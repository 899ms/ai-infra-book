<!-- 从 kernelagent.html 迁移的资料快照；原始 HTML SHA-256: ffeca80748e836ea3c760d46f1a5d554033c30b4cec4b1750251a6d0304a31b4。 -->
...
By [Kaiming Cheng, Laura Wang, Jack Khuu, Mark Saroufim, Wenyuan Chi, Jiannan Wang, and Joe Isaacson](#)March 6, 2026[No Comments](https://pytorch.org/blog/kernelagent-hardware-guided-gpu-kernel-optimization-via-multi-agent-orchestration/#respond)

## ![KernelAgent](https://pytorch.org/wp-content/uploads/2026/03/KernelAgent-1024x734.png)

## Summary

Recently, the PyTorch team released [KernelAgent](https://pytorch.org/blog/kernelfalcon-autonomous-gpu-kernel-generation-via-deep-agents/#), an open agentic system achieving 100% correctness across all 250 L1/L2/L3 [KernelBench](https://github.com/ScalingIntelligence/KernelBench) tasks. In this post, we extend that work by adding a **hardware-guided optimization layer** to the existing framework. Building on the previous correctness-focused pipeline, KernelAgent integrates GPU hardware-performance signals into a closed-loop multi-agent workflow to guide the optimization for Triton Kernels.

We evaluate the kernels generated by KernelAgent on all 100 L1 [KernelBench](https://github.com/ScalingIntelligence/KernelBench) tasks. Overall, it achieved **2.02x speedup** over generated kernels from earlier versions. On average, KernelAgent generated **1.56x** speedup when compared to default torch.compile, outperforming 65 of 100 KernelBench L1 tasks and achieving **89%** of the hardware roofline efficiency on the H100.

The optimization codebase is located at [KernelAgent repo](https://github.com/meta-pytorch/KernelAgent) with documentation to get started. We also share a selection of end-to-end KernelAgent optimization artifacts in the [open-source repo](https://github.com/kaiming-cheng/kernelagent-optimization-artifacts). 
...
We share a selection of end-to-end KernelAgent optimization artifacts in the [open source repo](https://github.com/meta-pytorch/KernelAgent). We also tested a few kernels across each category on different input shapes. Across 12 kernels / 144 shapes, we observe similar speedup. 

### On the effect of test time scaling: 

![Kernels making improvement by round](https://pytorch.org/wp-content/uploads/2026/03/Screenshot-2026-03-06-at-1.17.31-PM.jpg)
...
![Round-by-round kernel performance](https://pytorch.org/wp-content/uploads/2026/03/Screenshot-2026-03-06-at-1.17.41-PM.jpg)
...
![Memory throughput achieved](https://pytorch.org/wp-content/uploads/2026/03/Screenshot-2026-03-06-at-1.17.59-PM.jpg)
...
**Try it yourself.** [KernelAgent](https://github.com/meta-pytorch/KernelAgent) is an open-source project under active development. We welcome feedback, contributions, and new use cases from the community, and we hope this work helps advance practical, scalable kernel optimization within the PyTorch ecosystem.
