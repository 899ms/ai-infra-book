...
check(len(figs)==9,'nine figures missing')
check(len(re.findall(r'^\*图 3-\d+：',s,re.M))==9,'external captions missing')
...
for z in json.loads((HERE/'browser-validation.json').read_text()):
 check(z['width']==z['documentWidth'] and z['imagesLoaded'] and z['images']==9 and z['mathErrors']==0 and not z['unresolvedMath'],'browser check')
