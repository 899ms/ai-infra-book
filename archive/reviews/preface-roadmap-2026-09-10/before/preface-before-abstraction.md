...
模型执行时搬运数据的开销，也是我那时就遇到的问题。2016 年，围绕微软 Bing 搜索排序所用的深度神经网络，我们探索用 FPGA 加速推理。模型权重反复从片外内存搬进芯片，限制了计算速度。我想到，能否把模型拆到多块 FPGA，让各部分留在各自的片上存储中，只通过高速网络传递中间结果？我兴奋地把想法告诉徐宁仪老师，他说，这叫“模型并行”。[^preface-fpga] 计算放在哪里、数据怎样流动、专用化能省下什么，已经是我当时反复思考的问题。
...
今天回头看，万卡训练的必要性已经不难理解。但我想把当年的质疑写进这本书，因为站在当时，答案远没有今天看起来这样清楚。系统设计需要同时看清眼前的负载和正在变化的条件。八张卡能够完成当时熟悉的任务，是一种实际经验；更多计算会推动更大规模协作，是对未来需求的判断。要说服别人，就需要把这个判断所依赖的趋势、资源和代价讲清楚。后来的需求变化让大规模互联的重要性更加明确，也让我们面对更具体的问题：设备多了以后，怎样分配状态，怎样减少交接，怎样避免大量计算资源一起等待？
...
从可编程网卡到 AKG，再到 UB，工作跨越了不同的尺度，我反复遇到的却是相通的问题：计算需要的数据在哪里，现有接口增加了哪些工作，掌握更多应用信息以后，能否换一种执行方式？
...
这段经历让我更加确信，应用开发与基础设施之间有很直接的联系。同样是一次模型调用，多等几秒，产品体验就可能完全不同；同样是一小时语音服务，成本相差两个数量级，能够服务的用户和使用方式也会改变。前面在芯片、编译器和网络上积累的经验，最终都要回到这些具体的体验与成本上。
...
这种协作会直接改变模型的设计。模型架构不仅决定能够学到什么，也决定训练要保存多少状态、设备之间交换什么数据，以及生成答案时需要反复读取什么。在质量目标下，把这些执行代价一同纳入架构选择，往往能够取得单独优化算法或系统难以获得的收益。模型、数据与 Infra 之间的反馈越直接，团队就越有机会找到这样的设计。
...
模型结构给出了计算之间的依赖，运行时掌握数据的产生与使用时机，芯片和网络承担相应的运算与传输。利用这些信息，可以让相邻算子直接复用中间结果，重复使用已经准备好的执行计划，提前安排设备间的数据交接，也可以在多轮调用之间保留值得复用的状态。因此，模型结构、编译与运行时系统、硬件各层的设计选择会相互影响。理解模型训练和推理的完整执行过程，才能判断各层哪些设计值得重新考虑。
...
[^preface-voice]: 发布时间与约百分之一的成本比较据作者对 2024 年产品实践的回忆。比较的是自建系统的运行成本与调用 GPT-4o 实时语音 API 的费用，API 比较发生在其推出之后；并非两种模型在全部能力上的等价测试。取材说明见[作者经历记录](../references/editorial-context/2026-09-10/preface-author-stories.md)。
