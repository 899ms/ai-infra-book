...
f,a=canvas(5)
for row,(groups,label) in enumerate([(8,"八個單卡實例"),(2,"兩個四卡實例"),(1,"一個八卡實例")]):
 ...
 for g in range(groups):
  ...
  if groups<=2:a.text(x+w/2,y+.013,"一個獨立推理實例",ha='center',fontsize=9,color=C['teal'])
a.set_ylim(.32,1)
...
ax.set(xlim=(0,7.1),ylim=(-.6,3.6),yticks=[3,2,1,0],yticklabels=["階段 0","階段 1","階段 2","階段 3"],xticks=range(8),xlabel="時間 / ms")
ax.axvline(4,color=C['muted'],ls=':',lw=1);ax.text(4.05,3.4,"首個結果",fontsize=11)
ax.spines['left'].set_visible(False);ax.tick_params(axis='y',length=0)
save(f,'figure-6-3-pipeline')
# Independent token dispatch/combine ownership.
f,a=canvas(6.7)
box(a,.02,.29,.26,.29,"卡 0：token A","router 選擇專家 1、6",col='light',size=14)
box(a,.40,.64,.25,.21,"本地專家 1","計算 y1",size=14)
box(a,.40,.12,.25,.21,"卡 3：專家 6","計算 y6",size=14)
...
f,a=canvas(8)
panel(a,.025,.965,"塊 0：每經過一張卡，就加入該卡的貢獻")
for r,val in enumerate([1,11,111,1111]):
 x=.025+r*.248
 box(a,x,.70,.21,.16,f'卡 {r}',f'塊 0：{val}',col='light' if r==3 else 'pale',size=13)
 if r<3:
  arrow(a,(x+.215,.78),(x+.24,.78))
  a.text(x+.23,.64,f'第 {r+1} 輪',ha='center',fontsize=10)
panel(a,.025,.54,"ReduceScatter 完成：各卡持有一塊完整歸約結果")
for r,block in enumerate([1,2,3,0]):
 box(a,.025+r*.248,.34,.21,.13,f'卡 {r}：塊 {block}',size=13)
 arrow(a,(.13+r*.248,.33),(.13+r*.248,.23))
 box(a,.025+r*.248,.045,.21,.17,f'卡 {r}',"塊 0、1、2、3",col='light',size=13)
a.text(.5,.265,"再經三輪 AllGather，交換已歸約的資料塊",ha='center',fontsize=12)
save(f,'figure-6-6-ring-rounds')
# 3: Equal assignments, different expert reuse.
m=np.arange(1,65);expected=128*(1-(1-8/128)**m)
f,a=canvas(6)
for y,title,experts,rows,col in [(.61,"均勻覆蓋",128,4,'blue'),(.15,"集中到同八個專家",8,64,'orange')]:
 a.text(.02,y+.23,title,fontsize=15,weight='bold')
 box(a,.025,y,.23,.17,f'{experts} 個專家 × {rows} 行',f'共 {experts*rows} 次分派',size=13)
...
a.text(.37,.96,"權重讀取量",fontsize=15,weight='bold',va='top')
...
ax.set(xlabel="專家編號",ylabel="層編號",xticks=[0,64,128,192,255],yticks=[0,10,20,30,42]);f.colorbar(im,ax=ax,label="選擇次數 / token")
...
ax.loglog(M,ring*1e6,color=C['blue'],lw=2.2,label='Ring');ax.loglog(M,tree*1e6,color=C['orange'],lw=2.2,label="未分段二項樹");ax.set(xlabel="每卡完整輸入 / bytes",ylabel="模型時間 / μs",xlim=(100,1e8),ylim=(3,5000));ax.legend(frameon=False,loc='upper left');ax.grid(which='major',alpha=.2)
...
ax.annotate("約 680 KiB：排序翻轉",(cross,14*A*1e6+1.75*cross/BW*1e6),xytext=(3e6,40),fontsize=12,arrowprops={'arrowstyle':'->','color':C['muted']})
...
ax.set(yticks=[1,0],yticklabels=['4 MiB','64 MiB'],xlabel="時間 / ms",xlim=(0,50),ylim=(-.65,1.65));ax.set_title("通訊單獨執行",fontsize=14,loc='left')
ax=axs[1]
for base,comm,compute,col in [(1.1,6.141,11.523,'blue'),(0,45.209,12.885,'orange')]:
 ...
 ax.text(.015,base+.15,"通訊",va='center',fontsize=10)
 ax.text(.015,base-.15,"計算",va='center',fontsize=10,color='white')
...
ax.set(yticks=[1.1,0],yticklabels=['4 MiB','64 MiB'],xlabel="從同時開始起的時間 / ms",xlim=(0,56),ylim=(-.65,1.75));ax.set_title("同時執行",fontsize=14,loc='left')
...
for idx,z in enumerate(paths):
 ...
 ax.set_title(("遞迴" if idx==0 else 'Swing')+f'：第三輪 0 → {route["receiver"]}',fontsize=13)
ax=f.add_axes([.66,.21,.30,.61]);xx=np.arange(3)
for i,z in enumerate(paths):
 vals=[v['peak_link_bytes']/2**20 for v in z['rounds']]
 ax.bar(xx+(i-.5)*.34,vals,.32,color=C[['blue','orange'][i]],label=["遞迴",'Swing'][i])
 for x,v in zip(xx+(i-.5)*.34,vals):ax.text(x,v+.12,f'{v:g}',ha='center',fontsize=11)
ax.set(xticks=xx,xticklabels=['1','2','3'],xlabel="輪次",ylabel="最大單向鏈路傳輸量 / MiB",ylim=(0,5.6));ax.legend(ncol=2,frameon=False,fontsize=11)
...
for y,title,left,right,middle,detail in [
 (.72,"NVIDIA：機櫃級 NVLink","GPU / CPU 託盤","GPU / CPU 託盤","NVLink 交換",'GB200 NVL72：72 GPU、36 CPU'),
 (.40,"TPU：連線電互聯單元","64 晶片電互聯單元","64 晶片電互聯單元","OCS 光路交換","TPU v4：每單元 4 × 4 × 4 晶片"),
 (.08,"UB：連線跨主機資源","NPU / CPU 資源","NPU / CPU 資源","UB 交換",'CloudMatrix384：384 NPU、192 CPU')]:
...
ax.axhline(50,color=C['red'],ls='--',label="路徑：50 GB/s")
ax.axhline(win,color=C['orange'],ls=':',label="128 個在途事務：約 4.36 GB/s")
...
ax.set(xlabel="完整讀取 20 GB 的頻率 / 次每秒",ylabel="所需平均頻寬 / GB/s",xlim=(.01,40),ylim=(.1,1500))
...
f=plt.figure(figsize=(12,5.8));colors=['muted','blue','teal','orange'];labels=["八個單卡實例","四個兩卡實例","兩個四卡實例","一個八卡實例"]
curve_data={}
for pos,(phase,title) in enumerate([('healthy',"無故障條件"),('fault',"20 ms 故障，60 ms 開始重做")]):
 ...
 ax.axvline(130,color=C['line'],ls='--');ax.set(xlim=(20,240),ylim=(0,1.9),xlabel="八步續寫期限 / ms",ylabel="每個按時會話 / GPU·s")
...
page="<!doctype html><html lang=\"zh-CN\"><head><meta charset=\"utf-8\"><meta name=\"viewport\" content=\"width=device-width,initial-scale=1\"><title>第 6 章 超節點</title><style>"+css+math_css+"</style></head><body><main><nav aria-label=\"本章目录\">"+nav+'</nav>'+body+'</main></body></html>'
import sys
sys.path.insert(0,str(HERE.parent))
