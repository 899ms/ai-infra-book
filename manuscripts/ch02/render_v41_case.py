...
stage=['**表 2-D　V4.1 Flash 的 8K 输入如何分配到执行阶段（TFLOPs）**','','| 阶段 | 参考全层前向 | CED＋末尾窗口重放 |','| --- | ---: | ---: |']
for title,x,y in zip(['20 层编码器','解码器共享全局 KV 与索引键投影','20 层解码器的查询与前馈等计算','末尾 token词表头'],a,b):stage.append(f'| {title} | {x/1e12:.3f} | {y/1e12:.3f} |')
stage.append(f"| 合计 | {sum(a)/1e12:.3f} | {sum(b)/1e12:.3f} |")
stage+=['','参考路径执行全部 8192 个 token的 40 层，并按公开实现先计算矩形索引分数再屏蔽；CED 路径的编码器处理全部输入，解码器全局 KV 与索引键仍为全部输入生成，解码器主体只处理末尾 128 个 token，后续索引限制在候选集合内。表中分别包含各路径的全部文本矩阵工作，差异同时来自执行位置数与索引算法。','',
...
long='''**上下文从 8K 增至 1M 时的运算量与状态增长。** 长文档问答和多轮任务会放大上下文访问的成本。下面保持单请求、一次新增一个 token 和末尾 token词表头不变。8K 场景有 8192 个历史 token；1M 场景有 1,048,575 个历史 token，加上当前查询正好为 1,048,576 个可见位置。\n\n![五模型在 8K 与 1M 上下文下的单步矩阵运算量。](ch02/figure-2-long-context-compute.svg)\n\n*图 2-31　8K／1M 上下文下再处理一个 token 的矩阵运算量。沿用表 2-C 的执行路径，横轴为对数刻度；1M 包含当前查询。*\n\n表 2-E 单独累计当前查询的注意力评分与值汇总；V4-Flash 与 V4.1 Flash 再加入索引点积。状态列列出调用前的上下文和固定状态，精度与表 2-C 一致。[^long-context-data]\n\n**表 2-E　长上下文下的交互运算与状态容量**\n\n| 模型 | 待生成 |\n| --- | --- |\n\n'''
long+=f'''V4.1 Flash 的全局 KV 只有四份独立来源，增加上下文时不会为所有 40 层各增一份。主注意力每层最多使用 128 个局部 token和 512 个全局条目；解码器首个 Full 层生成候选集合，后续四个 Reindex 层各扫描至多 16384 个候选条目。编码器的三个索引器和解码器首个索引器仍需扫描增长的全局历史，因此计算量继续上升；跨层共享和分层索引改变了增长幅度。1M 下，其上下文交互为 {v['history_interaction_flops']/1e9:.2f} GFLOPs，完整单步矩阵工作为 {v['matrix_flops']/1e9:.2f} GFLOPs。\n\nV4-Flash 则沿用 CSA 与 HCA 的两种压缩粒度：CSA 的主注意力选择上限保持不变，但索引扫描随历史增长；HCA 会读取全部已完成的粗粒度条目。1M 下，其上下文交互为 {v4['history_interaction_flops']/1e9:.2f} GFLOPs，完整单步矩阵工作为 {v4['matrix_flops']/1e9:.2f} GFLOPs。较短上下文时，V4.1 Flash 更大的主干会增加投影和专家计算；历史足够长时，较少的索引器与分层候选选择才会抵消这部分新增工作。\n\nKimi K3 的 69 个 KDA 层保持固定递推状态，24 个 MLA 层继续访问随上下文增长的历史。紧凑 MLA 减少了缓存容量，却没有取消查询与历史 token之间的计算；1M 下，这部分交互达到 {k['history_interaction_flops']/1e9:.2f} GFLOPs，整模型达到 {k['matrix_flops']/1e9:.2f} GFLOPs。评估长上下文设计，应分别检查保存了多少状态、访问哪些位置，以及为索引和更新状态增加了多少工作。\n\n统一计算还保留了 200K 场景：V4.1 Flash 的完整单步矩阵工作为 {rows['deepseek-v4.1-flash','200K']['matrix_flops']/1e9:.2f} GFLOPs，V4-Flash 为 {rows['deepseek-v4-flash','200K']['matrix_flops']/1e9:.2f} GFLOPs。到 1M 时，两者的计算量差异进一步扩大，所以正文选用 1M 展示长上下文下的设计价值。\n\n'''
...
'engram.wkv.weight':'Engram 查询结果的 KV 投影','head.weight':'末尾 token词表头'}
buckets={}
...
'本表覆盖普通文本生成的全部矩阵投影。输入行数 $m_l$ 表示第 $l$ 层本次处理的查询位置数；$u_l$ 表示需要生成全局 KV 的位置数，$n_{\\mathrm{cmp},l}$ 表示本次新完成的压缩条目数。参考全层 prefill 中 $m_l=P$；CED 中编码器 $m_l=P$、解码器 $m_l=\\min(P,128)$，但解码器全局 KV 来源层仍有 $u_{20}=P$。所有行数还需乘请求数 $B$。','',
