...
commands=[]
def cmd(args):
 if args[0]=="docker":args=args[:1]+["-H","unix://"+"/run/book1102-bypass.sock"]+args[1:]
...
for mode in ['default','unconfined']:
 ...
 try:
  ...
  def watch():
   roots=[Path('/run/book1102-bypass-ctd/io.containerd.runtime.v2.task/book1102')/cid,Path('/run/book1102-bypass/runtime-runc/book1102')/cid,Path('/run/book1102-bypass/containerd/daemon/io.containerd.runtime.v2.task/book1102')/cid,P/'data/containers'/cid]
  ...
  if restore and 'already exists' in restore['stderr']:
   ...
   ctr=['ctr','--address','/run/book1102-bypass-ctd.sock','--namespace','book1102','content']
   listing=cmd(ctr+['ls'])
  ...
  row=dict(initial_restore=initial_restore,mode=mode,cid=cid,before=before,capture=capture,restore=restore,state=state)
