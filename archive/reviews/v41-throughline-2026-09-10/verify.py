...
for n in range(1,13):
 p=next(M.glob(f'{n:02}-*.md'));s=p.read_text();before=(R/'before'/p.relative_to(ROOT)).read_text();page=p.with_suffix('.html').read_text()
...
check(sum(r['added_figures'] for r in rows)==6,'six new figures')
