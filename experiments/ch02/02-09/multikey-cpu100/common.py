...
CONFIG=dict(model_path=str(MODEL),dtype='bfloat16',cpu_offload_gb=100,context_length=5120,max_total_tokens=5120,max_running_requests=1,chunked_prefill_size=256,mem_fraction_static=.9,swa_full_tokens_ratio=1.0,disable_cuda_graph=True,disable_radix_cache=True,log_level='info',port=18329,random_seed=20260909)
SAMPLING=dict(temperature=0,top_p=1,top_k=1,max_new_tokens=128,ignore_eos=False)
LIMITS=dict(start_available_gib=140,start_gpu_free_mib=80*1024,max_rss_gib=175,max_gpu_mib=88*1024,min_available_gib=24,min_gpu_free_mib=4*1024,init_timeout_s=1800,request_timeout_s=600,total_timeout_s=7200,poll_s=1)
def save(path,obj):
