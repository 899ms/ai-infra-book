...
md=HERE.parent/'11-资源调度与运行环境.md';s=md.read_text();o=(ROOT/'outlines/11-资源调度与运行环境.md').read_text();page=md.with_suffix('.html').read_text()
head=lambda text:re.findall(r'^#{2,3} (11\.\d+(?:\.\d+)?) (.+)$',text,re.M)
check('all five sections and eighteen subsection titles match outline',head(s)==head(o) and len(head(s))==23)
...
check('nineteen sequential external captions',re.findall(r'^\*图 11-(\d+)\s',s,re.M)==[str(i) for i in range(1,20)] and page.count('<figcaption>')==19)
imgs=re.findall(r'!\[[^\]]*\]\(([^)]+)\)',s);check('nineteen illustrations',len(imgs)==19)
for image in imgs:
...
check('all figure text within canvas',not read(HERE/'figure-layout-check.json')['text_extent_warnings'])
...
x=d['11-7'];check('routing curve independent formula',all(close(y,(.0424-.0342*h)/.98) for h,y in zip(x['h'],x['cost_B'])) and close(x['cost_crossover'],1291/1520) and close(x['joint_target_hit'],45/49))
...
check('zero denominator and later purchase audit explained','OPTIMIZATION-AUDIT.md' in s and '未经生产 trace 校准' in s)
...
check('four design costs', [F(r['submission_cost']) for r in rows]==list(map(F,['.0363','.03213','.0405','.03813'])))
check('deadline excludes ordinary service', [r['cost_per_on_time_quality'] is None for r in rows]==[True,True,False,False])
feasible=[r for r in rows if r['within_capacity'] and F(r['on_time_quality_probability'])>=F('.95')]
...
check('environment policy crossover',2*g*F('.0001')==F('.00041') and 2*2*F('.0001')<F('.00041')<2*6*F('.0001'))
...
check('pause and rebuild areas',d['pause']['paused_gib_seconds']==2*(1+1) and d['residency']['gib_seconds']==[2*30,3*2*3])
