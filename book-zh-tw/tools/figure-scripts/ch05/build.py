...
a.text(.55,.865,"for io, jo:   16 × 192 個輸出塊",fontsize=13)
...
a.text(.595,.63,"for ko:   128 個歸約塊",fontsize=13)
a.text(.595,.55,"A 塊 64 × 32：4 KiB\nW 塊 32 × 64：4 KiB",fontsize=12,va='top',linespacing=1.7)
a.text(.595,.395,"acc += A 塊 × W 塊",fontsize=13)
a.add_patch(Rectangle((.555,.155),.38,.12,facecolor=C['light'],edgecolor=C['teal']))
a.text(.575,.235,"c = BF16(acc)\n寫出 SiLU(c)",fontsize=13,va='top',linespacing=1.6)
arrow(a,(.745,.35),(.745,.28))
a.text(.76,.315,"歸約結束",fontsize=10,color=C['teal'])
a.text(.04,.025,"C 的寫出與讀取：48 MiB",fontsize=13,color=C['orange'])
a.text(.54,.025,"輸入槽隨 ko 更新；acc 跨 ko 保留",fontsize=12,color=C['blue'])
...
p=np.linspace(0,1,301);a.plot(p*100,np.full_like(p,10),lw=2.4,color=C['blue'],label="原實作：10 μs")
a.plot(p*100,20-15*p,lw=2.4,color=C['teal'],label="新實作：20 − 15p μs")
a.axvline(200/3,color=C['orange'],ls='--');a.text(200/3+2,19,"A 佔比 2/3",color=C['orange'],fontsize=11)
for x,y,label,offset in [(50,12.5,"各出現一次：新實作慢 25%",(-160,65)),(10000/101,520/101,"A ×100、B ×1：新實作約快 1.9 倍",(-260,-30))]:
...
a.set_xticks(range(0,101,20));a.set(xlim=(0,103),ylim=(0,22),xlabel="形狀 A 的呼叫佔比 / %",ylabel="每次呼叫平均時間 / μs");a.legend(frameon=False,loc='lower left')
...
f,a=plt.subplots(figsize=(12,5.5));f.subplots_adjust(left=.10,bottom=.17,right=.95,top=.93);rs=np.arange(1,151);labels={'generic':"通用",'bucket':"分桶",'specialized':"逐形狀特化"}
...
a.set_xticks(range(0,151,25));a.set(xlabel="指定十次呼叫組的重複次數",ylabel="準備 + 累計執行 / ms",xlim=(0,150),ylim=(0,2850));a.legend(frameon=False);a.text(14,2600,"1–64：通用",fontsize=11);a.text(66,2600,"65–89：分桶",fontsize=11);a.text(102,2600,"90 起：特化",fontsize=11)
...
for a,z,label in zip(axs,ranges,["普通提交","融合","圖重放","融合 + 圖重放"]):
 ...
 a.set_yticks([.28,.78],["裝置 kernel","主機 launch"]);a.set_ylim(0,1.05);a.set_xlim(0,900);a.set_xticks(range(0,901,150));a.set_xlabel("相對各自採集起點 / μs",fontsize=10);a.set_title(f"{label}：主機 kernel launch {z['launch_api_count']-z['graph_launch_count']}，graph launch {z['graph_launch_count']}；裝置 kernel {z['kernel_count']}",loc='left',fontsize=12)
...
f,a=canvas(6.3)
for y,hot,total,critical,label in [(.58,60,80,'A',"原始"),(.10,15,60,'B',"A 加快四倍後")]:
 a.text(.035,y+.29,label+'：'+str(total)+' μs',fontsize=15,weight='bold')
 box(a,.035,y+.07,.145,.105,"準備 10 μs",color='pale',size=12)
 ...
 a.text(.615,y+.26,"等待兩支完成",fontsize=11,color=C['muted'])
a.text(.035,.02,"橙色標出決定完成時間的路徑",fontsize=11,color=C['orange'])
...
a.axhline(np.median(v),ls='--',color=C['orange'],label=f'配對時間差中位數：{np.median(v):.1f} ms')
a.set_yticks(range(-3,6));a.set_xticks(x);a.set(xlim=(.5,11.5),ylim=(-3.2,5.3),xlabel="配對輪次",ylabel="替換前 − 替換後的請求時間 / ms");a.legend(frameon=False,loc='upper left',fontsize=11)
a.text(7.3,4.6,"9 對更快，2 對更慢\n請求總時間約 817 ms",ha='right',va='top',fontsize=11,linespacing=1.7)
...
for y,kernel,label in [(1,20,"原 kernel"),(0,5,"kernel 加快四倍")]:
 ...
 a.text(start+.5,y,f'{start} μs\n結果可用',va='center',fontsize=11)
a.set(xlim=(0,40),ylim=(-.65,1.65),xlabel="從 CPU 開始提交起計時 / μs");a.set_yticks([0,1],["kernel 加快四倍","原 kernel"]);a.set_xticks([0,3,11,20,31,35]);a.grid(axis='x',alpha=.15)
...
for y,fused in [(.57,False),(.12,True)]:
 a.text(.025,y+.31,"在矩陣乘中重新量化" if fused else "儲存一份 FP8 量化結果",fontsize=16,weight='bold')
 box(a,.03,y+.05,.18,.18,"FP16 輸入",'32 MiB',size=13)
 box(a,.34,y+.05,.24,.18,"求行 scale" if fused else "求行 scale，量化並儲存","不儲存 FP8 矩陣" if fused else "寫出 FP8：16 MiB",color='sand' if fused else 'light',size=12)
 arrow(a,(.22,y+.14),(.33,y+.14))
 box(a,.74,y+.05,.23,.18,"12 個輸出列塊","各讀 FP16 並量化" if fused else "各讀同一份 FP8",size=12)
 if fused:
  ...
  a.text(.45,y+.005,"原輸入：12 × 32 MiB",ha='center',fontsize=11,color=C['orange'])
 else:arrow(a,(.59,y+.14),(.73,y+.14),col='teal')
 if not fused:a.text(.66,y+.235,'12 × 16 MiB',ha='center',fontsize=11,color=C['teal'])
 a.text(.34,y-.040,"讀寫：32 + 12 × 32 = 416 MiB" if fused else "讀寫：32 + 16 + 12 × 16 = 240 MiB",fontsize=12)
a.text(.025,.015,"重複讀取的資料寬度翻倍，省下的一次寫出不足以抵消它",fontsize=12,color=C['blue'])
...
a.set_yticks([2,1,0],["普通執行","圖重放：2 MiB 輸入","圖重放：16 MiB 輸入"]);a.set(xlim=(0,52),xticks=range(0,51,10),ylim=(-.65,2.6),xlabel="完成時間 / μs");a.axvline(40,color=C['muted'],ls='--',lw=1)
a.legend(handles=[Rectangle((0,0),1,1,color=C[c],label=t) for c,t in [('orange',"準備 / 提交等待"),('teal',"複製輸入"),('blue',"裝置計算")]],loc='lower left',bbox_to_anchor=(-.2,1),frameon=False,ncol=3,fontsize=11)
...
figure_refs=re.findall("^!\\[圖 (5-\\d+) ([^\\]]+)\\]\\(ch05/([^)]+)\\)",raw,re.M)
...
def wrap_figure(match):
 ...
 content=('<div class="diagram-scroll">'+img+"</div><p class=\"diagram-hint\">左右滑動檢視完整配圖</p>") if revised else img
