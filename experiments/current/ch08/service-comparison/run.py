...
async def main(a):
 ...
 config=dict(model=a.model,dtype='bfloat16',kv_cache_dtype='auto',attention_backend='TRITON_ATTN',max_model_len=16384,max_num_seqs=4,max_num_batched_tokens=16384,enable_chunked_prefill=True,enable_prefix_caching=True,enforce_eager=True,async_scheduling=False,kv_cache_memory_bytes=12*1024**3,gpu_memory_utilization=.5,seed=809,disable_log_stats=False)
 if a.variant=='E':config['speculative_config']=dict(method='ngram',num_speculative_tokens=4,prompt_lookup_min=2,prompt_lookup_max=5)
 ...
 try:
  ...
  # Reset after full-path warmup, then prime only fixed input prefixes.
  await engine.reset_prefix_cache()
  state['group']='prime'
  for tid,t in tasks.items():
   prefix=t['prompt_token_ids'][:min(6144,len(t['prompt_token_ids'])-512)]
   async for unused in engine.generate({'prompt_token_ids':prefix},SamplingParams(temperature=0,max_tokens=1),'prime-'+tid):pass
  for g in protocol['groups']:
   state['group']=g['id'];begin=time.monotonic();sem=asyncio.Semaphore(1) if g['service']=='serial' else None
   await asyncio.gather(*(one(tid,g['id']+f'-{i}',begin+(i/g['rate'] if g['rate'] is not None else 0),sem) for i,tid in enumerate(g['tasks'])))
   end=time.monotonic();groups.write(json.dumps(dict(**g,start_s=begin,end_s=end))+'\n');print(g['id'],'complete',round(end-begin,3),flush=True)
...
if __name__=='__main__':
 p=argparse.ArgumentParser();p.add_argument('--variant',choices=['B','E'],required=True);p.add_argument('--model',required=True);p.add_argument('--out',type=Path,required=True);asyncio.run(main(p.parse_args()))
