...
commands=[]
def cmd(args):
 if args[0]=="docker":args=args[:1]+["-H","unix://"+"/run/book1102-iso2.sock"]+args[1:]
...
for mode in ['default','unconfined']:
 ...
 try:
  ...
  def watch():
   roots=[Path('/run/book1102-iso2/runtime-runc/book1102')/cid,Path('/run/book1102-iso2/containerd/daemon/io.containerd.runtime.v2.task/book1102')/cid,P/'data/containers'/cid]
