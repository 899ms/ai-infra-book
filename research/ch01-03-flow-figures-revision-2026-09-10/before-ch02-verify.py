...
check(re.findall(r'^> \*\*实验 (2-\d+)',s,re.M)==[f'2-{i}' for i in range(1,10)],'Exercise sequence')
check(re.findall(r'^> \*\*实验 (2-\d+)[^\n]*〔核心〕',s,re.M)==['2-2','2-5','2-7'],'Core exercise selection')
check(len(re.findall(r'!\[图 2-',s))==10,'Ten manuscript figures')
...
check(page.count('src="data:image/png;base64,')==10,'Ten embedded images')
...
browser=json.loads((H/'browser-validation.json').read_text());check(browser['loaded_images']==10 and browser['all_nav_targets_exist'],'Browser image/navigation');check(not browser['desktop_overflow'] and not browser['mobile_overflow'],'Reading layout overflow')
