...
输入：`{"dram_bytes_per_second": 40000000000, "intersocket_bytes_per_second": 8000000000, "order": "alternating", "pcie_bytes_per_second": 12000000000, "placement": "sender-local", "rank_to_gpu": [0, 2, 1, 3], "startup_ns": 0, "tokens": 1024}`
...
| --- | --- | --- |
| 0 → 2 | A | gpu0_to_host → dram_A → dram_A → A_to_B → gpu2_from_host |
| 2 → 1 | B | gpu2_to_host → dram_B → dram_B → B_to_A → gpu1_from_host |
| 1 → 3 | A | gpu1_to_host → dram_A → dram_A → A_to_B → gpu3_from_host |
