...
f,a=canvas(5.8)
for (sm,name),(y,title) in zip(alloc,[(.76,"inference request: 8192 input, 1025 output"),(.47,"hit 6144 prefix positions"),(.18,"output reduced to 129 tokens")]):
 ...
 a.text(.86,y+.05,f'{rate:.2f} req/s',va='center',fontsize=11)
a.text(.05,.02,"Blue: prefill     Green: decode",fontsize=11)
...
for ax,counts,title in [(axes[0],[64]*8,"even placement: eight cards finish simultaneously"),(axes[1],[512]+[0]*7,"eight hot experts on same card: other cards wait")]:
 times=np.array(counts)*2*18874368/H100_EFF*1e6
 ax.barh(range(8),times,color=C['teal'],height=.65);ax.invert_yaxis()
 ax.set(yticks=range(8),yticklabels=[f'card {i}' for i in range(8)],xlim=(0,46),xticks=[0,10,20,30,40])
 ax.set_title(title,loc='left',fontsize=13);ax.axvline(max(times),color=C['orange'],ls='--')
 ax.text(max(times)+.6,3,f'{max(times):.1f} μs\nbefore merge possible',fontsize=11)
 ax.grid(axis='x',alpha=.15)
axes[1].set_xlabel("time to finish expert compute on this card / μs")
...
serial_items=[(0,0,D,"dispatch"),(1,D,Cc,"compute"),(2,D+Cc,Rr,"merge")]
...
for ax,(title,items) in zip(axes,[(f'Serial:{D+Cc+Rr:.3f} ms',serial_items),(f'Two micro-batch pipeline:{pipeline:.3f} ms',pipe_items)]):
 ...
 ax.set(yticks=[0,1,2],yticklabels=["dispatch","Expert computation","Result merge"],xlim=(0,.9),ylim=(2.6,-.6))
 ax.set_title(title,loc='left',fontsize=13);ax.grid(axis='x',alpha=.15)
axes[1].set(xlabel="Time / ms",xticks=[0,.2,.4,.6,.8])
...
routes=[("A: local hit",250,None,warm),("B: local recompute",20,None,full),("B: remote 50 GbE",20,'50gbe',warm),("B: remote 200 GbE",20,'200gbe',warm)]
...
for ax,(title,q,key,c) in zip(axes,routes):
 ...
 ax.set(yticks=[0,1],yticklabels=['GPU',"Fetch"],ylim=(1.6,-.6),xlim=(0,1000),xticks=[0,200,400,600,800,1000])
 ax.set_title(title,loc='left',fontsize=12);ax.grid(axis='x',alpha=.12)
axes[-1].set_xlabel("time since request arrival / ms")
f.text(.14,.025,"Gray: queue/lookup    Orange: remote read    Blue: host mem → GPU    Green: compute",fontsize=11)
...
links=[(25,'teal',"target via 200 Gb/s NIC: 25 GB/s"),(6.25,'blue',"target via 50 GbE: 6.25 GB/s")]
catch=[V0/(B-g) for B,_,_ in links]
f,ax=plt.subplots(figsize=(10,5.5));f.subplots_adjust(left=.12,right=.96,bottom=.17,top=.91)
t=np.linspace(0,8,321);ax.plot(t,V0+g*t,label=f'source: {V0:.1f} GB, growing {g:.3f} GB',color=C['orange'],lw=2.5)
for (B,col,label),tc in zip(links,catch):
 ax.plot(t,np.minimum(B*t,V0+g*t),label=label,color=C[col],lw=2.5)
 ax.scatter([tc],[V0+g*tc],color=C['ink'],zorder=3);ax.annotate(f'~ {tc:.2f} s to catch up',xy=(tc,V0+g*tc),xytext=(tc+.35,V0-14),fontsize=12,arrowprops={'arrowstyle':'->'})
ax.set(xlabel="time since background copy start / s",ylabel="cumulative state size / GB",xlim=(0,8),ylim=(0,50));ax.legend(frameon=False,loc='lower right',fontsize=10);ax.grid(alpha=.15)
...
f,a=canvas(6)
for y,title in [(.72,"returned and recorded sequence"),(.43,"KV saved before failure"),(.14,"resume generation after rebuild")]:
 a.text(.025,y+.15,title,fontsize=12,weight='bold')
 box(a,.04,y,.39,.10,"input: 8192 tokens",col='pale',fs=11)
 if y!=.43:
  box(a,.45,y,.32,.10,"returned output 1–1024",col='green',fs=11)
  box(a,.80,y,.16,.10,"output 1025",col='sand',fs=11)
 else:
  a.text(.47,y+.05,"missing KV for generated part",va='center',fontsize=12,color=C['orange'])
a.text(.46,.32,"feed recorded outputs 1–1024 back into model",fontsize=11,color=C['teal'])
a.text(.05,.015,"KV restored to 9216 tokens first, then process output 1025 to generate next token.",fontsize=12)
