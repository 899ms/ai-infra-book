...
[^precision]: [The mixed-precision training paper](https://github.com/bojieli/ai-infra-book/blob/main/references/text/mixed-precision.txt) (loss-scaling mechanism, scaling factors from 8 to 32K, FP16 representable range), [the FP8 formats paper](https://github.com/bojieli/ai-infra-book/blob/main/references/text/fp8-formats.txt) (E4M3/E5M2 encoding and maximum normal values), [the DeepSeek-V3 technical report](https://github.com/bojieli/ai-infra-book/blob/main/references/files/papers/deepseek-v3.pdf) §3.3 (1×128 activation and 128×128 weight block scaling, promotion to FP32 accumulation every 128 elements, relative loss error below 0.25%). The exponent and mantissa comparison between FP16 and BF16 is given in [Section 4.6.1](https://github.com/bojieli/ai-infra-book/blob/main/04-加速器架构.md).
...
[^opd]: [DeepSeek V4 technical report](https://github.com/bojieli/ai-infra-book/blob/main/references/files/papers/deepseek-v4.pdf), §5.1 on domain experts and multi-teacher OPD; stage work division follows [Chapter 3](https://github.com/bojieli/ai-infra-book/blob/main/03-推理与训练负载.md).
...
[^design]: [Inputs, derivation, and recomputation for this chapter's design case](https://github.com/bojieli/ai-infra-book/blob/main/ch10/design-case.md). The capacity reservation, input waiting, and planned pauses for 32/48 cards are the given input; per-card compute efficiency uses Llama 3's MFU; the model's matrix work comes from the existing training-deadline computation, and the ZeRO-3 communication volume is derived from the same parameter table. The RTX 4090's PCIe 4.0 with no NVLink is given in the [RTX 4090 specification page](https://github.com/bojieli/ai-infra-book/blob/main/references/files/specs/nvidia-rtx4090.md); no P2P support between cards and eight 200 Gbit/s NICs per eight-card host are given in the [4090 cluster configuration analysis](https://github.com/bojieli/ai-infra-book/blob/main/references/files/documents/h100-vs-4090.md); within a single step, only the first AllGather and the last ReduceScatter cannot be hidden, per [MegaScale](https://github.com/bojieli/ai-infra-book/blob/main/references/text/megascale.txt) §3.2. Itemized values and independent recomputation are stored in [design-case.json](https://github.com/bojieli/ai-infra-book/blob/main/ch10/design-case.json).
