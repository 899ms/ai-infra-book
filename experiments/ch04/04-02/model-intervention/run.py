...
async def main(a):
    ...
    cfg=dict(model=a.model,dtype='auto',kv_cache_dtype='auto',attention_backend='TRITON_ATTN',
             ...
             async_scheduling=False,kv_cache_memory_bytes=2*1024**3,gpu_memory_utilization=.30,worker_extension_cls="intervention_probe.InterventionProbe",
    ...
    (out/'environment.json').write_text(json.dumps(dict(config=cfg,torch=torch.__version__,vllm=vllm.__version__,
        driver_sha256=hashlib.sha256(Path(__file__).read_bytes()).hexdigest(),scope='Layer 0 expert 0 gate_up output replacement only; quality ablation, no speed claim'),indent=2)+'\n')
    ...
    try:
        ...
        capture_info=await engine.collective_rpc('arm_intervention',args=(str(out),str(B/'reference-expert.pt')))
        (out/'capture-info.json').write_text(json.dumps(capture_info,indent=2)+'\n')
        with (out/'requests.jsonl').open('w',buffering=1) as f:
            for mode in ['direct','dequant']:
             for original_case in cases:
                c=dict(original_case,id=mode+'-'+original_case['id'])
                await engine.collective_rpc('set_intervention_case',args=(c['id'],mode))
        ...
        (out/'completion.json').write_text(json.dumps(dict(requests=2*len(cases),status='returned_not_quality_claim'))+'\n')
    finally:
