<!-- 从 h100-specs.html 迁移的资料快照；原始 HTML SHA-256: 685590ff9e263e8614b66902576ca715de75b344efe3cddc34a6902aead1306c。 -->

![](data:image/svg+xml;base64,)

Visit your regional NVIDIA website for local content, pricing, and where to buy partners specific to your country.

Argentina Australia België (Belgium) Belgique (Belgium) Brasil (Brazil) Canada Česká Republika (Czech Republic) Chile Colombia Danmark (Denmark) Deutschland (Germany) España (Spain) France India Italia (Italy) México (Mexico) Middle East Nederland (Netherlands) Norge (Norway) Österreich (Austria) Peru Polska (Poland) Rest of Europe România (Romania) Singapore Suomi (Finland) Sverige (Sweden) Türkiye (Turkey) United Kingdom United States 대한민국 (South Korea) 中国大陆 (Mainland China) 台灣 (Taiwan) 日本 (Japan)

[Continue](https://www.nvidia.com)

[Skip to main content](#page-content)

[![](data:image/svg+xml;base64,)](https://www.nvidia.com/en-sg/ "Artificial Intelligence Computing Leadership from NVIDIA")

- [Cloud and Data Center](https://www.nvidia.com/en-sg/data-center/)
- Solutions ![](data:image/svg+xml;base64,)
  Solutions
  [](https://www.nvidia.com/en-sg/data-center/solutions/accelerated-computing/)
  Accelerated Computing

  Accelerate enterprise IT and AI workloads with data center solutions

  [](https://www.nvidia.com/en-sg/data-center/ai-data-platform/)
  AI Data Platform for Enterprise

  Unify and accelerate enterprise AI workloads with a full-stack data platform

  [](https://www.nvidia.com/en-sg/solutions/ai-factories/)
  AI Factory

  Accelerate and deploy large-scale AI with full-stack factory solutions

  [](https://www.nvidia.com/en-sg/data-center/gpu-cloud-computing/)
  Cloud Computing

  Unleash enterprise AI and HPC with scalable accelerated cloud solutions

  [](https://www.nvidia.com/en-sg/edge-computing/)
  Edge Computing

  Accelerate AI, robotics, and IoT with scalable edge computing

  [](https://www.nvidia.com/en-sg/high-performance-computing/)
  High Performance Computing

  Advance discovery with energy‑efficient high‑performance computing

  [](https://www.nvidia.com/en-sg/data-center/sustainable-computing/)
  Sustainable Computing

  Save energy and reduce costs with sustainable, efficient computing

  [](https://www.nvidia.com/en-sg/data-center/solutions/mlops/)
  MLOps

  Scale AI solutions with enterprise MLOps tools, automation, and software

  [](https://www.nvidia.com/en-sg/networking/)
  Networking

  Networking for high-performance, scalable, and secure AI data centers

  [](https://www.nvidia.com/en-sg/data-center/dgx-platform/)
  DGX Platform

  Build next-generation AI factories for enterprises

  [](https://www.nvidia.com/en-sg/data-center/dgx-cloud/)
  DGX Cloud

  NVIDIA’s AI factory in the cloud
- Products ![](data:image/svg+xml;base64,)
  Products
  [](https://www.nvidia.com/en-sg/data-center/products/)
  Overview

  Accelerate AI, machine learning, and HPC with advanced data center platforms

  [](https://www.nvidia.com/en-sg/data-center/vera-rubin-nvl72/)
  Vera Rubin NVL72

  Rack-Scale Agentic AI Supercomputer

  [](https://www.nvidia.com/en-sg/data-center/lpx/)
  LPX

  Fast inference with full intelligence at scale

  [](https://www.nvidia.com/en-sg/data-center/dgx-superpod/)
  DGX SuperPOD

  Full-stack blueprint for gigascale AI infrastructure

  [](https://www.nvidia.com/en-sg/data-center/gb300-nvl72/)
  GB300 NVL72

  GB300 NVL72 rack-scale system built for the age of AI reasoning

  [](https://www.nvidia.com/en-sg/data-center/hgx/)
  HGX

  HGX platform specs for multi‑GPU AI, HPC, and simulation workloads

  [](https://www.nvidia.com/en-sg/data-center/grace-cpu/)
  CPU

  Architecture for data centers that transform data into intelligence

  [](https://www.nvidia.com/en-sg/data-center/gb200-nvl72/)
  GB200 NVL72

  GB200 NVL72 for scalable LLM inference and advanced AI data center needs

  [](https://www.nvidia.com/en-sg/data-center/products/rtx-pro-server/)
  RTX PRO Server

  Universal data center platform for enterprise AI and industrial AI

  [](https://www.nvidia.com/en-sg/products/workstations/professional-desktop-gpus/rtx-pro-6000-family/)
  RTX PRO 6000 Blackwell

  RTX PRO 6000 specs for professional AI, graphics, and visual computing tasks

  [](https://www.nvidia.com/en-sg/data-center/dgx-basepod/)
  DGX BasePOD

  Reference architecture for building and scaling AI infrastructure

  [](https://www.nvidia.com/en-sg/data-center/products/dsx/)
  DSX Platform

  Build AI factories optimized for lowest cost tokens per megawatt
- Software ![](data:image/svg+xml;base64,)
  Software
  [](https://www.nvidia.com/en-sg/data-center/products/ai-enterprise/)
  AI Enterprise Suite

  Enterprise AI software suite for data center deployments

  [](https://www.nvidia.com/en-sg/data-center/base-command/manager/)
  Base Command Manager

  Centralized platform to manage and monitor AI workloads in data centers

  [](https://www.nvidia.com/en-sg/technologies/cuda-x/)
  CUDA-X

  GPU-accelerated libraries and tools for AI, HPC, and data processing

  [](https://www.nvidia.com/en-sg/data-center/virtual-solutions/)
  Virtualization

  GPU virtualization software to accelerate and manage virtualized environments

  [](https://www.nvidia.com/en-us/accelerated-applications/)
  Accelerated Apps Catalog

  Browse DPU- and GPU-accelerated apps, tools, and services

  [](https://www.nvidia.com/en-sg/data-center/mission-control/)
  Mission Control

  Powers AI factory operations from developer workloads to infrastructure

  [](https://www.nvidia.com/en-sg/software/run-ai/)
  NVIDIA Run:ai

  AI Workload and GPU Orchestration
- Architectures ![](data:image/svg+xml;base64,)
  Architectures
  [](https://www.nvidia.com/en-sg/data-center/technologies/blackwell-architecture/)
  Blackwell Architecture

  The GPU architecture powering AI factories for the age of AI reasoning

  [](https://www.nvidia.com/en-sg/data-center/technologies/hopper-architecture/)
  Hopper Architecture

  Supercharging AI and HPC workloads for every data center

  [](https://www.nvidia.com/en-sg/technologies/ada-architecture/)
  Ada Lovelace Architecture

  Real-time ray tracing and AI for advanced design and visualization

  [](https://www.nvidia.com/en-sg/data-center/products/mgx/)
  MGX Reference Architecture

  Modular reference architecture for building accelerated data center servers
- Technologies ![](data:image/svg+xml;base64,)
  Technologies
  [](https://www.nvidia.com/en-sg/data-center/nvlink/)
  NVLink and NVLink Switch

  High-speed interconnect for multi-GPU communication and large AI models

  [](https://www.nvidia.com/en-sg/data-center/nvlink-fusion/)
  NVLink Fusion

  Semi-custom AI infrastructure with industry-proven AI scale-up performance

  [](https://www.nvidia.com/en-sg/data-center/tensor-cores/)
  Tensor Cores

  Specialized GPU cores that speed up AI and HPC math workloads

  [](https://www.nvidia.com/en-sg/technologies/multi-instance-gpu/)
  Multi-Instance GPUs

  GPU partitioning to securely share one GPU across workloads

  [](https://www.nvidia.com/en-sg/data-center/solutions/confidential-computing/)
  Confidential Computing

  Secure data and AI models in use
- Resources ![](data:image/svg+xml;base64,)
  Resources
  [](https://www.nvidia.com/en-sg/data-center/resources/)
  Resource Hub

  Resources to build and optimize AI-ready data centers and factories

  [](https://docs.nvidia.com/data-center-gpu/line-card.pdf)
  Data Center GPU Line Card

  Guide to selecting data center GPUs and networking for AI workloads

  [](https://resources.nvidia.com/l/en-us-gpu)
  Data Center GPUs Resource Center

  Documentation and guides for NVIDIA data center GPU products

  [](https://www.nvidia.com/en-sg/data-center/resources/mlperf-benchmarks/)
  MLPerf Benchmarks

  MLPerf training and inference results for the NVIDIA AI platform

  [](https://www.nvidia.com/en-sg/data-center/products/certified-systems/)
  NVIDIA-Certified Systems

  Accelerated partner systems delivering certified performance and scale

  [](https://marketplace.nvidia.com/en-us/enterprise/qualified-system-catalog/)
  Qualified System Catalog

  Catalog of qualified servers built with NVIDIA data center GPUs
- 

&nbsp;

- 

- [![](data:image/svg+xml;base64,PHN2ZyBjbGFzcz0ibjI0LWljb24gbWVnYS1uYXYtY2xvc2UtbWVudS1pY29uIiBhcmlhLWxhYmVsbGVkYnk9InNlYXJjaCI+CiAgICAgICAgICAgICAgICAgICAgICAgICAgICAgICAgICAgICAgICA8dXNlIHhsaW5rOmhyZWY9IiNuMjQtY2xvc2UiIC8+CiAgICAgICAgICAgICAgICAgICAgICAgICAgICAgICAgICAgIDwvc3ZnPg==)](#)

- [![](data:image/svg+xml;base64,PHN2ZyBjbGFzcz0ibjI0LWljb24gbmF2LXNlYXJjaC1pY29uIiBhcmlhLWxhYmVsbGVkYnk9InNlYXJjaCI+CiAgICAgICAgICAgICAgICAgICAgICAgICAgICAgICAgICAgIDx1c2UgeGxpbms6aHJlZj0iI24yNC1zZWFyY2giIC8+CiAgICAgICAgICAgICAgICAgICAgICAgICAgICAgICAgPC9zdmc+)](#)

- [![](data:image/svg+xml;base64,) US](#)

- - [](# "Sign In")
    Sign In

    [](https://www.nvidia.com/en-sg/account/edit-profile/)

    NVIDIA Account

    [](/)

    Logout

- [Log In](#) [LogOut](#)

[Skip to main content](#page-content)

-  ![](data:image/svg+xml;base64,PHN2ZyBjbGFzcz0ibjI0LWljb24gbWVnYS1uYXYtb3Blbi1tZW51LWljb24iPgogICAgICAgICAgICAgICAgICAgICAgICAgICAgICAgICAgICA8dXNlIHhsaW5rOmhyZWY9IiNuMjQtbWVudSIgLz4KICAgICAgICAgICAgICAgICAgICAgICAgICAgICAgICA8L3N2Zz4=) ![](data:image/svg+xml;base64,PHN2ZyBjbGFzcz0ibjI0LWljb24gbWVnYS1uYXYtY2xvc2UtbWVudS1pY29uIj4KICAgICAgICAgICAgICAgICAgICAgICAgICAgICAgICAgICAgPHVzZSB4bGluazpocmVmPSIjbjI0LWNsb3NlIiAvPgogICAgICAgICAgICAgICAgICAgICAgICAgICAgICAgIDwvc3ZnPg==)

- [![](data:image/svg+xml;base64,PHN2ZyBjbGFzcz0ibjI0LWljb24gbWVnYS1uYXYtY2xvc2UtbWVudS1pY29uIiBhcmlhLWxhYmVsbGVkYnk9InNlYXJjaCI+CiAgICAgICAgICAgICAgICAgICAgICAgICAgICA8dXNlIHhsaW5rOmhyZWY9IiNuMjQtY2xvc2UiIC8+CiAgICAgICAgICAgICAgICAgICAgICAgIDwvc3ZnPg==)](#)

- [![](data:image/svg+xml;base64,PHN2ZyBjbGFzcz0ibjI0LWljb24gbmF2LXNlYXJjaC1pY29uIj4KICAgICAgICAgICAgICAgICAgICAgICAgICAgIDx1c2UgeGxpbms6aHJlZj0iI24yNC1zZWFyY2giIC8+CiAgICAgICAgICAgICAgICAgICAgICAgIDwvc3ZnPg==)](#)

- [![](data:image/svg+xml;base64,)](https://www.nvidia.com/en-sg/ "Artificial Intelligence Computing Leadership from NVIDIA")

- 0 [![](data:image/svg+xml;base64,PHN2ZyBjbGFzcz0ibjI0LWljb24gbnZjYXJ0aWNvbiBuMjQtY2FydCIgc3R5bGUgYXJpYS1sYWJlbGxlZGJ5PSJzaG9wcGluZ19jYXJ0Ij4KICAgICAgICAgICAgICAgICAgICAgICAgICAgIDx1c2UgeGxpbms6aHJlZj0iI24yNC1jYXJ0IiAvPgogICAgICAgICAgICAgICAgICAgICAgICA8L3N2Zz4=)](/)

- [![](data:image/svg+xml;base64,) US](#)

-  ![](data:image/svg+xml;base64,PHN2ZyBjbGFzcz0ibjI0LWljb24gbWVnYS1uYXYtY2xvc2UtbWVudS1pY29uIj48dXNlIHhsaW5rOmhyZWY9IiNuMjQtY2xvc2UiIC8+PC9zdmc+)

- [](# "Sign In")
  Sign In

  [](https://www.nvidia.com/en-sg/account/edit-profile/)

  NVIDIA Account

  [](/)

  Logout

- [Login](#) [LogOut](#)

![](data:image/svg+xml;base64,)

[Cloud and Data Center](https://www.nvidia.com/en-sg/data-center/)

Solutions

[](https://www.nvidia.com/en-sg/data-center/solutions/accelerated-computing/)

Accelerated Computing

Accelerate enterprise IT and AI workloads with data center solutions

[](https://www.nvidia.com/en-sg/data-center/ai-data-platform/)

AI Data Platform for Enterprise

Unify and accelerate enterprise AI workloads with a full-stack data platform

[](https://www.nvidia.com/en-sg/solutions/ai-factories/)

AI Factory

Accelerate and deploy large-scale AI with full-stack factory solutions

[](https://www.nvidia.com/en-sg/data-center/gpu-cloud-computing/)

Cloud Computing

Unleash enterprise AI and HPC with scalable accelerated cloud solutions

[](https://www.nvidia.com/en-sg/edge-computing/)

Edge Computing

Accelerate AI, robotics, and IoT with scalable edge computing

[](https://www.nvidia.com/en-sg/high-performance-computing/)

High Performance Computing

Advance discovery with energy‑efficient high‑performance computing

[](https://www.nvidia.com/en-sg/data-center/sustainable-computing/)

Sustainable Computing

Save energy and reduce costs with sustainable, efficient computing

[](https://www.nvidia.com/en-sg/data-center/solutions/mlops/)

MLOps

Scale AI solutions with enterprise MLOps tools, automation, and software

[](https://www.nvidia.com/en-sg/networking/)

Networking

Networking for high-performance, scalable, and secure AI data centers

[](https://www.nvidia.com/en-sg/data-center/dgx-platform/)

DGX Platform

Build next-generation AI factories for enterprises

[](https://www.nvidia.com/en-sg/data-center/dgx-cloud/)

DGX Cloud

NVIDIA’s AI factory in the cloud

Products

[](https://www.nvidia.com/en-sg/data-center/products/)

Overview

Accelerate AI, machine learning, and HPC with advanced data center platforms

[](https://www.nvidia.com/en-sg/data-center/vera-rubin-nvl72/)

Vera Rubin NVL72

Rack-Scale Agentic AI Supercomputer

[](https://www.nvidia.com/en-sg/data-center/lpx/)

LPX

Fast inference with full intelligence at scale

[](https://www.nvidia.com/en-sg/data-center/dgx-superpod/)

DGX SuperPOD

Full-stack blueprint for gigascale AI infrastructure

[](https://www.nvidia.com/en-sg/data-center/gb300-nvl72/)

GB300 NVL72

GB300 NVL72 rack-scale system built for the age of AI reasoning

[](https://www.nvidia.com/en-sg/data-center/hgx/)

HGX

HGX platform specs for multi‑GPU AI, HPC, and simulation workloads

[](https://www.nvidia.com/en-sg/data-center/grace-cpu/)

CPU

Architecture for data centers that transform data into intelligence

[](https://www.nvidia.com/en-sg/data-center/gb200-nvl72/)

GB200 NVL72

GB200 NVL72 for scalable LLM inference and advanced AI data center needs

[](https://www.nvidia.com/en-sg/data-center/products/rtx-pro-server/)

RTX PRO Server

Universal data center platform for enterprise AI and industrial AI

[](https://www.nvidia.com/en-sg/products/workstations/professional-desktop-gpus/rtx-pro-6000-family/)

RTX PRO 6000 Blackwell

RTX PRO 6000 specs for professional AI, graphics, and visual computing tasks

[](https://www.nvidia.com/en-sg/data-center/dgx-basepod/)

DGX BasePOD

Reference architecture for building and scaling AI infrastructure

[](https://www.nvidia.com/en-sg/data-center/products/dsx/)

DSX Platform

Build AI factories optimized for lowest cost tokens per megawatt

Software

[](https://www.nvidia.com/en-sg/data-center/products/ai-enterprise/)

AI Enterprise Suite

Enterprise AI software suite for data center deployments

[](https://www.nvidia.com/en-sg/data-center/base-command/manager/)

Base Command Manager

Centralized platform to manage and monitor AI workloads in data centers

[](https://www.nvidia.com/en-sg/technologies/cuda-x/)

CUDA-X

GPU-accelerated libraries and tools for AI, HPC, and data processing

[](https://www.nvidia.com/en-sg/data-center/virtual-solutions/)

Virtualization

GPU virtualization software to accelerate and manage virtualized environments

[](https://www.nvidia.com/en-us/accelerated-applications/)

Accelerated Apps Catalog

Browse DPU- and GPU-accelerated apps, tools, and services

[](https://www.nvidia.com/en-sg/data-center/mission-control/)

Mission Control

Powers AI factory operations from developer workloads to infrastructure

[](https://www.nvidia.com/en-sg/software/run-ai/)

NVIDIA Run:ai

AI Workload and GPU Orchestration

Architectures

[](https://www.nvidia.com/en-sg/data-center/technologies/blackwell-architecture/)

Blackwell Architecture

The GPU architecture powering AI factories for the age of AI reasoning

[](https://www.nvidia.com/en-sg/data-center/technologies/hopper-architecture/)

Hopper Architecture

Supercharging AI and HPC workloads for every data center

[](https://www.nvidia.com/en-sg/technologies/ada-architecture/)

Ada Lovelace Architecture

Real-time ray tracing and AI for advanced design and visualization

[](https://www.nvidia.com/en-sg/data-center/products/mgx/)

MGX Reference Architecture

Modular reference architecture for building accelerated data center servers

Technologies

[](https://www.nvidia.com/en-sg/data-center/nvlink/)

NVLink and NVLink Switch

High-speed interconnect for multi-GPU communication and large AI models

[](https://www.nvidia.com/en-sg/data-center/nvlink-fusion/)

NVLink Fusion

Semi-custom AI infrastructure with industry-proven AI scale-up performance

[](https://www.nvidia.com/en-sg/data-center/tensor-cores/)

Tensor Cores

Specialized GPU cores that speed up AI and HPC math workloads

[](https://www.nvidia.com/en-sg/technologies/multi-instance-gpu/)

Multi-Instance GPUs

GPU partitioning to securely share one GPU across workloads

[](https://www.nvidia.com/en-sg/data-center/solutions/confidential-computing/)

Confidential Computing

Secure data and AI models in use

Resources

[](https://www.nvidia.com/en-sg/data-center/resources/)

Resource Hub

Resources to build and optimize AI-ready data centers and factories

[](https://docs.nvidia.com/data-center-gpu/line-card.pdf)

Data Center GPU Line Card

Guide to selecting data center GPUs and networking for AI workloads

[](https://resources.nvidia.com/l/en-us-gpu)

Data Center GPUs Resource Center

Documentation and guides for NVIDIA data center GPU products

[](https://www.nvidia.com/en-sg/data-center/resources/mlperf-benchmarks/)

MLPerf Benchmarks

MLPerf training and inference results for the NVIDIA AI platform

[](https://www.nvidia.com/en-sg/data-center/products/certified-systems/)

NVIDIA-Certified Systems

Accelerated partner systems delivering certified performance and scale

[](https://marketplace.nvidia.com/en-us/enterprise/qualified-system-catalog/)

Qualified System Catalog

Catalog of qualified servers built with NVIDIA data center GPUs

This site requires Javascript in order to view all its content. Please enable Javascript in order to access all the functionality of this web site. Here are the [instructions how to enable JavaScript in your web browser.](http://www.enable-javascript.com/)

![NVIDIA H100 Tensor Core GPU](data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mNkYAAAAAYAAjCB0C8AAAAASUVORK5CYII= "NVIDIA H100 Tensor Core GPU")

# NVIDIA H100 GPU

Extraordinary performance, scalability, and security for every data center.

[View Datasheet](https://resources.nvidia.com/en-us-hopper-architecture/nvidia-tensor-core-gpu-datasheet)

- 

- [Introduction](#introduction)

- [Workloads](#workloads)

- [H100 NVL](#h100-nvl)

- [NVIDIA Enterprise](#bm-uf)

- [Specifications](#specifications)

- [Introduction](#introduction)
- [Workloads](#workloads)
- [H100 NVL](#h100-nvl)
- [NVIDIA Enterprise](#bm-uf)
- [Specifications](#specifications)

![](data:image/svg+xml;base64,)

- [Introduction](#introduction)
- [Workloads](#workloads)
- [H100 NVL](#h100-nvl)
- [NVIDIA Enterprise](#bm-uf)
- [Specifications](#specifications)

## An Order-of-Magnitude Leap for Accelerated Computing

The NVIDIA H100 GPU delivers exceptional performance, scalability, and security for every workload. H100 uses breakthrough innovations based on the [NVIDIA Hopper™ architecture](https://www.nvidia.com/en-us/data-center/technologies/hopper-architecture/) to deliver industry-leading conversational AI, speeding up large language models (LLMs) by 30X. H100 also includes a dedicated Transformer Engine to solve trillion-parameter language models.

[ Read NVIDIA H100 Datasheet](https://resources.nvidia.com/en-us-hopper-architecture/nvidia-tensor-core-gpu-datasheet)

[ NVIDIA H100 NVL Product Brief](/content/dam/en-zz/Solutions/Data-Center/h100/PB-11773-001_v01.pdf)

## Securely Accelerate Workloads From Enterprise to Exascale

### Up to 4X Higher AI Training on GPT-3

![Up to 4X Higher AI Training on GPT-3](/content/nvidiaGDC/sg/en_SG/data-center/h100/_jcr_content/root/responsivegrid/nv_container_227392685/nv_container/nv_container/nv_image.coreimg.svg/1761542426118/nvidia-h100-workload-training.svg "Up to 4X Higher AI Training on GPT-3")

Projected performance subject to change.  GPT-3 175B training A100 cluster: HDR IB network, H100 cluster: NDR IB network \| Mixture of Experts (MoE) Training Transformer Switch-XXL variant with 395B parameters on 1T token dataset,  A100 cluster: HDR IB network, H100 cluster: NDR IB network with NVLink Switch System where indicated.

## Transformational AI Training

H100 features fourth-generation Tensor Cores and a Transformer Engine with FP8 precision that provides up to 4X faster training over the prior generation for GPT-3 (175B) models. The combination of fourth-generation NVLink, which offers 900 gigabytes per second (GB/s) of GPU-to-GPU interconnect; NDR Quantum-2 InfiniBand networking, which accelerates communication by every GPU across nodes; PCIe Gen5; and [NVIDIA Magnum IO™](https://www.nvidia.com/en-sg/data-center/magnum-io/) software delivers efficient scalability from small enterprise systems to massive, unified GPU clusters.

Deploying H100 GPUs at data center scale delivers outstanding performance and brings the next generation of exascale high-performance computing (HPC) and trillion-parameter AI within the reach of all researchers.

[Experience NVIDIA AI and NVIDIA H100 on NVIDIA LaunchPad](https://www.nvidia.com/en-sg/launchpad/ai/tuning-and-deploying-a-language-model-on-h100/)

## Real-Time Deep Learning Inference

AI solves a wide array of business challenges, using an equally wide array of neural networks. A great AI inference accelerator has to not only deliver the highest performance but also the versatility to accelerate these networks.

H100 extends NVIDIA’s market-leading inference leadership with several advancements that accelerate inference by up to 30X and deliver the lowest latency. Fourth-generation Tensor Cores speed up all precisions, including FP64, TF32, FP32, FP16, INT8, and now FP8, to reduce memory usage and increase performance while still maintaining accuracy for LLMs.

### Up to 30X Higher AI Inference Performance on the Largest Models

Megatron chatbot inference (530 billion parameters)

![Real-Time Deep Learning Inference](/content/nvidiaGDC/sg/en_SG/data-center/h100/_jcr_content/root/responsivegrid/nv_container_416312871/nv_container_326104294/nv_image.coreimg.svg/1761542426325/nvidia-h100-workload-inference-2c50-d.svg "Real-Time Deep Learning Inference")

Projected performance subject to change. Inference on Megatron 530B parameter model based chatbot for input sequence length=128, output sequence length =20 \| A100 cluster: HDR IB network \| H100 cluster: NVLink Switch System, NDR IB

### Up to 7X Higher Performance for HPC Applications

![AI-fused HPC Applications](/content/nvidiaGDC/sg/en_SG/data-center/h100/_jcr_content/root/responsivegrid/nv_container_700535989/nv_container/nv_image.coreimg.svg/1761542426414/nvidia-h100-workload-hpc-2c50-d.svg "AI-fused HPC Applications")

Projected performance subject to change. 3D FFT (4K^3) throughput \| A100 cluster: HDR IB network \| H100 cluster: NVLink Switch System, NDR IB \| Genome Sequencing (Smith-Waterman) \| 1 A100 \| 1 H100

## Exascale High-Performance Computing

The NVIDIA data center platform consistently delivers performance gains beyond Moore’s law. And H100’s new breakthrough AI capabilities further amplify the power of HPC+AI to accelerate time to discovery for scientists and researchers working on solving the world’s most important challenges.

H100 triples the floating-point operations per second (FLOPS) of double-precision Tensor Cores, delivering 60 teraflops of FP64 computing for HPC. AI-fused HPC applications can also leverage H100’s TF32 precision to achieve one petaflop of throughput for single-precision matrix-multiply operations, with zero code changes. 

H100 also features new DPX instructions that deliver 7X higher performance over A100 and 40X speedups over CPUs on dynamic programming algorithms such as Smith-Waterman for DNA sequence alignment and protein alignment for protein structure prediction.

[Review Latest GPU Performance on HPC Applications](https://developer.nvidia.com/hpc-application-performance)

DPX instructions comparison NVIDIA HGX™ H100 4-GPU vs dual socket 32-core IceLake.

## Accelerated Data Analytics

Data analytics often consumes the majority of time in AI application development. Since large datasets are scattered across multiple servers, scale-out solutions with commodity CPU-only servers get bogged down by a lack of scalable computing performance.

Accelerated servers with H100 deliver the compute power—along with 3 terabytes per second (TB/s) of memory bandwidth per GPU and scalability with NVLink and NVSwitch™—to tackle data analytics with high performance and scale to support massive datasets. Combined with NVIDIA Quantum-2 InfiniBand, Magnum IO software, GPU-accelerated Spark 3.0, and [NVIDIA RAPIDS™](https://www.nvidia.com/en-sg/deep-learning-ai/software/rapids/), the NVIDIA data center platform is uniquely able to accelerate these huge workloads with higher performance and efficiency.

![Accelerated servers with H100](/content/nvidiaGDC/sg/en_SG/data-center/h100/_jcr_content/root/responsivegrid/nv_container_4163128/nv_image.coreimg.jpeg/1761542426560/h100-tensor-core-data-analytics-2c50-d-2x.jpeg "Accelerated servers with H100")

![NVIDIA Multi-Instance GPU](/content/nvidiaGDC/sg/en_SG/data-center/h100/_jcr_content/root/responsivegrid/nv_container_739282046/nv_image.coreimg.jpeg/1761542426611/hopper-mig-2c50-d-2x.jpeg "NVIDIA Multi-Instance GPU")

## Enterprise-Ready Utilization

IT managers seek to maximize utilization (both peak and average) of compute resources in the data center. They often employ dynamic reconfiguration of compute to right-size resources for the workloads in use. 

H100 with MIG lets infrastructure managers standardize their GPU-accelerated infrastructure while having the flexibility to provision GPU resources with greater granularity to securely provide developers the right amount of accelerated compute and optimize usage of all their GPU resources.

[Learn More About MIG](https://www.nvidia.com/en-sg/technologies/multi-instance-gpu/)

## Built-In Confidential Computing

Traditional Confidential Computing solutions are CPU-based, which is too limited for compute-intensive workloads such as AI at scale. NVIDIA Confidential Computing is a built-in security feature of the [NVIDIA Hopper architecture](https://www.nvidia.com/en-sg/data-center/technologies/hopper-architecture/) that made H100 the world’s first accelerator with these capabilities. With [NVIDIA Blackwell](https://www.nvidia.com/en-sg/data-center/technologies/blackwell-architecture/), the opportunity to exponentially increase performance while protecting the confidentiality and integrity of data and applications in use has the ability to unlock data insights like never before. Customers can now use a hardware-based trusted execution environment (TEE) that secures and isolates the entire workload in the most performant way.

[Learn More About NVIDIA Confidential Computing](https://www.nvidia.com/en-sg/data-center/solutions/confidential-computing/)

![NVIDIA Confidential Computing](/content/nvidiaGDC/sg/en_SG/data-center/h100/_jcr_content/root/responsivegrid/nv_container_934925062/nv_image.coreimg.jpeg/1761542426751/hopper-h100-confidential-computing-2c50-d-2x.jpeg "NVIDIA Confidential Computing")

![NVIDIA Grace Hopper™ Superchip](/content/nvidiaGDC/sg/en_SG/data-center/h100/_jcr_content/root/responsivegrid/nv_container_177275295/nv_image.coreimg.jpeg/1761542426803/hopper-h100-grace-hopper-2c50-d-2x.jpeg "NVIDIA Grace Hopper™ Superchip")

## Exceptional Performance for Large-Scale AI and HPC

The Hopper GPU will power the NVIDIA Grace Hopper™ CPU+GPU architecture, purpose-built for terabyte-scale accelerated computing and providing 10X higher performance on large-model AI and HPC. The NVIDIA Grace CPU leverages the flexibility of the Arm® architecture to create a CPU and server architecture designed from the ground up for accelerated computing. The Hopper GPU is paired with the Grace CPU using NVIDIA’s ultra-fast chip-to-chip interconnect, delivering 900GB/s of bandwidth, 7X faster than PCIe Gen5. This innovative design will deliver up to 30X higher aggregate system memory bandwidth to the GPU compared to today's fastest servers and up to 10X higher performance for applications running terabytes of data.

[Learn More About NVIDIA Grace](https://www.nvidia.com/en-sg/data-center/grace-cpu/)

![Supercharge Large Language Model Inference with H100 NVL](/content/nvidiaGDC/sg/en_SG/data-center/h100/_jcr_content/root/responsivegrid/nv_container_1820991/nv_image.coreimg.jpeg/1761645622550/hpc-web-gtc23-h100-2560x1440.jpeg "Supercharge Large Language Model Inference with H100 NVL")

## Supercharge Large Language Model Inference With H100 NVL

For LLMs up to 70 billion parameters (Llama 2 70B), the PCIe-based NVIDIA H100 NVL with NVLink bridge utilizes Transformer Engine, NVLink, and 188GB HBM3 memory to provide optimum performance and easy scaling across any data center, bringing LLMs to the mainstream. Servers equipped with H100 NVL GPUs increase Llama 2 70B performance up to 5x over NVIDIA A100 systems while maintaining low latency in power-constrained data center environments.

![](data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mNkYAAAAAYAAjCB0C8AAAAASUVORK5CYII=)

## Enterprise-Ready: AI Software Streamlines Development and Deployment

NVIDIA H100 NVL is bundled with a five-year [NVIDIA Enterprise](https://www.nvidia.com/en-sg/data-center/products/ai-enterprise/) subscription. This subscription includes NVIDIA AI Enterprise to simplify the way you build an enterprise AI-ready platform. H100 accelerates AI development and deployment for production-ready generative AI solutions, including computer vision, speech AI, retrieval augmented generation (RAG), and more. NVIDIA AI Enterprise includes [NVIDIA NIM](https://www.nvidia.com/en-sg/ai/)^(TM), a set of easy-to-use microservices designed to speed up enterprise generative AI deployment. Together, deployments have enterprise-grade security, manageability, stability, and support. This results in performance-optimized AI solutions that deliver faster business value and actionable insights.

[Activate Your NVIDIA AI Enterprise License](https://www.nvidia.com/en-sg/data-center/activate-license/)

## Product Specifications

[TABLE]

  
\* With sparsity

Take a deep dive into the NVIDIA Hopper architecture.

[Read Whitepaper](https://resources.nvidia.com/en-us-hopper-architecture/nvidia-h100-tensor-c)

Products

- [Data Center GPUs](https://resources.nvidia.com/l/en-us-gpu)
- [NVIDIA DGX Platform](https://www.nvidia.com/en-sg/data-center/dgx-platform/)
- [NVIDIA HGX Platform](https://www.nvidia.com/en-sg/data-center/hgx/)
- [Networking Products](https://www.nvidia.com/en-sg/networking/products/)
- [Virtual GPUs](https://www.nvidia.com/en-sg/data-center/virtual-solutions/)

Technologies

- [NVIDIA Blackwell Architecture](https://www.nvidia.com/en-sg/data-center/technologies/blackwell-architecture/)
- [NVIDIA Hopper Architecture](https://www.nvidia.com/en-sg/data-center/technologies/hopper-architecture/)
- [MGX](https://www.nvidia.com/en-sg/data-center/products/mgx/)
- [Confidential Computing](https://www.nvidia.com/en-sg/data-center/solutions/confidential-computing/)
- [Multi-Instance GPU](https://www.nvidia.com/en-sg/technologies/multi-instance-gpu/)
- [NVLink-C2C](https://www.nvidia.com/en-sg/data-center/nvlink-c2c/)
- [NVLink/NVSwitch](https://www.nvidia.com/en-sg/data-center/nvlink/)
- [Tensor Cores](https://www.nvidia.com/en-sg/data-center/tensor-cores/)

Resources

- [Accelerated Apps Catalog](https://www.nvidia.com/en-us/accelerated-applications/)
- [Blackwell Resources Center](https://resources.nvidia.com/l/en-us-blackwell-architecture)
- [Data Center GPUs](https://www.nvidia.com/en-sg/data-center/technologies/rubin/)
- [Data Center GPU Line Card](https://docs.nvidia.com/data-center-gpu/line-card.pdf)
- [Data Center GPUs Resource Center](https://www.nvidia.com/en-sg/data-center/tesla-product-literature/)
- [Data Center Product Performance](https://developer.nvidia.com/deep-learning-performance-training-inference)
- [Deep Learning Institute](https://www.nvidia.com/en-sg/training/)
- [Energy Efficiency Calculator](https://www.nvidia.com/en-us/data-center/sustainable-computing/energy-efficiency-calculator)
- [GPU Cloud Computing](https://www.nvidia.com/en-sg/data-center/gpu-cloud-computing/)
- [MLPerf Benchmarks](https://www.nvidia.com/en-sg/data-center/resources/mlperf-benchmarks/)
- [NGC Catalog](https://www.nvidia.com/en-sg/gpu-cloud/)
- [NVIDIA-Certified Systems](https://www.nvidia.com/en-sg/data-center/products/certified-systems/)
- [NVIDIA Data Center Corporate Blogs](https://blogs.nvidia.com/blog/category/enterprise/)
- [NVIDIA Data Center Technical Blogs](https://developer.nvidia.com/blog/category/data-center-cloud/)
- [Qualified System Catalog](https://www.nvidia.com/en-sg/data-center/data-center-gpus/tesla-qualified-servers-catalog/)
- [Where to Buy](https://www.nvidia.com/en-sg/data-center/where-to-buy/)

Company Info

- [About Us](https://www.nvidia.com/en-sg/about-nvidia/)
- [Company Overview](https://www.nvidia.com/en-sg/about-nvidia/)
- [Investors](https://investor.nvidia.com/home/default.aspx)
- [Venture Capital (NVentures)](https://www.nvidia.com/en-us/startups/nventures/)
- [NVIDIA Foundation](https://www.nvidia.com/en-sg/foundation/)
- [Research](https://www.nvidia.com/en-us/research/)
- [Social Responsibility](https://www.nvidia.com/en-us/sustainability/)
- [Technologies](https://www.nvidia.com/en-sg/technologies/)
- [Careers](https://www.nvidia.com/en-us/about-nvidia/careers/)

Follow Data Center

[](http://www.facebook.com/NVIDIA "<util:I18n key="Follow GeForce on Facebook" />") [](https://www.linkedin.com/company/nvidia) [](http://twitter.com/nvidiadc "<util:I18n key="Follow GeForce on Twitter" />") [](http://www.youtube.com/user/nvidia)

![](data:image/svg+xml;base64,)

[ Singapore ](https://www.nvidia.com/object/country-selector.html)

- [Privacy Policy](https://www.nvidia.com/en-sg/about-nvidia/privacy-policy/)
- [Your Privacy Choices](https://www.nvidia.com/en-sg/about-nvidia/privacy-center/)
- [Legal](https://www.nvidia.com/en-sg/legal-info/)
- [Accessibility](https://www.nvidia.com/en-sg/about-nvidia/accessibility/)
- [Corporate Policies](https://www.nvidia.com/en-sg/about-nvidia/company-policies/)
- [Product Security](https://www.nvidia.com/en-us/security/)
- [Contact](https://www.nvidia.com/en-sg/contact/)

Copyright © 2026 NVIDIA Corporation
