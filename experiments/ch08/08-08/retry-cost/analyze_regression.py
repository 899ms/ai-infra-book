...
r=Path(__file__).absolute().parent;p=r/'results-regression';env=json.loads((p/'environment.json').read_text())
...
inp=json.loads((p/'inputs.json').read_text());tasks={x['id']:x for x in inp['tasks']};assert len(tasks)==4
for t in tasks.values():
 ...
 assert t['seed']==808512+int(t['id'].split('-')[1])
...
t=[json.loads(x) for x in (p/'tasks.jsonl').read_text().splitlines()];assert len(t)==8
assert [x['task_id'] for x in t]==[f'known-{i:02d}' for order in inp['orders'] for i in order]
...
summary=dict(repeat_output_differences=repeat_differences,tasks=8,distinct_documents=4,first_correct=sum(x['first_correct'] for x in rows),final_correct=sum(x['final_correct'] for x in rows),
 ...
 records=rows,scope='Known regression documents, not heldout evaluation; known-answer synthetic validator; includes actual failed attempts, switching and observation RPC overhead; excludes engine initialization, calibration and warmup. Serial tasks on shared GPU.')
