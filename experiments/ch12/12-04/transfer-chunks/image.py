...
SOURCE=R.parent/'image-records'
MODE=os.environ['BOOK_CHUNK_MODE'];WINDOW=None;CHUNK={'whole':0,'16k':16384,'64k':65536}[MODE]
FIXTURE=json.loads((SOURCE/'image-fixture.json').read_text());PAYLOAD=(SOURCE/FIXTURE['upload_file']).read_bytes();RESPONSE=(SOURCE/FIXTURE['response_file']).read_bytes();HASH=hashlib.sha256(PAYLOAD).hexdigest();RESPONSE_HASH=hashlib.sha256(RESPONSE).hexdigest();SERVICE_S=FIXTURE['service_s']
...
class H3(QuicConnectionProtocol):
 ...
 async def reply(self,stream,record):
  ...
  self.http.send_headers(stream,[(b':status',b'200'),(b'content-length',str(len(RESPONSE)).encode()),(b'content-type',b'image/jpeg')])
...
async def main(out,smoke):
 ...
 async def h1_server(reader,writer):
  ...
  try:
   while True:
    ...
    record['service_start']=NOW();await asyncio.sleep(SERVICE_S);record['service_end']=NOW()
    writer.write(conn.send(h11.Response(status_code=200,headers=[(b'content-length',str(len(RESPONSE)).encode()),(b'content-type',b'image/jpeg')])))
 ...
 env=dict(python=platform.python_version(),platform=platform.platform(),machine=platform.machine(),openssl=ssl.OPENSSL_VERSION,packages={p:importlib.metadata.version(p) for p in ['aioquic','h11','cryptography','pylsqpack']},certificate_sha256=fingerprint,source_sha256=hashlib.sha256(Path(__file__).read_bytes()).hexdigest(),mode=MODE,chunk_bytes=CHUNK,requested_window=WINDOW,listener_settings=listener_settings,quic_max_data=qc.max_data,quic_max_stream_data=qc.max_stream_data,tcp_port=tcp_port,udp_port=udp_port,payload_bytes=len(PAYLOAD),payload_sha256=HASH,response_bytes=len(RESPONSE),response_sha256=RESPONSE_HASH,service_time_replay_s=SERVICE_S,fixture=FIXTURE,smoke=smoke,server_and_client_same_event_loop=True,body_request_deadline_s=REQUEST_TIMEOUT)
   ...
   for index in range(lane_id,count,concurrency):
    ...
    try:
     ...
     with Image.open(io.BytesIO(data)) as im:
      im.load();rgb=im.convert('RGB');shape=[rgb.height,rgb.width,3];pixel_hash=hashlib.sha256(rgb.tobytes()).hexdigest()
     decode_end=NOW();rec.update(decoded_shape=shape,decoded_sha256=pixel_hash,decode_start=decode_start,decode_end=decode_end,connection=conn['record']['id'],fresh=fresh,connection_ready=conn['record']['ready'] if fresh else rec['start'],send=send,stream=stream,**result,bytes=len(data),sha256=hashlib.sha256(data).hexdigest(),valid=(data==RESPONSE and shape==FIXTURE['shape'] and pixel_hash==FIXTURE['decoded_sha256'] and dict(result['headers']).get(':status')=='200'),validation_end=NOW(),error=None)
