...
for k in range(3):
 a.broken_barh([(10*k,9)],(2.7,.5),facecolors=C['blue']);a.text(10*k+4.5,2.95,f'模型呼叫 {k+1}',color='white',ha='center',va='center',fontsize=12)
...
rows=[("按需建立",4,6),("提前 1 s",3,5),("提前 2 s",2,4),("提前 3 s",1,3)]
for i,(label,start,end) in enumerate(rows):
 ...
 a.text(6.15,y,["等待 2 s","等待 1 s","無需等待","多駐留 1 s"][i],va='center',fontsize=12)
a.axvline(4,color=C['red'],ls='--',lw=1.4);a.text(4,3.55,"呼叫到達",ha='center',fontsize=12,color=C['red'])
a.set(yticks=[3,2,1,0],yticklabels=[x[0] for x in rows],xticks=list(range(7)),xlim=(0,6),ylim=(-.5,3.9),xlabel="時間 / s");a.spines['left'].set_visible(False);a.tick_params(axis='y',length=0);a.grid(axis='x',alpha=.15)
a.plot([],[],color=C['blue'],lw=8,label="準備");a.plot([],[],color=C['orange'],lw=8,label="就緒未使用");a.legend(frameon=False,loc='upper left',ncol=2,fontsize=11)
...
f,a=canvas(6)
a.text(.5,.92,"新作業：4 張 H100 + 16 核 CPU，同一節點",ha='center',fontsize=16,weight='bold')
for x,title in [(.04,"整理前"),(.55,"整理後")]:
 a.text(x+.19,.79,title,ha='center',fontsize=14,weight='bold')
 if x<.5:
  box(a,x,.48,.37,.22,"節點 1 DGX H100：4 張 H100 空閒","僅 8 核可用 → 無法啟動",'sand')
  box(a,x,.15,.37,.22,"節點 2 DGX A100：4 張 A100 空閒","32 核可用 → 卡型不符",'pale')
 else:
  box(a,x,.48,.37,.22,"節點 1 DGX H100：4 張 H100 空閒","16 核可用 → 整組啟動",'green')
  box(a,x,.15,.37,.22,"節點 2 DGX A100：4 張 A100 空閒","24 核可用 + 接收遷移任務",'pale')
arrow(a,(.43,.57),(.53,.57));a.text(.48,.39,"遷移\n8 核任務",ha='center',fontsize=11)
a.text(.5,.055,"狀態傳輸與恢復 4 s；等待釋放 12 s；新作業本地執行 20 s",ha='center',fontsize=12,color=C['muted'])
...
for y,starts in [(1.3,[0,10,20]),(.3,[20,30,40])]:
 for k,start in enumerate(starts):
  a.broken_barh([(start,10)],(y,.5),facecolors=cols[k]);a.text(start+5,y+.25,f'樣本 {k+1}',ha='center',va='center',color='white',fontsize=12)
for k,t in enumerate([0,10,20]):a.annotate(f'樣本 {k+1} 到達',(t,2.05),(t,2.55),arrowprops={'arrowstyle':'-|>','color':C['muted']},ha='center',fontsize=11)
a.set(yticks=[.55,1.55],yticklabels=["整批後提交","逐條提交"],xlim=(-1,51),ylim=(0,2.9),xticks=[0,10,20,30,40,50],xlabel="時間 / s");a.spines['left'].set_visible(False);a.tick_params(axis='y',length=0);a.grid(axis='x',alpha=.18)
...
f,a=canvas(6)
box(a,.02,.51,.17,.22,"任務控制器","品質與期限")
box(a,.30,.51,.21,.22,"服務入口","鑑權、路由、限流")
box(a,.69,.69,.27,.21,"外部模型 API","佇列、快取、生成",'pale')
box(a,.69,.26,.27,.21,"自建模型副本","載入、佇列、執行",'green')
arrow(a,(.19,.62),(.30,.62));arrow(a,(.51,.67),(.69,.79));arrow(a,(.51,.55),(.69,.37))
a.text(.56,.83,"呼叫賬單",ha='center',fontsize=11);a.text(.57,.35,"裝置預留成本",ha='center',fontsize=11)
box(a,.15,.045,.48,.21,"記錄每項任務及各次嘗試的成本","普通輸入 / 建立 / 讀取 / 生成 / 其他成本",'sand',12)
arrow(a,(.40,.51),(.40,.255));arrow(a,(.81,.26),(.63,.16))
a.text(.07,.86,"請求與結果沿各自路徑往返",fontsize=12,color=C['muted'])
...
a.plot(h*100,cb,color=C['blue'],label="B Sonnet 5：命中率變化",lw=2.3);a.axhline(ca,color=C['orange'],label="A Haiku 4.5：字首全命中，10 s 超過期限",lw=2)
...
body=re.sub("<p>(<img [^>]+>)</p>\\s*<p><em>(圖 11-[\\s\\S]*?)</em></p>",r'<figure>\1<figcaption>\2</figcaption></figure>',body)
...
page="<!doctype html><html lang=\"zh-CN\"><meta charset=\"utf-8\"><meta name=\"viewport\" content=\"width=device-width,initial-scale=1\"><title>第 11 章 資源排程與執行環境</title><style>"+css+math_css+'</style><main><nav>'+nav+'</nav>'+body+'</main></html>'
import sys
sys.path.insert(0,str(HERE.parent))
from teaching_reading import readable_diagrams
from preview_output import preview_path
page=readable_diagrams(page)
html_path=preview_path(HERE.parent/"11-資源排程與執行環境.html");html_path.write_text(page)
