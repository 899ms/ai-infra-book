...
def main():
    ...
    save(O/'environment.json',dict(runid=runid,base_image_id=base,
        ...
        source_hashes={n:hashlib.sha256((R/n).read_bytes()).hexdigest() for n in ['run.py','worker.py','PROTOCOL.md']}))
    def inspect(cid):
    ...
    def create(label,image=base):
        ...
        row=docker('create','--name',name,'--label','book.experiment='+runid,'--cpus','1','--memory','256m',
            '--memory-swap','256m',image,'python','-u','/tmp/book-worker.py')
        cid=row['stdout'].strip();containers.append(cid)
        docker('cp',str(R/'worker.py'),cid+':/tmp/book-worker.py')
    ...
    def connect(cid):
        ...
        while True:
            try:
                s=socket.create_connection((info['ip'],8042),timeout=2)
                s.settimeout(5);connections.append(s)
                return s,dict(start=start,ready=time.monotonic(),container=info)
            except OSError:
            ...
            if path=='pause':
                ...
                if cp['exit_code']==0:
                    for number in range(2):
                        ...
                        restored=docker('start','--checkpoint-dir',copy,'--checkpoint','snap',new,check=False)
