...
r=Path(__file__).resolve().parent;os.chdir(r);records=[];name='book909-remote-immutable-001';cid=None
...
try:
 ...
 cid=cmd(['docker','create','--name',name,'--label','book.experiment=909-remote-immutable-001','--user',f'{os.getuid()}:{os.getgid()}','--network','host','--cpus','1','--memory','512m','--memory-swap','512m','--read-only','--tmpfs','/tmp','--mount',f'type=bind,source={r},target=/experiment',image,'python','-u','/experiment/store.py','--root','/experiment/docker-store','--port','18799'])
