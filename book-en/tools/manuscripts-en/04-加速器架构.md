# Chapter 4 Accelerator Architecture
...
![Both calls use the same 32 MiB of weights. The top shows a single row of input; the bottom illustrates 256 rows with partial strips; the weight-read amortized per row drops from 32 MiB to 128 KiB.](ch04/figure-4-1-reuse.svg)
*Figure 4-1　Both calls use the same 32 MiB of weights. The top shows a single row of input; the bottom illustrates 256 rows with partial strips; the weight-read amortized per row drops from 32 MiB to 128 KiB.*
...
![Relationship among the host, GPU memory, and on-chip components. Solid lines show data passing through cache, local buffer, matrix unit, and accumulator storage; dashed lines show the host submitting work. The diagram is grouped by hardware function.](ch04/figure-4-2-components.svg)
*Figure 4-2　Relationship among the host, GPU memory, and on-chip components. Solid lines show data passing through cache, local buffer, matrix unit, and accumulator storage; dashed lines show the host submitting work. The diagram is grouped by hardware function.*
...
![Cross-generation co-design of models and hardware. Solid lines run downward through time: existing accelerators influence model choices, software running in production exposes long-term bottlenecks, hardware design responds to these needs, and new accelerators make more model schemes possible.](ch04/figure-4-codesign-loop.svg)
*Figure 4-3　Cross-generation co-design of models and hardware. Solid lines run downward through time: existing accelerators influence model choices, software running in production exposes long-term bottlenecks, hardware design responds to these needs, and new accelerators make more model schemes possible.*
...
![Top: energy per byte for various storage levels and links, log scale. Bottom: the energy breakdown for one decode step of a single Qwen3-8B request with 8K context (weights 0.481 J, KV 0.038 J, compute 0.015 J), and the energy if the same bytes all came from a single level.](ch04/figure-4-energy-ladder.svg)
*Figure 4-5　Top: energy per byte for various storage levels and links, log scale. Bottom: the energy breakdown for one decode step of a single Qwen3-8B request with 8K context (weights 0.481 J, KV 0.038 J, compute 0.015 J), and the energy if the same 16.345 GB all came from a single level.*
...
![Top-down packaging schematic: two reticle-limit dies sit in the center, eight HBM stacks line both edges, and the interposer carries the wiring both between the dies and HBM and between the two dies.](ch04/figure-4-energy-package.svg)
*Figure 4-6　Top-down packaging schematic: two reticle-limit dies sit in the center, eight HBM stacks line both edges, and the interposer carries the wiring both between the dies and HBM and between the two dies. Bandwidth per stack is determined by the 1,024 pins and the pin rate; capacity per stack is determined by layer count and per-layer capacity.*
...
![Ratio of sustained frequency to peak frequency as a function of e/b: b/e when voltage is fixed, (b/e)^(1/3) when voltage drops with frequency. Peak frequency is sustainable when e/b does not exceed 1; at e/b = 1.5 the two curves give 0.67 and 0.87 respectively.](ch04/figure-4-energy-power-cap.svg)
*Figure 4-7　Ratio of sustained frequency to peak frequency as a function of the ratio of energy per FLOP $e$ to budget $b$: $b/e$ when voltage is fixed, $(b/e)^{1/3}$ when voltage drops with frequency. Peak frequency is sustainable when $e/b$ does not exceed 1; at $e/b = 1.5$ the two curves give 0.67 and 0.87 respectively.*
...
![Illustration of a 3×3 multiply-accumulate array. Inputs pass along rows, weights pass along columns, and each multiply-accumulate unit retains its own partial sum. This small array illustrates operand reuse.](ch04/figure-4-matrix-array.svg)
*Figure 4-8　Illustration of a 3×3 multiply-accumulate array. Inputs pass along rows, weights pass along columns, and each multiply-accumulate unit retains its own partial sum. This small array illustrates operand reuse.*
...
![A 16-row compute block for one expert. When an expert has only two rows, the remaining fourteen rows are zero-padded; when an expert has 64 rows, four complete blocks can be formed, and the figure shows one of them.](ch04/figure-4-3-expert-rows.svg)
*Figure 4-9　A 16-row compute block for one expert. When an expert has only two rows, the remaining fourteen rows are zero-padded; when an expert has 64 rows, four complete blocks can be formed, and the figure shows one of them.*

![Execution volume across all experts for the same 512 effective input rows. Spreading across 256 experts results in 4096 rows executed in total, while concentrating on eight experts executes only 512 rows.](ch04/figure-4-expert-padding-total.svg)
*Figure 4-10　Execution volume across all experts for the same 512 effective input rows. Spreading across 256 experts results in 4096 rows executed in total, while concentrating on eight experts executes only 512 rows.*
...
![Service cycles for the same attention block under four resource configurations. Comparing the matrix, shared-memory, and exponentiation items separately, boosting one capability alone may make another resource the new longer item.](ch04/figure-4-4-attention.svg)
*Figure 4-11　Service cycles for the same attention block under four resource configurations. Comparing the matrix, shared-memory, and exponentiation items separately, boosting one capability alone may make another resource the new longer item. "×2" indicates the corresponding resource's throughput capability is doubled; the horizontal axis shows the number of clock cycles needed to complete the same compute block.*
...
![Two computation paths for the same compressed weights. Expanding first produces a 32 MiB BF16 copy; the low-precision path completes scaling and combination during the computation process. The compressed weights and scales together total 8.5 MiB.](ch04/figure-4-5-precision.svg)
*Figure 4-12　Two computation paths for the same compressed weights. Expanding first produces a 32 MiB BF16 copy; the low-precision path completes scaling and combination during the computation process. The compressed weights and scales together total 8.5 MiB.*
...
![Weights, workspace, and KV within the RTX 4090's 24 GB of GPU memory. 8K with four requests and 16K with two requests fit; 8K with five requests exceeds the capacity limit marked by the dashed line.](ch04/figure-4-6-capacity.svg)
*Figure 4-13　Weights, workspace, and KV within the RTX 4090's 24 GB of GPU memory. 8K with four requests and 16K with two requests fit; 8K with five requests exceeds the capacity limit marked by the dashed line.*
...
![In-flight read requests occupy request-record space from issue until they return. Only when multiple independent requests overlap can the interface stay busy during the wait for a single access; the figure depicts only four representative requests.](ch04/figure-4-memory-inflight.svg)

*Figure 4-14　In-flight read requests occupy request-record space from issue until they return. Only when multiple independent requests overlap can the interface stay busy during the wait for a single access; the figure depicts only four representative requests.*
...
![At 128 bytes per transaction and 500 ns return latency, increasing the number of in-flight requests raises the bandwidth ceiling until it hits the RTX 4090's or RTX 5090's own memory interface rate limit.](ch04/figure-4-7-memory.svg)

*Figure 4-15　At 128 bytes per transaction and 500 ns return latency, increasing the number of in-flight requests raises the bandwidth ceiling until it hits the RTX 4090's or RTX 5090's own memory interface rate limit.*
...
![The first 256 bytes of each row form the actually read interval, while adjacent rows start 8192 bytes apart. The 128 rows together read 32 KiB, with the gray region between rows skipped due to the stride.](ch04/figure-4-8-layout.svg)

*Figure 4-16　The first 256 bytes of each row form the actually read interval, while adjacent rows start 8192 bytes apart. The 128 rows together read 32 KiB, with the gray region between rows skipped due to the stride.*
...
![The full lifecycle of one input slot from issue to release. Transfer takes 64 ticks, an additional 128-tick wait follows, the data becomes ready at 192 ticks, computation takes another 128 ticks, and the slot is released at 320 ticks.](ch04/figure-4-slot-lifetime.svg)

*Figure 4-17　The full lifecycle of one input slot from issue to release. Transfer takes 64 ticks, an additional 128-tick wait follows, the data becomes ready at 192 ticks, computation takes another 128 ticks, and the slot is released at 320 ticks. One tick is one clock cycle of a B200 SM.*
...
![Timing of four blocks with one input slot. Blue bars are transfers, orange lines mark readiness, green bars are computation, and light gray marks slot occupancy; the next block can only be issued once the previous one finishes, with completion at 1280 ticks.](ch04/figure-4-9-pipeline.svg)

*Figure 4-18　Timing of four blocks with one input slot. Blue bars are transfers, orange lines mark readiness, green bars are computation, and light gray marks slot occupancy; the next block can only be issued once the previous one finishes, with completion at 1280 ticks. One tick is one clock cycle of a B200 SM.*

![Two input slots on the same time scale. The first two blocks can be issued early, but the third block isn't ready until 512 ticks, while the second block already finished at 448 ticks, leaving 64 ticks idle.](ch04/figure-4-pipeline-two.svg)

*Figure 4-19　Two input slots on the same time scale. The first two blocks can be issued early, but the third block isn't ready until 512 ticks, while the second block already finished at 448 ticks, leaving 64 ticks idle. On the horizontal axis, one tick is one clock cycle of a B200 SM; each row corresponds to a data block, gray marks input slot occupancy, blue marks transfer, green marks computation, and the vertical tick marks data readiness.*
...
![Three input slots allow the first three blocks to be issued early; once the first slot is released, it takes on the fourth block. The matrix unit computes continuously from 192 to 704 ticks; a fourth slot would not shorten the completion time further.](ch04/figure-4-pipeline-three.svg)

*Figure 4-20　Three input slots allow the first three blocks to be issued early; once the first slot is released, it takes on the fourth block. The matrix unit computes continuously from 192 to 704 ticks; a fourth slot would not shorten the completion time further. On the horizontal axis, one tick is one clock cycle of a B200 SM; each row corresponds to a data block, gray marks input slot occupancy, blue marks transfer, green marks computation, and the vertical tick marks data readiness.*
...
![The matrix and vector units hand off via complete row groups. QK produces scores, softmax produces probabilities, and PV releases the slot after consuming the probabilities; a second slot holds the adjacent row group, letting different groups advance with overlap.](ch04/figure-4-matrix-vector-handoff.svg)
*Figure 4-21 The matrix and vector units hand off via complete row groups. QK produces scores, softmax produces probabilities, and PV releases the slot after consuming the probabilities; a second slot holds the adjacent row group, letting different groups advance with overlap.*
...
![When all computation is placed on die 0, die 1's 32 GiB of weights must be read cross-die. On B200, the cross-die read is limited by die 1's HBM, about 8.6 ms, the same as a local read; on Ascend 910C it is limited by the inter-die link, about 127 ms.](ch04/figure-4-10-locality.svg)
*Figure 4-22 When all computation is placed on die 0, die 1's 32 GiB of weights must be read cross-die. On B200, the cross-die read is limited by die 1's HBM, about 8.6 ms, the same as a local read; on Ascend 910C it is limited by the inter-die link, about 127 ms.*

![Once computation is placed on the die holding the weights, only 64 MiB of inputs and results cross the link, and each side reads its own local weights. B200 remains at about 8.6 ms; Ascend 910C drops from about 127 ms to about 21.5 ms.](ch04/figure-4-locality-compute.svg)
*Figure 4-23 Once computation is placed on the die holding the weights, only 64 MiB of inputs and results cross the link, and each side reads its own local weights. B200 remains at about 8.6 ms; Ascend 910C drops from about 127 ms to about 21.5 ms.*
...
![Startup and transfer time for a single 8 KiB transfer. The fixed startup overhead is 2 μs; switching NVLink from A100's 300 GB/s per direction to H100's 450 GB/s only shortens the thin blue transfer term.](ch04/figure-4-11-interconnect.svg)
*Figure 4-24 Startup and transfer time for a single 8 KiB transfer. The fixed startup overhead is 2 μs; switching NVLink from A100's 300 GB/s per direction to H100's 450 GB/s only shortens the thin blue transfer term.*

![Time for a single 2 MiB transfer under the same startup conditions. The blue transfer term dominates, and switching to H100's NVLink brings a more significant benefit; this figure's vertical-axis range is labeled separately from the previous figure.](ch04/figure-4-large-message.svg)
*Figure 4-25 Time for a single 2 MiB transfer under the same startup conditions. The blue transfer term dominates, and switching to H100's NVLink brings a more significant benefit; this figure's vertical-axis range is labeled separately from the previous figure.*
...
![The Q projection on the same V100. At 4,096 rows, Tensor Core substantially shortens matrix computation; at one row, both computation paths are shorter than weight transfer. The top and bottom groups use separately labeled time units.](ch04/figure-4-evolution-tensor-budget.svg)
*Figure 4-26 The Q projection on the same V100. At 4,096 rows, Tensor Core substantially shortens matrix computation; at one row, both computation paths are shorter than weight transfer. The top and bottom groups use separately labeled time units.*
...
![The storage and read budget for the same 4096×4096 weight set. Every 32 low-precision values share one one-byte scale. As bit width drops from BF16 to MXFP4, weights plus scales together drop from 32 MiB to 8.5 MiB.](ch04/figure-4-evolution-precision.svg)
*Figure 4-27 The storage and read budget for the same 4096×4096 weight set. Every 32 low-precision values share one one-byte scale. As bit width drops from BF16 to MXFP4, weights plus scales together drop from 32 MiB to 8.5 MiB.*
...
![From asynchronous copy, to TMA, to TMEM, dedicated units take on more of the data preparation and accumulation state management. Blue represents data storage and movement, orange represents matrix computation, and purple represents accumulation results.](ch04/figure-4-evolution-nvidia-path.svg)
*Figure 4-28 From asynchronous copy, to TMA, to TMEM, dedicated units take on more of the data preparation and accumulation state management. Blue represents data storage and movement, orange represents matrix computation, and purple represents accumulation results.*
...
![Two preparation paths for the same 3×3 convolution. Explicit unrolling forms an intermediate matrix nine times larger; MTE organizes windows on-chip, eliminating 6.891 MiB of intermediate-result write and read in HBM.](ch04/figure-4-evolution-img2col.svg)
*Figure 4-29 Two preparation paths for the same 3×3 convolution. Explicit unrolling forms an intermediate matrix nine times larger; MTE organizes windows on-chip, eliminating 6.891 MiB of intermediate-result write and read in HBM.*
...
![Early same-core division of labor, the independent control of 910B/910C, and the direct CV pathway added in 950. The ratio of compute units and the path for exchanging results together determine the efficiency of fused operators.](ch04/figure-4-evolution-ascend.svg)
*Figure 4-30 Early same-core division of labor, the independent control of 910B/910C, and the direct CV pathway added in 950. The ratio of compute units and the path for exchanging results together determine the efficiency of fused operators.*
...
![Unified memory handles the machine's overall data capacity, Dynamic Caching manages GPU local resources, and M5's Neural Accelerator adds dedicated in-GPU compute capability. These three feed respectively into the analysis of capacity, residency, and computation time.](ch04/figure-4-evolution-apple.svg)
*Figure 4-32 Unified memory handles the machine's overall data capacity, Dynamic Caching manages GPU local resources, and M5's Neural Accelerator adds dedicated in-GPU compute capability. These three feed respectively into the analysis of capacity, residency, and computation time.*
...
![Weights stay fixed while each request still independently reads KV. Given the assumption of 8K context retained per request, KV reads exceed shared weight reads starting from batch 13.](ch04/figure-4-12-specialization.svg)
*Figure 4-33 Weights stay fixed while each request still independently reads KV. Given the assumption of 8K context retained per request, KV reads exceed shared weight reads starting from batch 13.*
...
![Dedicated read-only weights change the two storage paths. In the upper path, weights and KV contend for HBM; in the lower path, ROM supplies weights while HBM holds writable state. Arrows indicate reads; KV also requires writing new state.](ch04/figure-4-rom-paths.svg)
*Figure 4-34 Dedicated read-only weights change the two storage paths. In the upper path, weights and KV contend for HBM; in the lower path, ROM supplies weights while HBM holds writable state. Arrows indicate reads; KV also requires writing new state.*
...
![How compute and memory-access time for the same Q projection vary with input row count. Compute scales with row count, while off-chip access includes both fixed weights and growing input/output; from row 179 onward the compute term is longer.](ch04/figure-4-13-roofline.svg)
*Figure 4-35　How compute and memory-access time for the same Q projection vary with input row count. Compute scales with row count, while off-chip access includes both fixed weights and growing input/output; from row 179 onward the compute term is longer.*
...
![Two stage budgets for the same Qwen3-8B. Single-request decode more directly reflects read bandwidth; 4K prefill's matrix budget more directly reflects the matrix rate at matched precision. The two plots' horizontal axes each label the computed time.](ch04/figure-4-evolution-convergence.svg)
*Figure 4-36　Two stage budgets for the same Qwen3-8B. Single-request decode more directly reflects read bandwidth; 4K prefill's matrix budget more directly reflects the matrix rate at matched precision. The two plots' horizontal axes each label the computed time.*
...
![Total projection time on the RTX PRO 6000. Each condition is tested over eleven rounds of sixteen calls each, taking the median of each round's average time; timing includes submission and synchronization.](ch04/figure-4-14-performance.svg)
*Figure 4-37　Total projection time on the RTX PRO 6000. Each condition is tested over eleven rounds of sixteen calls each, taking the median of each round's average time; timing includes submission and synchronization. "Reuse" means multiple calls read the same weight address; "rotation" means the weight address changes between calls.*

![DRAM read counts collected separately under the same four conditions. Single row reads are both about 32 MiB; for 256 rows, reuse reads 256 bytes and rotation reads about 32.1 MiB. Access counts and regular timing are measured separately.](ch04/figure-4-performance-traffic.svg)
*Figure 4-38　DRAM read counts collected separately under the same four conditions. Single row reads are both about 32 MiB; for 256 rows, reuse reads 256 bytes and rotation reads about 32.1 MiB. Access counts and regular timing are measured separately. "Reuse" and "rotation" respectively denote keeping and changing the weight address.*
...
[^qwen]: Qwen3-8B's fixed configuration, tensor indexing, and model implementation are given in the [model configuration](../calculations/configs/models/qwen3-8b/config.json) and [projection calculation results](../calculations/results/projection-qwen3-8b-rtx4090-b256.md).

[^projection]: [Q projection, RTX 4090, M=1](../calculations/results/projection-qwen3-8b-rtx4090-b1.md); [M=256](../calculations/results/projection-qwen3-8b-rtx4090-b256.md).

[^architecture]: [Accelerator architecture and execution comparison](../case-studies/accelerator-architecture.md).

[^tpu]: Jouppi et al., *In-Datacenter Performance Analysis of a Tensor Processing Unit*, ISCA 2017, [archived paper](../references/files/papers/tpu-v1.pdf).

[^nvidia]: [A100 architecture whitepaper](../references/files/specs/nvidia-a100.pdf), [H100 architecture whitepaper](../references/files/specs/nvidia-h100.pdf), [Hopper Tuning Guide](../references/files/documents/nvidia-hopper-tuning.md), [Blackwell technical brief](../references/files/specs/nvidia-blackwell-brief.pdf), and [CUTLASS Blackwell features](../references/outline-checks/2026-09-07/systems-cases/cutlass-blackwell.md).

[^ascend]: [Ascend 950 official architecture whitepaper](../references/files/specs/ascend-950-official.pdf), §4.1—4.1.6; page-level positioning of the earlier DaVinci and CANN separated architecture is in the [comparison notes](../case-studies/accelerator-architecture.md).

[^nonmatrix]: [From operator utilization to waiting during execution](../case-studies/component-utilization-and-overlap.md) and [paper reading notes](../references/proceedings/ASPLOS/2025/ascend-components-reading.json).

[^apple]: [M2 Pro/Max official specifications](../references/files/specs/apple-m2-pro-max.md), [Apple GPU architecture description](../references/files/documents/apple-gpu-architecture.md), [Metal storage modes](../references/files/documents/apple-metal-memory.json), [M5 GPU Neural Accelerator official description](../references/outline-checks/2026-09-07/systems-cases/apple-m5-evolution.md).

[^fa4]: *FlashAttention-4*, MLSys 2026, [paper](../references/proceedings/MLSys/2026/papers/mlsys2026-ae8b0b5838ba510daff1198474e7b984.pdf), §2.2, §3.1.1, Equations 1–3, and Table 1; [single-SM independent recomputation](../calculations/results/fa4-qwen8-resource-balance.md).

[^precision]: [Hardware precision audit](../calculations/HARDWARE-AUDIT.md), [DeepSeek V4-Flash and Qwen stage-by-stage resource conditions](../calculations/research/stage-resource-bounds/README.md), [low-precision and execution-path research](../case-studies/kernel-orchestration-and-quantization.md). For hardware evolution and cost attribution, see the [cost decline study](../research/token-cost-2023-2026/report.md).

[^quant-exp]: [Experiment 4-2](../experiments/ch04/04-02/README.md), [real routing activations](../experiments/ch04/04-02/routed-activations/README.md), [128-element grouping](../experiments/ch04/04-02/block-scales/README.md), [single-expert in-model substitution](../experiments/ch04/04-02/model-intervention/README.md).

[^workspace]: [Experiment 4-2 allocator peak](../experiments/ch04/04-02/workspace/README.md).

[^capacity]: [Qwen3-8B configuration](../calculations/configs/models/qwen3-8b/config.json); [full results for storage generations](../calculations/results/storage-generation-qwen8-235.md).

[^evolution]: [Understanding chip evolution from workload change](../case-studies/architecture-evolution.md).

[^storage]: [Storage-generation comparison results](../calculations/results/storage-generation-qwen8-235.md) and [computation notes](../calculations/research/storage-generation-comparison/README.md).

[^mess]: *Mess*, MICRO 2024, [author-accepted manuscript](../references/proceedings/MICRO/2024/paper-011.pdf), selected physical pages 3–6; [memory-bandwidth concurrency example and constraints](../case-studies/memory-bandwidth-and-concurrency.md).

[^coordinates]: [DeepSeek V4-Flash shared-expert movement coordinate results](../calculations/results/v4-copy-coordinates-m32.md) and [source-level counting notes](../calculations/research/v4-copy-coordinates/README.md).

[^transfer]: [Hopper Tuning Guide](../references/files/documents/nvidia-hopper-tuning.md), [Ascend 950 whitepaper](../references/files/specs/ascend-950-official.pdf), [Rubin official architecture description](../references/outline-checks/2026-09-07/systems-cases/rubin-rechecked.md).

[^pipeline]: [Qwen attention-input pipeline baseline](../calculations/results/attention-input-base.md), [matrix-rate doubling](../calculations/results/attention-input-matrix-double.md), [modeling and independent check](../calculations/research/attention-input-pipeline/README.md).

[^handoff]: [Matrix-vector handoff notes](../calculations/research/matrix-vector-handoff/README.md), [32-row two-slot direct path](../calculations/results/matrix-vector-direct-rows32-slots2.md).

[^package]: [Blackwell technical brief](../references/files/specs/nvidia-blackwell-brief.pdf), 10 TB/s NV-HBI; [CloudMatrix384 v2](../references/files/papers/cloudmatrix384-v2.pdf), §3.3.1 (910C, 64 GB per die, 1.6 TB/s, 270 GB/s per direction between dies), the start of §4.2 (one expert per die during decode) and §4.2.2; die-locality computations for the two products appear in the `die_locality` entry of the [teaching derivation script](ch04/derive.py); [Vera Rubin platform](../references/files/specs/nvidia-rubin-system.md), [UB and Ascend cross-check](../references/UB-ASCEND-NOTES.md).

[^tpu8]: [Inside the Eighth-Generation TPU: An Architecture Deep Dive](../references/files/specs/google-tpu8.md).

[^special]: [Groq TSP paper](../references/files/papers/groq-tsp.pdf), [IPU Programming Model](../references/files/documents/graphcore-programming.md), [Cerebras WSE-3 datasheet](../references/files/specs/cerebras-wse3-spec.pdf), [SambaNova SN40L paper](../references/files/papers/sambanova-sn40l-paper.pdf).

[^opentallas]: [OpenTallas case study](../case-studies/opentallas.md).

[^stage]: [Modeling notes on lower bounds for per-stage latency](../calculations/research/stage-resource-bounds/README.md); for the actual operator range of a real model, see the [Qwen3-8B prefill128 resource results](../calculations/results/stage-resources-qwen8-b1-prefill128.md).

[^hardware]: [Hardware sources and precision audit](../calculations/HARDWARE-AUDIT.md) and the [official baseline table](../calculations/results/hardware.md).

[^measurement]: [Experiment 4-6, full notes and raw records](../experiments/ch04/04-06/README.md), [projected timing summary](../experiments/ch04/04-06/results/projection-summary.json), [actual DRAM/L2 counts](../experiments/ch04/04-06/results/projection-traffic.json).

[^paired]: [Paired projected cost and average-power conditions](../calculations/results/paired-projection-unknown.md); the RTX PRO 6000's 600 W is taken from the [hardware input table](../calculations/configs/hardware.json), and the per-call energy at parity with the Mac's power is computed by the `energy` entry of the [teaching derivation script](ch04/derive.py); see the [derivation data](ch04/teaching-data.json).
...
[^rtx-spec]: [RTX Blackwell architecture whitepaper](../calculations/sources/hardware/nvidia-rtx-blackwell-whitepaper.pdf), Appendix A, Table 3: the RTX 4090 has 24 GB GDDR6X, 1,008 GB/s, PCIe Gen 4, and a 450 W TGP; the RTX 5090 has 32 GB GDDR7, 1,792 GB/s, PCIe Gen 5, and a 575 W TGP; [A100 80GB datasheet](../references/files/specs/nvidia-a100-80-spec.pdf), PCIe 4.0 at 64 GB/s (bidirectional aggregate). H2D and GPU-memory read times appear in the `host_link` entry of the [teaching derivation script](ch04/derive.py).

[^window-rtx]: [RTX 4090, 128 in-flight transactions](../calculations/results/window-qwen3-8b-rtx4090-n128.md), [RTX 4090, 4,096](../calculations/results/window-qwen3-8b-rtx4090-n4096.md), [RTX 5090, 4,096](../calculations/results/window-qwen3-8b-rtx5090-n4096.md), [RTX 5090, 800 ns latency](../calculations/results/window-qwen3-8b-rtx5090-l800.md); for H100 latency, see Mess's Table I and Figure 3(h).

[^allreduce-small]: [Experiment 7-3, public run-record verification](../experiments/ch07/07-03/README.md), [raw log from two HGX H100 nodes](../experiments/ch07/07-03/raw/hgx2.txt): 16 ranks, NCCL 2.26.2, 16-byte AllReduce out-of-place 24.96 μs, in-place 24.93 μs.

[^decode-measured]: [Experiment 8-1 batch sweep](../experiments/ch08/08-01/README.md) and its [row-by-row efficiency summary](../experiments/ch08/08-01/efficiency.json): RTX PRO 6000 Blackwell Workstation, Qwen3-8B BF16, vLLM 0.23.0, eager mode; each row takes the median of the pure decode iteration per round, then the median across three rounds. The ratio to the read lower bound is computed by the `measured_decode` entry of the [teaching derivation script](ch04/derive.py).

[^pipeline-extra]: The three-slot pipeline, the compute-doubling variant, and the design turning point are generated by the [teaching derivation script](ch04/derive.py); the full timeline appears in the [derivation data](ch04/teaching-data.json).

[^feedback]: [DeepSeek V3 technical report](../references/text/deepseek-v3.txt), §3.5, Suggestions on Hardware Design.

[^core-calculation]: [Item-by-item recomputation](../calculations/results/core-principles.json) for this chapter's condition comparisons, and the [computation program](../calculations/core_principles.py).

[^v41-case]: [DeepSeek V4.1 official technical report](../calculations/sources/deepseek-v4.1-flash/DeepSeek_V41_Tech_Report.pdf), §1, 2, 3, and 6; [fixed conditions and recomputation across chapters for this running session](../calculations/results/v41-throughline.json).

[^turing-evolution]: [Turing official architecture whitepaper](../research/ch04-architecture-evolution-2026-09-10/sources/nvidia-turing.pdf), Turing Tensor Cores and low-precision inference.

[^volta-evolution]: [Volta architecture whitepaper](../references/files/specs/nvidia-v100.pdf), Tensor Cores and mixed-precision chapter.

[^blackwell-evolution]: [Blackwell Tuning Guide](../references/files/specs/nvidia-blackwell-guide.md), [CUTLASS Blackwell feature notes](../references/outline-checks/2026-09-07/systems-cases/cutlass-blackwell.md), and the [SM100 TMEM example](../references/outline-checks/2026-09-07/systems-cases/cutlass-01_mma_sm100.cu); the 128 KB L1 data cache and 100 KB shared-memory cap per SM on SM120 (compute capability 12.0) appear in the [CUDA Programming Guide's compute-capability table](../references/files/documents/cuda-compute-capabilities.md).

[^rubin-evolution]: [NVIDIA Rubin GPU architecture description](../references/outline-checks/2026-09-07/systems-cases/rubin-rechecked.md), MoE data movement, K-dimension instruction throughput, and attention acceleration.

[^apple-generations]: [Apple M3 official architecture introduction](../references/outline-checks/2026-09-07/systems-cases/apple-m3-evolution.md), [M4 official release notes](../research/ch04-architecture-evolution-2026-09-10/sources/apple-m4-2024.md), and [M4 Mac mini specifications](../calculations/research/hardware-apple-closure/apple-m4-mini-specs.md).

[^ascend-history]: [DaVinci architecture paper](../references/files/specs/ascend-davinci.pdf), §3.1–3.4; [Ascend C Guide](../references/files/specs/ascend-c-guide.pdf), Chapter 4 on coupled versus separated architectures; [CloudMatrix384 v2](../references/files/papers/cloudmatrix384-v2.pdf), §3.3.1, §4.2.2.

[^evolution-quant]: [Quantitative computation program and notes for architecture evolution](../calculations/research/architecture-evolution-quantitative/README.md), [item-by-item computed results](../calculations/research/architecture-evolution-quantitative/result.json). Parameters are taken from the [hardware input table](../calculations/configs/hardware.json); the 3090 is supplemented from Table 9 of the [official GA102 whitepaper](../calculations/research/architecture-evolution-quantitative/sources/nvidia-ampere-ga102.pdf); the full model configurations and tensor inventory follow this book's calculations.

[^bf16-quant]: [A100 architecture whitepaper](../references/files/specs/nvidia-a100.pdf), chapter on BF16 and numerical formats; [recomputation of numerical range, storage, and quantization](../calculations/research/architecture-evolution-quantitative/result.json).

[^ascend-cnn-history]: The author's ["Where Should Network Intelligence Live"](../references/author-materials/2026-09-09/network-intelligence.md), a retrospective on the 2016 ResNet design background from a 2023 talk; the [DaVinci architecture paper](../references/files/specs/ascend-davinci.pdf) §3.2, §3.4, Table 5, and the Ascend 910 system give img2col, resource ratio, per-core bandwidth, and 1.2 TB/s HBM respectively.

[^nvidia-product-lines]: [RTX PRO 6000 product specifications](../references/files/specs/nvidia-rtx-pro6000-spec.pdf), ECC, MIG, and product configuration; [Hopper Tuning Guide](../references/files/documents/nvidia-hopper-tuning.md), NVLink; [GA102 whitepaper](../calculations/research/architecture-evolution-quantitative/sources/nvidia-ampere-ga102.pdf), 3090 NVLink; [Blackwell Tuning Guide](../references/files/specs/nvidia-blackwell-guide.md) and [CUTLASS Blackwell features](../references/outline-checks/2026-09-07/systems-cases/cutlass-blackwell.md), SM100/SM120.

[^apple-capacity-quant]: [Apple official configuration cross-check](../calculations/research/hardware-apple-closure.md), [GPU and capacity combinations](../calculations/research/hardware-apple-closure/gpu-memory-combinations.json), and the [hardware table](../calculations/configs/hardware.json). The M3 Ultra's 512 GB is cross-confirmed by the official release notes and recorded 80-core GPU full-system test configurations. The M5 Ultra's 512 GB/1,200 GB/s is a specification announced in August 2026; the 512 GB configuration is scheduled to ship in late October 2026.

[^logicfolding]: He Tingbo, *Huawei's τ Chip Was Supposed to Melt?*, ChinaXiv:202609.00031v1, 2026-09-04, [archived paper](../references/files/papers/202609.00031v1.pdf), §II–V and Figures 1–2: dynamic-power share, wiring capacitance, an NPU comparison at equal performance (29 TOPS, 0.85→0.55 V, a 63% frequency drop, a 66% power drop), a 25% drop in DSP power, and a projected 40% area reduction.

[^energy-table]: [Dally, Hardware for Deep Learning, Hot Chips 2023 keynote](../references/files/documents/dally-hotchips2023.pdf), pages 12, 51–52: page 12 gives HFMA at 1.5 pJ, HMMA at 110 pJ, instruction overhead of about 30 pJ, and an overhead share of 2000%/22% (45 nm); page 51's energy table notes it is taken from Horowitz, ISSCC 2014, at a 45 nm process; page 52 gives the three-level memory figures of 5/50/640 pJ per 32-bit word, with no process node noted; [Fine-Grained DRAM, MICRO 2017](../references/files/papers/fgdram-micro17.pdf), §1–2, HBM2 at 3.97 pJ/bit with its breakdown, DRAM energy modeled at 28 nm; [NVIDIA Grace Hopper Superchip Architecture In-Depth](../references/files/documents/nvidia-grace-hopper-blog.md), 2022-11-10, NVLink-C2C at 1.3 pJ/bit.

[^energy-ledger]: [Energy ledger results](../calculations/results/energy-ledger-book.md), read from the byte counts and FLOPs of the [single-request 8K decode result](../calculations/results/qwen3-8b-decode-b1-s8192.json), summed using the per-byte and per-FLOP energy figures from the table above; run `python3 calculations/calc.py energy-ledger --format md` to recompute.

[^hbm-stack]: [H100 architecture whitepaper](../references/files/specs/nvidia-h100.pdf), Table 4: five HBM3 stacks, a 5120-bit interface, 2619 MHz DDR, 3352 GB/s (this chapter's main text uses the rounded 3.35 TB/s, i.e., 3,350 GB/s, from the [datasheet](../references/files/specs/nvidia-h100-datasheet.pdf)), 814 mm², TSMC 4N process, with the A100 at 826 mm²; [Blackwell technical brief](../references/files/specs/nvidia-blackwell-brief.pdf), page 7, two reticle-limit dies and 10 TB/s NV-HBI, Table 3's B200 at 192 GB/7.7 TB/s; [HGX platform component description](../calculations/sources/hardware/nvidia-hgx-components.md), Table 1, HGX B200 at 180 GB per GPU and up to 8 TB/s; [H200 datasheet](../references/files/specs/nvidia-h200-datasheet.pdf), 141 GB/4.8 TB/s; [Micron HBM3E product page](../references/files/specs/micron-hbm3e-page.md), 1024 pins, eight-high 24 GB, twelve-high 36 GB; [SK hynix HBM product page](../references/files/specs/skhynix-hbm-page.md), 9.6 Gbit/s and 1.23 TB/s.

[^tdp]: [H100 datasheet](../references/files/specs/nvidia-h100-datasheet.pdf), SXM maximum thermal design power of 700 W; [H100 architecture whitepaper](../references/files/specs/nvidia-h100.pdf), Table 4, BF16 dense 989.4 TFLOPS with a TDP of 700 W; [Blackwell technical brief](../references/files/specs/nvidia-blackwell-brief.pdf), Table 3, B200 at 1000 W.
