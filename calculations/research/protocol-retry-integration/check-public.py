...
from infra_calc.topics import protocol_retry
...
def main():
    ...
    candidate = CALC / "research/protocol-retry"
    frozen = json.loads((candidate / "result.json").read_text())
    scenarios = json.loads((CALC / "scenarios/book.json").read_text())["protocol_retry"]
    checked = []
    with tempfile.TemporaryDirectory() as directory:
        inputs = Path(directory) / "inputs.json"
        output = Path(directory) / "output"
        for row in scenarios:
            name = row["id"].removeprefix("protocol-retry-")
            actual = protocol_retry.calculate(row["inputs"])
            assert normalized(actual) == normalized(frozen[name]), name
            inputs.write_text(json.dumps(row["inputs"]))
            for fmt in ("json", "md"):
                subprocess.run([sys.executable, str(CALC / "calc.py"), "protocol-retry",
