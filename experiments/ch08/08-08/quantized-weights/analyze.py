...
for name in ['bf16','fp8']:
 ...
 weights=snaps['weights'][0]
 assert any(p['dtype']=='torch.float8_e4m3fn' for p in weights['parameters'])
 if name=='bf16':reference_weights=weights
 else:assert weights==reference_weights
...
(r/'summary.json').write_text(json.dumps(dict(configurations=reports,output_differences=differences,repeat_output_differences=repeat_differences,
 scope='Eight distinct synthetic retrieval tasks, repeated across concurrency and modes. Same online FP8 weights and Triton backend. Natural generation quality assessed separately from forced-length timing; no retries performed.'),indent=2)+'\n')
