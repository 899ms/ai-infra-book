import argparse,ctypes,hashlib,json,os,time,fcntl,copy
...
class Records(Dataset):
 ...
 def __getitem__(self,i):
  ...
  with open(self.path,'rb') as f:
   fcntl.fcntl(f.fileno(),48,1);f.seek(start);data=f.read(end-start)
  after=time.monotonic()
  assert len(data)==end-start
  event(Path(self.out)/f'worker-{os.getpid()}.jsonl',dict(kind='read_complete',pid=os.getpid(),record=i,start=start,end=end,bytes=len(data),read_start=before,read_end=after,nocache_applied=True))
...
def main(a):
 ...
 pool=ThreadPoolExecutor(max_workers=1);pending=None;window_start=time.monotonic()
 def save_snapshot(path,snapshot):
  before=time.monotonic();durable(path,snapshot);event(out/'writes.jsonl',dict(step=snapshot['step'],start=before,end=time.monotonic(),bytes=path.stat().st_size,file=path.name))
 while step<a.steps:
  ...
  event(out/'steps.jsonl',dict(kind='update',step=step,input_start=input_start,input_end=input_end,start=start,end=time.monotonic(),loss=loss.item(),gradient=grad.item(),positions=ids,input_bytes=list(raw),input_sha256=hashlib.sha256(raw).hexdigest(),buffer_bytes=len(buf),cursor=cursor))
  if step in (16,32,48) and a.mode!='none':
   staging_start=time.monotonic()
   if pending is not None:pending.result()
   snapshot=copy.deepcopy(state());staged=time.monotonic()
   if a.mode=='sync':save_snapshot(out/f'save-{step}.pt',snapshot)
   else:pending=pool.submit(save_snapshot,out/f'save-{step}.pt',snapshot)
   event(out/'staging.jsonl',dict(step=step,start=staging_start,staged=staged,returned=time.monotonic(),mode=a.mode))
  if step==a.stop:
 ...
 if pending is not None:pending.result()
 pool.shutdown();write(out/'window.json',dict(start=window_start,end=time.monotonic(),mode=a.mode))
 durable(out/'final.pt',state());write(out/'completion.json',dict(done=True,step=step,pid=os.getpid(),time=time.monotonic()))
 del iterator,loader
if __name__=='__main__':
 p=argparse.ArgumentParser();p.add_argument('--mode',choices=['none','sync','async'],required=True);p.add_argument('--output',required=True);p.add_argument('--seed',type=int,required=True);p.add_argument('--steps',type=int,default=64);p.add_argument('--stop',type=int,default=-1);p.add_argument('--checkpoint');p.add_argument('--control',default='correct',choices=['correct','omit_buffer','dispatch_cursor']);main(p.parse_args())
