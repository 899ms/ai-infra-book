...
config={'experimental':True,'debug':True,'data-root':str(P/'data'),'exec-root':'/run/book1102-iso2','pidfile':str(P/'daemon.pid'),'hosts':['unix://'+'/run/book1102-iso2.sock'],'bridge':'none','iptables':False,'ip6tables':False,'ip-forward':False,'ip-masq':False,'containerd-namespace':'book1102','containerd-plugins-namespace':'plugins.book1102'}
...
host='unix://'+'/run/book1102-iso2.sock'
try:
 for _ in range(100):
  if proc.poll() is not None:raise RuntimeError('daemon exited')
  if Path('/run/book1102-iso2.sock').exists() and cmd(['docker','-H',host,'info','--format','{{.DockerRootDir}}'])['rc']==0:break
  time.sleep(.2)
 else:raise RuntimeError('daemon readiness timeout')
 assert cmd(['docker','image','save','-o',str(P/'base.tar'),'python:3.11-slim'])['rc']==0
