...
reports=[]
for name in ['results','results-default-restore','results-host-unix-v2']:
    out=R/name
    ...
    for row in cases:
        ...
        if row['path']=='pause':
            ...
            rec.update(state_contract='checkpoint_captured_restore_outcomes_retained',
...
summary=dict(status='measured_state_contracts_verified',author_substitution='Docker on rtx-pro instead of E2B',
    reports=reports,scope='36 cases across original bridge/custom-dir, bridge/default-dir and host-network UNIX controls. Do not pool timings across controls. Capture is not restore; failed restore latency remains null.',
    checkpoint_restore_complete=False)
