...
ex=read(R/'execution.json');env=read(O/'environment.json');rows=lines(O/'raw.jsonl');assert ex['exit_code']==0 and not ex['after_gpu'].strip() and len(rows)==32
for n,h in env['source_hashes'].items():assert sha(R/n)==h
order=[];rng=random.Random(1107)
for rep in range(2):
 cases=[dict(rep=rep,thinking=think,budget=cap,warm=warm,busy=busy) for think,cap in [(True,100),(True,1000),(True,4096),(False,1000)] for warm in [False,True] for busy in [False,True]];rng.shuffle(cases);order+=cases
...
groups=[]
for thinking,budget in [(True,100),(True,1000),(True,4096),(False,1000)]:
