...
SOURCE=R.parent/'tts-network'
MODE=os.environ['BOOK_WINDOW_MODE'];WINDOW={'default':None,'64k':65536,'4m':4194304}[MODE]
FIXTURE=json.loads((SOURCE/'tts-fixture.json').read_text());PAYLOAD=(SOURCE/FIXTURE['upload_file']).read_bytes();RESPONSE=(SOURCE/FIXTURE['response_file']).read_bytes();HASH=hashlib.sha256(PAYLOAD).hexdigest();RESPONSE_HASH=hashlib.sha256(RESPONSE).hexdigest();SERVICE_S=FIXTURE['service_s']
  ...
  async def lane(lane_id):
   ...
   for index in range(lane_id,count,concurrency):
    ...
    try:
     ...
     with wave.open(io.BytesIO(data),'rb') as w:fmt=[w.getnchannels(),w.getsampwidth(),w.getframerate(),w.getnframes()];pcm=w.readframes(w.getnframes())
     decode_end=NOW();rec.update(wav_format=fmt,pcm_sha256=hashlib.sha256(pcm).hexdigest(),decode_start=decode_start,decode_end=decode_end,connection=conn['record']['id'],fresh=fresh,connection_ready=conn['record']['ready'] if fresh else rec['start'],send=send,stream=stream,**result,bytes=len(data),sha256=hashlib.sha256(data).hexdigest(),valid=(data==RESPONSE and fmt==[1,2,44100,591872] and dict(result['headers']).get(':status')=='200'),validation_end=NOW(),error=None)
