...
config={'experimental':True,'debug':True,'data-root':str(P/'data'),'exec-root':str(P/'exec'),'pidfile':str(P/'daemon.pid'),'hosts':['unix://'+str(P/'docker.sock')],'bridge':'none','iptables':False,'ip6tables':False,'ip-forward':False,'ip-masq':False,'containerd-namespace':'book1102','containerd-plugins-namespace':'plugins.book1102'}
...
host='unix://'+str(P/'docker.sock')
try:
 for _ in range(100):
  if proc.poll() is not None:raise RuntimeError('daemon exited')
  if (P/'docker.sock').exists() and cmd(['docker','-H',host,'info','--format','{{.DockerRootDir}}'])['rc']==0:break
  time.sleep(.2)
