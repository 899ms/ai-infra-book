...
- 每历史 token 的 KV：147456 B；上下文 8192 时每序列每步读取 1207959552 B。
- 权重／KV 交叉点 batch：12.5309，即从 batch 13 起 KV 读取超过权重读取。
- 权重移出该接口后，仅此内存服务的理想加速比：13.531（batch=1）。
...
- 维持 10000 tok/s 仅 KV 读就需 12079595520000 B/s，按每 stack 1200000000000 B/s 至少 11 个 stack。
- 按密度 3009000 B/mm²，仅该状态需约 401.4 mm²。
...
- 直径传播每次 0.000001500 s，每 token 合计 0.000108 s。
- 单次预算内：否；token 预算内：否，超出倍数 1.08。
...
```json
{
  ...
  "residency_split": {
    ...
    "batch": 1,
    "kv_bytes_per_token": 147456,
    "kv_read_bytes_per_sequence_step": 1207959552,
    "active_weight_read_bytes": 15136811008,
    "step_bytes_with_resident_weights": 16344770560,
    "step_bytes_without_weight_reads": 1207959552,
    "crossover_batch_exact": {
      "numerator": 7391021,
      "denominator": 589824
    },
    "crossover_batch_first_integer": 13,
  ...
  "supply_backsolve": {
    "tokens_per_second": 10000,
    "kv_read_bandwidth_bytes_per_second": 12079595520000,
    "stack_bytes_per_second": 1200000000000,
    "stacks_required": 11,
