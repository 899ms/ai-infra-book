    ...
    with lock:
     if self.path=='/exists':info=dict(exists=[path(k).exists() for k in meta['keys']])
      ...
      for e in meta['entries']:
       ...
       if existed:assert hashlib.sha256(p.read_bytes()).hexdigest()==e['sha256'],'same key different bytes'
       else:
        temp=p.with_suffix('.tmp');temp.write_bytes(b);temp.replace(p)
       entries.append(dict(key=e['key'],bytes=n,sha256=e['sha256'],new_write=not existed))
