<!-- 从 163-meta-blog.html 迁移的资料快照；原始 HTML SHA-256: 60c516d1f30e50066176fd6a326f979c15df9b99075a1edfd9cf4c29de964907。 -->

POSTED ON JULY 22, 2025 TO [Data Infrastructure](https://engineering.fb.com/category/data-infrastructure/)

# 

How Meta keeps its AI hardware reliable

![](https://engineering.fb.com/wp-content/uploads/2025/07/Meta-AI-Hardware-Reliability-at-Scale-HERO.png)

By [Harish Dattatraya Dixit](https://engineering.fb.com/author/harish-dattatraya-dixit/ "Posts by Harish Dattatraya Dixit"), [Sriram Sankar](https://engineering.fb.com/author/sriram-sankar/ "Posts by Sriram Sankar")
...
From our experience running [the Llama 3 herd of models](https://arxiv.org/abs/2407.21783), we find that hardware failures in components such as SRAMs, HBMs, processing grids, and network switch hardware significantly impact AI cluster reliability, with over 66% of training interruptions due to such failures. Some of the challenges for AI clusters include accelerators that might be less reliable than CPUs due to complexity and limited telemetry, network complexity that could result in misattributed failures, and errors within the GPU software stack that may require  extensive configuration to correct. Hence, reducing hardware and configuration failures greatly enhances cluster efficiency.

![](https://engineering.fb.com/wp-content/uploads/2025/07/Meta-AI-hardware-reliability-at-scale-image-1.png)
...
![](https://engineering.fb.com/wp-content/uploads/2025/07/Meta-AI-hardware-reliability-at-scale-image-2.png)

## Novel SDC detection mechanisms 

To protect applications from silent data corruption, Meta employs several detection mechanisms, as detailed in the papers, [“Detecting Silent Errors in the Wild”](https://arxiv.org/abs/2203.08989) and [“Hardware Sentinel.”](https://dl.acm.org/doi/abs/10.1145/3676641.3716258)

1.  [Fleetscanner](https://engineering.fb.com/2022/03/17/production-engineering/silent-errors/): Fleetscanner captures performance outliers at scale with targeted micro-benchmarks for identifying hardware defects. These benchmarks’ signatures are integrated into telemetry for non-benchmark-based detection. This approach involves running directed tests during maintenance operations such as firmware upgrades and hardware repairs. Tests are scheduled periodically, covering the entire fleet every 45 to 60 days. While it provides dedicated testing on hosts, it may be too slow for some SDCs.
2.  [Ripple:](https://arxiv.org/abs/2203.08989) Ripple co-locates with workloads, executing tests in milliseconds to seconds, allowing fleet-wide coverage in days. It overlaps test instructions across cores and threads, providing faster detection than Fleetscanner.
3.  [Hardware Sentinel:](https://dl.acm.org/doi/abs/10.1145/3676641.3716258) This novel, test-and-architecture-agnostic approach evaluates application exceptions in kernel space. It identifies core-based anomalies as silent data corruption without requiring test allocations, operating solely in the analytical plane. Hardware Sentinel outperforms testing-based methods by 41% across architectures, applications, and data centers.
...
![](https://engineering.fb.com/wp-content/uploads/2025/07/Meta-AI-hardware-reliability-at-scale-image-3.png)
...
#### [Algorithmic fault tolerance](https://ieeexplore.ieee.org/abstract/document/10020972)
...
#### [Parameter vulnerability factors](https://arxiv.org/abs/2405.01741)
...
#### [Divergence detection](https://dl.acm.org/doi/10.1145/3620666.3651349)
...
Meta’s journey toward industry leadership in SDC began with identifying frequent fleet issues in 2016, scaling SDC detection in 2018, and implementing detection frameworks by 2019. By 2020, detection mechanisms were integrated into accelerators, and Meta published the paper, [“Silent Data Corruptions at Scale.”](https://arxiv.org/abs/2102.11245) In 2022, Meta introduced [“FleetScanner and Ripple”](https://arxiv.org/abs/2203.08989) and conducted an [RFP](https://research.facebook.com/blog/2022/2/engineering-director-sriram-sankar-discusses-metas-first-research-award-opportunity-in-silent-data-corruptions-at-scale) for [academic awards](https://fburl.com/n5h6hlml), funding five winners. 

In 2023, Meta collaborated with industry leaders (Google, Microsoft, ARM, AMD, NVIDIA, and Intel) to enhance server resilience, defining test architectures and metrics. A joint [RFP](https://www.opencompute.org/blog/computings-hidden-menace-the-ocp-takes-action-against-silent-data-corruption-sdc) with partners from the[ Open Compute Project selected six winners for cross-domain SDC research](https://www.opencompute.org/blog/ocps-server-resilience-initiative-sdc-academic-research-awards-announced). By 2024, Meta’s fleet had advanced [AI SDC detection methodologies](https://dl.acm.org/doi/abs/10.1145/3676641.3716258) in production, contributing to research through publications, tutorials, and talks at major conferences and forums, addressing at-scale reliability challenges.

![](https://engineering.fb.com/wp-content/uploads/2025/07/Meta-AI-hardware-reliability-at-scale-image-4.png)

## The Meta Training and Inference Accelerator 

Meta is on an ambitious journey toward enabling training and inference accelerators under the [Meta Training and Inference Accelerator (MTIA)](https://ai.meta.com/blog/next-generation-meta-training-inference-accelerator-AI-MTIA/) family. On this journey, our goal is to utilize all the lessons learned from the fleet and move toward industry-leading, fleet-reliability practices in MTIA architecture and design practices. Using the factory-to-fleet approach, and consistently revisiting our reliability solutions across the stack, our goal is to deliver a best-in-class, reliable-and-performant solution to add to our infrastructure portfolio of AI hardware and to power AI applications at scale. 
...
![](https://engineering.fb.com/wp-content/uploads/2025/07/Meta-AI-hardware-reliability-at-scale-image-5.png)
...
![](https://engineering.fb.com/wp-content/uploads/2025/07/Meta-AI-hardware-reliability-at-scale-image-6.png)
...
*The authors would like to thank all the cross-functional engineers and teams instrumental in landing these solutions over the years. This blog accompanies the* [*@Scale conference talk*](#talk)*, please check out the talk for more details.*

### Share this:

- [Share on Facebook (Opens in new window) Facebook](https://engineering.fb.com/2025/07/22/data-infrastructure/how-meta-keeps-its-ai-hardware-reliable/?share=facebook)
- [Share on Threads (Opens in new window) Threads](https://engineering.fb.com/2025/07/22/data-infrastructure/how-meta-keeps-its-ai-hardware-reliable/?share=threads)
- [Share on WhatsApp (Opens in new window) WhatsApp](https://engineering.fb.com/2025/07/22/data-infrastructure/how-meta-keeps-its-ai-hardware-reliable/?share=jetpack-whatsapp)
- [Share on LinkedIn (Opens in new window) LinkedIn](https://engineering.fb.com/2025/07/22/data-infrastructure/how-meta-keeps-its-ai-hardware-reliable/?share=linkedin)
- [Share on Reddit (Opens in new window) Reddit](https://engineering.fb.com/2025/07/22/data-infrastructure/how-meta-keeps-its-ai-hardware-reliable/?share=reddit)
- [Share on X (Opens in new window) X](https://engineering.fb.com/2025/07/22/data-infrastructure/how-meta-keeps-its-ai-hardware-reliable/?share=x)
- [Share on Bluesky (Opens in new window) Bluesky](https://engineering.fb.com/2025/07/22/data-infrastructure/how-meta-keeps-its-ai-hardware-reliable/?share=bluesky)
- [Share on Mastodon (Opens in new window) Mastodon](https://engineering.fb.com/2025/07/22/data-infrastructure/how-meta-keeps-its-ai-hardware-reliable/?share=mastodon)
- [Share on Hacker News (Opens in new window) Hacker News](https://engineering.fb.com/2025/07/22/data-infrastructure/how-meta-keeps-its-ai-hardware-reliable/?share=custom-1699562127)
- [Email a link to a friend (Opens in new window) Email](mailto:?subject=%5BShared%20Post%5D%20How%20Meta%20keeps%20its%20AI%20hardware%20reliable&body=https%3A%2F%2Fengineering.fb.com%2F2025%2F07%2F22%2Fdata-infrastructure%2Fhow-meta-keeps-its-ai-hardware-reliable%2F&share=email)
- 
...
![](https://engineering.fb.com/wp-content/uploads/2026/08/MetaRoCE-HERO-final.png?w=580&h=326&crop=1)
...
![](https://engineering.fb.com/wp-content/uploads/2026/08/Meta-MTIA-300-Hero-final.png?w=580&h=326&crop=1 "Meta MTIA 300 - Hero - final")
...
![](https://engineering.fb.com/wp-content/uploads/2026/06/Metas-AI-Storage-Blueprint-at-Scale-Hero-1.png?w=580&h=326&crop=1)
