...
a.text(.76,.37,"啟用往返",ha='center',fontsize=11)
...
box(a,.03,.70,.25,.14,'CPU DRAM',"專家權重",col='sand');box(a,.39,.70,.24,.14,"GPU 權重緩衝","等待載荷就緒");box(a,.75,.70,.22,.14,"GPU 專家計算","輸出啟用",col='green');arrow(a,(.285,.77),(.38,.77),'orange');arrow(a,(.635,.77),(.74,.77));a.text(.335,.86,'W',ha='center',color=C['orange'])
label(a,.025,.56,"啟用交接：CPU 就地計算專家")
box(a,.03,.30,.25,.15,'GPU attention',"輸入啟用 X");box(a,.39,.30,.24,.15,"CPU 專家計算","本地讀取權重",col='sand');box(a,.75,.30,.22,.15,"同步合併","繼續下一層",col='green');arrow(a,(.285,.375),(.38,.375));arrow(a,(.635,.375),(.74,.375));a.text(.335,.465,'X',ha='center',color=C['teal']);a.text(.69,.465,'Y',ha='center',color=C['teal'])
...
ax.set(xlabel="每次啟動與同步 / μs",ylabel="序列交接時間 / ms",xlim=(0,20),ylim=(48.2,50),xticks=[0,5,10,15,20])
