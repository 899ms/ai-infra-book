...
names=['gradient-fp32-flat-contiguous-nic8','gradient-fp32-flat-interleaved-nic8','gradient-fp32-hierarchical-nic8'];g=[calc(x) for x in names];labels=["contiguous ring","interleaved ring","hierarchical reduction"]
...
f,a=canvas(9)
rows=[("host-handled RPC",["Application / CPU","NIC / network","peer CPU"],["request serialization","payload transfer","execute and respond"]),("CPU-submitted GPUDirect RDMA",["CPU submission","NIC ↔ GPU memory","remote NIC ↔ GPU"],["submit / read completion status","direct data delivery","target memory write"]),("GPU access via NVLink",["initiating GPU","NVLink / switching","peer GPU memory"],["device instruction","access path","wait for access completion"]),("device-initiated URMA async read/write",["initiator device / queue","UB / switching","target device memory"],["submit / completion notification","data path","write / notify"])]
for i,(title,boxes,notes) in enumerate(rows):
 ...
 a.annotate('',xy=(.17,y-.04),xytext=(.83,y-.04),arrowprops={'arrowstyle':'->','ls':'--','color':C['muted'],'lw':1});a.text(.50,y-.075,"Response / completion and synchronization relationship",fontsize=9,ha='center',color=C['muted'])
...
f=plt.figure(figsize=(13,8));a=f.add_axes([.04,.58,.92,.37]);a.set(xlim=(0,1),ylim=(0,1));a.axis('off');panel(a,.01,.98,"app endpoint separated from transfer state")
for i in range(3):
 y=.63-i*.24;box(a,.015,y,.23,.17,f'application endpoint{i}',"Independent submission / completion",size=11);box(a,.36,y,.23,.17,"relationship binding","target, permission, identity",size=11);arrow(a,(.25,y+.085),(.35,y+.085));arrow(a,(.60,y+.085),(.735,.42))
box(a,.745,.30,.23,.26,"same-dest transfer state","Reliable delivery / congestion",col='light',size=11)
...
for j,(title,col) in enumerate([("endpoint",'blue'),("relationship binding",'orange'),("transfer status",'teal')]):ax.barh(range(3),vals[:,j],left=left,color=C[col],label=title);left+=vals[:,j]
ax.set(yticks=range(3),yticklabels=["per-relationship exclusive","shared by target","eight-way isolation"],xlabel="State capacity / MiB",xlim=(0,10.4),xticks=[0,2,4,6,8,10]);ax.invert_yaxis();ax.axvline(1,ls='--',color=C['red']);ax.legend(frameon=False,ncol=3,loc='lower right')
...
for x,t,c in [(.015,"write A",'pale'),(.26,"Recovery / visibility",'sand'),(.505,"notify B",'pale'),(.75,"independent transfer C",'light')]:box(graph,x,.32,.20,.34,t,col=c,size=11)
...
graph.text(.365,.83,"notify only after write",ha='center',fontsize=12,color=C['teal']);graph.text(.82,.12,"extra wait dependency",ha='center',fontsize=11,color=C['red'])
ax=f.add_axes([.18,.11,.77,.50])
colors={'write_data':'blue','recover_and_make_visible':'orange','publish_notification':'red','independent_transfer':'teal'};task_names={'write_data':"write",'recover_and_make_visible':"recovery",'publish_notification':"notify",'independent_transfer':"independent transfer"}
for i,key in enumerate(['strict','necessary_dependencies']):
 ...
 ax.text(-3,2.75-i*2,["execute all sequentially","keep only necessary dependencies"][i],ha='right',va='center',fontsize=11)
 ax.text([112,10][i]+1,2.45-i*2,f'{[112,10][i]} μs',va='center',color=C['teal'])
ax.set(xlim=(0,126),ylim=(0,3.6),yticks=[],xlabel="time / μs",xticks=[0,20,60,100,112]);ax.legend(handles=[Patch(color=C[v],label=task_names[k]) for k,v in colors.items()],frameon=False,ncol=4,loc='upper center',bbox_to_anchor=(.5,1.12))
...
queue_data={}
for name,label,col,ls in [('periodic-queue-aligned',"overlap 20 ms",'blue','-'),('periodic-queue-drift',"overlap 5 ms",'orange',':'),('periodic-queue-staggered',"no overlap",'teal','--')]:
...
axs[0].axhline(50,c=C['red'],ls='--',lw=1);axs[0].text(44,53,"Egress 50 GB/s",color=C['red'],fontsize=10)
axs[0].set(ylim=(-3,96),ylabel="Arrival rate / GB/s");axs[0].legend(frameon=False,ncol=3,loc='upper center',bbox_to_anchor=(.5,1.23))
axs[1].set(xlim=(0,60),ylim=(-20,1120),xlabel="Time / ms",ylabel="Backlog / MB");axs[1].text(21,1010,'1000 MB',color=C['blue']);axs[1].text(21,260,'250 MB',color=C['orange'])
...
f,a=canvas(6);panel(a,.015,.98,"resource dependency causing circular wait");panel(a,.53,.98,"reserve response resource; fixed request order")
xy=[(.04,.63),(.30,.63),(.30,.19),(.04,.19)]
for i,(x,y) in enumerate(xy):box(a,x,y,.15,.18,f'resource {chr(65+i)}',size=12)
arrow(a,(.195,.72),(.29,.72),'red');arrow(a,(.375,.625),(.375,.385),'red');arrow(a,(.295,.28),(.20,.28),'red');arrow(a,(.115,.385),(.115,.625),'red')
a.text(.245,.495,"hold prior\nwait for next",ha='center',va='center',fontsize=11,color=C['red'])
for i,(t,b) in enumerate([("request resource","sequential request"),("execution resource","process data"),("response resource","reserved buffer / priority send")]):
 y=.66-i*.25;box(a,.60,y,.31,.17,t,b,col='light',size=12)
 if i<2:arrow(a,(.755,y-.005),(.755,y-.07))
a.text(.755,.03,"release original request resource on response",ha='center',fontsize=10,color=C['muted'])
...
for group,(label,rd,ex) in enumerate([("original accelerator",ready,.4),("switching halved",ready,.2),("eliminate readiness skew",[0]*4,.4)]):
 base=10-group*4
...
ax.set(xlim=(0,2.85),xticks=[0,.5,1,1.5,2,2.5],ylim=(-.4,11),yticks=[],xlabel="Time / ms");ax.legend(handles=[Patch(color='#dbe2e6',label="not yet ready"),Patch(color='#f6e3c9',label="wait for other participants"),Patch(color=C['blue'],label="switching")],ncol=3,frameon=False,loc='upper center',bbox_to_anchor=(.5,1.15))
