...
def analyze():
 ...
 for iscancel in [False,True]:
  for round,pool in enumerate([True,False,False,True]):
   for stack in ['baseline','queqiao']:
    name=f'cancel-{str(iscancel).lower()}/round-{round}-pool-{str(pool).lower()}-{stack}';d=B/'runs'/name
 ...
 assert len(rows)==16
 return dict(conditions=rows,full_pcm_equal=8,cancel_prefix_equal=8,boundary='Actual muted output device callbacks. Python/pipe overhead included; DAC times are API estimates, not acoustic measurement; no WAN/model generation.')
