...
def create_sender(inputs):
    ...
    controller_config = p.get("controller")
    adapter = None
    ...
    def context(**extra):
        ...
        if pacer_hook:
            result["pacer_next_eligible"] = pacer_hook(extra.get("now", now), None)
        result.update(extra)
        return result

    def normalize(record, **extra):
        return {**record, "at": record["time"], **extra}

    def pacing_rate():
        if adapter:
            return F(adapter.pacing_rate_quic_bytes_per_second)
        return (
            F(str((controller_config or {}).get("pacing_gain", "5/4")))
            * cwnd
            / smoothed
        )
    ...
    def set_limited(at, app_limited, flow_limited):
        limited_flags.update(app_limited=app_limited, flow_limited=flow_limited)
        if adapter:
            adapter.on_limited(context(now=F(at)))

    def check_persistent():
        nonlocal cwnd
        if not controller_config or not unconsumed_losses:
            return
        eligible = [
            history[pn]
            for pn in unconsumed_losses
            if history[pn]["status"] == "lost"
            and history[pn]["ack_eliciting"]
            and rtt_known_at_send[pn]
        ]
        if not eligible:
            return
        all_lost = [
            h
            for h in lost_history.values()
            if h["status"] == "lost"
            and h["ack_eliciting"]
            and rtt_known_at_send[h["pn"]]
        ]
        if (
            len(all_lost) < 2
            or max(h["time"] for h in all_lost) - min(h["time"] for h in all_lost)
            <= 3 * pto_period()
        ):
            return
        records = [
            dict(
                ...
                sent_at=h["sent_at"],
                status=h["status"],
                ack_eliciting=h["ack_eliciting"],
                rtt_known_at_send=rtt_known_at_send[h["pn"]],
            )
        ...
        selected = [
            span
            for span in evidence["qualifying_spans"]
            if any(
                span["first_pn"] <= pn <= span["last_pn"] for pn in unconsumed_losses
            )
        ]
        if not evidence["established"] or not selected:
            return
        consumed = {
            pn
            for pn in unconsumed_losses
            if any(span["first_pn"] <= pn <= span["last_pn"] for span in selected)
        }
        unconsumed_losses.difference_update(consumed)
        if adapter:
            adapter.on_persistent(
                context(
                    evidence_key={"new_loss_declarations": sorted(consumed)},
                    loss_declarations=sorted(consumed),
                )
            )
            cwnd = F(adapter.cwnd_bytes)
        else:
            cwnd = F(2 * mds)
        persistent_events.append(
            dict(
                at=str(now),
                evidence=evidence,
                consumed_loss_declarations=sorted(consumed),
                cwnd_after=str(cwnd),
            )
        )
    ...
    def arm():
        ...
        if quantum is not None:
            ...
            for key, value in values.items():
                if value is None or (key == "cwnd" and adapter is not None):
                    continue
        ...
        if adapter:
            cwnd = F(adapter.cwnd_bytes)
        if controller_config and pacer_hook:
            pacer_hook(now, pacing_rate())
        generation += 1
        timer = None
    ...
    def state():
        ...
        if controller_config:
            result["controller"] = (
                adapter.snapshot()
                if adapter
                else {"controller": "newreno", "cwnd_bytes": str(cwnd)}
            )
            result["pacing_rate_quic_bytes_per_second"] = str(pacing_rate())
        return result
    ...
    def detect_loss():
        ...
        lost = []
        flight_before_loss = sum(
            h["sent_bytes"] for h in active.values() if h["in_flight"]
        )
        delay = max(F(9, 8) * max(latest, smoothed), granularity)
        ...
        if congestion:
            ...
            new_epoch = recovery_start is None or latest_lost > recovery_start
            if new_epoch:
                recovery_start = now
            if adapter:
                adapter.on_loss(
                    [normalize(h) for h in congestion],
                    context(
                        flight_before=flight_before_loss, new_recovery_epoch=new_epoch
                    ),
                )
                cwnd = F(adapter.cwnd_bytes)
                if new_epoch:
        ...
        if not controller_config and first_sample is not None:
            candidates = sorted(
                (
                    h
                    for h in history.values()
    ...
    def acknowledge(e):
        ...
        if not newly:
            if adapter and late:
                adapter.on_ack(
                    [
                        normalize(h, was_lost=True, inflight_released=False)
                        for h in late
                    ],
                    context(),
                )
        ...
        if adapter:
            records = [
                normalize(h, was_lost=False, inflight_released=h["in_flight"])
                for h in newly
            ]
            records += [
                normalize(h, was_lost=True, inflight_released=False) for h in late
            ]
            adapter.on_ack(
                records,
                context(
                    flight_before=ack_flight_before,
                    rtt_sample=(
                        rtt_events[-1] if len(rtt_events) > sample_cursor else None
                    ),
                ),
            )
            cwnd = F(adapter.cwnd_bytes)
    ...
    def process_next():
        ...
        if kind == "timer":
            ...
            if event["type"] == "sent":
                ...
                history[pn] = h
                if controller_config:
                    rtt_known_at_send[pn] = first_sample is not None
                ...
                if adapter:
                    adapter.on_sent(
                        ...
                        context(
                            flight_before=(
                                flight - h["sent_bytes"] if h["in_flight"] else flight
                            )
                        ),
                    )
        elif event["type"] == "ack":
        ...
        if kind == "external" and event["type"] == "ack":
            check_persistent()
        arm()
        ledger.append(
            {
    ...
    return dict(
        ...
        set_pacer=set_pacer,
        set_limited=set_limited,
        persistent_events=persistent_events,
        rtt_known_at_send=rtt_known_at_send,
        unconsumed_losses=unconsumed_losses,
    )
