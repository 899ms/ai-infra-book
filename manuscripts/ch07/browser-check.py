...
with sync_playwright() as p:
    ...
    for label,width,height in [('desktop',1440,1000),('mobile',390,844)]:
        ...
        page.goto((HERE.parent/'07-数据中心网络.html').as_uri(),wait_until='load')
        ...
        result['device']=label
        result['passed']=result['images']==result['loadedImages']==21 and result['formulas']==98 and result['mathErrors']==0 and result['documentWidth']<=width and not result['brokenAnchors']
        page.screenshot(path=str(HERE/f'preview-{label}.png'))
        page.locator('img').nth(2).scroll_into_view_if_needed()
        page.screenshot(path=str(HERE/f'preview-{label}-hierarchy.png'))
