...
fig.text(.055,.365,'B  H100 SXM：本章固定资源卡（标称供给，分别计量）',fontsize=13,weight='bold')
...
fig.text(.055,.914,'上排：70B／H100 教学下界，仅计权重与近似矩阵工作',fontsize=12,color=C['blue'])
...
print(json.dumps({'figures':6,'formats':['SVG','PNG'],'html':str(ht),'font':family},ensure_ascii=False))
