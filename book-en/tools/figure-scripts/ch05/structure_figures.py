...
def draw(here, data=None):
    ...
    with plt.rc_context(STYLE):
        ...
        f, a = canvas(3.6)
        for x, title in [(.02, "host memory (CPU)"), (.56, "GPU memory (GPU)")]:
            a.add_patch(Rectangle((x, .06), .42, .82, facecolor=COL['white'], edgecolor=COL['line'], lw=.9, ls=(0, (1, 2))))
            text(a, x + .21, .93, title, 12, ha='center')
        box(a, .06, .66, .34, .16, "Regular memory\npages can be swapped out", 'gray', 11)
        box(a, .06, .42, .34, .16, "Pinned buffer\nresident, directly transferable", 'blue', 11)
        box(a, .06, .14, .34, .16, "Result buffer", 'green', 11)
        box(a, .60, .66, .34, .16, "Weights\nload once, resident long-term", 'gray', 11)
        box(a, .60, .42, .34, .16, "input buffer", 'blue', 11)
        box(a, .60, .14, .34, .16, "Output", 'green', 11)
        arrow(a, (.23, .66), (.23, .58)); text(a, .26, .625, "Host-side copy", 11)
        ...
        f, a = canvas(3.4)
        text(a, .0, .70, "Copy stream", 12); text(a, .0, .32, "Compute stream", 12)
        box(a, .14, .62, .20, .16, "H2D batch 0\nwrite slot A", 'blue', 11)
        box(a, .38, .62, .20, .16, "H2D batch 1\nwrite slot B", 'green', 11)
        box(a, .76, .62, .20, .16, "H2D batch 2\nwrite slot A", 'blue', 11)
        box(a, .38, .24, .30, .16, "Compute batch 0\nread slot A", 'blue', 11)
        box(a, .72, .24, .24, .16, "Compute batch 1\nread slot B", 'green', 11)
        arrow(a, (.34, .62), (.40, .40), 'control'); text(a, .10, .49, "event: batch 0 transfer complete", 11)
        arrow(a, (.68, .40), (.76, .62), 'control'); text(a, .44, .53, "event: batch 0 consumed", 11)
        a.plot([.14, .96], [.10, .10], color=COL['line'], lw=.8); text(a, .96, .05, "time", 11, ha='right')
        out.save(f, 'figure-5-stream-event')

        # 5.5.1 Host preparation and accelerator compute: serial, pipelined, faster accelerator.
        f, a = canvas(4.6)
        scale = .80 / 160; x0 = .16
        panels = [(.70, "serial: 20 μs prep, 20 μs compute per segment, 4 segments, 160 μs total", 40, 20, 20, 0),
                  (.38, "pipelined: next segment prep overlaps current compute, 100 μs total", 20, 20, 20, 20),
                  (.06, "accelerator sped up to 5 μs: host still needs 20 μs per segment, 85 μs total", 20, 20, 5, 20)]
        for y, title, period, host, acc, lag in panels:
            text(a, .0, y + .27, title, 11)
            text(a, .0, y + .175, "host preparation", 11); text(a, .0, y + .045, "accelerator compute", 11)
            for i in range(4):
        ...
        f, a = canvas(3.2)
        for x, title, host_label in [(.03, "normal submission", "host: 6 kernel launches"), (.53, "graph replay", "host: 1 graph launch")]:
            text(a, x, .93, title, 12); text(a, x, .80, host_label, 11); text(a, x, .08, "accelerator: 6 kernels", 11)
            for i in range(6):
                bx = x + .072 * i
                box(a, bx, .18, .06, .14, str(i + 1), 'blue', 11)
                if x < .5:
                    box(a, bx, .56, .06, .14, str(i + 1), 'orange', 11); arrow(a, (bx + .03, .56), (bx + .03, .32))
        box(a, .53, .56, .16, .14, "figure", 'orange', 11)
        ...
        rows = [256, 1536, 2048]; buckets = [512, 2048, 2048]; labels = ["256 rows, 8 times", "1536 rows, 1 time", "2048 rows, 1 time"]
        a.barh(range(3), rows, color=COL['blue'], edgecolor=COL['line'], height=.55, label="actual row count")
        a.barh(range(3), [b - r for b, r in zip(buckets, rows)], left=rows, color=COL['gray'], edgecolor=COL['line'], height=.55, label="pad to bucket")
        for i, b in enumerate(buckets): a.text(b + 40, i, f'bucket {b}', va='center', fontsize=11)
        a.set(yticks=range(3), yticklabels=labels, xlim=(0, 2500), xlabel="rows executed per call"); a.invert_yaxis()
        ...
        for row, (title, left, right, result, c) in enumerate([
                ("split M: output rows", "input row sharding", "weight replication", "different output rows", 'blue'),
                ("split N: output columns", "input replication", "weight column sharding", "different output columns", 'green'),
                ("split K: reduction dim", "input column sharding", "weight row sharding", "partial sums of same output", 'orange')]):
            y = .71 - row * .30
        ...
        text(a, .50, .04, "output shards gathered on demand; reduction shards must be summed", 12, ha='center')
        ...
        f, a = canvas(3.6)
        text(a, .0, .95, "two resident thread blocks (blue: block 0, green: block 1)", 12)
        x0, w = .30, .62
        rows = [("registers: 64K", block['threads'] * block['registers_per_thread'] / limits['registers'], "two blocks fill up, third block doesn't fit", True),
                ("shared memory: 228 KB", (block['shared_bytes'] + block['reserved_shared_bytes']) / limits['shared_bytes'], "34 KiB left, third block doesn't fit", True),
                ("thread slots: 2048", block['threads'] / limits['threads'], "512 used, 6 more blocks fit", False)]
        for i, (label, frac, note, binding) in enumerate(rows):
            ...
            text(a, x0, y - .065, ("limit residency:" if binding else '') + note, 11)
        text(a, .0, .03, f'resident {resident} blocks = {resident * block["threads"] // 32} warps, occupancy {resident * block["threads"] // 32}/64 = {resident * block["threads"] // 32 * 100 // 64}%', 12)
        ...
        a.text(full[0][0] + .05, .5, "full", fontsize=11, va='center'); a.text(empty[0][1] + .05, .62, "empty", fontsize=11, va='center')
        a.text((computes[0]['start'] + compute_us + computes[1]['start']) / 2, -.42, "waiting for data", fontsize=11, ha='center', va='center')
        a.set(yticks=[0, 1], yticklabels=["consumer warp\ncompute", "producer warp\ncopy"], xlim=(0, 5.8), ylim=(-.6, 1.6),
              xticks=[0] + [round(c['start'] + copy_us, 2) for c in copies], xlabel="Time (μs)")
        a.set_xticklabels(['0'] + [f"{c['start'] + copy_us:.2f}" for c in copies])
        f.legend(handles=[Patch(fc=COL[c], ec=COL['line'], label=l) for c, l in [('blue', "slot A: blocks 0, 2"), ('green', "slot B: blocks 1, 3")]],
