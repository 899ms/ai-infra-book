...
with sync_playwright() as p:
    ...
    for label,width,height in [('desktop',1440,1000),('mobile',390,844)]:
        ...
        page.goto((HERE.parent/'10-训练系统.html').as_uri(),wait_until='load')
        ...
        result['device']=label
        result['passed']=result['images']==result['loadedImages']==20 and result['formulas']==json.loads((HERE/'math-validation.json').read_text())['expressions'] and result['mathErrors']==0 and result['documentWidth']<=width and not result['brokenAnchors']
        page.screenshot(path=str(HERE/f'preview-{label}.png'))
        page.locator('img').nth(6).scroll_into_view_if_needed()
        page.screenshot(path=str(HERE/f'preview-{label}-pipeline.png'))
