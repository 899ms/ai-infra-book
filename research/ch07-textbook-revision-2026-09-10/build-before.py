...
n=np.arange(1,9);cut=8*2**30/50e9*1000;compute=100/n
...
a.text(4.8,151,'固定割集：约 172 ms',color=C['teal'])
a.set(xlabel='设备数相对倍数',ylabel='每步时间 / ms',xticks=n,yticks=[0,50,100,150,200,250,300],ylim=(0,315));a.legend(frameon=False,loc='upper right')
...
f,ax=plt.subplots(figsize=(10,5.8));f.subplots_adjust(left=.12,right=.96,bottom=.14,top=.89)
...
save(f,'figure-7-3-hierarchy');data['7-3']={'kind':'saved_calculation','sources':names,'local_MiB':local,'remote_MiB':remote,'lower_ms':ts,'plotted':'local_MiB and remote_MiB; lower_ms supports prose only'}
...
f,ax=plt.subplots(figsize=(11,5.8));f.subplots_adjust(left=.18,right=.95,top=.83,bottom=.14)
...
for i,key in enumerate(['strict','necessary_dependencies']):
 ...
 ax.text(-3,2.75-i*2,['全部按完成串行','仅保留必要依赖'][i],ha='right',va='center',fontsize=11)
 ax.text([112,10][i]+1,2.45-i*2,f'{[112,10][i]} μs',va='center',color=C['teal'])
ax.set(xlim=(0,126),ylim=(0,3.6),yticks=[],xlabel='时间 / μs',xticks=[0,20,60,100,112]);ax.legend(handles=[Patch(color=C[v],label=task_names[k]) for k,v in colors.items()],frameon=False,ncol=4,loc='upper center',bbox_to_anchor=(.5,1.2))
save(f,'figure-7-6-ordering');data['7-6']={'kind':'saved_calculation','schedules':{k:order['request_schedules'][k] for k in ['strict','necessary_dependencies']},'stale_read_example':order['stale_read_example'],'plotted':'schedules only; stale_read_example supports prose'}
...
save(f,'figure-7-7-congestion');data['7-7']={'kind':'saved_calculation','periodic_segments':queue_data,'feedback':calc('feedback-queue-overflow'),'packets':packet_data,'plotted':'periodic_segments only; feedback and packets support prose'}
...
save(f,'figure-7-9-progress');data['7-9']={'kind':'teaching','ready_ms':ready,'exchange_ms':.4,'ring_us':ring.tolist(),'measured':m,'plotted':'ready_ms and exchange_ms only; ring and CPU observations support prose'}
