...
[独立GPU实验](gpu-tiles/README.md)补齐K4096/N12288实际GEMM、六份完整FP64数值核验及六组NCU流量。M1的本组最快候选32×64，M1024为128×128；后者64×64／128×128块的L2流量约3096／1560MiB，显存读却均约56MiB，说明L2重读不能全部算作显存访问。固定缓冲、共享GPU，记录不作为冷缓存或端到端收益；bank布局和拆分归约仍待。

## RMSNorm拆分归约补测

[整行／拆分及分段整行控制](rmsnorm-split/README.md)完成两行宽、三行数的实际CUDA trace与15份全输出核验。4096列整行更快；65536列整行向量出现溢出，分段控制消除溢出后，1024行时间从约2286µs降至267µs，与拆分约262µs接近，不能把原差距全部归于并行度。各路径容差均过，同路径跨batch前缀逐位相同，不同路径则有少量不同元素；270批计时与九trace完整保留。bank布局及跨kernel并发仍待。
