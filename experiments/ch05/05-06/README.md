...
**2026-09-14更新：** [独占GPU复测](EXCLUSIVE-2026-09-14.md)与[第四次真实profiler反馈Agent](FEEDBACK-2026-09-14.md)已完成；后者产生5个实际新候选并通过所选候选的5组留出。原schedule三次完整请求复测均无中位收益；native/schedule/Agent三策略99请求对照已完成，输出一致但没有重复确认的正向请求收益，保留native，不能支持有限正向摊销。该实验按完整结果交付，以下partial状态仅描述早期批次。以下旧批次限制按历史保留，不代表最新候选仍未产生。
...
[实际预算、profiler及交错计时](EXCLUSIVE-2026-09-14.md)：六个schedule全部通过，256/4仍最低分；Agent六次重复AST，55.729秒内无新候选。已核验原trace热点没有内部GPU工作重叠；不把无并发的原路径当作未做争用实验。后续起始profiler反馈和同窗口请求验证仍待。
