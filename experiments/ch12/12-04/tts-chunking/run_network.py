...
MODE=os.environ['BOOK_CHUNK_MODE'];FIXTURE=json.loads((R/(MODE+'-fixture.json')).read_text());PAYLOAD=(R/FIXTURE['upload_file']).read_bytes();RESPONSE=(R/FIXTURE['response_file']).read_bytes();HASH=hashlib.sha256(PAYLOAD).hexdigest();RESPONSE_HASH=hashlib.sha256(RESPONSE).hexdigest();SERVICE_S=FIXTURE['service_s']
...
class H3(QuicConnectionProtocol):
 ...
 def quic_event_received(self,event):
  ...
  for e in self.http.handle_event(event):
   ...
   if isinstance(e,HeadersReceived):state['headers']=[(k.decode(),v.decode()) for k,v in e.headers];state['first_headers']=NOW()
    ...
    state.setdefault('arrivals',[]).append(dict(at=NOW(),bytes=len(e.data),total=len(state['body'])))
 ...
 async def reply(self,stream,record):
  record['service_start']=NOW();record['releases']=[]
  self.http.send_headers(stream,[(b':status',b'200'),(b'content-length',str(len(RESPONSE)).encode()),(b'content-type',b'audio/wav')]);self.transmit()
  for index,part in enumerate(FIXTURE['release_plan']):
   await asyncio.sleep(max(0,record['service_start']+part['ready_s']-NOW()))
   offset=part['offset'];data=RESPONSE[offset:offset+part['bytes']]
   self.http.send_data(stream,data,end_stream=index==len(FIXTURE['release_plan'])-1);self.transmit()
   record['releases'].append(dict(offset=offset,bytes=len(data),at=NOW()))
...
async def main(out,smoke):
 ...
 async def h1_server(reader,writer):
  ...
  try:
   while True:
    ...
    record=dict(at=NOW(),bytes=len(body),sha256=hashlib.sha256(body).hexdigest(),headers=headers,peer=writer.get_extra_info('peername'));server_rows.append(record)
    record['service_start']=NOW();record['releases']=[]
    writer.write(conn.send(h11.Response(status_code=200,headers=[(b'content-length',str(len(RESPONSE)).encode()),(b'content-type',b'audio/wav')])));await writer.drain()
    for part in FIXTURE['release_plan']:
     await asyncio.sleep(max(0,record['service_start']+part['ready_s']-NOW()))
     offset=part['offset'];data=RESPONSE[offset:offset+part['bytes']]
     writer.write(conn.send(h11.Data(data=data)));await writer.drain()
     record['releases'].append(dict(offset=offset,bytes=len(data),at=NOW()))
    writer.write(conn.send(h11.EndOfMessage()));await writer.drain();record['service_end']=NOW();conn.start_next_cycle()
 ...
 env=dict(python=platform.python_version(),platform=platform.platform(),machine=platform.machine(),openssl=ssl.OPENSSL_VERSION,packages={p:importlib.metadata.version(p) for p in ['aioquic','h11','cryptography','pylsqpack']},certificate_sha256=fingerprint,source_sha256=hashlib.sha256((R/'run_network.py').read_bytes()).hexdigest(),tcp_port=tcp_port,udp_port=udp_port,payload_bytes=len(PAYLOAD),payload_sha256=HASH,response_bytes=len(RESPONSE),response_sha256=RESPONSE_HASH,service_time_replay_s=SERVICE_S,fixture=FIXTURE,smoke=smoke,server_and_client_same_event_loop=True)
 ...
 try:
  ...
  order=[]
  for trial in [int(os.environ['BOOK_TRIAL'])]:
   conditions=[(proto,reuse,c) for proto in ['h1','h3'] for reuse in [False,True] for c in [1]];random.Random(1204+trial).shuffle(conditions)
