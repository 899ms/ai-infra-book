...
for j,(label,color) in enumerate(zip(["model weights","KV state","per-card workspace"],['blue','teal','orange'])):
...
ax.axvline(80,color=C['red'],ls='--');ax.text(82,1.48,"H100 capacity 80 GB",ha='left',color=C['red'])
ax.set(yticks=[1,0],yticklabels=["single-card deployment","each card in 8-card TP"],xlim=(0,560),xticks=[0,80,200,300,400,500],ylim=(-.65,1.7),xlabel="Memory footprint per card / GB");ax.legend(loc='lower left',frameon=False,ncol=3,fontsize=10)
...
f,a=canvas(7.6)
a.text(.04,.965,"each row: one expert group, 2-card TP",fontsize=14,va='top')
a.text(.59,.965,"each column: reduction across 4 expert groups",fontsize=14,va='top',color=C['orange'])
for ep in range(4):
 y=.74-ep*.205
 a.text(.018,y+.06,f'EP {ep}\nexpert {ep*32}—{ep*32+31}',va='center',fontsize=11)
 for tp in range(2):
  x=.21+tp*.43
  box(a,x,y,.28,.15,f'card {2*ep+tp}: expert matrix'+("left half" if tp==0 else "right half"),f'same-request KV head '+('0、1' if tp==0 else '2、3'),col='light' if tp==0 else 'pale',size=11)
 a.annotate('',xy=(.635,y+.075),xytext=(.495,y+.075),arrowprops={'arrowstyle':'<->','color':C['teal'],'lw':1.5})
 if ep==0:a.text(.565,y+.12,"① TP sum",ha='center',fontsize=10,color=C['teal'])
for x in [.35,.78]:
 for ep in range(3):a.annotate('',xy=(x,.74-(ep+1)*.205+.155),xytext=(x,.74-ep*.205-.005),arrowprops={'arrowstyle':'<->','color':C['orange'],'lw':1.5})
a.text(.975,.40,"② EP\nsum",ha='right',fontsize=11,color=C['orange'])
a.text(.5,.025,"each row repeats attention for same request; expert compute split across 4 rows",ha='center',fontsize=12)
save(f,'ep-layout')
...
for ax,vals,title,detail in zip(axs,loads,["128 experts selected uniformly","eight experts concentrated in one group","same eight experts spread across 4 groups"],["read 4.5 GiB weights","read 288 MiB weights","read 288 MiB weights"]):
...
axs[0].set_ylabel("token-expert compute count per group")
...
for key,label,color in [('local_s',"local memory access",'blue'),('communication_s',"128 reductions",'orange')]:
...
ax.set(yticks=ys,yticklabels=['TP 1','TP 2','TP 4','TP 8'],xlim=(0,34),xticks=[0,5,10,15,20,25,30],xlabel="Execution time per decode / ms");ax.legend(loc='upper center',bbox_to_anchor=(.5,1.18),ncol=3,frameon=False)
save(f,'tp-time')
# Ports are discrete resources; the same 32 slots must be split between directions.
f,a=canvas(5.8)
for y,down,up in [(.62,32,32),(.18,48,16)]:
 box(a,.015,y,.21,.23,f'{down} GPU ports',f'total input {down*25} GB/s',col='light')
 box(a,.77,y,.215,.23,f'{up} uplink ports',f'total bandwidth {up*25} GB/s',col='sand')
 arrow(a,(.23,y+.12),(.30,y+.12));arrow(a,(.70,y+.12),(.76,y+.12),'orange')
 a.text(.5,y+.27,f'64 ports:{down} downlink / {up} uplink',ha='center',fontsize=13)
 for i in range(64):
  row=i//32;col=i%32
  a.add_patch(Rectangle((.305+col*.012,y+.145-row*.09),.009,.06,fc=C['teal' if i<down else 'orange']))
a.text(.5,.045,"fixed total ports: more devices connected reduces uplink bandwidth",ha='center',fontsize=13)
...
for x,label in [(.05,"left half"),(.57,"right half")]:
...
arrow(a,(.415,.50),(.56,.50),'orange');a.text(.49,.59,"middle cross-section\nk² links",ha='center',fontsize=11,color=C['orange'])
# Wrap-around connections meet the other cut surface.
a.plot([.09,.09,.88,.88],[.28,.13,.13,.28],color=C['teal'],lw=2)
a.text(.49,.04,"end-to-end connection: k² more links",ha='center',fontsize=12,color=C['teal'])
a.text(.49,.89,"split in half along one dimension",ha='center',fontsize=14)
ax=f.add_axes([.67,.23,.30,.56]);xx=np.arange(2)
ax.bar(xx-.17,[1,8],.32,color=C['blue'],label="device count")
ax.bar(xx+.17,[1,4],.32,color=C['orange'],label="bisection link count")
for x,y,t in [(-.17,1,'64'),(.17,1,'32'),(.83,8,'512'),(1.17,4,'128')]:ax.text(x,y+.18,t,ha='center',fontsize=11)
ax.set(xticks=xx,xticklabels=['k = 4','k = 8'],ylim=(0,10),ylabel="multiple relative to k = 4");ax.legend(frameon=False,loc='upper left',fontsize=10)
...
for ax,title in zip(axs,["before borrowing: task 0 short 20 GB","after borrowing: 20 GB stored on node 1"]):
 ax.bar(range(4),[80]*4,color='white',edgecolor=C['line'],width=.65)
 ax.set(xticks=range(4),xticklabels=["node 0","node 1","node 2","node 3"],ylim=(0,111),yticks=[0,20,40,60,80,100]);ax.set_title(title,fontsize=13)
 ax.axhline(80,ls=':',color=C['muted']);ax.text(3.4,81,"capacity 80 GB",ha='right',fontsize=10)
...
axs[1].annotate("task 0's remote data",xy=(1,80),xytext=(1.6,99),arrowprops={'arrowstyle':'->','color':C['blue']},fontsize=11,color=C['blue'])
axs[0].set_ylabel("Memory requirement and footprint / GB")
...
f,a=canvas(6.5)
a.text(.11,.94,"issue read request",ha='center',fontsize=13);a.text(.68,.94,"receive returned data",ha='center',fontsize=13)
for i in range(4):
 y=.76-i*.155
 box(a,.025,y,.16,.09,f'request {i}',size=11)
 arrow(a,(.19,y+.045),(.61,y+.045),'blue')
 box(a,.62,y,.14,.09,"q bytes",col='light',size=11)
 a.text(.38,y+.068,"waiting for remote return",ha='center',fontsize=10,color=C['muted'])
a.annotate('',xy=(.18,.16),xytext=(.77,.16),arrowprops={'arrowstyle':'<->','color':C['ink']});a.text(.475,.09,"round-trip time L",ha='center',fontsize=12)
box(a,.81,.36,.17,.28,"in-flight window","u requests\ntotal uq bytes",col='sand',size=12)
a.text(.48,.015,"Window bandwidth upper bound = uq / L",ha='center',fontsize=13)
...
for ax,c in zip(axs.flat,continuous['candidates'][1:]):
 ...
 for request in range(4):
  ...
  ax.text(start+service/2,shown-1-instance,f'Session {request}',ha='center',va='center',color='white',fontsize=10)
 ax.axvline(130,color=C['muted'],ls='--',lw=1)
 ax.set(yticks=range(shown),yticklabels=[f'Instance {i}' for i in range(shown-1,-1,-1)],xlim=(0,180),ylim=(-.6,max(shown-.4,1)),xticks=[0,30,60,90,120,150,180])
 ax.set_title(f'{instances} '+("single card" if p==1 else f'{p} cards')+"instance"+("(4 idle instances)" if p==1 else ''),fontsize=12,loc='left')
 ax.text(max(c['healthy_completion_ms']),-.42,f"{max(c['healthy_completion_ms']):.1f} ms",ha='right',va='top',fontsize=10)
for ax in axs[1]:ax.set_xlabel("Time from simultaneous arrival of four sessions / ms")
