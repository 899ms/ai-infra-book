...
for L,col in [(2048,'blue'),(8192,'teal')]:
 kv=L*147456/2**30;a.plot(bs,weight+kv,color=C[col],lw=2.5,label=f'{L//1024}K 上下文：總讀取');a.axhline(kv,color=C[col],ls='--',lw=1.2,label=f'{L//1024}K 上下文：KV')
...
for j,(policy,label) in enumerate([('fixed',"固定 batch"),('continuous',"連續批處理"),('chunked',"Decode 優先分塊")]):
 ...
 s=d['summary'];a.set(yticks=range(4),yticklabels=['r3','r2','r1','r0'],xlim=(0,900),xticks=[0,150,300,450,600,750,900],ylim=(-.85,3.85),xlabel="時間 / ms");a.set_title(label+f"  ·  總時間 {s['finish_ns']/1e6:.0f} ms，最大間隔 {s['max_itl_ns']/1e6:.0f} ms",loc='left',fontsize=12,pad=9)
...
f,a=canvas(8.5);lengths=[9,13,5,15];styles=[("整段預留",64),("每塊 4 位置",52),("A、B 共享前 8 位置",44)]
# A and B both have at least eight valid positions; two complete blocks can be shared.
for j,(lab,total) in enumerate(styles):
 top=.93-j*.29;a.text(.025,top,lab,fontsize=14,weight='bold');a.text(.975,top,f'{total} 個物理位置',ha='right',fontsize=13,color=C['teal'])
 ...
 a.text(.20,top-.24,"藍：有效位置    灰：分配但未用    斜線：引用已有塊（不新分配）",fontsize=10,color=C['ink'])
...
tree(list(range(4)),0)
f=plt.figure(figsize=(13,6.5));a=f.add_axes([.05,.14,.39,.73]);a.set(xlim=(-.3,4.7),ylim=(-.5,3.6));a.axis('off');a.set_title("實際前四輪的壓縮字首樹",loc='left',fontsize=14)
levels={}
...
for i,n in enumerate(nodes):
 ...
 if len(n['ids'])==1:a.text(x+.12,y,f"輪 {n['ids'][0]+1}：{n['depth']}",va='center',fontsize=10)
 elif n['parent'] is None:a.text(x-.08,y-.32,f"共同 {n['depth']}",fontsize=10)
a.text(0,-.43,"邊：新增 token 數；葉：總輸入 token 數",fontsize=10)
...
for y,buff,count in [(.66,288,2),(.29,576,1)]:
 ...
 if remaining:
  a.add_patch(Rectangle((pos,y),remaining*scale,.13,fc=C['gray']));a.text(pos+remaining*scale/2,y+.065,f'餘 {remaining}',ha='center',va='center',fontsize=12)
 a.text(.20,y-.085,f'淨空間 {2592-buff} MiB → {count} 條完整上下文',fontsize=13,color=C['ink'])
a.text(.5,.04,"橙：預取緩衝    綠：完整 KV 上下文    灰：不足一條的餘量（單位：MiB）",ha='center',fontsize=11)
...
for a,m,title,total in zip(axs,quality_matrices,['BF16 KV',"原 FP8 實作",'FP8 KV ＋ BF16 Q'],correct):
 a.imshow(m,cmap=ListedColormap([C['red'],C['teal']]),vmin=0,vmax=1,aspect='auto');a.set(xticks=range(4),xticklabels=['1 / 1','1 / 2','4 / 1','4 / 2'],yticks=range(8),yticklabels=task_ids,xlabel="併發數 / 重複序號");a.set_title(f'{title}\n正確 {total}/32',fontsize=12)
...
axs[0].set_ylabel("任務 ID");f.text(.55,.035,"每格為一次自然生成：綠色 ○ 正確，紅色 × 錯誤",ha='center',fontsize=11)
...
for name,prob,col in [('AAAA',.25,'red'),('BBBB',.75,'teal')]:
 ...
 a.plot(queries,(queries+T0)/out,color=C[col],lw=2.5,label=f'{name}：平均輸出數 {out:.2f} token/輪')
...
for x,col in zip(crosses,['red','teal']):
 a.scatter([x],[T0],color=C[col],s=45);a.annotate(f'查詢約 {x:.1f} ms',xy=(x,T0),xytext=(x+1.5,T0+6),arrowprops={'arrowstyle':'->','color':C[col]},fontsize=11)
a.set(xlabel="每輪查詢時間 / ms",ylabel="平均每個輸出 token 的耗時 / ms",xlim=(0,60),ylim=(0,70),xticks=[0,10,20,30,40,50,60]);a.grid(alpha=.2);a.legend(frameon=False,loc='upper left')
...
for frac,col in [(.2,'blue'),(.5,'teal'),(.8,'orange')]:
 yy=1/(1-frac+frac/ss);a.plot(ss,yy,lw=2.4,color=C[col],label=f'Decode 佔基線 {int(frac*100)}%')
a.scatter([4],[1.6],color=C['teal']);a.annotate("區域性 4 倍 → 任務 1.6 倍",xy=(4,1.6),xytext=(4.4,2.25),arrowprops={'arrowstyle':'->','color':C['teal']},fontsize=11);a.set(xlabel="Decode 區域性加速比",ylabel="完整任務加速比",xlim=(1,8),ylim=(1,3.8));a.grid(alpha=.2);a.legend(frameon=False,loc='upper left');a.set_title("其他階段保持不變、無新增成本〔教學上限〕",loc='left',fontsize=13)
...
f,a=plt.subplots(figsize=(12,6.5));f.subplots_adjust(left=.09,right=.97,bottom=.20,top=.91);labels=["逐個\n1/s","逐個\n4/s","逐個\n16/s","連續\n1/s","連續\n4/s","連續\n16/s","離線\n突發"];xs=np.arange(7)
...
a.set(ylabel="視窗吞吐 / req/s",ylim=(0,3.1),yticks=[0,.5,1,1.5,2,2.5,3],xticks=xs,xticklabels=labels);a.legend(frameon=False,fontsize=11,loc='upper left');a.grid(axis='y',alpha=.15)
...
page="<!doctype html><html lang=\"zh-CN\"><head><meta charset=\"utf-8\"><meta name=\"viewport\" content=\"width=device-width,initial-scale=1\"><title>第 8 章 推理最佳化</title><style>"+css+mathcss+"</style></head><body><main><nav aria-label=\"本章目录\">"+nav+'</nav>'+body+'</main></body></html>';hp=preview_path(md)
zoom="<dialog id=\"figure-view\" aria-label=\"放大插图\"><button id=\"close-figure\" type=\"button\">關閉</button><div id=\"figure-content\"></div></dialog><script>\nconst view=document.getElementById('figure-view'),content=document.getElementById('figure-content');\nfor(const img of document.querySelectorAll('main img')){img.tabIndex=0;img.setAttribute('role','button');img.setAttribute('aria-label',img.alt+'，點選放大');const show=()=>{content.replaceChildren(img.cloneNode());content.firstChild.removeAttribute('role');content.firstChild.removeAttribute('tabindex');view.showModal();};img.addEventListener('click',show);img.addEventListener('keydown',e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();show();}});}\ndocument.getElementById('close-figure').onclick=()=>view.close();view.addEventListener('close',()=>content.replaceChildren());\n</script>"
