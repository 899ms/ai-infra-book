...
前面分别分析了计算、存储、搬运和互联。本节把这些部件放回 NVIDIA、昇腾、Apple 的代际变化中，计算新增机制究竟减少了什么工作、改变了哪项资源需求，再解释这些变化如何回应模型负载。

比较时固定模型、精度和输入形状，沿用前面工作量除以资源速率的方法：矩阵改进看乘加时间，数据通路改进看传递的字节与接口带宽，存储扩展看能够驻留的状态。先算局部变化，再判断剩余工作会不会成为新的瓶颈。全部数字由硬件参数表、固定模型配置和配套计算程序生成；第 4.8 节再将这些结果合成为设备选择所需的时间模型。[^evolution-quant]
...
![同一个 4096×4096 权重的存储与读取预算。每 32 个低精度值共享一个一字节缩放因子。位宽从 BF16 降至 MXFP4 后，权重与缩放因子合计从 32 MiB 减至 8.5 MiB。](ch04/figure-4-evolution-precision.svg)

*图 4-23　同一个 4096×4096 权重的存储与读取预算。每 32 个低精度值共享一个一字节缩放因子。位宽从 BF16 降至 MXFP4 后，权重与缩放因子合计从 32 MiB 减至 8.5 MiB。*

从 33.29 μs 到 4.97 μs，权重读取预算缩小约 6.69 倍。这笔收益由两部分相乘：带宽约提高 1.78 倍，存储量约缩小 3.76 倍。若软件每次将 MXFP4 展开成 BF16 再乘，还要加入转换和展开副本的访问；原生低精度矩阵路径则把相应表示交给专门指令。转换与缩放的逐步计算见第 4.2.4 节。
...
取 $M=N=128,K=64$ 的 BF16 计算块，A、B 合计 32 KiB，FP32 累加结果为 64 KiB。三组输入缓冲加一组累加结果，共占 160 KiB 的逻辑存储。累加结果改由 TMEM 保存，便把这 64 KiB 从通用寄存器承担的矩阵状态中分离出来；三组输入保留在缓冲中，通用寄存器腾出的空间可用于线程状态。第 4.4 节的三槽算例从两槽的 768 tick 降到 704 tick，节省约 8.3%；多出的缓冲使数据准备提前完成，矩阵单元的等待随之减少。
...
RTX 3090 与 4090 容量相同，BF16 矩阵速率约提高 2.33 倍，带宽却只提高约 7.7%。前者主要缩短大矩阵计算，后者才直接缩短单行权重读取。RTX 5090 的容量提高到 32 GB，带宽再提高约 1.78 倍；其新增 FP4 路径还让能够采用低精度的模型改变执行方法。这三代产品分别改变了矩阵处理、权重读取和模型驻留的资源预算。
...
数据中心卡还提供 HBM 与面向多卡的高速互联。A100 SXM 的 NVLink 双向聚合带宽为 600 GB/s，H100 SXM 为 900 GB/s，换成单向发送分别为 300、450 GB/s。按第 4.5 节的方法，传送 64 MiB 数据的链路序列化时间分别约为 224、149 μs。计算规模增加后，张量并行每层的通信、训练梯度交换和专家分派会反复使用这些连接。RTX 4090、5090 没有 NVLink；RTX 3090 支持双卡 NVLink。后续并行计算章节将用这些链路参数计算张量切分、梯度交换与专家分派的时间。[^nvidia-product-lines]
...
若先在 HBM 中生成展开矩阵，再由矩阵乘法读入，额外发生一次 3.445 MiB 写入和一次读取，共 6.891 MiB。早期 910 的 HBM 带宽为 1.2 TB/s，仅这笔展开矩阵的写读就占约 6.02 μs 的带宽时间。MTE 在片上搬运时完成窗口展开，便省去这份 HBM 中间副本的写读。输入窗口在片上组织后直接送入矩阵计算，HBM 中保留原始输入和权重。[^ascend-history]
...
同一 Cube 上，CNN 卷积的矩阵工作占 28,224 周期，注意力块只占 1,024 周期；向量工作却从简单 ReLU 变成指数、归约与归一化。早期论文的 BERT 分析中，多数层由 Cube 时间主导。把注意力单独拆开后，可以看到 Softmax 对向量吞吐提出的新需求。[^ascend-history]
...
*图 4-27　同一注意力块的两次交接。外层交换接口承担写出和读入共 256 KiB；直接 CV 通路传递两个 64 KiB 张量，将这笔流量移出外层接口。在 1.024 μs 内完成交接，对应直接通路的带宽需求为 128 GB/s。*

950 同时将单个 Vector Core 的 FP16／FP32 吞吐提高一倍，在 UB 与向量 ALU 之间增加寄存器文件，并优化 Softmax、GELU 的执行。对相同普通向量工作 $F_v$，$F_v/P_v$ 降为 $F_v/(2P_v)$；如果原来向量阶段是矩阵阶段的两倍，这项改动能把二者重新配平。前面的注意力块进一步把指数吞吐需求确定为每周期 16 个结果。新增 NDDMA 与 BufferID 则分别减少地址、布局准备和缓冲同步管理。[^ascend]
...
Apple 的 CPU、GPU 和独立 Neural Engine 共享统一内存。它减少了不同处理器间复制数据的需要，也让本地模型使用较大的内存池。下面先算统一内存能够装下的模型，再算生成一个 token 需要读取多少数据。
...
取 Qwen3-8B、BF16、单请求、8,192 位置，本书张量清单给出每步权重与 KV 的接口载荷约 16.345 GB。M1／M2 Ultra 的读取下界为 $16.345/800\approx20.43$ ms，M3 Ultra 为 $16.345/819\approx19.96$ ms，M4 Max 为 $16.345/546\approx29.94$ ms。同样是 128 GB，M1 Ultra 读取这一步数据比 M4 Max 少用约 9.51 ms。[^evolution-quant]
...
再把 Qwen3-235B-A22B 增加到八条并发请求，比较两组专家路由。八条请求、8K 上下文的常驻预算都为约 137.906 GB；若请求集中使用相同专家，每步载荷约 24.737 GB，在 M3 Ultra 上为 30.20 ms；若路由分散、覆盖更多专家，每步载荷约 75.967 GB，为 92.76 ms。每步产出八个 token，对应的读取吞吐预算分别为 $8/0.03020\approx265$ token/s 和 $8/0.09276\approx86$ token/s。专家复用减少了约 51.23 GB 的每步读取量，使同一台机器的这项预算提高约 3.07 倍。
...
参数表中还收录了已公布的 M5 Ultra 512 GB、1,200 GB/s 规格。对相同 16.345 GB 载荷，读取预算为约 13.62 ms，比 M3 Ultra 的 19.96 ms 缩短约 31.8%；这项计算由带宽变化得出。[^apple-capacity-quant]

三条路线都可以用本节的工作量变化理解：Tensor Core 缩短规则矩阵计算，img2col 省去卷积展开的中间写读，CV 通路减少注意力的外层交换，统一内存扩展模型与 KV 的驻留空间。大矩阵、卷积、注意力和大型 MoE 分别把这些改动变成了乘加时间、展开流量、交接流量和常驻容量的变化。图 4-3 的跨代协同由这些量连接起来：负载改变了资源需求，架构改变了执行成本，模型的精度、批次和并行方式再据此调整。第 4.8 节将这些代际参数用于同一模型的设备比较；下面先考察进一步固定计算与数据条件的专用设计。
...
令 $BK=W$，可求得 KV 读取量与权重读取量相等的转折点，约为 $B=12.5$；从整数 batch 13 起，KV 读取量超过权重读取量。图 4-29 用水平线与斜线展示了这一转移。

![固定权重的同时，各请求仍独立读取 KV。题设每请求保留 8K 上下文，batch 从十三起 KV 读取超过共享权重读取。](ch04/figure-4-12-specialization.svg)

*图 4-29　固定权重的同时，各请求仍独立读取 KV。题设每请求保留 8K 上下文，batch 从十三起 KV 读取超过共享权重读取。*
...
![图 4-30 独立只读权重改变两条存储路径。上方权重与 KV 争用 HBM；下方 ROM 提供权重，HBM 存放可写状态。箭头表示读取，KV 还需写入新状态。](ch04/figure-4-rom-paths.svg)

*图 4-30　独立只读权重改变两条存储路径。上方权重与 KV 争用 HBM；下方 ROM 提供权重，HBM 存放可写状态。箭头表示读取，KV 还需写入新状态。*

## 4.8 根据架构分析性能、选择加速器

第 4.6、4.7 节计算了不同架构怎样改变计算、存储与交换工作。本节把这些结果用于设备选择：先合成执行时间模型，再用同一模型比较候选设备，随后用实测校准，最后比较完成任务的成本与能耗。
...
资源耗时与执行依赖确定以后，还可以求出负载变化时的瓶颈转折点。对本章的 Q 投影，Roofline 模型把这一关系写成算术强度与吞吐的关系。
...
*图 4-31　同一 Q 投影的计算与访存耗时随输入行数的变化。计算量按行数增长，片外访问同时包含固定权重和增长的输入输出；从 179 行起计算项较长。*

图 4-31 中，计算时间随行数成比例增加，数据传输时间却只缓慢增长，因为 32 MiB 权重始终只读一次。曲线交点左侧，等待数据占主要时间；右侧，每次读入的数据已经支持足够多运算，矩阵计算成为较慢的一项。
...
现在用这一方法从单个投影扩展到整个模型，分别计算 decode 的主要读取与 prefill 的矩阵工作，并检查哪些设备能够容纳所需状态。
...
矩阵列统一采用 BF16 输入、FP32 累加、稠密执行的速率；横线对应硬件表中尚未给出这一完整口径的速率。decode 列用 16.345 GB 除以带宽，prefill 列用 61.85 TFLOPs 除以矩阵速率。
...
*图 4-32　同一 Qwen3-8B 的两种阶段预算。单请求 decode 更直接反映读取带宽，4K prefill 的矩阵预算更直接反映匹配精度的矩阵速率。两图横轴分别标注所计算的时间。*
...
**容量如何改变可选择的设备？** BF16 Qwen3-8B 的单请求预算约 19.737 GB，表中的 24 GB 卡能够容纳；增加到八条 8K 请求，总预算为约 28.193 GB，便超过 3090／4090 的 24 GB，进入 5090 的 32 GB 容量范围。模型和精度没有改变，只增加请求数，就会改变可行的设备集合。因此，这组八请求任务从 3090 升到 4090 仍装不下，升到 5090 则可以单卡驻留。

**同样在本地运行，Qwen3-8B 与 Qwen3-235B-A22B 应怎样选？** 先比较单请求生成。Qwen3-8B 使用 BF16、8K 上下文，在 RTX 4090、5090、M3 Ultra 上都能驻留。把表中的每步读取时间换成每秒 token 数，分别得到 $1000/16.22\approx61.7$、$1000/9.12\approx109.6$ 和 $1000/19.96\approx50.1$ token/s 的读取上限。这个 8B 稠密模型没有用到 Ultra 的额外容量，5090 的高带宽直接体现为更高的生成预算。

再换成第 4.6.3 节的 Qwen3-235B-A22B，采用 4-bit 权重、单请求和 8K 上下文。它需要约 126.867 GB 的模型、KV 与工作区空间，3090、4090、5090 均无法单卡驻留，M3 Ultra 256 GB 和 H200 141 GB 可以。每步读取约 13.697 GB，M3 Ultra 用约 16.72 ms，H200 用约 2.85 ms，对应约 59.8 与 350.4 token/s 的读取上限。Ultra 的大内存使这一个 235B MoE 能在本地整机上运行，H200 的高带宽则将同一步读取缩短到约六分之一。

把模型、任务与设备放在一起，选择过程就变成下面几笔具体计算。

| 模型与任务 | 算出的资源需求 | 对应的设备选择 |
| --- | --- | --- |
| Qwen3-8B，BF16，单请求 8K decode | 常驻 19.737 GB；每步读取 16.345 GB | 4090、5090 均能装下；5090 的读取时间由 16.22 ms 降至 9.12 ms |
| Qwen3-8B，BF16，八请求各 8K | 常驻 28.193 GB | 超过 3090／4090 的 24 GB；5090 的 32 GB 可容纳 |
| Qwen3-8B，BF16，4K prefill | 矩阵工作 61.85 TFLOPs | 3090、4090、H100 的矩阵时间依次为 871.1、374.4、62.5 ms |
| Qwen3-235B-A22B，4-bit，单请求 8K | 常驻 126.867 GB；每步读取 13.697 GB | M3 Ultra 256 GB 与 H200 可容纳；读取时间分别为 16.72、2.85 ms |

这张表先用容量排除装不下的设备，再用每步读取与输入矩阵工作比较速度。多卡运行的张量切分与专家分派在后续并行计算章节展开；这里继续沿单设备任务计算实测速度和使用成本。

### 4.8.3 预测与实测为什么有差距

第 4.8.2 节的设备比较给出了容量与主要资源预算。运行库为数学算子选择具体内核，缓存改变片外流量，提交和同步也占用时间。测量的用途是识别这些执行步骤，补全第 4.8.1 节的时间模型。

先把 Qwen3-8B 在 RTX 4090 上的读取预算换成一次具体的生成任务。仍用 BF16、单请求、8K 上下文，取这一上下文附近的 128 个 decode 步骤，每步载荷按 16.345 GB 计算。以峰值带宽读取，128 步合计约 $128\times16.22\approx2076$ ms，即 2.08 s。

设这一任务的执行参数如下：权重与 KV 读取阶段的有效带宽为 806.4 GB/s，即 4090 峰值的 80%；每步另有 2.00 ms 串行执行时间，用于向量运算、提交与同步。每步读取变为 $1000\times16.345/806.4\approx20.27$ ms，总时间为 $20.27+2.00=22.27$ ms，生成速度为 $1000/22.27\approx44.9$ token/s，128 步合计约 2.85 s。

同一任务若将有效带宽提高到峰值的 90%，读取时间降至约 18.02 ms；保留每步 2.00 ms 的串行时间，总时间变为约 20.02 ms，生成速度升至约 50.0 token/s。带宽利用改善十个百分点，128 步节省约 0.29 s。若保持 80% 带宽，把串行时间从 2.00 ms 减至 1.00 ms，则每步用 21.27 ms，速度约 47.0 token/s。两项优化分别减少读取与串行执行的时间，收益可以直接比较。

实际实验中的带宽与串行时间来自计时、流量和内核记录。下面用本书已有的 Q 投影实测，逐步展示怎样从这些记录找到时间差的来源。
...
选择硬件时，也要为后续的切分选择留下足够信息。除了显存容量与峰值算力，应记录目标数据类型下的有效计算能力、HBM 带宽、每计算单元可用的寄存器与共享内存，以及设备间的单向带宽、启动延迟和共享出口。第 5 章用前几项约束 tile 与并发块数，第 6 章用后几项约束跨卡通信组；同一模型形状必须与这些资源逐项匹配。若硬件只能在特定精度、布局或较大矩阵上达到标称算力，成本模型也应使用对应条件，而不能给所有候选填同一个峰值。
