...
for panel_index in range(3):
 ...
 if panel_index<2:
  ...
  ax.text(.5,.04,'本地分散归约后：每人 48 MiB',ha='center',fontsize=10)
...
graph.text(.365,.83,'保留的发布依赖',ha='center',fontsize=12,color=C['teal']);graph.text(.82,.12,'额外串行边',ha='center',fontsize=11,color=C['red'])
