...
def main(out,smoke):
 ...
 env=dict(torch=torch.__version__,device=torch.cuda.get_device_name(),capability=torch.cuda.get_device_capability(),platform=platform.platform(),source_sha256=hashlib.sha256((R/'run_matrix.py').read_bytes()).hexdigest(),smoke=smoke)
 (out/'environment.json').write_text(json.dumps(env,indent=2))
 k,n=2048,1536
 prepared=torch.load(R/'reference-expert.pt',map_location='cpu',weights_only=True)
 w=prepared['reference_fp32'].to('cuda');torch.cuda.synchronize()
 begin=torch.cuda.Event(enable_timing=True);end=torch.cuda.Event(enable_timing=True);start=time.perf_counter();begin.record()
 qw=prepared['int8_weight'].to('cuda');sw=prepared['int8_scale'].to('cuda');end.record();end.synchronize()
 transfer=dict(wall_s=time.perf_counter()-start,event_ms=begin.elapsed_time(end),weight_bytes=qw.numel()*qw.element_size(),scale_bytes=sw.numel()*sw.element_size(),scope='Pageable CPU to CUDA .to calls; reference FP32 transfer excluded; no pinning or bandwidth claim')
 (out/'initial-transfer.json').write_text(json.dumps(transfer,indent=2)+'\n')
 assert tuple(w.shape)==(k,n) and qw.dtype==torch.int8
 ...
 pool=torch.load(R/'activation-pool.pt',map_location='cpu',weights_only=True)['input_bf16']
 assert pool.shape[0]>=512
 rows=[]
 for m in ([1,8] if smoke else [1,8,64,512]):
  x=pool[:m].float().to('cuda');q,sx=quant(x);reference=x@w;integer_ref=(q.double()@qw.double()).to(torch.int32);native=torch._int_mm(q,qw)
  ...
  for name,fn in [('int8_direct',direct),('int8_dequant_bf16',dequant)]:
   ...
   checks[name]=dict(relative_l2_to_checkpoint_fp32=error,max_abs_error=float((y-reference).abs().max()),output_sha256=hashlib.sha256(y.cpu().contiguous().numpy().tobytes()).hexdigest(),passed=error<=.02)
   (out/f'quality-m{m}.json').write_text(json.dumps(checks,indent=2))
   torch.save(y.cpu(),out/f'output-m{m}-{name}.pt')
   # Keep numerical quality failures as measured outcomes; do not select successful samples.
  torch.cuda.synchronize()
