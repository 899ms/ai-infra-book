...
![Figure 12-1: The raw image travels uplink to the server, the server processes it once fully received, and then the complete finished image returns via downlink. Solid arrows indicate data and processing order; the connection is already established, and round-trip propagation totals 0.1 s.](images/figure-12-image-path.pdf)
...
![Figure 12-3: Serial whole-image processing: all 30 MB is uploaded and propagates to the server before processing begins, followed by a 5 MB return. Uplink 20 Mbit/s, downlink 100 Mbit/s, one-way propagation 0.05 s; the task completes at 12.8 s.](images/figure-12-2-overlap.pdf)

![Figure 12-4: Each of the three chunks uploads for 4 s; each is processed independently for 0.1 s upon arrival, followed by returns of 1, 2, and 2 MB. The processing and return of the first two chunks overlap with subsequent uploads, and the complete finished image returns at 12.36 s.](images/figure-12-overlap-chunks.pdf)
...
![Figure 12-5: The first audio chunk timed from the start of capture: ready at 20 ms, done processing at 32 ms, done sending at 33 ms, arrives at 38 ms; after an initial 40 ms buffer it plays at 78 ms. Dots mark arrivals; the bottom row is the playback device's clock.](images/figure-12-audio-clocks.pdf)
...
![Figure 12-8: One round of a screenshot agent's execution. Arrows indicate dependency order; box widths do not indicate duration. The bottom return arrow must pass through action execution and interface update before the next round's screenshot can be obtained.](images/figure-12-5-agent.pdf)
...
![Figure 12-14: Migrating 64 MiB of state over an 80 Mbit/s link, saving 0.4 seconds per round after migration. The two curves take recovery time as 1 second and 2 seconds respectively; net saving is N×0.4 minus transmission and recovery time. Dots mark the first integer round at which the benefit turns positive; above the zero line migration is faster, with the first benefit occurring at round 20 and round 22 respectively.](images/figure-12-7-migration.pdf)
...
![Figure 12-15: One attention-output reduction under tensor parallelism: the two cards each compute first, then exchange and sum, obtaining the complete output before entering the feedforward network. Horizontal arrows indicate execution order; the vertical arrow in the middle indicates communication between the two cards.](images/figure-12-8-sync.pdf)

![Figure 12-16: The feedforward network also computes local outputs separately, then performs a two-card reduction. Only once the complete feedforward result is ready can the next layer begin; this structure repeats across all 36 layers.](images/figure-12-sync-ffn.pdf)
...
![Figure 12-19: Whole-connection in-order delivery: the audio is fully received by second 3 but still has to wait until second 7 because of the image's gap. The gray segment marks the wait after the data is already complete; the dot marks the moment the data is handed to the application.](images/figure-12-10-transport.pdf)

![Figure 12-20: Per-stream ordered delivery: the audio is delivered immediately once received at second 3, while the image is still delivered at second 7. The send, arrival, and recovery times are identical in both figures; the difference is whether the streams must wait on one another.](images/figure-12-transport-per-stream.pdf)
...
![Figure 12-21: With a new connection, the median request times for the direct path and Queqiao on a fixed 354,640-byte audio file are 1185.3 and 301.6 ms, respectively. Timing runs from request initiation to receipt of the full result; the input file is identical, and the two paths are run alternately.](images/figure-12-13-queqiao.pdf)

![Figure 12-22: After reusing the connection and tuning it, the median request times for the direct path and Queqiao on the same fixed audio are 240.9 and 236.5 ms. This figure uses the same vertical axis as the previous one; both paths now approach the total time required for data sending, the necessary round trip, and processing.](images/figure-12-queqiao-tuned.pdf)
...
![Figure 12-23: Single-factor experiment method: holding the same file, path, and other settings fixed, change only one factor at a time and compare intermediate events against total completion time.](images/figure-12-experiment-design.pdf)
...
![Figure 12-26: Two independent paths split the input in proportion to bandwidth, sending 20 MB and 10 MB respectively, each taking 8 s. The task completes once both parts are done.](images/figure-12-multipath-independent.pdf)
...
![Figure 12-28: Local device: 6 s of terminal work, 58.0 s of model computation, 64.0 s total. All three figures share a 45 s deadline line and the same horizontal axis; colors aggregate the time spent on each type of work across twenty rounds.](images/figure-12-14-budgets.pdf)

![Figure 12-29: Nearby workstation (RTX PRO 6000): 0.27 s preparation, twenty rounds of 6 s terminal work, 2.7 s model, 0.4 s propagation, 1.6 s upload, 11.0 s total, meeting the 45 s deadline. The dashed line marks the 45 s completion deadline.](images/figure-12-budgets-1.pdf)

![Figure 12-30: Cloud (H100 SXM): 0.14 s preparation, twenty rounds of 6 s terminal work, 1.5 s model, 4 s propagation, 20 s upload, 31.6 s total. Computation is faster but transfer takes longer, making it about 20.6 s slower than the nearby workstation, though still within the deadline. The dashed line marks the 45 s completion deadline.](images/figure-12-budgets-2.pdf)
...
![Figure 12-32: Retaining the commit records for the first nine rounds (green), redoing only the unconfirmed tenth round (orange). The cloud takes 1.57 s per round, reconnection takes 1 s, adding 2.57 s, for a total time of 34.2 s. This example uses operations that can be safely replayed; already-committed external operations instead have their results queried.](images/figure-12-16-recovery.pdf)

![Figure 12-33: When ten rounds of progress are lost, reconnecting and redoing ten rounds adds 1+10×1.57≈16.7 s, extending the total task from 31.6 s to 48.3 s, over the 45 s deadline.](images/figure-12-recovery-all.pdf)
...
![Figure 12-34: Speedup ceiling within a fixed task trajectory. The model shrinks from 8 seconds to 0.8 seconds while the other serial stages remain at 2 seconds; the third row shows the ideal lower bound as model time approaches zero.](images/figure-12-task-counterfactual.pdf)
...
[^phone]: [Snapdragon 8 Elite product brief](https://github.com/bojieli/ai-infra-book/blob/main/references/files/specs/qualcomm-8elite-brief.pdf) and [fifth-generation brief](https://github.com/bojieli/ai-infra-book/blob/main/references/files/specs/qualcomm-8elite-gen5-brief.pdf): supports LPDDR5X up to 5,300 MHz, capacity up to 24 GB, with no bus width given. [Micron LPDDR5X product page](https://github.com/bojieli/ai-infra-book/blob/main/references/files/specs/micron-lpddr5x-page.md): top speed grade of 10.7 Gbit/s, JEDEC standard is x16 single-channel devices, with the 1γ process adding 6/12/24 GB capacity options plus a 16 GB device aimed at flagship phones; [Samsung LPDDR5X product page](https://github.com/bojieli/ai-infra-book/blob/main/references/files/specs/samsung-lpddr5x-page.md) likewise tops out at 10.7 Gbit/s. [Apple iPhone 16 Pro spec page](https://github.com/bojieli/ai-infra-book/blob/main/references/files/specs/apple-iphone16-pro-specs.md) does not publish memory capacity or speed.

[^phone-ledger]: [Local-device bus and energy ledger](https://github.com/bojieli/ai-infra-book/blob/main/calculations/results/energy-ledger-book.md): channel count is taken as four x16 (`declared_phone_channels_x16=4`; the SoC brief gives no bus width), per-pin rate taken as the SoC-supported 10.6 Gbit/s, totaling 84.8 GB/s; the 15,136,819,200 bytes of weights and 1,207,959,552 bytes of KV are read directly from the [single-request 8K decode results](https://github.com/bojieli/ai-infra-book/blob/main/calculations/results/qwen3-8b-decode-b1-s8192.json).

[^melt]: [MELTing Point: Mobile Evaluation of Language Transformers](https://github.com/bojieli/ai-infra-book/blob/main/references/files/papers/melting-point.pdf), arXiv 2403.12844, §5.2.2 and Appendix Table 5: 0.16/0.20/0.21 mWh per token; the iPhone 14 Pro has a sustained power ceiling of 13.8 W with instantaneous peaks over 18 W, while the Galaxy S23 stays under 8.5 W sustained; on the iPhone 14 Pro, Zephyr-3B q4_k runs at 14.8 token/s and Llama-2 7B q3_k at 6.0 token/s; one charge can run 490.05–590.93 prompts (542.78 for the S23). Line-by-line verification of extracted values is in the [value extraction table](https://github.com/bojieli/ai-infra-book/blob/main/research/gap-plan-2026-09-11/sources-extract.json).

[^mathis]: [Mathis et al., The Macroscopic Behavior of the TCP Congestion Avoidance Algorithm](https://github.com/bojieli/ai-infra-book/blob/main/references/files/papers/mathis-tcp-model.pdf), ACM CCR 1997. Equation (3): $BW=(MSS/RTT)\cdot C/\sqrt{p}$; for periodic loss with per-packet acknowledgment $C=\sqrt{3/2}\approx1.22$, and 1.31 for random loss; Equation (4) gives a simpler upper bound.

[^bbr]: [BBR Congestion Control, IETF draft draft-cardwell-iccrg-bbr-congestion-control-02](https://github.com/bojieli/ai-infra-book/blob/main/references/files/standards/bbr-ietf-draft.txt): BBR.max_bw is the maximum of recent bandwidth samples within a window, BBR.min_rtt is the minimum RTT sample within a window, and BBR.bdp is their product; Startup's pacing gain is $4\ln2\approx2.77$ with a cwnd_gain of 2, and ProbeBW_UP's pacing gain is 1.25.

[^wan-loss]: Calculations at [14% packet loss](https://github.com/bojieli/ai-infra-book/blob/main/calculations/results/wan-loss-model-book.md) and [3.6% packet loss](https://github.com/bojieli/ai-infra-book/blob/main/calculations/results/wan-loss-model-p036.md). Path parameters are taken from the [Queqiao path record](https://github.com/bojieli/ai-infra-book/blob/main/references/author-context/queqiao-168ff4b/PATH-CHARACTER-DC-20260826.md): round trip 199–207 ms, capacity knee about 333 Mbit/s, download-direction loss from Irvine to Guiyang about 14% (3.6% in another period), zero loss across 41,663 packets from Guiyang to Irvine, MSS 1,448 bytes, fixed file size 354,640 bytes; model time is taken from the roughly 30 ms in the fixed-file remeasurement section, consistent with measurement: the tuned median of 236.5 ms minus the same period's shortest round trip of 197 ms and 8.5 ms of sending leaves a model time no greater than about 31 ms; the earlier-noted 38 ms comes from an earlier test that rotated through eight files. FEC symbol size is taken as the MSS, and the block-recovery success target is taken as 99.9%. The retransmission model performs one ideal selective retransmission per round, ignoring window shrinkage and timeout retransmission, making it a lower bound favorable to TCP.

[^raw]: [RAW image enhancement case and conditional budget](https://github.com/bojieli/ai-infra-book/blob/main/case-studies/raw-retouching.md). The example sets the original image at 30 MB, the finished image at 5 MB, and image processing time at 0.3 seconds.

[^stream]: The two computations for three blocks are 12.8 s and 12.36 s respectively; the main text explains the overlap benefit using the roughly 0.4 s difference; an additional preview adds further encoding and transmission. Sources: [whole-image barrier](https://github.com/bojieli/ai-infra-book/blob/main/calculations/results/image-stream-whole-image-without-preview.md), [independent blocks](https://github.com/bojieli/ai-infra-book/blob/main/calculations/results/image-stream-independent-blocks-without-preview.md), and [additional preview](https://github.com/bojieli/ai-infra-book/blob/main/calculations/results/image-stream-independent-blocks-with-preview.md).

[^audio]: [Audio block timing calculation](https://github.com/bojieli/ai-infra-book/blob/main/calculations/results/audio-timing-base.md); the one-to-one correspondence between input frames and output audio blocks is a simplification of this model, used to illustrate the dependency between block-by-block processing and in-order playback.

[^author]: [The author's materials and technical judgment](https://github.com/bojieli/ai-infra-book/blob/main/case-studies/author-context-and-design.md). AOI and Latent Bridge are used to illustrate what interface information should be observed and what data should be passed between devices.

[^ec]: [Fixed vision shape with full EC/KV computation](https://github.com/bojieli/ai-infra-book/blob/main/calculations/results/multimodal-cache-single.md), [multimodal stage placement case](https://github.com/bojieli/ai-infra-book/blob/main/case-studies/multimodal-stage-placement.md). Fixed vision variants, DeepStack, and language state are measured separately.

[^encode-place]: [Device computation for encoding location and the edge/edge-server/cloud three-tier setup](https://github.com/bojieli/ai-infra-book/blob/main/calculations/results/edge-tiers-agent-book.md): the matrix workload for vision encoding is taken from the [single-image vision encoding result](https://github.com/bojieli/ai-infra-book/blob/main/calculations/results/vision-encoding-single.md), and the peak value is taken from the hardware parameter table; [ConnectX-7 datasheet](https://github.com/bojieli/ai-infra-book/blob/main/references/files/specs/nvidia-connectx7-datasheet.pdf): single-port rate up to 400 Gbit/s.

[^ec-test]: [Qwen3-VL-8B CPU encoding and local TCP transfer experiment](https://github.com/bojieli/ai-infra-book/blob/main/experiments/ch12/12-03/README.md). The input is a synthetic 256×256 test image; the full EC is about 2 MiB, PNG is about 3.4 KiB; the data is bit-for-bit identical before and after 8 transfers. Encoding completes before the transfer timing starts; the times listed correspond to the feature transfer stage.

[^dense]: [Per-operation communication volume computation for Qwen3-8B with TP=2, PP=4](https://github.com/bojieli/ai-infra-book/blob/main/calculations/results/dense-comm-qwen8-tp2-pp4-t1.md). Each of the 36 layers has two output reductions, producing 72 communication launches in total; embeddings, stage activations, logits, and token returns add further communication.

[^survey]: [FlexSP/llm.npu paper and implementation materials](https://github.com/bojieli/ai-infra-book/blob/main/references/framework-history/2026-09-09/sequence-and-npu/NOTES.md).

[^quic]: [RFC 9000: QUIC Transport](https://github.com/bojieli/ai-infra-book/blob/main/references/files/standards/rfc9000.txt), §2, §7, §9, §13; [RFC 9001: TLS](https://github.com/bojieli/ai-infra-book/blob/main/references/outline-checks/2026-09-07/rfc9001.txt); [RFC 9114: HTTP/3](https://github.com/bojieli/ai-infra-book/blob/main/references/outline-checks/2026-09-07/edge-media/rfc9114.txt); [RFC 9221: Unreliable Datagrams](https://github.com/bojieli/ai-infra-book/blob/main/references/outline-checks/2026-09-07/rfc9221.txt). HTTP/3 specifies how to transmit HTTP requests and responses over QUIC.

[^closed]: The exact packet breakdown is 25,685 packets uploaded, 4,281 packets downloaded; 29,966 data packets total plus an equal number of ACKs; actual network transmission is 39,554,832 bytes; the complete response takes 14.36339008 s. Source: [complete image reference closed loop](https://github.com/bojieli/ai-infra-book/blob/main/calculations/results/closed-loop-book-30mb-5mb.md).

[^ack]: Under the same padding conditions, immediate ACK is 39,555,120 bytes/14.52103724 s, and aggregated ACK is 38,177,328 bytes/14.564459796 s — a difference of about 1.38 MB and 43.4 ms. Source: [immediate-ACK NewReno control](https://github.com/bojieli/ai-infra-book/blob/main/calculations/results/controller-loop-book-newreno.md), [aggregated-ACK control](https://github.com/bojieli/ai-infra-book/blob/main/calculations/results/ack-policy-book-newreno.md), and [CUBIC control-stage record](https://github.com/bojieli/ai-infra-book/blob/main/calculations/results/controller-loop-book-cubic_hystart.md).

[^media]: [FIFO](https://github.com/bojieli/ai-infra-book/blob/main/calculations/results/shared-media-schedule-fifo.md) and [audio priority](https://github.com/bojieli/ai-infra-book/blob/main/calculations/results/shared-media-schedule-priority.md); with fixed send records, [connection-wide ordering](https://github.com/bojieli/ai-infra-book/blob/main/calculations/results/shared-media-hol-connection.md) and [per-stream delivery](https://github.com/bojieli/ai-infra-book/blob/main/calculations/results/shared-media-hol-per_stream.md). These respectively examine the effects of send scheduling and receiver-side delivery dependencies.

[^http]: [Local TCP/HTTP3 transfer](https://github.com/bojieli/ai-infra-book/blob/main/experiments/ch12/12-04/loopback/README.md) and [HTTP3 single-connection multistream](https://github.com/bojieli/ai-infra-book/blob/main/experiments/ch12/12-04/h3-multistream/README.md).

[^air]: [Shared-spectrum image computation](https://github.com/bojieli/ai-infra-book/blob/main/calculations/results/shared-airtime-image-baseline.md). Data and MAC ACK use OFDM 54/6 Mbit/s respectively, without frame aggregation; the channel access wait is SIFS 16 μs plus two 9 μs slots, i.e., the 802.11a OFDM DIFS of 34 μs, excluding random backoff; MAC-layer and end-to-end acknowledgment are measured separately.

[^wireless-survey]: [The New Golden Age of Computer Networking (Part 3)](https://github.com/bojieli/ai-infra-book/blob/main/references/files/documents/network-golden-3.md), introducing the historical background of TACK, Link Turbo, and proxy deployment.

[^mixed]: [Mixed feedback computation for media priority and aggregated ACK](https://github.com/bojieli/ai-infra-book/blob/main/calculations/results/media-feedback-mixed-priority-aggregate.md) and the FIFO/immediate-ACK controls in the same directory. Model processing time and scheduled playback timing are set by the calculation.

[^dual]: [Dual-process, dual-TCP-connection experiment](https://github.com/bojieli/ai-infra-book/blob/main/experiments/ch12/12-06/README.md): the experiment simulates two transmission paths using two processes and two TCP connections; the 30 formal attempts include cases of shared-endpoint failure.

[^queqiao-readme]: For the system's positioning and the motivation for cross-datacenter operation, see [Queqiao README, fixed archive](https://github.com/bojieli/ai-infra-book/blob/main/references/author-context/queqiao-496ca627/README.md), corresponding to [GitHub commit `496ca627`](https://github.com/bojieli/queqiao/blob/496ca6278e359c02b5107dfab77c6a3db585f80d/README.md), read on 2026-09-10. The system description follows this version; the performance comparison data comes from commit `168ff4b`.

[^queqiao]: [Fixed-archive path record](https://github.com/bojieli/ai-infra-book/blob/main/references/author-context/queqiao-168ff4b/PATH-CHARACTER-DC-20260826.md) and [design record](https://github.com/bojieli/ai-infra-book/blob/main/references/author-context/queqiao-168ff4b/DESIGN-DC-PROFILE.md), from commit `168ff4b`. Files, tuning settings, and timing start/end points all follow the definitions in the original records.

[^queqiao-calc]: For newly established connections, the direct/Queqiao p50 is 1185.3/301.6 ms; with tuned persistent connections it is 240.9/236.5 ms; the two median ratios are about 3.93/1.019 respectively. Taking the median of the per-round time ratios instead gives 3.96/1.03. Source: [Queqiao same-condition statistics](https://github.com/bojieli/ai-infra-book/blob/main/calculations/results/queqiao-records-conditions.md).

[^physical]: Across 8 complete reception tests, the PCM received and the PCM read by the playback callback were bit-for-bit identical in every case; the playback callback accumulated 360–1440 ms of being unable to obtain sufficient audio samples. The experiment used a muted output device; the record includes both reception and playback-callback events. A further 3 requests were canceled before the first playback callback; the bytes received, connection-close events, and remote EOF record the cancellation process. The complete four-round data appears in the [accompanying evidence notes](https://github.com/bojieli/ai-infra-book/blob/main/manuscripts/ch12/evidence-notes.md). Source: [audio experiment with explicit physical interface binding](https://github.com/bojieli/ai-infra-book/blob/main/experiments/ch12/12-07/audio-physical/README.md), fixed at commit `496ca627`; the baseline includes the socket-binding adaptation.

[^region]: The cross-cloud networking and Regionless discussion in [The New Golden Age of Computer Networking (Part 2)](https://github.com/bojieli/ai-infra-book/blob/main/references/files/documents/network-golden-2.md). This chapter's regional scheme measures cost in device energy consumption and GPU time.

[^region-trace]: For a replay of the same model, token sequence, and round, the data sent to the server was 66,207/10,193 bytes respectively, and the data returned to the client was 2,527 bytes in both cases. The matrix workloads were 296,505,803,538,432 and 60,380,764,176,384 FLOPs respectively; dividing by the H100 SXM's BF16 dense peak gives the GPU-time lower bound; the state is 465,371,136 bytes, and converting its residency time by its share of the 80 GB of GPU memory shows that after about 41.0 s the saved 0.239 GPU-seconds is offset. Source: [regional placement for a fixed agent trajectory](https://github.com/bojieli/ai-infra-book/blob/main/calculations/results/region-placement-agent-session.md); the bytes and tokens come from the original trajectory.

[^edge-tiers]: [Device computation for the edge/edge-server/cloud three-tier setup](https://github.com/bojieli/ai-infra-book/blob/main/calculations/results/edge-tiers-agent-book.md). The per-step read volume is taken from the weights and KV in the [single-request 8K decode result](https://github.com/bojieli/ai-infra-book/blob/main/calculations/results/qwen3-8b-decode-b1-s8192.md), with weights rearranged as q4_0 at 18 bytes per 32 values; the phone's bus figures are taken from [edge-side bus and energy accounting](https://github.com/bojieli/ai-infra-book/blob/main/calculations/results/energy-ledger-book.md); bandwidth, BF16 dense peak, and rated power come from the [hardware parameter table](https://github.com/bojieli/ai-infra-book/blob/main/calculations/results/hardware.md); the measured multiplier comes from the batch-1, 8K row of the [efficiency table for Experiment 8-1](https://github.com/bojieli/ai-infra-book/blob/main/experiments/ch08/08-01/efficiency.json). The results file also lists all cases for ten rounds, tightened deadlines, and disconnect recovery.

[^core-calculation]: The [itemized recomputation](https://github.com/bojieli/ai-infra-book/blob/main/calculations/results/core-principles.json) and [computation program](https://github.com/bojieli/ai-infra-book/blob/main/calculations/core_principles.py) for this chapter's condition comparisons.
