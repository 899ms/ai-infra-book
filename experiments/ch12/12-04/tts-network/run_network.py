...
FIXTURE=json.loads((R/'tts-fixture.json').read_text());PAYLOAD=(R/FIXTURE['upload_file']).read_bytes();RESPONSE=(R/FIXTURE['response_file']).read_bytes();HASH=hashlib.sha256(PAYLOAD).hexdigest();RESPONSE_HASH=hashlib.sha256(RESPONSE).hexdigest();SERVICE_S=FIXTURE['service_s']
