...
func (c *Client) dial(ctx context.Context, tlsCfg *tls.Config) (*quic.Conn, error) {
	...
	binding, err := netbind.ResolveWithInterface(c.cfg.LocalAddress)
	if err != nil {
	...
	listener := net.ListenConfig{Control: netbind.InterfaceControl(binding.InterfaceName)}
	packet, err := listener.ListenPacket(ctx, "udp", net.JoinHostPort(binding.Addr.String(), "0"))
	if err != nil {
