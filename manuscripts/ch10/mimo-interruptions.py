...
def draw(language="zh", output_dir=None):
    ...
    legend = [Patch(facecolor=blue, label='Completed steps' if english else '完成步骤'), Patch(facecolor=red, label='Interrupted' if english else '中断区间'),
              Patch(facecolor='#fbf0e5', edgecolor=orange, hatch='////', label='Rolled back' if english else '回滚作废'),
    ...
    for run, y, label in [('pro', 1, 'Pro'), ('flash', 0, 'Flash')]:
        ...
        fig.text(.14, .86 if y else .705,
                 (f"{label} · {d['restarts']} restarts · {d['wall_seconds']/3600:.1f} h total" if english else
                  f"{label}  ·  {d['restarts']} 次重启  ·  总运行 {d['wall_seconds']/3600:.1f} 小时"),
        ...
        fig.text(.14, .825 if y else .67,
                 (f"Wasted: {wasted/3600:.1f} h ({wasted/d['wall_seconds']:.1%} of total)" if english else
                  f"浪费约 {wasted/3600:.1f} 小时（占总时间 {wasted/d['wall_seconds']:.1%}）"),
    ...
    ax.set(xlim=(0,141), ylim=(-.48,1.5), yticks=[], xticks=range(0,131,20), xlabel='Elapsed time from each launch (hours)' if english else '各自启动后经过的时间 / 小时')
    ...
    fig.text(.14,.40,'Estimated interruption and rollback costs' if english else '中断与回滚对应的费用估算',fontsize=12,weight='bold')
    ...
    for run,y,label in [('pro',1,'Pro'),('flash',0,'Flash')]:
        ...
        cost.text(v+w+1.4,y,f'${(v+w)*10:.0f}k' if english else f'{v+w:.1f} 万美元',va='center',fontsize=11,weight='bold')
    cost.set(yticks=[1,0],yticklabels=['Pro','Flash'],xlim=(0,81),ylim=(-.5,1.5),
             xticks=[0,20,40,60],xlabel='Cost (USD thousands)' if english else '费用 / 万美元')
    if english:
    ...
    fig.text(.14,.028,"Halving Pro's interruption time saves ~15 h / ~$310k." if english else 'Pro 中断时间减少一半：约省 15 小时、31 万美元。',
