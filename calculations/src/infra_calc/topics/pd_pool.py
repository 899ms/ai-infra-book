...
Stage rates come either from locked hardware peaks times declared efficiency
(device-form workers, see stage_rates) or from explicit effective token rates.
Results are steady-state resource bounds, not SLOs.
"""
...
def calculate(model='qwen3-8b', prompt_tokens=8192, output_tokens=1025,
              cached_prefix_tokens=0, workers=None, network_bytes_per_second=25*10**9,
              arrival_requests_per_second='4', kv_layout='gqa', kv_model='deepseek-v3'):
    ...
    workers = ([dict(name='A100',count=4,device='a100-80gb-sxm',decode_batch=32,
                     compute_efficiency=0.5,bandwidth_efficiency=0.5),
                dict(name='H20',count=4,device='h20-sxm5-96gb',decode_batch=32,
                     compute_efficiency=0.5,bandwidth_efficiency=0.5)] if workers is None else workers)
    ...
    # Device-form workers derive both stage rates from locked hardware peaks and the model forward.
    derived=[]; resolved=[]
    for worker in workers:
        if isinstance(worker,dict) and 'device' in worker:
            converted,rates=stage_rates.worker_from_device(worker,model,prompt_tokens,output_tokens,cached_prefix_tokens)
            derived.append(rates); resolved.append(converted)
        else:
    ...
    kv_sources=state_result['sources']
    if kv_layout=='mla':
        # Compact MLA per-token bytes from the locked DeepSeek-V3 config, applied to the same prompt length.
        snapshot=prompt_tokens*kv_comparison.per_token_bytes(kv_model,'mla')
        kv_sources=list({s['file']:s for s in state_result['sources']+provenance(kv_model)}.values())
    ...
    hardware_sources=[src for rates in derived for src in rates['sources']]
    kv_sources=list({s['file']:s for s in kv_sources+hardware_sources}.values())
    return dict(schema_version=1,calculation='pd-pool',sources=kv_sources,derived_stage_rates=derived,
                scenario=dict(model=model,prompt_tokens=prompt_tokens,output_tokens=output_tokens,
                              ...
                              arrival_requests_per_second=arrival_requests_per_second,kv_layout=kv_layout,kv_model=kv_model),
                summary=dict(new_prefill_tokens_per_request=new_tokens,decode_calls_per_request=decode_calls,
                             kv_layout=kv_layout,kv_bytes_per_token=snapshot//prompt_tokens,
