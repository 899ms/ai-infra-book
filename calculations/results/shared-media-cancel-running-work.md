...
| all_businesses_usable | `false` |
| all_reliable_packets_delivered | `false` |
| data_wire_bytes | `2` |
| ack_wire_bytes | `2` |
| unique_received_payload_bytes | `2` |
| unique_delivered_payload_bytes | `2` |
...
| --- | --- | ---: | ---: | --- |
| infer | server/server | 2 | 5 | True |

取消信号经过实际网络与可靠顺序交付后只影响接收端。已开始计算和发送保留。

- 4 s：`server` 收到应用控制 `cancel`，取消 `screen`。
  ...
  "credit_events": [
    {
      ...
      "reliable_receive_outstanding": 1
    },
    {
      "at": "2",
      "event": "send",
      ...
      "direction": "c2s",
      "reserved_bytes": 1,
      "shared_outstanding": 2,
      "reliable_receive_outstanding": 2
    },
    {
      "at": "4",
      "event": "ack_received",
  ...
  "work": [
    {
      ...
      "start": "2",
      "end": "5",
      "cancel_received_while_running": true
    }
  ],
  "cancellations": [
    {
