...
R=Path(__file__).resolve().parent; O=R/'structured-server-results'
M='/home/ubuntu/.cache/huggingface/hub/models--Qwen--Qwen3-VL-8B-Instruct/snapshots/0c351dd01ed87e9c1b53cbc748cba10e6187ff3b'
SCHEMA={'type':'object','properties':{'action':{'type':'string','enum':['click','type','key','done']},'x':{'type':'number'},'y':{'type':'number'},'text':{'type':'string'},'key':{'type':'string','enum':['Tab','Enter','Backspace','Escape']}},'required':['action'],'additionalProperties':False}
async def main():
  ...
  try:
   ...
   if path=='/stop': response={'stopping':True};stop.set()
    ...
    async with lock:
     ...
     async for value in engine.generate({'prompt':prompt,'multi_modal_data':{'image':picture}},SamplingParams(temperature=0,max_tokens=request.get('max_tokens',160),structured_outputs=StructuredOutputsParams(json=SCHEMA)),request['id']):
