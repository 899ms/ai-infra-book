...
class RemoteKV(HiCacheFile):
 ...
 def batch_get(self,keys,target_locations,target_sizes=None):
  result=[]
  for i in range(0,len(keys),1):result.extend(self._batch_get_chunk(keys[i:i+1],target_locations[i:i+1]))
  return result
 ...
 def batch_set(self,keys,values=None,target_locations=None,target_sizes=None):
  for i in range(0,len(keys),1):
   if not self._batch_set_chunk(keys[i:i+1],values[i:i+1]):return False
  return True
