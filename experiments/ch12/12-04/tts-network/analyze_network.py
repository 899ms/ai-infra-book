...
parser=argparse.ArgumentParser();parser.add_argument('profile',choices=['loss0','loss0.1']);args=parser.parse_args()
...
assert env['fixture']==json.loads((R/'tts-fixture.json').read_text())
assert env['payload_sha256']==hashlib.sha256((R/'request.json').read_bytes()).hexdigest()
assert env['response_sha256']==hashlib.sha256((R/'audio.wav').read_bytes()).hexdigest()
with wave.open(str(R/'audio.wav')) as audio:
 assert (audio.getnchannels(),audio.getframerate(),audio.getsampwidth())==(1,44100,2)
 assert audio.getnframes()*2+44==env['response_bytes']
...
summary=dict(status='26/26 actual TTS audio availability-trace replays under Docker netem including 2 warmups; not new model execution',conditions=result,handshake_median_ms={p:statistics.median((c['ready']-c['start'])*1000 for c in connections.values() if c['protocol']==p) for p in ['h1','h3']},qlog_events=dict(qlog_events),qlog_drop_triggers=dict(drops),configured_loss_percent=0.1 if args.profile=='loss0.1' else 0,configured_rtt_ms=80,configured_rate_mbit_s=20,calibration_file='../controlled-network/calibration-summary.json',model_service_replay_s=env['service_time_replay_s'])
(R/(args.profile+'-summary.json')).write_text(json.dumps(summary,indent=2)+'\n');print(json.dumps(summary,indent=2))

frame_rows=[]
for protocol in ['h1','h3']:
 for server in json.loads((D/(protocol+'-server.json')).read_text()):
  assert len(server['releases'])==len(env['fixture']['release_plan'])
  for observed,planned in zip(server['releases'],env['fixture']['release_plan']):
   assert (observed['offset'],observed['bytes'])==(planned['offset'],planned['bytes'])
   assert observed['at']-server['service_start']>=planned['ready_s']
for row in raw:
 offset=0
 for event in row['arrivals']:
  offset+=event['bytes'];assert offset==event['total']
 assert offset==env['response_bytes']
 one=next(e['at'] for e in row['arrivals'] if e['total']>=44+1764)
 three=next(e['at'] for e in row['arrivals'] if e['total']>=44+3*1764)
 assert row['first_data']<=one<=three<=row['end']
 frame_rows.append(dict(trial=row['trial'],protocol=row['protocol'],reuse=row['reuse'],warmup=row['warmup'],index=row['index'],
  first_frame_ms=(one-row['start'])*1000,three_frames_ms=(three-row['start'])*1000,
  first_body_ms=(row['first_data']-row['start'])*1000,complete_ms=(row['validation_end']-row['start'])*1000))
frame_groups=[]
for protocol in ['h1','h3']:
 for reuse in [False,True]:
  selected=[r for r in frame_rows if not r['warmup'] and (r['protocol'],r['reuse'])==(protocol,reuse)]
  assert len(selected)==6
  frame_groups.append(dict(protocol=protocol,reuse=reuse,requests=len(selected),**{key:statistics.median(r[key] for r in selected) for key in ['first_frame_ms','three_frames_ms','first_body_ms','complete_ms']}))
(R/(args.profile+'-playable.json')).write_text(json.dumps(dict(scope='Client availability of 20ms PCM frames; no acoustic playback measurement',groups=frame_groups,requests=frame_rows),indent=2)+'\n')
print(json.dumps(frame_groups,indent=2))
