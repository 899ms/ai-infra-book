...
- [2025-05-05 H100 部署](sglang-ep-2025.md)：读 Two-batch Overlap、对应消融、Limitations and Future Work。重点为 DeepEP 元数据等待时的提交顺序，以及切小 batch 后的效率损失；模拟 MTP 与实测分开。
- [2025-06-16 GB200 第一期](sglang-gb200-2025-june.md)：读 Methods、End-to-end Performance、batch 消融与 Future Work。NVLink 环境改变两微批重叠的取舍；沿用旧基线与人为饱和负载不作单变量对照。
- [2025-09-25 GB200 第二期](sglang-gb200-2025-sept.md)：读 Methods、End-to-end Performance、Accuracy 的文字段落；质量图未估读。combine 与 down GEMM／共享专家重叠，精度、内核、EP 和 batch 同时变化，不能独立归因总加速。
...
[2025-09-23 内存调度公告](ollama-memory-2025.md)正文与支持范围已读。其“精确”描述限定当时新引擎路径，不能替代预留、可用内存变化和实际分配检查。2024 的 llm server 原件随比较保存，但本轮未读实现主体；当前代码的后续子进程分配实现未作全量追踪。
