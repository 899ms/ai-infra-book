...
七幅图均由本书自行绘制。数据图直接读取固定计算结果与原始实验的分析记录，结构图采用教学示意；均提供可编辑 SVG 和 160 dpi PNG。SVG 保留文字，需要中文字体；HTML 阅读版嵌入 PNG，避免缺少字体导致显示变化。
...
| 图号 | SVG | PNG |
| --- | --- | --- |
| 1-1 | [AI Infra 六层全景：从应用需求到设备与互联，右侧为跨层平台能力，底部为共同物理条件。](figure-1-1-panorama.svg) | [PNG](figure-1-1-panorama.png) |
| 1-2 | [请求经过应用、入口、路由、实例调度、GPU 执行与输出返回；另标模型加载、KV 状态与多设备协作。](figure-1-2-request.svg) | [PNG](figure-1-2-request.png) |
| 1-3 | [数据中心通过网络连接多个超节点，放大一个超节点展示计算托盘、CPU 与主存、GPU 与 HBM、内部互联和网卡。](figure-1-3-datacenter.svg) | [PNG](figure-1-3-datacenter.png) |
| 1-4 | [关键数字分为历史访问延迟与 H100 SXM 资源卡。延迟采用对数刻度；容量、带宽与计算吞吐分别列示。](figure-1-4-numbers.svg) | [PNG](figure-1-4-numbers.png) |
| 1-5 | [每卡容量与执行时的权重读取](figure-1-capacity-path.svg) | [PNG](figure-1-capacity-path.png) |
| 1-6 | [教学模型的算力、带宽和批复用变化，以及独立 Qwen3-8B 实测中的吞吐和每请求时间。](figure-1-5-budget.svg) | [PNG](figure-1-5-budget.png) |
| 1-7 | [TPU、SmartNIC 与 Unified Bus 分别改变专用资源配置、处理位置和协作范围。](figure-1-6-designs.svg) | [PNG](figure-1-6-designs.png) |
