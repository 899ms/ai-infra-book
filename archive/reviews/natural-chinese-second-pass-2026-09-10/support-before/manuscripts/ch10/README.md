...
[阅读版 HTML](../10-训练系统.md) · [正文 Markdown](../10-训练系统.md) · [写作大纲](../../outlines/10-训练系统.md)
...
正文现引用 41 幅图，以下清单按当前阅读顺序列出；上文旧图号用于追溯构建记录。
...
| 10-21 | 链路空闲时，3 ms 归约（橙色）可以在 5 ms 独立计算（蓝色）结束前完成。虚线标出计算结束。 | [SVG](figure-10-8-overlap.svg) | [PNG](figure-10-8-overlap.png) | [PDF](figure-10-8-overlap.pdf) |
| 10-22 | 链路先被其他通信占用 4 ms（灰色），归约推迟到 4—7 ms。虚线后多出的 2 ms 延长训练步骤。 | [SVG](figure-10-overlap-busy.svg) | [PNG](figure-10-overlap-busy.png) | [PDF](figure-10-overlap-busy.pdf) |
...
| 10-37 | 稳态中生成下一批与学习上一批在独立资源上重叠；两者完成后同步权重，周期为 44 s。橙色同步阶段阻塞两侧。 | [SVG](figure-10-async-overlap.svg) | [PNG](figure-10-async-overlap.png) | [PDF](figure-10-async-overlap.pdf) |
| 10-38 | 离散专家 ID 连接生成端与训练端，当前权重继续参与数值计算。示意记录为样本 A、token 17、层 3 的 top-2 选择；虚线表示 ID 重放，实线表示当前计算数据流。 | [SVG](figure-10-17-replay.svg) | [PNG](figure-10-17-replay.png) | [PDF](figure-10-17-replay.pdf) |
| 10-39 | 每条横条依次累计基础训练时间、保存与故障恢复的附加时间，以及预留的 5 天计划性停顿。基础训练时间已经包括通信和输入等待；橙色小段为检查点模型得到的额外耗时。两套方案使用相同任务和全局批次，虚线标出 30 天期限。 | [SVG](figure-10-18-deadline.svg) | [PNG](figure-10-18-deadline.png) | [PDF](figure-10-18-deadline.pdf) |
| 10-40 | 资源能力变化的收益由原有等待时间占比决定。曲线按 $T'/T=1-f+f/r$ 计算，固定单卡计算与依赖，通信时间随有效能力成反比。 | [SVG](figure-10-19-hardware.svg) | [PNG](figure-10-19-hardware.png) | [PDF](figure-10-19-hardware.pdf) |
| 10-41 | 固定 90 天执行期限，Dense 规模增大要求更多计算设备。数据量为 20T token，算法工作为 $6ND$，效率为 50%，使用 BF16 dense 峰值。各线为向上取整前的连续计算边界；横线标出 16,384 张卡。 | [SVG](figure-10-20-scale.svg) | [PNG](figure-10-20-scale.png) | [PDF](figure-10-20-scale.pdf) |
