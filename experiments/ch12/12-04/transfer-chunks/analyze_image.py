...
R=Path(__file__).resolve().parent;O=R/'results/image';order=json.loads((O/'order.json').read_text());offsets=json.loads((R/'tcp-offsets.json').read_text());F=json.loads((R.parent/'image-records/image-fixture.json').read_text())
...
for c in order:
 ...
 env=json.loads((D/'environment.json').read_text());assert env['machine']=='x86_64';assert env['mode']==c['mode'] and env['requested_window']==expected and env['fixture']==F and not env['smoke'];assert env['source_sha256']==hashlib.sha256((R/'image.py').read_bytes()).hexdigest();assert env['quic_max_data']==env['quic_max_stream_data']==(1048576 if expected is None else expected)
