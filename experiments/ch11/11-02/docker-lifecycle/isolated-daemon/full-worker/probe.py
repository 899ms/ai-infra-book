...
def main():
    ...
    def cmd(args, check=True, timeout=60):
        ...
        return row
    def docker(*args, **kw):
        row=cmd(['docker', *map(str,args)], **kw)
        if args and args[0]=='start' and '--checkpoint' in args and row['exit_code'] and 'already exists' in row['stderr']:
            digest=re.search(r'content (sha256:[0-9a-f]{64}): already exists',row['stderr']).group(1)
            ctr=['ctr','--address','/run/book1102-worker-ctd.sock','--namespace','book1102','content']
            listing=cmd(ctr+['ls'])
            if digest in listing['stdout']:cmd(ctr+['rm',digest])
            row=cmd(['docker', *map(str,args)], **kw)
        return row
    ...
    save(O/'environment.json',dict(runid=runid,base_image_id=base,
        ...
        source_hashes={n:hashlib.sha256((R/n).read_bytes()).hexdigest() for n in ['probe.py','worker_unix.py','PROTOCOL.md']}))
...
"""
    ...
    try:
            ...
            if path=='pause':
                ...
                if cp['exit_code']==0:
                    for number in range(2):
                        ...
                        derivative=dict(container=new,creation=timing,restore=restored)
                ...
                row['derivatives_rechecked']=[]
                for derivative in row['derivatives']:
                    if derivative['restore']['exit_code']==0:
                        ds,dc=connect(derivative['container'])
                        row['derivatives_rechecked'].append(dict(container=derivative['container'],state=call(ds)))
                        ds.close()
        ...
        save(O/'completion.json',dict(status='all_12_cases_recorded',count=len(rows),runid=runid))
    finally:
