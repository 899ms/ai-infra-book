"""QUIC v1 HRR, Retry and PSK decisions with declared packet replay ledgers."""

from fractions import Fraction
import heapq
import json
from ..sources import provenance, read_source
from .protocol_early_stream import example as early_example, markdown as early_markdown
...
def example():
    """Reuse the early-stream baseline with declared event control datagrams."""
    inputs = early_example()
    inputs["packets"].update(
        retry=[100],
...
def markdown(result):
    """Explain handshake decisions before reusing the shared byte/trace report."""
    ...
    fields = (
        ...
        "application_early_data_authorized",
        "retry_early_policy",
        "retry_early_replay_authorized",
        "application_retry_authorized",
        "application_policy",
        "retry_token_bytes",
        "retry_token_valid",
        "retry_integrity_valid",
        "initial_dcid",
        "retry_scid",
        "client_scid",
    )
    rows.extend(
        "| " + name + " | `" + json.dumps(inputs[name], ensure_ascii=False) + "` |"
        for name in fields
    )
    rows.extend(
        [
    ...
    if inputs["handshake_event"] in ("psk_unknown_abort", "selected_binder_invalid"):
        rows.append(
            "| "
            + result["milestones"].get("client_hello_received", "未到达")
            + " | 服务器收到完整 ClientHello，检测握手失败并发送声明的 Initial CONNECTION_CLOSE |"
        )
    for event in result["state_transitions"]:
        rows.append("| " + event["at"] + " | " + event["event"] + " |")
    if not result["state_transitions"]:
        rows.append(
            "| — | 本场景没有额外 HRR／Retry／失败状态转换；普通密钥安装见后续时刻表 |"
        )
    rows.extend(
        [
            "",
    ...
    text = early_markdown(result)
    return text.replace("## 业务与握手时刻", "\n".join(rows) + "\n## 业务与握手时刻", 1)
