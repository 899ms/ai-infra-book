...
SOURCE=R.parent/'computer-use'
MODE=os.environ['BOOK_CHUNK_MODE'];WINDOW=None;CHUNK={'whole':0,'16k':16384,'64k':65536}[MODE]
FIXTURE=json.loads((SOURCE/'network-fixture.json').read_text());STEPS=[dict(f,payload=(SOURCE/f['upload_file']).read_bytes(),response=(SOURCE/f['response_file']).read_bytes()) for f in FIXTURE['steps']]
for f in STEPS:assert hashlib.sha256(f['payload']).hexdigest()==f['upload_sha256'] and hashlib.sha256(f['response']).hexdigest()==f['response_sha256']
BYHASH={f['upload_sha256']:f for f in STEPS};PAYLOAD=STEPS[0]['payload']
REQUEST_TIMEOUT=120
NOW=time.perf_counter

def chunks(data):
 size=CHUNK or len(data)
 for offset in range(0,len(data),size):yield offset,data[offset:offset+size]
async def h1_write_body(conn,writer,data,ledger):
 for offset,part in chunks(data):
  rec=dict(offset=offset,bytes=len(part),sha256=hashlib.sha256(part).hexdigest(),before_s=NOW());writer.write(conn.send(h11.Data(data=part)));rec['queued_s']=NOW();await writer.drain();rec['complete_s']=NOW();ledger.append(rec)
async def h3_write_body(protocol,stream,data,ledger):
 for offset,part in chunks(data):
  rec=dict(offset=offset,bytes=len(part),sha256=hashlib.sha256(part).hexdigest(),before_s=NOW());protocol.http.send_data(stream,part,end_stream=offset+len(part)==len(data));protocol.transmit();rec['queued_s']=NOW();await asyncio.sleep(0);rec['complete_s']=NOW();ledger.append(rec)
...
class H3(QuicConnectionProtocol):
 ...
 async def reply(self,stream,record):
  fixture=BYHASH[record['sha256']];record['step']=fixture['step'];response=fixture['response'];record['service_start']=NOW();await asyncio.sleep(fixture['service_s']);record['service_end']=NOW()
  self.http.send_headers(stream,[(b':status',b'200'),(b'content-length',str(len(response)).encode()),(b'content-type',b'application/json')])
  record['response_chunks']=[];await h3_write_body(self,stream,response,record['response_chunks'])
 async def request(self):
  ...
  send=NOW();self.http.send_headers(stream,[(b':method',b'POST'),(b':scheme',b'https'),(b':authority',b'localhost'),(b':path',b'/echo'),(b'content-length',str(len(PAYLOAD)).encode())])
  upload_chunks=[];await h3_write_body(self,stream,PAYLOAD,upload_chunks)
  result=await asyncio.wait_for(future,REQUEST_TIMEOUT);result=dict(result);result['upload_chunks']=upload_chunks;del result['future'];del self.states[stream];return send,stream,result
...
async def main(out,smoke):
 ...
 async def h1_server(reader,writer):
  ...
  try:
   while True:
    ...
    fixture=BYHASH[record['sha256']];record['step']=fixture['step'];response=fixture['response'];record['service_start']=NOW();await asyncio.sleep(fixture['service_s']);record['service_end']=NOW()
    writer.write(conn.send(h11.Response(status_code=200,headers=[(b'content-length',str(len(response)).encode()),(b'content-type',b'application/json')])))
    record['response_chunks']=[];await h1_write_body(conn,writer,response,record['response_chunks']);writer.write(conn.send(h11.EndOfMessage()));await writer.drain();conn.start_next_cycle()
 ...
 env=dict(python=platform.python_version(),platform=platform.platform(),machine=platform.machine(),openssl=ssl.OPENSSL_VERSION,packages={p:importlib.metadata.version(p) for p in ['aioquic','h11','cryptography','pylsqpack']},certificate_sha256=fingerprint,source_sha256=hashlib.sha256(Path(__file__).read_bytes()).hexdigest(),mode=MODE,chunk_bytes=CHUNK,requested_window=WINDOW,listener_settings=listener_settings,quic_max_data=qc.max_data,quic_max_stream_data=qc.max_stream_data,tcp_port=tcp_port,udp_port=udp_port,fixture=FIXTURE,smoke=smoke,server_and_client_same_event_loop=True,body_request_deadline_s=REQUEST_TIMEOUT)
 ...
 async def request(conn):
  ...
  c=conn['h11'];send=NOW()
  conn['writer'].write(c.send(h11.Request(method='POST',target='/echo',headers=[('Host','localhost'),('Content-Length',str(len(PAYLOAD)))])))
  upload_chunks=[];await h1_write_body(c,conn['writer'],PAYLOAD,upload_chunks);conn['writer'].write(c.send(h11.EndOfMessage()));await conn['writer'].drain();state=dict(body=bytearray(),first_headers=None,first_data=None,headers=None,upload_chunks=upload_chunks)
  while True:
 ...
 async def group(trial,proto,reuse,concurrency,count,warmup=False):
  ...
  async def lane(lane_id):
   ...
   for index in range(lane_id,count,concurrency):
    fixture=STEPS[index];PAYLOAD=fixture["payload"];response=fixture["response"]
    ...
    try:
     ...
     action=json.loads(data);decode_end=NOW();rec.update(step=fixture['step'],action=action,decode_start=decode_start,decode_end=decode_end,connection=conn['record']['id'],fresh=fresh,connection_ready=conn['record']['ready'] if fresh else rec['start'],send=send,stream=stream,**result,bytes=len(data),sha256=hashlib.sha256(data).hexdigest(),valid=(data==response and action==fixture['action'] and dict(result['headers']).get(':status')=='200'),validation_end=NOW(),error=None)
 ...
 try:
  ...
  for c in order:await group(c['trial'],c['protocol'],c['reuse'],c['concurrency'],len(STEPS))
 finally:
  sampler.cancel()
  try:await sampler
