import io,wave,collections,gzip,hashlib,json,statistics,struct
from pathlib import Path
R=Path(__file__).resolve().parent;O=R/'results/tts';order=json.loads((O/'order.json').read_text());offsets=json.loads((R/'tcp-offsets.json').read_text());F=json.loads((R.parent/'tts-network/tts-fixture.json').read_text())
assert order==[dict(trial=t,mode=m) for t in range(3) for m in (['default','64k','4m'][t:]+['default','64k','4m'][:t])]
AUDIO=(R.parent/'tts-network/audio.wav').read_bytes()
with wave.open(io.BytesIO(AUDIO),'rb') as w:PCM=w.readframes(w.getnframes());assert [w.getnchannels(),w.getsampwidth(),w.getframerate(),w.getnframes()]==[1,2,44100,591872]
assert hashlib.sha256(AUDIO).hexdigest()==F['response_sha256']
def tcp_info(r):
...
for c in order:
 ...
 env=json.loads((D/'environment.json').read_text());assert env['machine']=='x86_64';assert env['mode']==c['mode'] and env['requested_window']==expected and env['fixture']==F and not env['smoke'];assert env['source_sha256']==hashlib.sha256((R/'tts.py').read_bytes()).hexdigest();assert env['quic_max_data']==env['quic_max_stream_data']==(1048576 if expected is None else expected)
 ...
 for proto in ['h1','h3']:
  ...
  for r in server:
   assert r['sha256']==F['upload_sha256'] and r['bytes']==env['payload_bytes'] and r['service_end']-r['service_start']>=F['service_s']
   assert len(r['releases'])==len(F['release_plan'])
   for observed,planned in zip(r['releases'],F['release_plan']):
    assert (observed['offset'],observed['bytes'])==(planned['offset'],planned['bytes']) and observed['at']-r['service_start']>=planned['ready_s']
 ...
 for r in rows:
  assert r['valid'] and r['error'] is None and r['sha256']==F['response_sha256'] and r['wav_format']==[1,2,44100,591872] and r['pcm_sha256']==hashlib.sha256(PCM).hexdigest();assert r['bytes']==env['response_bytes'];assert dict(r['headers'])[':status']=='200'
  ...
  offset=0
  for event in r['arrivals']:
   offset+=event['bytes'];assert offset==event['total'] and r['first_data']<=event['at']<=r['end']
  assert offset==len(AUDIO)
  one=next(e['at'] for e in r['arrivals'] if e['total']>=1808);three=next(e['at'] for e in r['arrivals'] if e['total']>=5336);assert r['first_data']<=one<=three<=r['end']
  allrows.append(dict(**r,first_frame_ms=(one-r['start'])*1000,three_frames_ms=(three-r['start'])*1000,mode=c['mode'],outer_trial=c['trial'],subrun=name))
 for g in groups:
