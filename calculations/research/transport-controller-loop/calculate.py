...
spec = importlib.util.spec_from_file_location("closed_sender", ROOT / "sender.py")
sender = importlib.util.module_from_spec(spec)
spec.loader.exec_module(sender)
pacer_spec = importlib.util.spec_from_file_location(
    "controller_pacer", ROOT / "pacer.py"
)
...
def calculate(inputs):
    ...
    if p.get("ack_policy", "immediate_each_packet") != "immediate_each_packet":
        raise ValueError("only explicitly selected immediate ACK policy implemented")
