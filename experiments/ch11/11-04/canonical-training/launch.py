...
try:
 with (R/'train.log').open('x') as f,(R/'gpu.jsonl').open('x') as g:
  p=subprocess.Popen([E,'-u',str(R/'train.py')],stdout=f,stderr=subprocess.STDOUT,start_new_session=True,env={**os.environ,'OMP_NUM_THREADS':'4'});state['pid']=p.pid
