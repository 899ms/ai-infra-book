...
lines+=['','其他包含 router、潜空间进出投影、归一化、卷积和连接等参数；注意力投影包括 V4 的压缩与索引投影。Qwen3-8B 的主要参数集中在每个 token 都执行的稠密 FFN，三个 MoE 模型则将大部分参数放进路由专家。Qwen3.6 的路由专家占约 93%，V4 与 K3 约为 97%—98%。因此，它们的总容量主要由专家集合决定，单 token 工作则由每层选出的专家决定。','','**表 2-C　相同输入条件下的计算与状态**','','| 模型 | 统一 BF16 权重（GB） | 8K prefill（TFLOPs） | 8K 历史单步 decode（GFLOPs） | 8K 状态（MiB） | 单 token 选中路由专家 BF16 载荷（GiB） |','| --- | ---: | ---: | ---: | ---: | ---: |']
...
lines+=['', '两种调用均取 $B=1$，prefill 为 $S=0,P=8192$，decode 为 $S=8192,P=1$，词表头均只处理末位置。状态列取已有 8192 位置，历史表示采用 BF16，递推和压缩缓冲采用相应 FP32。最后一列按统一 BF16 计算一次选中的路由专家集合。表内的权重、计算与状态分别对应驻留规模、当前调用和请求历史。', '', '各模型按自己的执行路径累计：Qwen3-8B 为有效因果注意力，Qwen3.6 为 eager 矩形注意力与块式线性分支，V4 为有效主注意力及参考索引，K3 为紧凑 MLA 与块式 KDA。Qwen3.6 保存的权重约为 Qwen3-8B 的四倍，单步矩阵工作却更少；V4 权重继续扩大，8K 状态反而因压缩与选择缩小；K3 的巨大专家集合、较宽主干和更多层共同提高了容量与激活工作。三个资源维度沿各自的结构增长。', '', '**从保存量继续推到访问量。** Qwen3-8B 每步使用 1152 MiB 旧 KV；V4 的主历史选择与索引扫描合计 27.625 MiB，并更新压缩器。K3 紧凑 MLA 使用 216 MiB 历史，414 MiB 递推矩阵还要一读一写，形成 828 MiB 访问；Qwen3.6 对应 160 MiB 全局历史和 120 MiB 递推矩阵读写。固定状态每步仍被更新，全局历史每步仍被查询，二者从不同路径产生持续访问。', '', '**从参数转到存储格式。** 统一 BF16 给出相同每参数字节数下的结构规模；发布 checkpoint 采用各自格式，V4 主干约为 156.02 GB，K3 文本约为 1559.97 GB。低位宽矩阵、较高精度保留项和格式元数据共同形成文件载荷。加载时再按执行需要转换或分配，第二章的容量筛选由此连接到第五章的格式与运行时。', '', '表 2-B、2-C 的逐项整数、来源与复算见[四模型比较数据](ch02/model-comparison.json)和[生成程序](ch02/compare_models.py)。Qwen3.6 使用[末位置输出头结果](../calculations/results/qwen36-prefill-last-head.md)，V4 单步由相同公共计算入口生成；完整算子与 checkpoint 范围仍以对应模型表的结果链接为准。']
