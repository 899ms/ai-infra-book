...
class BbrAdapter:
    ...
    def active(self):
        # Dict insertion order matches historical send order after removals.
        return list(self.active_records.values())

    def _remove_active(self, pn):
        record = self.active_records.pop(pn, None)
        if record is not None:
            self.active_bytes -= record["sent_bytes"]

    def _flight(self, supplied):
        if supplied is not None and supplied != self.active_bytes:
            raise ValueError(
                f"Sender flight mismatch {supplied} != {self.active_bytes}"
            )
    ...
    def _on_sent(self, packet, now, flight_before=None, limited_flags=None):
        ...
        if counted and not self.active_records:
        ...
        self.highest_sent_pn = (
            pn if self.highest_sent_pn is None else max(pn, self.highest_sent_pn)
        )
        if counted:
            self.active_records[pn] = record
    ...
    def _on_loss(self, records, now, flight_after=None):
        ...
        for row in records:
            ...
            if r["counted"] and not r["acked"] and not r["lost"]:
                r["lost"] = True
                self._remove_active(pn)
                new.append(pn)
        if new:
            if self.recovery_cutoff is None:
                self.core.on_ssthresh()
                self.recovery_cutoff = self.highest_sent_pn
    ...
    def on_sent(self, record, context):
        ...
        if flags["app_limited"] and not flags["flow_limited"]:
            self.app_limited_marker = (
                self.delivered + len(self.active_records)
            ) & 0xFFFFFFFF or 1
    ...
    def status(self):
        return dict(
            ...
            pending_losses=self.pending_losses,
            active_packet_count=len(self.active_records),
