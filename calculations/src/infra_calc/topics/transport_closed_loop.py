...
def calculate(inputs=None):
    if isinstance(inputs, dict) and ("application" in inputs or "network" in inputs):
        if set(inputs) != {"application", "network"}:
            raise ValueError("media input requires exactly application and network")
        if isinstance(inputs["network"], dict) and "wireless_access" in inputs["network"]:
            from ..transport.shared_airtime_network import calculate as calculate_media
        else:
            from ..transport.media_network import calculate as calculate_media
        return calculate_media(inputs)
    ...
    if aggregate:
        ...
        result["rtt_samples"] = {d: states[d]["result"]()["rtt_samples"] for d in directions}
    return result
...
def markdown(result):
    ...
    aggregated_ack = isinstance(p.get("ack_policy"), dict)
    if aggregated_ack:
        lines[2] = (
            "这是已确认单路径的有限 ACK 聚合与真实反馈计算。接收端按声明包数或期限触发，"
            "ACK 真正开始发送时才冻结范围和编码延迟；沿用同一发送方、流控、恢复与可选控制器。"
            "不是浏览器默认策略、ACK_FREQUENCY 协商或完整 TCP/QUIC 实测；媒体业务接入仍另行核算。"
        )
    for key, label in [
        ("upload", "服务端完整收到上传"),
        ("model_start", "服务端模型开始"),
    ...
    if aggregated_ack:
        lines += [
            "", "## ACK 聚合、范围与延迟", "",
            f"声明策略：`{json.dumps(p['ack_policy'], ensure_ascii=False)}`。",
            "",
            "范围保留数只限制报告范围，不等于接收端全部历史的内存上限。"
            "ACK frame 按实际 QUIC varint 宽度核算；声明报文空间不足时明确拒绝。"
            "若反向排队超过 max_delay，保留真实值并标记超期，不能夹短延迟掩盖期限未满足。",
            "",
            "发送方首个 RTT 样本不扣 ACK delay；已有样本时按 minRTT 与声明上限决定扣除。"
            "纯 ACK PN 的 lost 身份不等于物理丢包或业务重传；应与数据 PN、实际 drop 和恢复记录分别计数。",
            "",
        ...
        for packet in result["transmissions"]:
            if packet["kind"] != "ack":
                continue
            snap = packet["ack_snapshot"]
            lines.append(
                f"| {packet['direction']}/{packet['pn']} | {packet['send_start']} | {snap['ranges']} | "
                f"{snap['raw_delay']} | {snap['decoded_delay']} | {snap['frame_bytes']} | {snap['exceeds_max_delay']} |"
            )
        lines += ["", "逐次 raw/adjusted RTT、SRTT 和 variance 记录见同名 JSON 的 rtt_samples。", ""]
    lines += ["", "## 已校验官方来源", ""]
