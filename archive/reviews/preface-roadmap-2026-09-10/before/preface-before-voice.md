...
我对这些问题的兴趣，可以追溯到在微软亚洲研究院与中科大联合培养读博的时候。那时，我研究怎样用 FPGA 这样的可重构硬件加速数据中心的软件。我的博士论文《基于可编程网卡的高性能数据中心系统》，把计算、存储和网络放在一起考虑：让网卡承担网络功能、直接处理内存中的数据结构，再与主机上的软件配合，减少通信过程中的拷贝和同步。[^preface-phd] 我关心的是，怎样利用应用的特点重新划分软硬件的工作，同时保留编程的灵活性。

模型执行的数据代价，也是那时就遇到的问题。2016 年，围绕微软 Bing 搜索排序所用的深度神经网络，我们探索用 FPGA 加速推理。模型反复从片外内存搬进芯片，限制了计算速度。我想到，能否把模型拆到多块 FPGA，让各部分留在各自的片上存储中，只通过高速网络传递中间结果？我兴奋地把想法告诉徐宁仪老师，他说，这叫“模型并行”。[^preface-fpga] 后来，我在《芯片架构换血！如何评价微软在数据中心使用 FPGA？》中整理了对这类硬件加速的认识。计算放在哪里、数据怎样流动、专用化能省下什么，已经是我当时反复思考的问题。

2019 年加入华为后，我参与了 MindSpore 的自动算子生成项目 AKG，担任动态形状团队的架构师和技术负责人。这项工作延续了此前的软硬件协同思路：把模型计算转换为适合昇腾神经网络处理器执行的程序，由编译器安排数据分块、存储和执行顺序。[^preface-akg] 我还记得为 softmax 算子做融合时，到处寻找合适的算法，后来找到 NVIDIA 在 2018 年发表的在线 softmax 工作，才有了继续实现融合的办法。[^preface-softmax] 要让中间结果尽量留在片内，就需要把算法、编译和硬件放在一起考虑。
...
从可编程网卡到 AKG，再到 UB，工作跨越了不同的尺度，我反复遇到的却是相通的问题：计算需要的数据在哪里，现有接口增加了哪些工作，掌握更多应用信息以后，能否换一种执行方式？后来写《Unified Bus 背后的思考》时，我尝试把早期设计中的这些问题和选择记录下来。今天产品化的 UB 已经经历了后续团队的大量工作，具体实现也可能与早期方案不同。本书会结合亲历、公开规范与具体计算，解释设计的依据和适用条件。
...
[^preface-phd]: 李博杰，《基于可编程网卡的高性能数据中心系统》，博士论文，2019 年。[论文摘要与入口](https://ring0.me/2019/12/phd-thesis/)。
