...
f,a=canvas(7)
box(a,.03,.78,.18,.13,"host CPU\nprepares input, submits task")
box(a,.32,.78,.24,.13,"off-chip memory\nweights, input, final output")
box(a,.71,.78,.23,.13,"device control and task scheduling")
arrow(a,(.22,.845),(.31,.845));a.plot([.12,.12,.825],[.915,.96,.96],ls='--',color=C['gray'],lw=1.4);arrow(a,(.825,.96),(.825,.915),True);a.text(.46,.969,"submit command",ha='center',fontsize=10);arrow(a,(.83,.77),(.58,.38),True)
box(a,.31,.57,.25,.12,"shared cache / memory interface");arrow(a,(.435,.77),(.435,.70))
a.add_patch(FancyBboxPatch((.04,.05),.90,.43,boxstyle='round,pad=.012',facecolor='none',edgecolor=C['gray'],linestyle='--'))
a.text(.06,.445,"local compute unit group (multiple possible)",fontsize=11,color=C['gray'])
box(a,.08,.25,.20,.12,"local input buffer\ntiling and reuse",color='light')
box(a,.38,.25,.20,.12,"matrix compute unit\nmultiply-add, local accumulation",color='sand')
box(a,.69,.25,.20,.12,"accumulator storage\nregister / dedicated storage")
box(a,.69,.08,.20,.10,"vector and general compute")
arrow(a,(.37,.56),(.18,.38));a.text(.12,.52,"load / move",fontsize=11)
arrow(a,(.29,.31),(.37,.31));arrow(a,(.59,.31),(.68,.31));arrow(a,(.79,.24),(.79,.19));arrow(a,(.68,.12),(.48,.12));arrow(a,(.48,.12),(.48,.24));a.text(.35,.075,"intermediate result continues computing",fontsize=10)
arrow(a,(.91,.37),(.91,.62));arrow(a,(.90,.63),(.57,.63));a.text(.70,.65,"write back as needed",fontsize=11)
...
for i,(label,col) in enumerate([("matrix",'blue'),("shared memory read",'teal'),("exponent",'orange')]):
...
a.set_yticks(range(4),["original config","matrix only ×2","matrix and exponent ×2","all three ×2"]);a.invert_yaxis();a.set_xlabel("cycles per SM per tile");a.set_xlim(0,1150);a.legend(ncol=3,frameon=False,loc='upper left',bbox_to_anchor=(0,1.18))
...
# Precision paths.
f,a=canvas(7)
box(a,.03,.43,.20,.23,"compressed weights + scale\n8.5 MiB",'light')
for y in [.78,.22]:arrow(a,(.235,.55),(.32,y))
box(a,.33,.68,.22,.20,"unroll / dequantize\nBF16 copy 32 MiB")
box(a,.63,.68,.22,.20,"matrix compute\nmatching accumulation precision",'sand');arrow(a,(.56,.78),(.62,.78))
box(a,.33,.12,.22,.20,"low-precision matrix compute\npadding / partial sum",'sand')
box(a,.63,.12,.22,.20,"scale, merge, convert\nto required output format");arrow(a,(.56,.22),(.62,.22))
...
for threshold,rate in zip(thresholds,[b/1e12 for b in bandwidths]):
 a.plot(threshold,rate,'o',color=C['orange']);a.annotate(f'at least {threshold} ',xy=(threshold,rate),xytext=(threshold+350,rate-.23),fontsize=11,arrowprops={'arrowstyle':'-','color':C['gray']})
a.axvline(4096,color=C['gray'],ls=':',lw=1);a.set(xlim=(0,10000),ylim=(0,2.25),xlabel="concurrent outstanding requests N",ylabel="achievable bandwidth ceiling / TB/s");a.legend(frameon=False,loc='upper left');a.grid(axis='y',alpha=.15)
...
for a,slots in zip(axes,[1,2,3]):
 ...
 a.set_yticks(range(4),["block 0","block 1","block 2","Block 3"]);a.invert_yaxis();a.set_xlim(0,1320);a.set_title(f'{slots} input slots: complete {r["finish_tick"]} tick',loc='left',fontsize=12);a.set_xticks([0,320,640,960,1280]);a.tick_params(labelbottom=True)
axes[-1].set_xlabel("time / tick (B200 SM clock cycle)")
f.text(.12,.966,"blue: input transfer  orange: data ready  green: compute  light gray: slot occupied until compute ends",fontsize=11)
...
for a,size,label in zip(axes,payload,['M=1：8 KiB','M=256：2 MiB']):
 ser=size/rates*1e6;total.append((ser+alpha).tolist());a.bar([0,1],[alpha,alpha],color=C['orange'],label="startup");a.bar([0,1],ser,bottom=alpha,color=C['blue'],label="transmission");a.set_xticks([0,1],['A100 300 GB/s','H100 450 GB/s']);a.set_title(label,loc='left',fontsize=12);a.set_ylabel("time / μs");a.set_ylim(0,max(ser+alpha)*1.2)
...
b=np.arange(1,33);W=15136811008;K=1207959552;a.plot(b,np.full(len(b),W/1e9),color=C['blue'],lw=2,label="weights this step: read once per batch");a.plot(b,b*K/1e9,color=C['teal'],lw=2,label="KV: read once per request");a.axvline(13,color=C['orange'],ls=':');a.annotate("from batch 13, KV exceeds weights",xy=(13,13*K/1e9),xytext=(16,9),arrowprops={'arrowstyle':'->','color':C['orange']},fontsize=11);a.set_xlabel('batch B');a.set_ylabel("per-step read / GB");a.set_xlim(1,32);a.set_ylim(0,42);a.legend(frameon=False,fontsize=11,loc='upper left')
...
labels=[f'M={n}\n'+("reuse" if mode=='reused' else "rotation") for n,mode in conditions];colors=[C['blue'],C['teal']]*2
...
for a,values,ylabel,limit in [(axes[0],wall,"total time per call / μs",62),(axes[1],dram,"separately measured DRAM read / MiB",40)]:
...
page="<!doctype html><html lang=\"zh-CN\"><meta charset=\"utf-8\"><meta name=\"viewport\" content=\"width=device-width,initial-scale=1\"><title>Chapter 4 Accelerator Architecture</title><style>"+css+'</style><main><nav>'+nav+'</nav>'+body+'</main></html>'
import sys
sys.path.insert(0,str(HERE.parent))
from teaching_reading import readable_diagrams
from preview_output import preview_path
page=readable_diagrams(page)
html_path=preview_path(HERE.parent/"04-Accelerator Architecture.html");html_path.write_text(page)
...
(HERE/'manifest.json').write_text(json.dumps({'chapter':4,'generator':'manuscripts/ch04/build.py','figures':len(re.findall(r'!\[',raw)),'font_family':family,'outputs':[{'path':str(p.relative_to(ROOT)),'sha256':hashlib.sha256(p.read_bytes()).hexdigest()} for p in artifacts if not (p.parent==ROOT/'manuscripts' and re.match(r'^[01][0-9]-',p.name))]},ensure_ascii=False,indent=2)+'\n')
