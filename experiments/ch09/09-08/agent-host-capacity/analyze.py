...
ex=read(O/'execution.json');assert len(ex['rows'])==3 and all(x['exit_code']==0 for x in ex['rows']) and not ex['after_gpu'].strip()
...
for capacity in [4096,8192,16384]:
 d=O/str(capacity);raw=read(d/'raw.json');rows=lines(d/'requests.jsonl');assert raw['status']=='all_requests_returned' and raw['config']['max_total_tokens']==capacity and len(rows)==36 and rows==raw['requests']
 ...
 for index,(item,other) in enumerate(zip(inputs,pressure)):
  ...
  for j,(phase,ids) in enumerate([('target_before',item['input_ids']),('pressure',other),('target_after',item['input_ids'])]):
   ...
   r=dict(capacity=capacity,host_ratio=2,turn=item['turn'],phase=phase,input_tokens=len(ids),cached_tokens=m['cached_tokens'],levels=detail,complete_request_s=row['end_s']-row['start_s']);records.append(r)
