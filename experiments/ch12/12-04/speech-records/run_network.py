...
FIXTURE=json.loads((R/'asr-fixture.json').read_text());PAYLOAD=(R/FIXTURE['upload_file']).read_bytes();RESPONSE=(R/FIXTURE['response_file']).read_bytes();HASH=hashlib.sha256(PAYLOAD).hexdigest();RESPONSE_HASH=hashlib.sha256(RESPONSE).hexdigest();SERVICE_S=FIXTURE['service_s']
...
class H3(QuicConnectionProtocol):
 ...
 async def reply(self,stream,record):
  ...
  self.http.send_headers(stream,[(b':status',b'200'),(b'content-length',str(len(RESPONSE)).encode()),(b'content-type',b'text/plain')]);self.http.send_data(stream,RESPONSE,end_stream=True);self.transmit()
 ...
 async def h1_server(reader,writer):
  ...
  try:
   while True:
    ...
    record['service_start']=NOW();await asyncio.sleep(SERVICE_S);record['service_end']=NOW()
    for event in [h11.Response(status_code=200,headers=[(b'content-length',str(len(RESPONSE)).encode()),(b'content-type',b'text/plain')]),h11.Data(data=RESPONSE),h11.EndOfMessage()]:writer.write(conn.send(event))
