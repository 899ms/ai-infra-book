...
def main(a):
 ...
 loader=DataLoader(data,batch_size=None,num_workers=a.workers,prefetch_factor=2,multiprocessing_context='spawn',sampler=sampler,collate_fn=ident,worker_init_fn=init_worker,generator=generator)
 ...
 write(out/'identity.json',dict(pid=os.getpid(),pgid=os.getpgrp(),workers=a.workers,seed=a.seed,control=a.control,resume_from=a.checkpoint,start_step=step,torch=torch.__version__,threads=torch.get_num_threads(),time=time.monotonic(),record_cursor=cursor,parameters=sum(p.numel() for p in model.parameters())))
...
if __name__=='__main__':
 p=argparse.ArgumentParser();p.add_argument('--workers',type=int,choices=[1,4],required=True);p.add_argument('--output',required=True);p.add_argument('--seed',type=int,required=True);p.add_argument('--steps',type=int,default=64);p.add_argument('--stop',type=int,default=-1);p.add_argument('--checkpoint');p.add_argument('--control',default='correct',choices=['correct','omit_buffer','dispatch_cursor']);main(p.parse_args())
