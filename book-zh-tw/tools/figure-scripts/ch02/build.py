...
f,a=canvas("圖 2-1  序列模型的計算依賴","同一座標：橫軸為序列位置，縱軸為網路深度；顏色區分已完成狀態與新增工作。",8.2)
for panel,x0,title in [(0,.065,"RNN：同層時間遞推"),(1,.39,"因果 Transformer：跨層存取"),(2,.715,"快取生成：僅追加新 token")]:
 ...
 for l,y in enumerate(ys):
  ...
  a.text(x0-.025,y,"輸入" if l==0 else f'L{l}',ha='right',va='center',fontsize=9)
 for t,x in enumerate(xs):a.text(x,.255,f't{t+1}',ha='center',fontsize=10)
 a.text(x0+.10,.17,["固定寬度狀態\n位置 t 等待位置 t−1","允許存取上一層的因果字首\n同層已知位置可並行","綠色：可複用的上下文\n橙色：當前 token 前向"][panel],ha='center',fontsize=11,linespacing=1.7)
a.text(.05,.065,"參數共享方式和資料邊不同，「旋轉 90 度」僅是區域性類比；未知輸出之間仍有跨呼叫依賴。",fontsize=11,color=C['muted'])
...
f,a=canvas("圖 2-2  Qwen3-8B 單層的尺寸與資料流","36 層 · d=4096 · 32 個 Q 頭／8 個 KV 頭 · 頭寬 128；m=BP（prefill）或 B（decode）。",10)
box(a,.055,.75,.18,.075,"輸入 X",'$m\\times 4096$')
...
f.suptitle("圖 2-3  KV 共享與潛變數壓縮",x=.05,ha='left',y=.96,fontsize=23,weight='bold')
f.text(.05,.875,"相同上下文長度 8192、BF16；兩個面板分別比較自己的模型設定。",fontsize=12,color=C['muted'])
for ax,labels,vals,title in [(axs[0],["MHA\n32 KV 頭","GQA\n8 KV 頭","MQA\n1 KV 頭"],[4608,1152,144],"A  固定 Qwen 層數與頭寬"),(axs[1],["緊湊潛變數\n512＋64 維","參考展開 K/V"],[216,11520],"B  K3 的 24 個 MLA 層")]:
 ax.bar(range(len(vals)),vals,color=[C['blue'],C['teal'],C['orange']][:len(vals)],width=.55);ax.set_xticks(range(len(vals)),labels);ax.set_yscale('log');ax.set_ylim(50,30000);ax.set_ylabel("上下文狀態（MiB，對數軸）");ax.set_title(title,loc='left',fontsize=14,pad=15)
 for i,vv in enumerate(vals):ax.text(i,vv*1.18,f'{vv:,}',ha='center',fontsize=12)
 ax.grid(axis='y',alpha=.15)
f.text(.05,.095,"A：僅 GQA 是真實基線，MHA／MQA 是機制變體；Q 頭互動不按 KV 頭數同比減少。",fontsize=11)
f.text(.05,.05,"B：未加 KDA／卷積／工作區；緊湊代數路徑不代表固定參考程式碼的實際分配。",fontsize=11,color=C['muted'])
...
f,a=canvas("圖 2-4  V4-Flash 的視窗、CSA 與 HCA","43 層主幹：2 個純視窗層＋21 個 CSA 層＋20 個 HCA 層；S=8192，B=1。",9)
for y,title,body in [(.71,"視窗 128","儲存最近位置；每步存取區域性上下文"),(.53,"CSA：壓縮比 4","儲存 2048 個壓縮條目與索引；主注意力最多選擇 512 條"),(.35,"HCA：壓縮比 128","儲存 64 個粗粒度條目；存取視窗與粗粒度上下文")]:
 box(a,.055,y,.27,.13,title,body.split('；')[0],color='light',size=13)
 arrow(a,(.335,y+.065),(.405,y+.065))
 box(a,.415,y,.52,.13,"存取與新增工作",("主注意力最多選擇 512 條；索引掃描全部 2048 條" if title.startswith('CSA') else body.split('；')[-1] if '；' in body else "區域性查詢與滑動更新"),color='pale',size=12)
a.text(.065,.255,"整模型常駐參考狀態（MiB）",fontsize=13,weight='bold')
com=v['components'];names=["視窗","壓縮上下文","索引上下文","壓縮緩衝"];vals=[com['window_history_bytes']/MiB,com['compressed_history_bytes']/MiB,com['index_history_bytes']/MiB,v['summary']['compressor_buffer_bytes']/MiB]
for i,(n,z) in enumerate(zip(names,vals)):box(a,.06+i*.23,.135,.20,.075,n,f'{z:.3f} MiB',size=11)
a.text(.06,.075,"最後查詢：主注意力 17.125 MiB ＋ 索引掃描 10.500 MiB ＝ 27.625 MiB。",fontsize=11)
a.text(.06,.035,"索引仍掃描壓縮索引上下文；未完成塊與更新峰值單列。圖中資料量不是 HBM 實測。",fontsize=10,color=C['muted'])
...
f.suptitle("圖 2-5  狀態容量、存取與輸入工作",x=.05,ha='left',y=.96,fontsize=24,weight='bold');f.text(.05,.875,"B=1；容量曲線為結構推算，非長上下文品質驗證；K3 採用緊湊 BF16 MLA＋FP32 遞推狀態。",fontsize=11)
for ax,series,title in [(axs[0],[qr,vr,kr],"A  常駐狀態"),(axs[1],[qaccess,vaccess,kaccess],"B  每步已計狀態存取")]:
 ...
 ax.set_xscale('log',base=2);ax.set_yscale('log');ax.set_xticks([8,32,128,512,1024],['8K','32K','128K','512K','1M']);ax.set_xlabel("上下文長度");ax.set_ylabel("GiB（對數軸）");ax.set_title(title,loc='left',fontsize=14);ax.grid(alpha=.17);ax.legend(fontsize=8,frameon=False)
a=axs[2];a.bar(range(3),pre,color=[C['blue'],C['teal'],C['orange']],width=.55);a.set_xticks(range(3),['Qwen3\n8B','V4\nFlash','K3\ncompact']);a.set_yscale('log');a.set_ylim(50,4500);a.set_ylabel("矩陣 TFLOPs（對數軸）");a.set_title('C  8192-token prefill',loc='left',fontsize=14)
...
f,a=canvas("圖 2-6  稠密層與 V4 層的資料流","同為 4096 維子層輸入；連線狀態、子層矩陣與專家路由分別標註。",10)
for x,title in [(.10,'Qwen3-8B'),(.60,'V4-Flash')]:a.text(x+.14,.82,title,ha='center',fontsize=17,weight='bold')
for y,title,body in [(.66,"GQA 注意力","QKV → 上下文存取 → Wo"),(.45,"殘差相加／歸一化",'$m\\times 4096$'),(.24,"稠密 SwiGLU","所有 token 使用同組 FFN 權重")]:box(a,.10,y,.28,.12,title,body,color='light')
arrow(a,(.24,.65),(.24,.58));arrow(a,(.24,.44),(.24,.37));arrow(a,(.24,.23),(.24,.15));a.text(.24,.115,"殘差相加 → 下一層",ha='center',fontsize=12)
for y,title,body in [(.66,"mHC 匯合 → 注意力","視窗／壓縮／索引；輸入 $m\\times 4096$"),(.45,"mHC 殘差混合","保留四路狀態；子層維度不乘 4"),(.24,"mHC 匯合 → MoE","256 選 6＋1 共享專家；每專家 $t_e$ 行")]:box(a,.60,y,.31,.12,title,body,color='sand')
arrow(a,(.755,.65),(.755,.58));arrow(a,(.755,.44),(.755,.37));arrow(a,(.755,.23),(.755,.15));a.text(.755,.115,"殘差混合 → 下一層",ha='center',fontsize=12)
...
a.text(.045,.05,"橙色側路表示殘差傳遞；完整歸一化、混合與輸出連線必須計量，不能僅累加注意力和專家矩陣。",fontsize=10.5,color=C['muted'])
...
f.suptitle("圖 2-7  模型容量與架構形狀",x=.05,ha='left',y=.96,fontsize=24,weight='bold');f.text(.05,.88,"容量使用十進位制 GB；固定工作區 2 GiB，BF16 KV，8192 上下文，每柱包含一個請求。",fontsize=11)
...
for field,label,col in [('weight_bytes',"權重",'blue'),('workspace_bytes',"固定預留",'orange'),('kv_bytes_per_request',"1 條請求 KV",'teal')]:
 vals=np.array([z[field]/1e9 for z in entries]);a.barh(y,vals,left=left,label=label,color=C[col],height=.5);left+=vals
for i,z in enumerate(entries):a.plot([z['capacity_bytes']/1e9]*2,[i-.32,i+.32],color=C['ink'],lw=2);a.text(2,i+.37,f"{ {24_000_000_000:'RTX 4090',80_000_000_000:'H100 SXM'}[z['capacity_bytes']]} {z['capacity_bytes']/1e9:.0f} GB：最多容納 {z['maximum_requests']} 條",fontsize=10)
a.set_yticks(y,['Qwen8\nBF16',"70B\n8-bit 方案","70B\n4-bit 方案"]);a.invert_yaxis();a.set_xlim(0,87);a.set_xlabel('GB');a.set_title("A  權重、狀態與容量",loc='left',fontsize=14);a.legend(loc='upper center',bbox_to_anchor=(.5,-.13),ncol=3,fontsize=9,frameon=False)
...
a.bar(["8K 上下文","32K 上下文"],[y8,y32],color=[C['blue'],C['orange']]);a.set_ylabel("容量允許的獨立請求數");a.set_title("B  上下文增長擠佔併發容量",loc='left',fontsize=14)
for i,z in enumerate([y8,y32]):a.text(i,z+.25,str(z),ha='center')
a.set_ylim(0,17);a.text(.5,.88,"70B · 4-bit 量化 · H100 SXM 80 GB\n固定預留 2 GiB",transform=a.transAxes,ha='center',fontsize=11)
...
f,a=canvas("圖 2-8  請求輸入與計算狀態對映","統一資源請求：S=0、P=128、G=4、B=1；首輸出來自 prefill，decode 呼叫為 G−1=3。",9)
...
box(a,.65,.235,.29,.27,"同一文字的檢索實驗","Qwen 與 V4-Flash 各 8/8\n輸入 token 數、部署與精度不同\nPro／K3 未測",color='sand',size=13)
# Scope is stated in the external caption.
a.text(.055,.055,"左側按給定 token 數估算運算量；右側用同一文字檢查回答，並記錄各模型的輸入長度。",fontsize=11,color=C['muted'])
...
for x,title,groups in [(.05,'MHA',4),(.375,'GQA',2),(.70,'MQA',1)]:
 ...
 a.text(x+.15,.27,f'{groups} 組上下文 K、V',ha='center',fontsize=13)
 a.text(x+.15,.19,"4 組查詢評分與加權彙總",ha='center',fontsize=11,color=C['muted'])
...
f,a=canvas('','',8.8)
box(a,.35,.75,.30,.075,"當前層輸入",r'$X\in\mathbb{R}^{m\times2048}$',size=13)
for x,title,body in [(.08,"線性注意力層（共 30 層）","固定遞推矩陣＋短卷積"),(.57,"完整注意力層（共 10 層）","隨上下文增長的全域 KV")]:
 box(a,x,.54,.35,.13,title,body,color='light',size=13)
 arrow(a,(.50,.74),(x+.175,.68))
 arrow(a,(x+.175,.53),(.50,.45))
box(a,.32,.345,.36,.095,"歸一化與殘差連線","執行模型設定指定的注意力分支",size=13)
arrow(a,(.50,.335),(.50,.285))
box(a,.10,.16,.37,.115,"路由專家：256 選 8","專家 $e$ 處理 $t_e$ 行",color='sand',size=13)
box(a,.59,.16,.31,.115,"共享專家","處理全部 $m$ 行",color='sand',size=13)
...
page="<!doctype html><html lang=\"zh-CN\"><meta charset=\"utf-8\"><meta name=\"viewport\" content=\"width=device-width, initial-scale=1\"><title>第 2 章 模型架構</title><style>"+css+'</style><main><nav>'+nav+'</nav>'+body+'</main></html>'
import sys
sys.path.insert(0,str(HERE.parent))
from teaching_reading import readable_diagrams
from preview_output import preview_path
page=readable_diagrams(page)
html_path=preview_path(HERE.parent/"02-模型架構.html");html_path.write_text(page)
