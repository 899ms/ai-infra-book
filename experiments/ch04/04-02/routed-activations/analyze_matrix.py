...
def main(out):
 e=json.loads((out/'environment.json').read_text());assert e['source_sha256']==hashlib.sha256((R/'run_matrix.py').read_bytes()).hexdigest()
 ...
 result=[]
 for r in rows:
  assert r['integer_exact'] and r['padded_m']==max(32,r['m']);assert r['k']==2048 and r['n']==1536
  assert all(x['passed']==(x['relative_l2_to_checkpoint_fp32']<=.02) for x in r['checks'].values())
 ...
 (out/'summary.json').write_text(json.dumps(dict(smoke=e['smoke'],rows=result,scope='Checkpoint expert matrix, actual routed activations, GPU-resident timed operator paths; no full model quality or cold-memory claim'),indent=2)+'\n');print('Verified',len(rows),'shapes')
