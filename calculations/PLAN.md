...
本项目把大纲中散落的推导、算例和实验中的计算部分收拢为可复现 Python 项目。范围包括十二章正文、逐章扩写资料及其案例笔记，不能以 36 项核心练习代替全部计算。正式 GPU 测量、质量评测和作者尚未提供的记录按原大纲保持独立；需要这些输入的计算先提供明确标记的情景扫描及数据导入，不能填造实测。

## 当前状态速览（2026-09-10，非全书完成率）

当前93个顶层工作包中32个已勾选、61个未勾选。未勾选包含“部分实现”和“尚未逐原文验收”，不能解释为61项完全没做；脚本数、测试数也不能换算为完成百分比。

本轮收口了状态复核队列里剩余的跨章综合条目：C70–C76 七项全部按原文契约交付并勾选，各自的模块、结果、独立测试与明确未计入项写回原条目。队列中仍未勾选的 F01、F04、C37、C58、C63、C68、C69 已各自登记具体缺口，不属于状态不清。
...
| C70/C71 端边云记录与跨地域    | 本轮完成并勾选                | 有新增实测记录时复核受影响结论            |
| C72–C76 跨章综合与设计记录    | 本轮完成并勾选                | 待真实观测填入设计记录的“观察”一栏         |
| F01/F04 全书覆盖与最终交付    | 未完成                   | 对全部原文要求与现有成果逐项核对        |

维护约定：新成果同时更新对应工作包的“已完成/仍缺”，PROGRESS只保留过程和验证记录。末尾历史追加说明不能替代本表及原条目。已回填C10/C13/C14及本轮C24、C77–C81的确定过时内容；其余条目仍须逐项复核，不能假称整份PLAN已全部核准。最新状态复核与收口顺序区分维护欠账和真实剩余工作。
...
- [ ] F01 原文清单：正文小节、练习、扩写资料与所有案例逐条纳入，审查计算范围及旧脚本迁移。清单本身已捕获 95 份原件、1,326 个文本块；本轮补上一直缺的那一半——从文本块回到工作包的映射，见[覆盖清单](inventory/f01-coverage.json)与 `python3 calculations/calc.py coverage --format md`。映射按各条自带的小节引用机械生成，直接命名与仅命名其子节两种关系分开记，并逐块记录量化信号与已引用的生成结果。当前：1,054 块有量化信号，878 块有直接候选工作包，27 块仅通过子节相关，351 块有量化信号却无候选；146 块引用了生成结果且全部解析成功（0 处失效链接）。**由此发现并已修复的计划欠账**：第 3 章改写后小节全部移位，C15–C20 仍写着旧编号，C16/C17/C18 声称的视觉、语音与训练小节实际已成为无人认领的空白；C56 的 `10.4.1、3` 简写此前也解析不出 10.4.3。已按标题逐条对应更新 C15–C20 与 C56/C57 的小节引用并注明原编号。本轮（清单刷新后）继续修复改号欠账：第 4 章插入 4.6 模型需求演进后旧 4.6 专用架构／4.7 性能与选择顺延为 4.7／4.8（C73 改引 4.7，C75 回本算例改引 4.8.4，C22/C24 补引 4.8.1–2／4.8.3–4）；第 8 章小节移位（C42 补 8.2，C43 改 8.3，C44 改 8.5，C46 改 8.6）；综合比较 9.6→9.7（C72、C74 改引，C51/C52 补 9.7.1／9.7.3）；跨地域 12.6→12.5（C71、C75）；并认领新增小节 5.2.4（C25）、5.6（C31）、6.7（C36）与 10.4.5 掉队者（C57，straggler_max 已并入该条）。剩余唯一有小节的空白是第 1 章 1.1 全景（4 块：1.1、1.1.2、1.1.3），是否需要计算由作者判断。另有 334 个未映射块来自案例笔记——没有任何工作包按文件名认领案例，这是工作包写法的问题而非案例未计算。**仍未完成**：1,326 块的 review_status 全部仍为 pending，本轮不设置任何一块；机械映射不是审查，有候选工作包不等于该块要求已实现。逐块审查、1.1 与案例笔记的认领决定、旧脚本迁移仍待。22 项独立tests通过。
...
- [ ] F04 全部结果可再生、原文到工作包覆盖审计、outline 与网页同步、读者入口。四项检查已实现为一条命令：`python3 calculations/calc.py delivery --format md`，见[交付检查](src/infra_calc/delivery.py)。四项各自独立报告、不合并成完成度数字，只读盘上状态、不重新生成任何东西。**当前 2 项通过、2 项未通过**。通过：结果可再生——1,171 个场景 id 与 2,388 份产物互相对上，没有场景缺产物、也没有产物无场景（`states`／`generation` 两组按行内字段构造名字，审计按 reproduce 的同一规则还原）；读者入口——73 个子命令全部在 README 中以可直接运行的形式列出，210 个专题模块全部可从命令行或其他模块到达，本轮据此补文档 7 条（fetch、verify-results、stage-resource-bounds、v4-compressor-online、plot-image-request、plot-supernode-cost、delivery），并确认无孤立模块。未通过：原文审查——覆盖清单是最新的、129 处结果引用全部解析成功，但 1,268 块的 review_status 仍全为 pending，机械映射不是审查，因此按契约判为未通过，只能由逐块审查关闭；正文与网页同步——转交本书自己的 `scripts/verify_outline.py` 并原样复述结论，当前 failed、71 条错误。根因是仓库既有状态：`scripts/render_outline.py` 要求每章都有 `## 本章的设计决定`，而第 5 章用的是 `## 本章小结与常见误区`（`git show HEAD:` 同样如此），脚本在该章抛 IndexError，skeleton.html 因而停在第 5 章改写之前。第 5 章补回该节或让脚本接受现用标题之后重跑 render 即可。15 项独立tests通过。四项全通过之前不勾选。
...
- [x] C04 TPU历史语音需求与SmartNIC核时／包率／PCIe约束（1.4.1、1.4.2 扩写）。证据：[TPU计算](src/infra_calc/topics/tpu_demand.py)、[NIC计算](src/infra_calc/topics/nic_budget.py)、11个复现场景、7项独立测试、官方论文原件锁与全局哈希、第1章扩写C04-tpu-demand/C04-nic-budget落点。历史锚点与线性/容量/卸载条件分开；完成原手算计算契约，不声称真实历史服务器库存、完整虚拟化、实测卸载收益或成本。
- [ ] C05 历史 UB 容量聚合、交接频次及当时条件下的选择翻转（1.4.3 扩写）。 [ub-scope](src/infra_calc/topics/ub_scope.py)现接10个现代Qwen教学场景、逐卡/消息图与精确带宽/容量边界，CLI及第6章扩写C05-modern-scope已接入；[历史缺口](research/c05-integration/acceptance-gaps.md)明确当年负载/设备/接口条件未取得，不能用现代示例勾选原历史要求。
...
- [x] C07 Qwen3-8B 逐层 Q/K/V/O、QK Norm、RoPE、QKᵀ、Softmax、PV、SwiGLU、残差、最终 RMSNorm、嵌入／输出头；8192 prefill、B=1/64 decode、6144+2048 前缀续算（2.2，实验 2-2）。证据：[实现](src/infra_calc/models/qwen3.py)、[场景](scenarios/book.json)、[结果](results/README.md)、[官方索引与守恒检查](tests/test_accounting.py)、[正文回填](../archive/outlines/02-模型架构.md)。完成的是声明的逻辑图与操作数载荷；tile、后端 HBM 与性能测量归 C25–C31。
...
- [ ] C15 成对长度与到达轨迹、同均值不同分布、prefill/decode 资源随形状变化（3.1.1、3.1.3；原编号 3.1.1–2，随第 3 章改写更新）。[成对轨迹重放](src/infra_calc/topics/request_trace.py)已覆盖 Qwen8/235 的逐请求矩阵、FIFO 等待／TTFT／p95、KV 存活与均值相同的相关性／到达对照；有限共享 KV 预算已加入按声明输出上限预留的 FIFO 准入，活跃／预留峰值分开；V4、batch／命中扫描、真实放置与服务校准仍待完成。
- [ ] C16 reasoning 预算／成功率／有效输出成本；Agent 多轮前缀、工具等待、状态驻留、视觉 E→P→D 与 EC（3.1.2、3.2.1–4、3.3.1；原编号 3.1.2–3，随第 3 章改写更新）。[真实 Agent 轨迹](src/infra_calc/topics/agent_trace.py)已导入实验 3-4 的两条哈希锁定原始记录，核对逐轮 token／命中、冷重算对照、实测关键路径替换与假设 KV 工具等待驻留，保留失败／截断和两层任务判据；多任务质量、分支工具、实际缓存生命周期及视觉仍待完成。
- [ ] C17 实时语音帧率、流水／响应时限、抖动与尾部预算（3.3.3；原编号 3.1.4，随第 3 章改写更新）。[逐块教学时序](src/infra_calc/topics/audio_timing.py)已覆盖显式模型／链路流水、乱序传播、固定截止／按序停顿、PCM ready queue 和设备量子静音响应；[当前证据复核](research/c17-status-review/review.json)另确认两份历史记录已分析首块接收与到达间隔，并核对软件播放位置的源码契约；这些不能证明设备首播或取消。仍缺固定记录的实际首播/打断、模型/网络/缓冲可比较证据及图3-4的实际时序；声学配置与真实取消／flush继续待办。
- [ ] C18 Dense 6ND 与注意力修正、MoE 实际矩阵、SFT token mask、RL rollout/reward/update 分账（3.4.1–3、3.5.1–2；原编号 3.2.1–2，随第 3 章改写更新）。[Dense 训练矩阵子账](src/infra_calc/topics/training_matrix.py)已覆盖 Qwen8/32/235 的前向／两个梯度、有效因果注意力、6ND 差额、显式输出头筛选与参数状态；Qwen MoE 已按各专家 token 数计量；非矩阵反向、重计算、[RL 批次子账](src/infra_calc/topics/rl_cycle.py)已连接 Qwen rollout／reference／teacher／更新及权重载荷，保持有效样本目标并计拒收开销；[条件式资源供给](src/infra_calc/topics/rl_supply.py)已支持阶段速率／验证服务槽／同步带宽导入、共享池累加和倍率扫描；V4/K3 训练、完整验证工作与实际阶段供给仍待完成。
- [x] C19 scaling law 受控数据拟合与预算最优、训练加生命周期服务成本、外推敏感性（3.6.1；原编号 3.2.3，随第 3 章改写更新）。 [scaling-law](src/infra_calc/topics/scaling_law.py)已接三组合成教学场景、解析/有限候选最优、独立留出、声明生命周期费用和指数敏感性，原13项加10项边界测试通过，另7项独立缺陷回归已修复；[real-scaling-fit](src/infra_calc/topics/real_scaling_fit.py)已接作者8个C4点、6fit/2预定holdout及四项敏感性；[real-scaling-lifecycle](src/infra_calc/topics/real_scaling_lifecycle.py)已接主law与敏感性的调用量交叉和外推边界，plot-real-scaling已生成真实点/生命周期双图。[逐项状态复核](research/c19-status-review/README.md)确认原工作包及实验3-8的有限计算要求已交付，39专项测试全部通过。完整论文复现、同任务质量映射、实际硬件价格与置信区间未提供，继续作为适用限制；不追加为本计算条目的完成门槛。旧教学生成段落的过时待办已从生成源修正，结果再生、图依赖更新、正文及网页同步验证通过；该编辑欠账已关闭。
- [x] C20 Llama/Qwen/DeepSeek公开训练投入复算（3.6.2；原编号 3.2.4，随第 3 章改写更新）。证据：[实现](src/infra_calc/topics/training_history.py)、[13模型/16归档历史表](results/training-history-published.md)、五场景、15项独立测试、[复核](research/c20-independent-review/REVIEW.md)、[验收记录](inventory/c20-review.json)、正文C20-training-history。总/激活参数、6ND代理、阶段小时/替代分支、reported阶段MFU和跨source条件日历分列；生命周期费用仅声明scope。完成公开字段与计算契约，原图计划、质量对齐实验及未披露的全程日志不借此标完成。
...
- [ ] C22 HBM/GDDR/统一内存容量与带宽、GEMM/Decode Roofline 及 batch 交叉点（4.3、4.5、4.8.1–2；4.8.1 从资源预算建立执行时间模型、4.8.2 用同一模型比较候选加速器对应 Roofline 与交叉点）。 已有25场景[逐阶段资源界](src/infra_calc/topics/stage_resource_bounds.py)，覆盖Qwen8/V4的阶段、batch和资源倍率；不能把空框解释为未实现。实验4-3已接[公共容量/带宽对照](results/storage-generation-qwen8-235.md)：官方Qwen8/235逐张量三格式、两上下文三batch及MoE路由，五产品与两种独立资源替换共54工作负载/810对照；6384独立闭式检查、3专项tests通过。[公共接入验收](research/storage-generation-integration/acceptance.json)已完成CLI、固定场景、结果再生与正文/网页同步（803测试781通过22跳过，1793产物/22图）；原要求的完整Roofline代际范围、图4-4/4-6及各小节覆盖仍须逐项验收，本条不勾选。
...
- [ ] C24 Mac/RTX 与公开芯片参数的任务／整机／能耗成本比较，面积／功耗／封装／互联资源扫描（4.5–6、4.8.3–4；4.8.3 预测与实测差距含实验 4-6 Mac/RTX 对照，4.8.4 容量、速度、功耗与成本选择）。 [公共配对投影费用/功率条件](results/paired-projection-unknown.md)复用实验4-6同夹具BF16原记录，11组16次批平均墙钟重算门槛，费用/功率缺失默认null，3场景136独立检查通过；[公共接入验收](research/paired-projection-integration/acceptance.json)已完成CLI、固定输入、六份JSON/Markdown结果及正文同步，4专项tests包含原始输入破坏拒绝，全套821项（799通过、22跳过）、1833产物/22图校验通过。仍缺：同期整机功率与实际费用未观测，不以TDP替代；生命周期、面积/封装/互联及剩余原文要求继续待办。

### 第 5 章：算子、融合与运行时

- [x] C25 GEMM 循环与 tile 枚举、128 MiB 下界、3096/1560 MiB 流量、24/80 KiB 缓冲，行归约与 partial buffer（5.1、5.2.4；5.2.4 沿哪个维度切分对应行归约 partial、bank 映射与 tile 枚举）。[GEMM tile 账](src/infra_calc/topics/gemm_tiles.py)已覆盖官方 Qwen 单支投影、正文全部容量／接口常数、output-stationary 与 K 外层 partial、双缓冲／尾块及容量筛选；[行归约](src/infra_calc/topics/row_reduction.py)已补三阶段 partial／inverse、重读、尾分片与舍入反例；[bank 枚举](src/infra_calc/topics/bank_mapping.py)已补32／33跨度、端口与同址广播，官方映射原件已锁定；[小循环访问](src/infra_calc/topics/loop_access.py)已逐数组表达式计量、尾块事件／数值回放，并匹配实验5-1的32组原始实测记录，哈希与离线验证通过。四类计算均有CLI、固定场景、结果、独立检查和第5章C25回填；完成本工作包的计量范围，真实GPU归约调度／完整缓存流量仍属于后端实测，不以CPU源码字节替代。
...
- [ ] C31 Amdahl 与依赖图的请求收益、共享资源竞争及瓶颈转移；校准记录导入（5.5、5.6；5.6 从局部优化到完整请求对应 Amdahl 热点占比与关键路径切换）。[请求DAG](src/infra_calc/topics/request_dag.py)已补确定性列表调度、依赖／串行资源边、关键路径转移、旧路径误推与串行等价Amdahl分列，四个教学情景、JSON输入及正文回填。[两微批争用账](src/infra_calc/topics/microbatch_overlap.py)已补官方FFN工作守恒／288与576MiB、1.20/1.36/1.16/1.28ms、联合窗口严格收益阈值和不等分变体，复用DAG保留填充排空。实际并发资源减速与实验5-9同引擎替换前后校准仍待补。
...
- [x] C36 池化容量、远端访问频次、故障域、复制与多副本成本选择（6.6、6.7；6.7 从并行方案到超节点规模含实验 6-10 超节点规模与系统成本）。按[原文逐要求验收](research/supernode-integration/scope-review.md)完成有限教学计算：[内存池](src/infra_calc/topics/memory_pool_access.py)覆盖4×64GiB容量、迁移/借用、1/2/3副本与45组故障依赖和访问频次；[增长KV](src/infra_calc/topics/growing_remote_kv.py)补Qwen8/Qwen3.6逐位置读取、初始化/追加复制与commit epoch；[实验6-10](src/infra_calc/topics/supernode_cohort_cost.py)接同8卡独立物理超节点分组、Qwen8/32真实容量、54个模型/并发/deadline/恢复条件、完整响应SLO与有效请求费用。图6-8明确取回为远端读取，图6-9及精确阶梯已生成。1701独立tick候选检查、11新增专项tests、108实际CLI及108冻结结果一致；[公共验收](research/supernode-integration/acceptance.json)：846项tests（824通过/22可选跳过）、1998产物/24注册图、正文网页通过。物理分组/服务/恢复/费率为声明教学输入；不据此声称实测性能、完整KV服务或真实状态恢复协议，9.5及其他原工作包继续保留。
...
- [ ] C46 TTFT/TPOT/E2E/SLO goodput、质量一致条件下设备与到达率扫描（8.6）。原编号 8.5，请求负载下的性能与配置选择现位于 8.6。[真实交付复算](src/infra_calc/topics/service_replay.py)已导入四条件72请求13固定文件，核对输入／源码／输出、逐请求联合计时阈值与三轮窗口加权goodput，四场景及独立事件统计／边界检查完成；真实任务质量、逐tokenSLO及设备／到达率扫描仍待补。

### 第 9 章：分布式服务

- [ ] C47 合置、PD、AF 基线资源图；阶段能力转请求/s、A100/H20 配比（9.1–2）。[PD池配比](src/infra_calc/topics/pd_pool.py)已完成有效阶段token/s转请求/s、异构整数分配、共置资源秒、网络上界及首输出／前缀／短输出／4K输出场景，回填9-2。2026-09-11 起worker可按设备给出：[stage_rates](src/infra_calc/topics/stage_rates.py)从hardware.json峰值×声明效率与Qwen3逐算子账推出两阶段Roofline时间，A100 80GB SXM＋H20 96GB（H20峰值取MegaScale-Infer表3）；效率50%以08-01批量64校准。完整放置、AF基线与时变SLO仍待完成。
...
- [ ] C51 缓存亲和与排队／新鲜度、事件丢失／索引恢复、PD+AF+池组合（9.5、9.7.1；9.7.1 组合副本、PD、AF 与共享 KV 为部署综合比较的组合小节）。[缓存路由](src/infra_calc/topics/cache_route.py)已复算官方KV、队列／取回依赖、远端等时带宽、失效两点分布及p99等号，五场景回填9-9；[原生路由回放](src/infra_calc/topics/router_trace.py)已导入9-9三策略36请求和实际worker日志，核对真实命中与官方矩阵节省，三场景回填9-9；[真实路由压力](src/infra_calc/topics/router_pressure.py)另导入9-9六组／18调用，配对核算目标节省与整组变慢，回填9-9；共享链路动态队列、预测规则、事件缺口与组合仍待完成。
- [ ] C52 TP/EP 重配置权重与状态迁移、摊销阈值、分流与恢复后的完整部署成本（9.6、9.7.3；9.7.3 负载变化后的部署调整对应重配置与分流）。[迁移子账](src/infra_calc/topics/reconfiguration.py)已接9个官方Qwen3配置场景、逐卡权重/KV所有权、显式同卡物化/网络载荷与双缓冲峰值、条件有限寿命摊销，18项聚焦验证；[独立审查与集成](research/c52-independent-review/INTEGRATED.md)列明静止快照/单播/PP=DP=1边界。真实排空、分流、恢复重试、请求身份验证与SLO有效产出尚缺，带宽下界不能替代切换时间，原项保持未完成。
...
- [ ] C57 故障率、checkpoint 周期 c/τ+λτ/2+λr、最优间隔和相关故障敏感性（10.4.2、10.4.4、10.4.5；原仅 10.4.2，故障规模与保存周期现位于 10.4.4，掉队者与慢节点为 10.4.5）。[周期与重试](src/infra_calc/topics/checkpoint_interval.py)已实现官方载荷的阻塞c、一阶分项／连续最优、独立设备与作业共同冲击、另列Poisson重试期望／数值最优，五场景含高率／零率／恢复敏感性；[掉队者与慢节点](src/infra_calc/topics/straggler_max.py)已接同步步时间取 N 卡最大值（正态最大值期望）、AllReduce 等待检测信号与等待／重分配／驱逐三响应计价，损失尖峰回滚作为额外作业率经 checkpoint_interval 并入同一 L(τ)=c/τ+(λ_hw+λ_spike)(τ/2+r) 模型；实际故障分布、一般相关故障、恢复再失败和异步策略仍待。
...
- [ ] C68 连接复用／握手/RTT/拥塞窗口、带宽时延积、小请求/大文件/帧截止时间（12.3）。原要求审计保留BDP与TCP/HTTP3局部实测；[有限ACK窗口](research/connection-window-integration/acceptance.json)、[连续请求](research/connection-sequence-integration/acceptance.json)、[握手/长0RTT](research/protocol-integration/acceptance.json)、[HRR/Retry/PSK](research/protocol-retry-integration/acceptance.json)、[共享媒体与图12-4教学面板](research/shared-media-integration/acceptance.json)均已公共接入。新增[上传—模型—响应闭环](src/infra_calc/topics/transport_closed_loop.py)公共CLI/10场景20产物，连接实际ACK、绝对流控、有限路由器队列、接收消费与新PN恢复；原书30MB/5MB、20/100Mbps、各50ms、模型0.3s得到14.36339008s、59932包、39554832线上bytes，明确布局/立即ACK和1e-12局部量化。[最新公共验收](research/transport-closed-loop-integration/acceptance.json)：10完整候选数学payload与20实际CLI/20冻结产物一致，8专项tests；全套904项（882通过/22可选跳过），2244产物28注册图，十二章正文/网页验证通过。[CUBIC首版](research/congestion-controllers/cubic-acceptance.json)为明确Reno启动纯状态，[BBR固定源码状态对照](research/congestion-controllers/BBR-STATE-HARNESS-REVIEW.md)197回调×38字段及6时间差边界通过，[HyStart++](research/hystart-plus-plus/REVIEW.md)18独立组/9非法输入通过；研究候选已按[共同网络接口合同](research/transport-controller-loop/INTERFACE-CONTRACT.md)接入真实反馈，并已迁入公共共享transport包和原CLI。[独立适配与反馈审查](research/transport-controller-loop/ADAPTER-FEEDBACK-REVIEW.md)13组adapter/发送重放及12组小型网络反馈通过；[动态pacer](research/transport-controller-loop/pacer-root-check.json)24项局部检查与[3个手算网络例](research/transport-controller-loop/network-pacer-root-check.json)17项检查通过。[持续拥塞纯证据](research/transport-controller-loop/persistent-acceptance.json)17主审/13独立例通过，新增[实际窗口动作](research/transport-controller-loop/persistent-sender-root-check.json)21项NewReno/CUBIC发送方检查，覆盖迟ACK与未消费loss声明去重、loss_timer证据等到真实ACK。[同网络控制器公共验收](research/transport-controller-loop-integration/acceptance.json)已完成6场景12产物、6完整公共/研究数学比较及12实际CLI逐字节一致；三大例统一59932包/39555120线上B，NewReno/CUBIC14.52103724s、BBR14.974125992s，CUBIC大例仍在slow_start。最终全suite909项（887通过/22可选跳过）、2256产物28图和十二章同步/网页校验通过。复算另修复stage资源报告的无序字段输出，并以25场景跨进程字节对照证明；同步器保留正文原始空行。ACK聚合已接入公共CLI与10场景20产物，完整公共/研究数学比对及917项测试（895通过/22可选跳过）通过，[最终公共验收](research/transport-ack-policy-integration/acceptance.json)包含20次实际CLI逐字节一致、2276产物28图及十二章网页检查；明确实际ACK发送时刻、ranges、raw/decoded delay与逐样本RTT，纯ACK身份lost不当作data丢失。[共享媒体真实反馈研究验收](research/media-feedback-loop/acceptance.json)已完成14作者小场景、12独立网络例、4公共单图退化逐包回归及完整单图/混合四格实际复算；5完整轨迹277331包独立守恒/ACK因果/内存/业务依赖通过。FIFO缺0.16s音频、priority全播且四格截图动作不可用，不能把审查通过解释为全业务成功。来源追踪补network_validation后五例重跑且完整字节不变，旧数据保留。[媒体公共最终验收](research/media-feedback-public-integration/acceptance.json)已完成19场景38产物、19完整数学比较、38实际CLI逐字节一致；全suite931项（909通过/22可选跳过），最终2314产物30图及十二章5136链接验证通过。图注修正后二次正式生成的全部2314结果字节不变，38CLI证据与最终manifest续核一致；图12-4新增成片/音频质量/真实发送与ACK面板，保留作者第3/8章同期编辑。剩余完整persistent场景覆盖、同媒体默认/调优TCP/H3匹配对照、TCP HRR/PSK及运行时Retry组合/票据/完整编码和完整图12-4；不以有限上传闭环或已有教学媒体图关闭整项。
- [ ] C69 ACK 空口占用、上下行争用、双路径分流/冗余及合并等待（12.4）。原要求审计确认已有[分包乱序恢复](src/infra_calc/topics/packet_reorder.py)及[双TCP音频取消/复制/共同故障](../experiments/ch12/12-06/README.md)真实记录；[共享空口研究候选](research/shared-airtime-loop/README.md)已分别计算传输ACK与MAC ACK，官方实现来源与精确OFDM服务经独立复核；当前内核37c44282下16小输入及14旧例两种关闭入口共386检查、9场景196个观察截止共980前缀检查通过。[五完整无线例及九格载荷/ACK扫描](research/shared-airtime-book-analysis/acceptance.json)已实跑，273438条完整包/无线尝试通过有限逐轨迹审查；19旧例关闭无线完整回归通过。公共CLI/图/章节接入仍待，不能以有限研究验收关闭此项。仍缺TCP ACK对应比较、共享空口/双路径下的完整双向媒体期限、RAW汇合瓶颈/蜂窝费用能耗、截图版本与重试到任务结果的连接及图12-5。应用bytes不冒充物理无线计费/耗电，保持未勾选。
- [x] C70 Queqiao 原始可用记录的同条件统计、流水阶段与时间戳误差；缺失帧分位数不得补造（12.5）。已交[记录再核算](src/infra_calc/topics/queqiao_records.py)与[结果](results/queqiao-records-conditions.md)：两份固定文档按SHA锁入[来源组](configs/queqiao-records.lock.json)，逐行转录只保留文档打印的数字。同条件比较按代次／负载／连接状态／内核设置四项全等才允许，8组比值独立复现文档自身的15.5×、8.8×、1.24×、1.3×与1.03×口径；配对比逐轮原值不在记录中，只携带不重算。算术下界＝往返＋拐点串行＋模型服务的区间，据此判定被撤回的225.8 ms在199–207 ms往返带内对任何载荷都不可行，仅记录最小往返185.9 ms才容许，并给出各行最大可行载荷。分段残差与0.1 ms分辨率界分离：direct新建连接各段中位数和1135.3 ms反超总计1133.5 ms，残差−1.8 ms为分辨率界0.15 ms的12倍，证明逐段中位数不可相加而非存在未测阶段；采集、编码、播放缓冲与播放在所有记录段之外。13组帧运行按提供帧数给出分位数样本支撑，1200帧的p999仅由1个样本决定并标为不支撑，9组因文档未打印p50而拒绝给出p99/p50。持续供给对播放消费的比较指出仅0.13 Mbit/s直连下行低于16 kHz单声道PCM的0.256 Mbit/s，60 ms预缓冲在121.9 ms后必然卡顿。慢启动5轮995 ms与实测948.2 ms相差−46.8 ms。23项独立tests通过。缺失分位数一律保持缺失。
- [x] C71 跨地域输入/输出/状态传输与计费、放置盈亏平衡和同质量任务成本（12.5）。原编号 12.6，跨地域部署的时限／成本／恢复比较现位于 12.5.1–2。已交[跨地域放置](src/infra_calc/topics/region_placement.py)与[结果](results/region-placement-agent-session.md)、[紧交付期变体](results/region-placement-tight-deadline.md)：同一固定Agent轨迹重放三种放置，模型revision、token序列与轮次相同，质量按构造固定而非按声明相等。请求与回复字节取轨迹自身消息体（无状态66,207 B、保温10,193 B、本地0），会话状态按官方K/V几何每token 147,456 B。硬约束先于价格：功率上限与交付期不满足的地域在比较之前排除，交付期扫描给出26周仅near、30周起far重新入选。盈亏平衡两条：保温可持有121.8 s，超过后避免的prefill不再抵偿驻留；出网价格边界84.17 /GB对声明价0.08 /GB留约1000倍余量，说明文本Agent的放置由算力价格而非出网费决定。复用扫描按模型适配器重算各命中比例的prefill工作，0命中时保温更贵、0.1起转为更便宜，实测命中0.8337远在保温一侧。19项独立tests通过。价格、功率上限与交付期为声明输入，不冒称任何运营商资费。

### 跨章综合计算：协同设计

- [x] C72 完整执行 DAG 资源下界／关键路径／面积／功率／容量，多目标候选筛选（1.3、9.7；配套综合练习）。原编号 9.6，部署方案综合比较现位于 9.7。已交[执行图与筛选](src/infra_calc/topics/execution_dag.py)与[结果](results/execution-dag-qwen8-h100.md)、[功率受限变体](results/execution-dag-qwen8-h100-power-capped.md)：节点取固定逐阶段账，38节点关键路径、基线下界6.707 ms，节点标运算与位置、边标接口字节，状态按产生／复用／更新／释放列出（权重16,381,470,720 B、KV 622,854,144 B、激活仅供下一阶段）。五个候选改写各自登记去掉什么与新增什么：缓存前缀降到4.720 ms、压缩状态把容量从17,004,324,864 B降到16,692,897,792 B、批量放大到22.018 ms但每token成本最低、分离把整份会话状态622,854,144 B推过接口后升到6.892 ms。多目标筛选先按容量／功率／面积上限排除再比较，分离被baseline与压缩状态双重支配，构成“多加一台设备什么都没换来”的反例；官方未公布这些部件裸片面积，面积轴仅在声明占地时参与。未公布速率的special资源逐项列为缺口，不并入下界。20项独立tests通过。
- [x] C73 OpenTallas 权重驻留后的激活扇出/扇入、总 hop、同步跨度、供给倒推及遗漏约束（4.7、6.5.1）。原编号 4.6.5，第 4 章插入"模型需求如何推动架构演进"后专用架构移至 4.7，OpenTallas 固定权重案例在 4.7.3。已交[驻留后剩余流量](src/infra_calc/topics/weight_resident_traffic.py)与[结果](results/weight-resident-qwen8-8k.md)、[长上下文](results/weight-resident-qwen8-32k.md)、[宽网格](results/weight-resident-qwen8-wide-mesh.md)：全部由固定官方配置重建并与案例自身数字逐项吻合——checkpoint 16,381,470,720 B、活跃decode读取15,136,811,008 B（差额恰为decode不流式读取的embedding查找表）、KV每token 147,456 B、8,192上下文每步1,207,959,552 B、交叉点12.53（整数13起）、B=1加速比13.53、B=16为1.78、32,768上下文交叉点3.13；供给倒推得12.08 TB/s、11个stack、401 mm²、理想75,674 lane与降额315,306 lane。扇出／扇入按列切分与行切分分别计量并给出网格边长、直径跳、生成树跳与每token总跳；同步跨度按真实36层×2次给出每次预算，并解出预算允许的最大直径为单次4跳、整token 13跳，14跳起仅传播就超过整个token预算，案例的15跳网格落在失败侧。七条遗漏约束逐条列明。23项独立tests通过。本设计没有流片，无一项作为实测速率。
- [x] C74 两侧独立量化／副本／并行选择，负载／面积／quality 固定下的反例和翻转条件（9.7.2）。原编号 9.6.2，同质量、同资源约束下的方案比较现位于 9.7.2。已交[同上限两侧比较](src/infra_calc/topics/iso_resource_comparison.py)与[结果](results/iso-resource-h100-a100.md)、[给定FP8质量证据变体](results/iso-resource-h100-a100-fp8-evidence.md)：先按两套固定配置定价，再在同一5,600 W上限下让两侧各自选位宽、TP与副本，上限不要求所有设备组成同一同步域。A100官方未公布dense FP8张量速率，四个FP8候选按“未公布速率”拒绝而非按上一位宽推定，这正是“两侧量化选择并不对称”的证据；H100的FP8候选在未声明质量证据时列出但不参与同质量排名。逐目标结果：H100以BF16/tp8取最低延迟（1.276×），A100以BF16/tp1取SLO内最高吞吐与最低每token成本（1.217×）。翻转条件两条——把A100声明价乘以超过1.217倍，H100成为每token更便宜的一侧；仅把FP8的质量证据从无改为有，硬件、价格与字节数不变，SLO内最高吞吐由A100翻转为H100，价格比同时收窄到1.046。20项独立tests通过。
- [x] C75 NRE/交付/更新风险、盈亏平衡用量、整站电力/冷却/网络/利用率成本（4.8.4、11.4.1、12.5.2）。原编号 4.6.5、12.6.1，第 4 章插入 4.6 后回本算例现位于 4.8.4，跨地域成本比较现位于 12.5.2。已交[专用化回本](src/infra_calc/topics/specialization_payback.py)与[结果](results/specialization-payback-baseline.md)、[254台变体](results/specialization-payback-fleet-254.md)：回本量4×10^13 token、一年有效产出1.5768×10^13 token、需要254台，与案例自身数字一致。三类风险各自改变结论——经济寿命减半需508台，交付滑26周需506台（交付期从寿命里扣除而非往后加，滑到60周则整段寿命无服务期），有效速率降到6,000 tok/s需423台。整站费用按设施计量：散热系数只乘电费，不乘网络与备份；标称情形站点合计1,311,040、折合每百万token 0.0831，占声称0.50节省的16.6%，净节省0.4169仍不足以回本；有效速率下降时同一设施摊到更少token，站点成本升到0.1386。20项独立tests通过。全部金额为教学假设，机器数只是产出折算，仍需真实需求消化。
- [x] C76 留出配置、输入不确定区间、最有价值的补测与可证伪预测，跨章最终设计记录（8.1.4、11.3.2；阅读配套与设计记录）。已交[设计记录与不确定性](src/infra_calc/topics/design_record.py)与[结果](results/design-record-uncertainty.md)：对C75回本与C72图筛选两项决定，逐个输入在其声明区间内单独移动，其余保持标称。两项决定的任何单一输入区间都不足以改变结论，因此“最小补测”明确写成买到信心而非买到另一种选择；翻转以二分区间给出而非精确根，其中机器数区间[253,254]独立重新发现案例的254台答案，节省需超过约1.268/百万token、经济寿命需超过30个月，利用率与电价在64倍搜索范围内单独都无法翻转。逐输入标注证据类型，对同一模型的复算不充当该模型的独立测量，因此“观察”一栏明确为空、修改后的决定与初次预测一致。留出配置给出可证伪区间：life_halves与output_rate_falls各自的覆盖率区间不同，图筛选的disaggregate预测只会是被支配或被上限排除、从不在前沿，任一落在区间外的观测即证伪。八项记录字段全部填写。21项独立tests通过。
