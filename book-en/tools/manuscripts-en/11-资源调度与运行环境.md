# Chapter 11 Resource Scheduling and Execution Environments
...
![Three-round timeline of model calls, tool work, and environment residency](ch11/figure-11-1-timeline.svg)
*Figure 11-1: The three sequential rounds take 30 s total. Blue shows the three model calls of 9 s each, orange the three tool executions of 1 s each; the tool consumes 3 CPU-seconds cumulatively.*

![The same task's environment occupies 2 GiB throughout, for 30 s, giving a rectangle area of 60 GiB-seconds](ch11/figure-11-memory-area.svg)
*Figure 11-2: The same task's environment occupies 2 GiB throughout, for 30 s, giving a rectangle area of 60 GiB-seconds. The environment is retained even while waiting for the model, so memory occupancy lasts longer than the tool's CPU execution time.*
...
![Different impacts of slower model response on the three resource requirements](ch11/figure-11-capacity.svg)
*Figure 11-3: At 10 tasks/s with a 9 s model call, the proportion of each resource's demand to its capacity. Bar-end labels give demand/available capacity; the dashed line marks 100%.*

![Extending each model call from 9 s to 12 s leaves CPU demand at 30 cores; concurrent calls rise to 360, environment memory rises to 780 GiB](ch11/figure-11-capacity-slow.svg)
*Figure 11-4: Extending each model call from 9 s to 12 s leaves CPU demand at 30 cores; concurrent calls rise to 360, environment memory rises to 780 GiB. The dashed line marks 100% capacity.*
...
![Processes have independent address spaces, with access restricted by permissions; processes on the same node share the host kernel](ch11/figure-11-isolation-0.svg)
*Figure 11-6: Processes have independent address spaces, with access restricted by permissions; processes on the same node share the host kernel.*

![Containers establish separate resource views and quotas for tasks, while still using the same underlying host kernel](ch11/figure-11-isolation-1.svg)
*Figure 11-7: Containers establish separate resource views and quotas for tasks, while still using the same underlying host kernel.*

![microVMs let each task environment run an independent virtual-machine kernel, accessing host resources through virtual hardware](ch11/figure-11-isolation-2.svg)
*Figure 11-8: microVMs let each task environment run an independent virtual-machine kernel, accessing host resources through virtual hardware.*
...
![The template contains shared files and dependencies; the private portion records each environment's modifications; running programs also occupy process memory and buffers](ch11/figure-11-template-runtime.svg)
*Figure 11-9: The template contains shared files and dependencies; the private portion records each environment's modifications; running programs also occupy process memory and buffers. The space needed to load the template and the memory occupied by the active environment are computed separately.*
...
![Local data volume for different environment content loading approaches](ch11/figure-11-pages.svg)
*Figure 11-10: The four schemes use the same horizontal-axis scale, comparing the data saved locally by each environment. There is also one shared 2 GiB template, not counted in any environment's bar length. Values are from Example 11-2; management overhead is 4 MiB per environment.*
...
![Environment residency versus pause-and-resume during the same model-wait interval](ch11/figure-11-pause.svg)
*Figure 11-11: Retaining a 2 GiB environment throughout the 10–19 s wait for the model occupies 18 GiB-seconds. The horizontal axis matches the next figure.*

![Saving 2 GiB of state from 10–18 s and resuming from 18–19 s leaves memory unreleased for the entire interval](ch11/figure-11-pause-release.svg)
*Figure 11-12: Saving 2 GiB of state at 4 s per GiB from 10–18 s, then resuming from 18–19 s, leaves memory unreleased for the entire interval. The colored interval still occupies 18 GiB-seconds, and the next tool execution starts at second 19.*
...
![Memory footprint over time for always-resident environments versus per-round rebuilding](ch11/figure-11-residency.svg)
*Figure 11-13: The environment is retained from the start of the task to its end. The model takes 9 s per round and the tool takes 1 s per round, for a cumulative footprint of 60 GiB·seconds across three rounds. Uses the same horizontal axis as the next figure.*

![Each round retains the environment only during the 2 s preparation and 1 s execution, totaling 18 GiB·seconds across three rounds](ch11/figure-11-residency-rebuild.svg)
*Figure 11-14: Each round retains the environment only during the 2 s preparation and 1 s execution, totaling 18 GiB·seconds across three rounds. The working directory and results have already been persisted, and the task still finishes at 30 s across three rounds.*
...
![Relationship between preparation lead time and call waiting](ch11/figure-11-3-lifecycle.svg)
*Figure 11-15: On-demand creation: preparation begins only when the call arrives at second 4, becomes ready at second 6, for a wait of 2 s. Blue denotes preparation, the dashed line the call moment; the following four figures use the same horizontal axis.*

![Correct prediction with 1 s lead time: preparation runs from second 3 to second 5, and the call arriving at second 4 still waits 1 s](ch11/figure-11-prewarm-1.svg)
*Figure 11-16: Correct prediction with 1 s lead time: preparation runs from second 3 to second 5, and the call arriving at second 4 still waits 1 s. Blue denotes environment preparation, the dashed line marks the call's arrival at second 4.*

![Correct prediction with 2 s lead time: preparation finishes exactly when the call arrives at second 4](ch11/figure-11-prewarm-2.svg)
*Figure 11-17: Correct prediction with 2 s lead time: preparation finishes exactly when the call arrives at second 4. Blue denotes environment preparation, the dashed line marks the call's arrival at second 4.*

![Correct prediction with 3 s lead time: preparation finishes at second 3, and the orange segment represents the 1 s of idle time during which the environment sits ready waiting for the call](ch11/figure-11-prewarm-3.svg)
*Figure 11-18: Correct prediction with 3 s lead time: preparation finishes at second 3, and the orange segment represents the 1 s of idle time during which the environment sits ready waiting for the call. Blue denotes environment preparation, the dashed line marks the call's arrival at second 4.*

![Wrong prediction: an unneeded environment (gray) is prepared during seconds 2–4; after cancellation at second 4, another 2 s is spent preparing the actually needed environment](ch11/figure-11-prewarm-wrong.svg)
*Figure 11-19: Wrong prediction: an unneeded environment (gray) is prepared during seconds 2–4; after cancellation at second 4, another 2 s is spent preparing the actually needed environment. This figure assumes immediate cancellation with no resource contention.*
...
![Node configurations before and after resource fragmentation and migration reorganization](ch11/figure-11-4-placement.svg)
*Figure 11-20: Before reorganization: the new job needs four H100s and 16 cores on the same node. Node 1 lacks CPU cores, and the idle capacity on Node 2 consists of A100s.*

![After migrating the 8-core CPU task from Node 1 to Node 2, Node 1 simultaneously has four idle H100s and 16 idle cores, allowing the new job to start](ch11/figure-11-placement-after.svg)
*Figure 11-21: After migrating the 8-core CPU task from Node 1 to Node 2, Node 1 simultaneously has four idle H100s and 16 idle cores, allowing the new job to start.*
...
![Queueing process under uniform arrival versus simultaneous arrival](ch11/figure-11-queue.svg)
*Figure 11-22: Ten calls arrive 1 s apart; a single execution process handles each for 1 s. Dots mark arrivals, blue marks execution, and each call can start immediately.*

![Ten calls all arrive at second 0; gray denotes waiting, blue denotes execution, and the waiting times across the rows run from 0–9 s](ch11/figure-11-queue-burst.svg)
*Figure 11-23: Ten calls all arrive at second 0; gray denotes waiting, blue denotes execution, and the waiting times across the rows run from 0–9 s. The execution process still works only 10 s in total, with an average queueing time of 4.5 s.*
...
![Other stages left unshortened even after doubling generation speed](ch11/figure-11-rl-stages.svg)
*Figure 11-24: Four stages execute sequentially. Generation time drops from 40 seconds to 20 seconds, while the other three stages still take 45 seconds combined, so the total time per round drops from 85 seconds to 65 seconds. Bar lengths are drawn proportional to time.*
...
![Verification timeline under per-sample submission versus batch submission](ch11/figure-11-5-stages.svg)
*Figure 11-26: Three samples arrive at seconds 0, 10, and 20 respectively and are submitted immediately to the sole verification process, each taking 10 s to process; the whole batch finishes at second 30. Dots mark arrival moments.*

![All three samples wait until they are ready together at second 20, then verify in sequence, finishing at second 50](ch11/figure-11-stages-batch.svg)
*Figure 11-27: All three samples wait until they are ready together at second 20, then verify in sequence, finishing at second 50. The same 30 s of verification work executes as in the previous figure; the difference comes from submission timing.*
...
![Nine verifications each take 1 s, and one takes 100 s](ch11/figure-11-remaining-time.svg)
*Figure 11-28: Nine verifications each take 1 s, and one takes 100 s. The dashed line marks the 10 s already elapsed; any sample still unfinished at this point can only belong to the 100 s class, leaving 90 s remaining.*
...
![Model service endpoint, backend selection, and billing location](ch11/figure-11-6-service.svg)
...
![Intersection of the cost curve and the on-time pass requirement](ch11/figure-11-7-routing.svg)
*Figure 11-31: B's successful-task cost decreases as hit rate increases, equaling A's at about 79.5%. Both totals are each divided by their own number of successfully completed tasks first; this figure compares cost alone, the next figure adds the deadline separately.*

![Only B's 4 s hit path satisfies the 6 s deadline; multiplying by the 98% acceptance pass rate gives an on-time success fraction of 0.98h](ch11/figure-11-routing-deadline.svg)
*Figure 11-32: Only B's 4 s hit path satisfies the 6 s deadline; multiplying by the 98% acceptance pass rate gives an on-time success fraction of 0.98h. Reaching the 90% target requires $h$ to be at least about 91.8%.*
...
![The external system has already executed and committed operation K, but the confirmation was lost](ch11/figure-11-commit-ack.svg)
*Figure 11-34: The external system has already executed and committed operation K, but the confirmation was lost. Recovering the environment does not roll back the result of the external operation; the controller queries the result by the same operation identifier and then resumes the task. Solid lines represent requests and execution, dashed lines represent confirmation and recovery queries.*
...
![Probabilities and cumulative completion times of retry branches](ch11/figure-11-retry-tree.svg)
*Figure 11-36: The first attempt, after 10 s, splits into three outcomes: success, local repair, and direct escalation. The proportions in the boxes use the total submissions as the denominator; the next figure expands the conditional branches of repair.*

![Zooming in on the repair node: of the 12% entering here, 60% repair successfully, accounting for 7.2% of total submissions; 40% move on to escalation, accounting for 4.8% of total submissions](ch11/figure-11-retry-conditional.svg)
*Figure 11-37: Zooming in on the repair node: of the 12% entering here, 60% repair successfully, accounting for 7.2% of total submissions; 40% move on to escalation, accounting for 4.8% of total submissions. Escalation takes another 8 s, so this latter path totals 22 s.*
...
![After model acceleration, environment creation becomes the source of waiting before the tool starts executing. Both cases begin creating the environment at time zero; the tool must wait on both the model's decision and the environment's readiness. The horizontal axis is in seconds.](ch11/figure-11-environment-overlap.svg)
*Figure 11-39: After model acceleration, environment creation becomes the source of waiting before the tool starts executing. Both cases begin creating the environment at time zero; the tool must wait on both the model's decision and the environment's readiness. The horizontal axis is in seconds.*
...
![Completion times for three rounds under ordinary and fast models](ch11/figure-11-decision.svg)
...
The following exercises continue the numbering of materials 11-1 through 11-10. Core exercises 11-1, 11-2, and 11-10 form a complete progression from requirements to design; raw experiment records and optional run entry points are in [this chapter's companion materials](ch11/README.md).
...
[^environment]: This book's [fixed environment-resources calculation](../calculations/results/environment-resources-book.md) and [tool subprocess records](../experiments/ch11/11-01/process-resources/README.md). The tool experiments comprise nine groups and 36 processes; cumulative memory footprint is obtained by integrating sampled RSS over time.

[^lifecycle]: The [environment-lifecycle fixed budget](../calculations/results/environment-lifecycle-default.md) distinguishes shared/private capacity, the lower bound on serial-transfer time, and measured local warmup. This chapter uses a fixed capacity budget to compare four content-loading approaches.

[^platform]: [Study of scheduling, model routing, and cloud environments](../case-studies/platform-routing.md), covering ASI, RLBoost, DistRS, and SpecBox. The ASI study covers six months and 155,410 GPUs, with its allocation ratio measuring device ownership. Token unit prices are taken from the [Claude API pricing page snapshot](../references/outline-checks/2026-09-07/platform-routing/claude-api-pricing.md).

[^e2b]: [E2B fixed architecture](../references/outline-checks/2026-09-07/platform-routing/e2b-architecture.md), [persistence documentation](../references/outline-checks/2026-09-07/platform-routing/e2b-persistence.md) (pausing takes about 4 seconds per GiB of memory, resuming about 1 second, and pausing that saves only the filesystem), [checking of pause/snapshot interface behavior](../experiments/ch11/11-02/README.md). E2B uses Firecracker.

[^ub]: Device virtualization and delivery paths in the [UB Operating System Reference Design](../references/files/specs/UB-Software-Reference-Design-for-OS-2.0-zh.pdf), used as an implementation illustration for QEMU/VFIO; this describes a VM configuration supporting device passthrough.

[^prewarm]: [Local tool warmup replay](../experiments/ch11/11-06/README.md). Actual calls, predicted hits, and sampling-window boundaries are preserved in the original records.

[^scheduling]: [DRF](../references/files/papers/drf.pdf), [Gavel](../references/files/papers/gavel.pdf), and [Pollux](../references/files/papers/pollux.pdf) as background on scheduling mechanisms; [Resource sharing and placement](../case-studies/resource-sharing-and-placement.md) supplements the distinction between resource availability and state handoff.

[^rollout]: [Survey of weight preparation and effective output](../case-studies/preemptible-rollout-and-weight-readiness.md), recording the RLBoost paper's conditions and PolyRL's fixed source path. Qwen3-8B's weight byte count is taken from the [safetensors index](../calculations/sources/qwen3-8b/model.safetensors.index.json); the 200/50 Gbit/s frontend NIC and instance prices are taken from the RLBoost paper.

[^recovery]: [Qwen3-8B preemption recovery experiment](../experiments/ch11/11-04/README.md), separately recording the token counts used for generation, client receipt, and recovery.

[^reward]: [DistRS verification scheduling and verl implementation analysis](../case-studies/reward-deadlines-and-feedback.md).

[^validation]: [Timeout and resource-release experiment](../experiments/ch11/11-05/README.md), [dual-batch scheduling records](../experiments/ch11/11-05/batch-scheduling/README.md). The experiments use controlled CPU processes.

[^thinking]: [Thinking-budget and quality records](../experiments/ch11/11-08/README.md), [checks after increasing the thinking budget](../experiments/ch11/11-08/wide-budget/README.md). The records separately check whether generation ends naturally and whether the input state meets requirements.

[^routing]: [routing-cost fixed results](../calculations/results/routing-cost-book.md), [crossover-point results](../calculations/results/routing-cost-crossover.md), and the [full derivation](../case-studies/routing-cost-and-completion.md). Unit prices for Haiku 4.5 and Sonnet 5 are taken from the [Claude API pricing page snapshot](../references/outline-checks/2026-09-07/platform-routing/claude-api-pricing.md); the calculation uses an acceptance-pass probability independent of cache hits, with the successful-task count computed from the given probability.

[^purchase]: The B200 Pod price of 6.79 dollars per card-hour and Serverless price of 8.64 dollars per card-hour are taken from the [Runpod pricing page snapshot](../experiments/ch13/13-06/single-agent-serving/comparison-sources/gpu-prices.md). The computed result is 25.317 ms, of which decode is 15.918 ms and the remaining stages together are 9.399 ms. Here, per-layer fixed overhead and full periodic rebuilding are treated as given inputs; see the [continuous agent serving comparison baseline](../experiments/ch13/13-06/single-agent-serving/COMPARISON-RESULTS.md), [optimization audit](../experiments/ch13/13-06/single-agent-serving/OPTIMIZATION-AUDIT.md), and [time attribution and sensitivity](../experiments/ch13/13-06/single-agent-serving/AUDIT-RESULTS.md) for details.

[^records]: [Task records and scope of evidence](../case-studies/evaluation-and-agent-records.md), [file-queue fault injection](../experiments/ch11/11-09/collector-faults/README.md), [task exit-status records](../experiments/ch11/11-09/exit-status/README.md). These records are used respectively to illustrate result persistence, fault diagnosis, and cost accounting.
...
[^retry]: Exact results: success probability 0.99744, probability of passing tests and completing on time 0.9504, expected cost per task 0.01456 dollars, cumulative CPU time 3.376 seconds, residency 23.008 GiB·seconds. The computed results list resource usage and cost separately; the given cost at each node is already a total price and should not be billed again by resource usage. See the [retry-paths finite conditional graph](../calculations/results/retry-paths-book.md). Costs at each node are computed from Claude's standard pricing and the token counts given in the problem; probabilities are as given in the problem, and retries unfold along a finite execution tree; tasks exceeding the evaluation deadline still continue executing to completion.

[^design]: Inputs for the running platform and Example 11-9, the stage-by-stage timetable, cost, the condition for equal cost, and the recalculation appear in [running design data](ch11/platform-design.json) and [verification program](ch11/verify.py). Model invocation times come from this book's estimate for running DeepSeek V4-Flash on 4 B200s with a 200K context per session: 25.3 ms per output token at 32 sessions per replica, 16.9 ms at 16 sessions, both including amortized input processing and context reconstruction ([comparison results](../experiments/ch13/13-06/single-agent-serving/COMPARISON-RESULTS.md), [full data](../experiments/ch13/13-06/single-agent-serving/comparison.json)); a 355-token call takes about 8.99 s and 6.01 s respectively, and the design rounds these to 9 s and 6 s. The B200 cost of $6.79 per card-hour comes from a [Runpod pricing page snapshot](../experiments/ch13/13-06/single-agent-serving/comparison-sources/gpu-prices.md). The m5d.metal's 48 cores (hyperthreading disabled) and 384 GB memory come from the evaluation environment in section 5 of the [Firecracker paper](../references/text/firecracker.txt); the 1 vCPU, 2 GiB specification and the CPU and memory unit prices come from an [E2B pricing page snapshot](../references/files/documents/e2b-pricing.md). The 2 seconds and 0.1 CPU-seconds of hot-path preparation and the 95% acceptance pass rate are given assumptions; result saving and working-state cleanup are included within the tool's 1 second. The template capacity example separately covers content loading.

[^nodes]: [DGX H100 system specification](../references/text/dgx-superpod-h100-ra.txt) (dual Xeon Platinum 8480C, 112 cores total); [DGX A100 system specification](../references/text/nvidia-a100.txt) (Appendix A, Table 10, dual EPYC 7742, 128 cores total).

[^firecracker-history]: Agache et al., AWS, [Firecracker: Lightweight Virtualization for Serverless Applications](../references/text/firecracker.txt), NSDI 2020.

[^core-calculation]: [Itemized recalculation](../calculations/results/core-principles.json) and [calculation program](../calculations/core_principles.py) for the conditional comparisons in this chapter.

[^v41-case]: [DeepSeek V4.1 official technical report](../calculations/sources/deepseek-v4.1-flash/DeepSeek_V41_Tech_Report.pdf), sections 1, 2, 3, and 6; [fixed conditions and recalculation for the cross-chapter running session](../calculations/results/v41-throughline.json).
