...
FIXTURE=json.loads((R/'network-fixture.json').read_text());STEPS=FIXTURE['steps']
for f in STEPS:
 f['payload']=(R/f['upload_file']).read_bytes();f['response']=(R/f['response_file']).read_bytes()
 assert hashlib.sha256(f['payload']).hexdigest()==f['upload_sha256'] and hashlib.sha256(f['response']).hexdigest()==f['response_sha256']
...
async def main(out,smoke):
 ...
 env=dict(python=platform.python_version(),platform=platform.platform(),openssl=ssl.OPENSSL_VERSION,packages={p:importlib.metadata.version(p) for p in ['aioquic','h11','cryptography','pylsqpack']},certificate_sha256=fingerprint,source_sha256=hashlib.sha256(Path(__file__).read_bytes()).hexdigest(),fixture=json.loads((R/'network-fixture.json').read_text()),smoke=smoke,body_request_deadline_s=REQUEST_TIMEOUT)
