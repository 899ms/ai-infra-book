...
P=Path(__file__).resolve().parent;O=P/'validation'
async def main():
 ...
 (O/'protocol.json').write_text(json.dumps(dict(runid=runid,base_image=base,tasks=tasks,order=order,execution_deadline_s=2,slots=2,platform=platform.platform(),source_sha256={n:hashlib.sha256((P/n).read_bytes()).hexdigest() for n in ['schedule.py','validate_worker.py']}),indent=2)+'\n')
 try:
  for case in order:
   ...
   async def launch(job):
    ...
    cid=(await command('docker','create','--label','book.experiment='+runid,'--network','none','--cpus','1','--memory','128m','--memory-swap','128m','--pids-limit','32','--cap-drop','ALL','--security-opt','no-new-privileges','--read-only','--user','65534:65534',base,'python','-u','/tmp/worker.py'))['stdout'].strip();owned.add(cid)
