<!-- 从 megatron-context-parallel.html 迁移的资料快照；原始 HTML SHA-256: 6ba5e5cf445a4571645250cf378d6567e05780cbf6951b9e5727ecdf7af10d06。 -->
...
<a id="context-parallel-package"></a>

# Context Parallel Package
<a id="context-parallelism-overview"></a>

## Context Parallelism Overview
![Diagram of a transformer layer with tensor parallelism 2 and context parallelism 2, showing CP and TP communication patterns around attention and other blocks.](../../_images/CP_overview.png)

Figure 1: A transformer layer running with TP2CP2. Communications next to Attention are for CP, others are for TP. (AG/RS: all-gather in forward and reduce-scatter in backward, RS/AG: reduce-scatter in forward and all-gather in backward, /AG: no-op in forward and all-gather in backward).[\#](#id1 "Link to this image")
...
<a id="context-parallelism-benefits"></a>

## Context Parallelism Benefits
![Chart of speedup for 175B GPT with different tensor parallelism and context parallelism combinations compared with full activation recomputation.](../../_images/CP_results.png)

Figure 2: Speedup of 175B GPT with various TP+CP combinations compared to full recomputation (that is, TP8CP1).[\#](#id2 "Link to this image")
...
<a id="enabling-context-parallelism"></a>
...
[](../../api-guide/router_replay.html "previous page")

previous

Design Document: MoE Router Replay Feature

[](megatron_fsdp.html "next page")
...
- [Context Parallelism Overview](#context-parallelism-overview)
- [Context Parallelism Benefits](#context-parallelism-benefits)
- [Enabling Context Parallelism](#enabling-context-parallelism)
