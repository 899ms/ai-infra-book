...
![Figure 8-1: A continues execution across two iterations. B finishes in the first round, and in the next round C takes over its execution slot (each round reserves one row of computation for each active request); request state is still stored by its own identity.](images/figure-8-request-iterations.pdf)
...
![Figure 8-4: HBM read volume amortized per output token. With a fixed 8K context, the conventional path is W/B+K and the independent fast-ROM path is K; the vertical axis is computed by the formula above. W is the number of weight bytes read in the whole batch, B is the number of requests in the batch, and K is the number of KV bytes read per request in this step; W/B+K is the read volume amortized per output token.](images/figure-8-batch-counterfactual.pdf)
...
![Figure 8-7: Splitting a long input into chunks, letting existing requests generate first each round before processing one chunk of new input. Each row is one request; blue is input processing, green is output generation, and triangles mark arrival times. Chunking reduces the longest output interval to about 133 ms, with all requests finishing in about 844 ms. The horizontal axis uses the same time scale as Figures 8-5 and 8-6.](images/figure-8-scheduling-chunked.pdf)
...
![Figure 8-8: When processing 4 tokens for the first time, there is no old context — only causal attention pairs among the new tokens, forming 1 + 2 + 3 + 4 = 10 green cells. White cells represent future tokens, which do not participate in the current query.](images/figure-8-4-attention.pdf)

![Figure 8-9: With an existing context of 8 tokens, the 4 new tokens form 4 × 8 = 32 blue cells against the old context, while the within-block portion remains 10 green cells. With the same block length, the total pair count rises from 10 to 42.](images/figure-8-attention-history.pdf)
...
![Figure 8-10: Logical blocks 0, 1, 2 form the sequence in order; the block table points them to physical blocks 2, 0, 3 respectively. Physical block 1 is free. Attention restores logical order via the block table.](images/figure-8-page-map.pdf)
...
![Figure 8-11: Four requests contain 9, 13, 5, and 15 tokens respectively; each reserves KV space for 16 tokens, for a total allocation of KV space for 64 tokens. Blue is used; gray is reserved but unused.](images/figure-8-5-pages.pdf)

![Figure 8-12: Under KV allocation with 4 tokens per block, the four requests obtain capacity for 12, 16, 8, and 16 tokens respectively, totaling 52. Gray denotes the unused capacity at block tails, reduced from 22 tokens to 10 tokens.](images/figure-8-pages-paged.pdf)
...
![Figure 8-13: A's and B's two shared prefix blocks are stored only once, with each block table pointing to them. A and B keep their own private tail blocks; the actual allocated KV capacity across the four requests drops further, to 44 tokens.](images/figure-8-pages-shared.pdf)
...
![Figure 8-14: After A ends, the reference count drops from 2 to 1, and B can still use the block; only after the last reference is released and the accelerator has finished using it can the block return to the free pool.](images/figure-8-reference-release.pdf)
...
![Figure 8-15: A tail block holding 4 tokens per block already contains the shared a, b, c. The branches append x and y respectively, requiring different physical tail blocks; the already-full block continues to be shared.](images/figure-8-copy-on-write.pdf)
...
![Figure 8-16: The cancel interface returning means the cancellation request has been received. Only after already-submitted accelerator operations complete are private blocks and their references released; the observed 1.6 ms and 31 ms correspond to different events.](images/figure-8-cancel-lifetime.pdf)
...
![Figure 8-17: Compressed prefix tree of the first four rounds of input from a code agent. The root already has 206 common tokens; edges show the number added; leaves are the input round. A branch indicates that subsequent content differs.](images/figure-8-6-prefix.pdf)

![Figure 8-18: Blue is the prefix identical token-for-token to the previous round; orange is the rest of the input. This figure describes how reusable the input content is; actual cache hits still depend on whether the state has been retained.](images/figure-8-prefix-lengths.pdf)
...
![Figure 8-19: The text matches up to 10752, with the most recent state snapshot at 8192. After restoring, 2560 tokens must still be recomputed to reach the recurrent state at the matched endpoint. In the figure, 10752 and 8192 are token counts accumulated from the start of the sequence.](images/figure-8-prefix-restore.pdf)
...
![Figure 8-20: Three ways of updating the same history. Blue denotes the reusable prefix, orange denotes input that must be reprocessed; the summary scheme first generates a summary, then rebuilds the cache. Lengths are illustrated in K tokens.](images/figure-8-context-edits.pdf)
...
![Figure 8-21: The encoder's three prefix-restoration paths and the shared decoder replay. "SWA" in the cache-hit states refers specifically to the encoder; all paths still build the decoder SWA during prefill before generation begins. Arrows indicate execution order; box size does not represent duration.](images/figure-8-v41-recovery.pdf)
...
![Figure 8-23: Space required by BF16, q8_0, and q4_0 to store the same group of 32 values. Bars are drawn proportional to byte count; orange represents the 2-byte scale per group. Multiplying the total length per group by the number of groups gives the KV capacity for an entire context.](images/figure-8-8-kv-format.pdf)
...
![Figure 8-24: Offloading nine FFN sets frees up 2592 MiB total. Orange is the prefetch buffer kept on the accelerator; green is the net space that can be reallocated. One buffer set nets 2304 MiB; two sets net 2016 MiB.](images/figure-8-9-offload.pdf)
...
![Figure 8-25: The per-round copy volume stays at 2592 MiB (2.72 GB). Over PCIe Gen5 x16 (64 GB/s per direction), this takes at least about 42.5 ms; over GH200's NVLink-C2C (450 GB/s per direction), it takes at least about 6.0 ms.](images/figure-8-offload-copy.pdf)
...
![Figure 8-26: BF16 KV baseline. Each row is one fixed task; the four columns are two natural-generation runs each at concurrency 1 and 4. Green circles mark correct answers, orange crosses mark errors, totaling 28/32 correct.](images/figure-8-10-kv-quality.pdf)

![Figure 8-27: Same tasks and run order; the original FP8 implementation gets 26/32 correct overall. This comparison includes the effect of implementation choice on Q's precision. Circles mark correct answers, crosses mark errors; each row corresponds to the same question, and columns represent concurrency level and repeat run index.](images/figure-8-kv-quality-1.pdf)
...
![Figure 8-28: Keeping query Q at BF16 while KV stays in FP8, totaling 28/32 correct; the questions answered incorrectly differ from the BF16 baseline. All three figures use the same tasks and column order, with model weights in BF16 throughout. Circles mark correct answers, crosses mark errors; each row corresponds to the same question, and columns represent concurrency level and repeat run index.](images/figure-8-kv-quality-2.pdf)
...
![Figure 8-29: The process of greedy verification. a, b, c, d, x denote tokens; the target model selects x at the third position, which differs from draft token c. The fourth token and its subsequent verification results are discarded; the next round continues generation from a, b, x. The boxes represent token positions in the sequence, not execution time.](images/figure-8-11-verification.pdf)
...
![Figure 8-30: The probability of accepting A is 1/4; the probability of outputting B after rejection is 3/4. The two paths together yield the target distribution.](images/figure-8-sample-A.pdf)

![Figure 8-31: The probability of accepting B is 3/4; the probability of outputting A after rejection is 1/4. The target distribution is the same, but the draft's acceptance probability is higher.](images/figure-8-sample-B.pdf)
...
![Figure 8-33: Each admission-and-arrival condition uses three windows of 16 requests. Bars show the median throughput of all completed requests; dots show results for individual windows. Time is measured from the scheduled arrival.](images/figure-8-13-service.pdf)

![Figure 8-34: The numerator counts only requests that are correct and complete within 3 seconds. Both figures use the same horizontal-axis ordering and vertical-axis range; the gap comes from wrong or late responses.](images/figure-8-service-goodput.pdf)
...
![Figure 8-36: Amdahl speedup curves with other stages' time fixed and no added preparation work. The three curves take decode as 20%, 50%, and 80% of the baseline time, respectively. The higher the original share of time occupied by this stage, the greater the benefit from a local speedup.](images/figure-8-15-task.pdf)
...
[^batch]: The fixed model's [batching computation](https://github.com/bojieli/ai-infra-book/blob/main/calculations/results/batch-reuse-pro6000-2k.md) and [8K comparison](https://github.com/bojieli/ai-infra-book/blob/main/calculations/results/batch-reuse-pro6000-8k.md) on the RTX PRO 6000. Full BF16 weights are 16,381,470,720 bytes, and one step of ideally shared matrix weight reads is 15,136,811,008 bytes; embeddings are looked up by row per request, but the full embedding matrix must still be kept resident in memory. The reads table is first computed with exact byte counts and then each item is rounded.

[^device]: [NVIDIA RTX Blackwell PRO specification](https://github.com/bojieli/ai-infra-book/blob/main/references/files/specs/nvidia-rtx-blackwell-pro.pdf), Appendix A, Table 4, compiled in the rtx-pro6000-blackwell-ws row of the [hardware parameters table](https://github.com/bojieli/ai-infra-book/blob/main/calculations/results/hardware.md).

[^batch-measure]: [Raw records of the batch sweep](https://github.com/bojieli/ai-infra-book/blob/main/experiments/ch08/08-01/README.md). RTX PRO 6000, vLLM 0.23.0, BF16, three runs; the main text reports the median, throughput covers the remaining prefill and generation, and excludes loading and warmup; the 8K condition first establishes a shared prefix.

[^schedule-survey]: [Orca original paper](https://github.com/bojieli/ai-infra-book/blob/main/references/files/papers/orca.pdf) and [chunked computation and versioning notes](https://github.com/bojieli/ai-infra-book/blob/main/case-studies/chunking-and-state-transfer.md).

[^schedule]: [Fixed batch size](https://github.com/bojieli/ai-infra-book/blob/main/calculations/results/iteration-batching-pro6000-fixed.md), [continuous batching](https://github.com/bojieli/ai-infra-book/blob/main/calculations/results/iteration-batching-pro6000-continuous.md), [chunked strategy](https://github.com/bojieli/ai-infra-book/blob/main/calculations/results/iteration-batching-pro6000-chunked.md). Per-round latency parameters are fitted from [Experiment 8-1's per-round records](https://github.com/bojieli/ai-infra-book/blob/main/experiments/ch08/08-01/efficiency.json): for batch 1, a 2K decode round is 26.26 ms, prefill is 94.8 ms; the last output token has not yet been written into the KV cache when the request ends.

[^chunk]: [Chunked computation and archiving measurements](https://github.com/bojieli/ai-infra-book/blob/main/calculations/results/chunk-history-book.md). 11 requests, 16 chunks each; median first- and last-chunk times are 25.30 and 35.07 ms; the CUDA event interval covers the full model path and possible host-side gaps, excluding external logits/sampling. Each chunk executes in its original order, with input tokens and content changing chunk by chunk.

[^nanoflow]: [OSDI 2025 survey](https://github.com/bojieli/ai-infra-book/blob/main/research/2026-infra-survey/reading-osdi-2025.md), [NanoFlow final paper](https://github.com/bojieli/ai-infra-book/blob/main/references/proceedings/OSDI/2025/selected/osdi25-zhu-kan.pdf), and [resource sharing and placement case study](https://github.com/bojieli/ai-infra-book/blob/main/case-studies/resource-sharing-and-placement.md).

[^replay]: [Six-request measurement](https://github.com/bojieli/ai-infra-book/blob/main/experiments/ch08/08-02/README.md) and [graph execution trade-offs](https://github.com/bojieli/ai-infra-book/blob/main/case-studies/graph-execution-tradeoffs.md). The experiment executes six synthetic text requests in a fixed order on a shared GPU.

[^pages]: [Paging, cancellation, and preemption](https://github.com/bojieli/ai-infra-book/blob/main/experiments/ch08/08-03/README.md), [four-branch sharing](https://github.com/bojieli/ai-infra-book/blob/main/experiments/ch08/08-03/branch-sharing/README.md), [cold-branch comparison](https://github.com/bojieli/ai-infra-book/blob/main/experiments/ch08/08-03/cold-branches/README.md). Shared branches produce identical outputs; cold and hot branches have the same peak block count but different prefill volumes.

[^lora]: [MLSys 2024 survey](https://github.com/bojieli/ai-infra-book/blob/main/research/2026-infra-survey/reading-mlsys-2024.md) and [multi-LoRA computation](https://github.com/bojieli/ai-infra-book/blob/main/case-studies/multi-lora-serving.md).

[^context]: [Context engineering case study](https://github.com/bojieli/ai-infra-book/blob/main/case-studies/author-context-and-design.md).

[^hybrid]: [Hybrid state recovery and checkpoint computation](https://github.com/bojieli/ai-infra-book/blob/main/case-studies/hybrid-prefix-state.md), [MLSys 2025 survey](https://github.com/bojieli/ai-infra-book/blob/main/research/2026-infra-survey/reading-mlsys-2025.md). Kimi K3's capacity is computed based on the specified storage format and parallelism configuration.

[^prefix]: [Cache replay of 12 rounds of real input](https://github.com/bojieli/ai-infra-book/blob/main/experiments/ch08/08-04/README.md). The replay uses the input sequence of the original task, executing requests serially, generating one token per round.

[^files]: [Fixed model files and service budget](https://github.com/bojieli/ai-infra-book/blob/main/case-studies/inference-training-scenarios.md) and [235B shard metadata](https://github.com/bojieli/ai-infra-book/blob/main/references/outline-checks/2026-09-07/systems-cases/qwen235-gguf-metadata.json). Exact bytes, GB, and GiB are reported separately.

[^kv-format]: [KV format and execution cost](https://github.com/bojieli/ai-infra-book/blob/main/case-studies/kv-quantization-and-execution.md). The material includes the GGML grouped format, FP8 scale granularity, and fitted results on the H100.

[^offload]: [Weight offloading and prefetch computation](https://github.com/bojieli/ai-infra-book/blob/main/case-studies/weight-offload-execution.md); the copy and prefetch scheduling for the two link tiers [PCIe Gen5](https://github.com/bojieli/ai-infra-book/blob/main/calculations/results/weight-offload-pcie5.md) and [NVLink-C2C](https://github.com/bojieli/ai-infra-book/blob/main/calculations/results/weight-offload-gh200-c2c.md), with per-layer computation time taken as the batch-1 decode round time of 26.26 ms divided by 36 layers; [GH200 architecture notes](https://github.com/bojieli/ai-infra-book/blob/main/references/files/documents/nvidia-grace-hopper-blog.md): NVLink-C2C totals 900 GB/s, 450 GB/s per direction.

[^codec]: [Tensor compression and transfer](https://github.com/bojieli/ai-infra-book/blob/main/case-studies/tensor-codec-and-transfer.md). The material includes the LLM.265 video engine implementation, dedicated hardware design, and performance estimation.

[^mac]: [M2 Max small-model capacity records](https://github.com/bojieli/ai-infra-book/blob/main/experiments/ch08/08-07/README.md) and [32K supplementary test](https://github.com/bojieli/ai-infra-book/blob/main/experiments/ch08/08-07/context32k/README.md). Each slot executes prefill in sequence.

[^quality]: [KV and Q precision comparison](https://github.com/bojieli/ai-infra-book/blob/main/experiments/ch08/08-08/README.md), [Q control with fixed FP8 weights](https://github.com/bojieli/ai-infra-book/blob/main/experiments/ch08/08-08/quantized-q-control/README.md), [concurrency capacity records](https://github.com/bojieli/ai-infra-book/blob/main/experiments/ch08/08-08/concurrency/README.md). The main text uses the BF16 weight group. In another group with fixed FP8 weights, BF16 Q and the default path are 32/32 and 28/32 respectively.

[^retry]: [Natural output and retry cost](https://github.com/bojieli/ai-infra-book/blob/main/experiments/ch08/08-08/retry-cost/README.md). The experiment uses answer verification with known answers; retry cost includes the generation cost of the incorrect first answer.

[^spec]: [Speculative sampling original paper](https://github.com/bojieli/ai-infra-book/blob/main/references/files/papers/speculative-sampling.pdf), [speculative decoding original paper](https://github.com/bojieli/ai-infra-book/blob/main/references/files/papers/speculative-decoding.pdf), [framework counting and budget notes](https://github.com/bojieli/ai-infra-book/blob/main/case-studies/speculative-execution.md), [framework coverage in the survey](https://github.com/bojieli/ai-infra-book/blob/main/research/2026-infra-survey/framework-coverage.md). The basic probability algorithm, draft routes, and specific implementations are cited separately.

[^history]: [Distribution and cost computation for context drafts](https://github.com/bojieli/ai-infra-book/blob/main/case-studies/history-drafts-and-rollout.md) and [rational-number exhaustive results](https://github.com/bojieli/ai-infra-book/blob/main/research/2026-infra-survey/history-speculation-arithmetic.json). The example uses a two-symbol target distribution; per-round times on the RTX PRO 6000 are given in [AAAA](https://github.com/bojieli/ai-infra-book/blob/main/calculations/results/speculative-round-pro6000-aaaa.md), [BBBB](https://github.com/bojieli/ai-infra-book/blob/main/calculations/results/speculative-round-pro6000-bbbb.md), and [the 256-token request with a 2-second index](https://github.com/bojieli/ai-infra-book/blob/main/calculations/results/speculative-budget-pro6000-index.md); the 26.26 ms ordinary decode round is taken from [Experiment 8-1's per-round records](https://github.com/bojieli/ai-infra-book/blob/main/experiments/ch08/08-01/efficiency.json), and DFlash's per-round time is taken from the step-by-step records of [Experiment 8-5](https://github.com/bojieli/ai-infra-book/blob/main/experiments/ch08/08-05/README.md).

[^spec-measure]: [DFlash same-executor comparison](https://github.com/bojieli/ai-infra-book/blob/main/experiments/ch08/08-05/README.md) and [per-request stage observations](https://github.com/bojieli/ai-infra-book/blob/main/experiments/ch08/08-05/kernel-trace/README.md). The experiment executes a given short problem set in fixed order under low concurrency. At concurrency 1, the median times for ordinary, K7, and K15 to pass the short problem set are 90.02, 30.58, and 29.43 ms respectively; the paired outputs total 32 pairs.

[^service]: [Raw records of service and energy](https://github.com/bojieli/ai-infra-book/blob/main/experiments/ch08/08-09/README.md). Same device, eight problems, seven conditions, three 16-request windows each; the same problem produces consistent output across conditions, and 42 errors come from the same problem. The throughput table uses the median of each window's metrics. Each window contains 16 requests; p95 is computed at the ceiling of the sorted position, which yields the maximum value. The NVML and RAPL component counting windows are slightly wider than the request windows, with other services still running.

[^service-plan]: [Experiment 8-1's efficiency table](https://github.com/bojieli/ai-infra-book/blob/main/experiments/ch08/08-01/efficiency.json) is exported by the [computation script](https://github.com/bojieli/ai-infra-book/blob/main/experiments/ch08/08-01/efficiency.py) from the [per-round records of the batch sweep](https://github.com/bojieli/ai-infra-book/blob/main/experiments/ch08/08-01/README.md); the peaks are taken from the hardware table's RTX PRO 6000 values of 1792 GB/s and 503.8 TFLOP/s; DFlash's peak memory footprint, first-token time, and per-round time are taken from [Experiment 8-5](https://github.com/bojieli/ai-infra-book/blob/main/experiments/ch08/08-05/README.md). Each scheme's memory and time are recomputed by the [example check script](https://github.com/bojieli/ai-infra-book/blob/main/manuscripts/ch08/teaching-check.py).

[^task]: [Generation experiment and agent run records](https://github.com/bojieli/ai-infra-book/blob/main/experiments/ch08/08-06/README.md). The agent records use non-streaming invocation; the stage-time figure uses the parameters given in the problem statement.

[^gguf]: [Q2_K per-tensor layout](https://github.com/bojieli/ai-infra-book/blob/main/calculations/results/gguf-layout-q2k.md), [8K file capacity](https://github.com/bojieli/ai-infra-book/blob/main/calculations/results/gguf-inventory-8k.md), and [32K comparison](https://github.com/bojieli/ai-infra-book/blob/main/calculations/results/gguf-inventory-32k.md). Total file size is 85,691,002,112 bytes, code-value/floating-point payload is 69,178,275,840 bytes, quantization metadata is 16,506,720,256 bytes, and headers and padding are 6,006,016 bytes. The above file sizes are computed from the archived headers and released metadata.

[^pcie]: [RTX PRO 6000 Blackwell Workstation Edition specification](https://github.com/bojieli/ai-infra-book/blob/main/references/files/specs/nvidia-rtx-pro6000-spec.pdf): system interface PCIe 5.0 x16, with bandwidth twice that of PCIe Gen4; [A100 80GB specification](https://github.com/bojieli/ai-infra-book/blob/main/references/files/specs/nvidia-a100-80-spec.pdf): PCIe 4.0 is 64 GB/s (combined send and receive), i.e., x16 gives 32 GB/s per direction. Hence PCIe Gen5 x16 gives 64 GB/s per direction. The main text computes using nominal values, yielding a lower bound on transfer time. The prefix recomputation time is computed from [Experiment 8-1's efficiency table](https://github.com/bojieli/ai-infra-book/blob/main/experiments/ch08/08-01/efficiency.json).

[^cancel]: The cancellation observations come from the [state experiment](https://github.com/bojieli/ai-infra-book/blob/main/experiments/ch08/08-03/README.md). Recorded values are approximately 1.55 ms and 31.41 ms, with timing including observer overhead.

[^core-calculation]: [Item-by-item recomputation](https://github.com/bojieli/ai-infra-book/blob/main/calculations/results/core-principles.json) and [computation procedure](https://github.com/bojieli/ai-infra-book/blob/main/calculations/core_principles.py) for this chapter's condition comparisons.
