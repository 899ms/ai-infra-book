...
a.text(.07,.625,'推理使用前向；预训练、中期训练与 SFT 使用同一基本更新路径，数据、标签与长度另行声明。',fontsize=11)
ax=f.add_axes([.10,.23,.37,.29]);vals=[t['summary'][k]/1e12 for k in ['forward_matrix_flops','backward_matrix_flops','training_matrix_flops','six_nd_flops']];ax.barh(range(4),vals,color=[C['blue'],C['teal'],C['ink'],C['orange']]);ax.set_yticks(range(4),['前向矩阵','反向矩阵','两者合计','总参数 $6ND$']);ax.invert_yaxis();ax.set_xlim(0,515);ax.set_xlabel('TFLOPs');ax.set_title('A  矩阵子账',loc='left',fontsize=13)
for i,v in enumerate(vals):ax.text(v+7,i,f'{v:.3f}',va='center',fontsize=10)
ax=f.add_axes([.64,.23,.29,.29]);states=t['parameter_state_bytes'];labels=['BF16 权重','FP32 梯度','FP32 master','Adam 一阶','Adam 二阶'];sv=[v/1e9 for v in states.values()];ax.barh(range(5),sv,color=[C['blue'],C['teal'],C['orange'],C['muted'],C['muted']]);ax.set_yticks(range(5),labels);ax.invert_yaxis();ax.set_xlim(0,42);ax.set_xlabel('十进制 GB');ax.set_title('B  声明的无分片参数状态',loc='left',fontsize=13)
...
for x,title,body in [(.06,'策略生成','候选轨迹'),(.30,'反馈 / 验证','规则、模型或环境'),(.54,'筛选与组织','明确接受条件'),(.78,'策略更新','前向 / 反向 / 更新')]:box(a,x,.70,.17,.12,title,body,size=12)
...
ax.set_yticks([0,1],['32 候选 → 16 接受','64 候选 → 16 接受']);ax.invert_yaxis();ax.set_xlim(0,4050);ax.set_xlabel('已计矩阵 TFLOPs');ax.legend(ncol=4,frameon=False,fontsize=10,loc='upper center',bbox_to_anchor=(.5,1.37))
for i,v in enumerate(left):ax.text(v+45,i,f'{v:.3f}',va='center',fontsize=10)
footer(a,'本次数值设教师 0 次；规则验证、损失、优化器和真实供给未知。一次 BF16 权重快照 16.381 GB，非优化器状态。')
...
f=plt.figure(figsize=(15,10));f.suptitle('图 3-7  训练预算与生命周期成本',x=.045,y=.97,ha='left',fontsize=24,weight='bold');f.text(.045,.91,'六个 C4 点拟合、两个事前留出；费用来自同一拟合的条件代理，不是硬件价格或同任务质量证明。',fontsize=11,color=C['muted'])
...
f,a=canvas('图 3-8  Llama 与 Qwen 的模型—数据选择','历史投入使用报告规模代理；Qwen token 为 family 披露，产品点不是受控 Scaling Law 实验。',9)
...
ax=f.add_axes([.63,.26,.31,.51]);mids=['deepseek-v3-pretraining','deepseek-v4-flash','deepseek-v4-pro'];ctx=[hr[k]['parameter_context'] for k in mids];y=np.arange(3);tot=[z['total_reported']/1e9 for z in ctx];act=[z['active_reported']/1e9 for z in ctx];ax.barh(y-.17,tot,height=.30,color=C['blue'],label='总参数：容量侧');ax.barh(y+.17,act,height=.30,color=C['orange'],label='激活参数：计算代理');ax.set_yticks(y,['V3 · 2024\n14.8T','V4-Flash · 2026\n32T','V4-Pro · 2026\n33T']);ax.invert_yaxis();ax.set_xlim(0,1900);ax.set_xlabel('十亿参数 B');ax.set_title('B  MoE 总参数与激活参数',loc='left',fontsize=13);ax.legend(frameon=False,fontsize=10,loc='upper center',bbox_to_anchor=(.5,-.17))
...
a.text(.06,.135,'D/N 从约 149 到约 4500，说明相近参数规模可投入更多训练；能力差异还涉及数据、配方和评测。',fontsize=11)
footer(a,'6ND 需注明参数代理；MoE 激活量不能替代全部矩阵、状态更新与优化器工作，跨模型不据本图排名质量。')
...
box(a,.06,.155,.41,.115,'完整预训练小时未闭合','Qwen2.5 / Qwen3 / Qwen3.5 / V4\n未知值不画成零',color='pale',size=12)
...
(HERE/'manifest.json').write_text(json.dumps({'chapter':3,'generator':'manuscripts/ch03/build.py','figures':9,'font_family':family,'outputs':[{'path':str(p.relative_to(ROOT)),'sha256':hashlib.sha256(p.read_bytes()).hexdigest()} for p in artifacts]},ensure_ascii=False,indent=2)+'\n')
