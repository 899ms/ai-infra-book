...
asr=json.loads((R/'asr-summary.json').read_text())
a=json.loads((R/'loss0-summary.json').read_text())
b=json.loads((R/'loss0.1-summary.json').read_text())
text='''# Actual ASR records and controlled-network replay
...
for x,y in zip(a['conditions'],b['conditions']):
    assert (x['protocol'],x['reuse'])==(y['protocol'],y['reuse'])
    text+=f"| {x['protocol']} | {x['reuse']} | {x['median_attempt_ms']:.3f} | {y['median_attempt_ms']:.3f} |\n"
text+='''
...
(R/'manifest.json').write_text(json.dumps(dict(scope='Actual ASR plus calibrated real-network record replay; parent incomplete',files=files),indent=2)+'\n')
