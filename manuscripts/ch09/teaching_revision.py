    ...
    with plt.rc_context(STYLE):
        for i,title in enumerate(['多卡共同执行一个完整模型','完整副本分别接收请求','PD：按输入处理与生成阶段分工','AF：按每层的算子分工']):
            ...
            if i==0:
                ...
                arrow(a,(.39,.46),(.61,.46));text(a,.5,.68,'交接历史 KV' if i==2 else '逐层传递激活',12,ha='center')
        ...
        for stage,name in [(0,'kv-residency'),(1,'kv-publish'),(2,'kv-release')]:
            f,a=canvas(3.9);text(a,.04,.94,['传输开始：两端都占用完整空间','传输完成：发布目的端可用状态','交接完成：源端释放，D 继续生成'][stage],13)
        ...
        f,a=canvas(3.8);box(a,.04,.50,.34,.24,'目录记录\n标识 → 存储位置','orange',11);box(a,.62,.50,.34,.24,'KV 数据对象\n实际历史状态','blue',11);arrow(a,(.38,.62),(.62,.62),'control');text(a,.5,.22,'先按目录定位，再确认对象可用并取回',12,ha='center');save(f,'cache-directory')
        f,a=canvas(4.5)
        for i in range(64):
        ...
        for pooled,name in [(False,'16-composition'),(True,'composition-pool')]:
            ...
            text(a,.5,.18,'整份写入并发布后，D 再取回' if pooled else 'P 直接将完整状态交给 D',12,ha='center');save(f,name)
