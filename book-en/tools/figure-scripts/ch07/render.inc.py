...
page="<!doctype html><html lang=\"en\"><head><meta charset=\"utf-8\"><meta name=\"viewport\" content=\"width=device-width,initial-scale=1\"><title>Chapter 7 Datacenter Network</title><style>"+css+math_css+"</style></head><body><main><nav aria-label=\"Chapter contents\">"+nav+'</nav>'+body+'</main></body></html>'
import sys
...
(HERE/'manifest.json').write_text(json.dumps({'chapter':7,'generator':'manuscripts/ch07/build.py','figures':len(outputs)//3,'font_family':family,'outputs':[{'path':str(p.relative_to(ROOT)),'sha256':hashlib.sha256(p.read_bytes()).hexdigest()} for p in artifacts if not (p.parent==ROOT/'manuscripts' and p.suffix=='.md' and p.name[:2].isdigit())]},ensure_ascii=False,indent=2)+'\n')
