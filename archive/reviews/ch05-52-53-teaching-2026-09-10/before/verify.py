...
check(len(re.findall(r'^!\[图 5-',s,re.M))==17,'seventeen illustrations')
check(len(re.findall(r'^\*图 5-\d+：',s,re.M))==17,'seventeen external captions')
check(len(re.findall(r'<img ',page))==17,'seventeen embedded images')
check(re.findall(r'^!\[图 (5-\d+)',s,re.M)==[f'5-{i}' for i in range(1,18)],'figure reading order')
...
check(set(figdata)=={f'5-{i}' for i in range(1,18)},'complete figure data in reading order')
bank=figdata['5-3']
...
report={'status':'passed' if not errors else 'failed','sections':len(re.findall(r'^## 5\.\d+ ',s,re.M)),'subsections':len(re.findall(r'^### 5\.\d+\.\d+ ',s,re.M)),'exercises':9,'worked_examples':12,'figures':17,'local_links':links,'footnotes':len(defs),'han_characters':len(re.findall(r'[\u4e00-\u9fff]',s)),'numerical_checks':'worked-example thresholds; RMSNorm input/gamma traffic; rectangular tile and prefetch exercises; critical path; tile capacity/traffic; quantization rereads; double-buffer dependencies and slot lifetimes; workload and payback thresholds; graph payload limit; persistent task schedule; attention budget; online softmax; ragged GEMM; activation counterexample; specialization thresholds; trace counts','errors':errors}
