<!-- 从 pytorch-pinmem.html 迁移的资料快照；原始 HTML SHA-256: ec6e81d4d186e35601c928d35ef2241e3978572c597463002b631e3076ccdae7。 -->
...
non_blocking and `pin_memory()` in PyTorch"\>
...
[Go to the end](#sphx-glr-download-intermediate-pinmem-nonblock-py) to download the full example code.

<a id="a-guide-on-good-usage-of-non-blocking-and-pin-memory-in-pytorch"></a>

# A guide on good usage of `non_blocking` and `pin_memory()` in PyTorch
Created On: Jul 31, 2024 \| Last Updated: Apr 01, 2026 \| Last Verified: Nov 05, 2024
...
## Introduction
Transferring data from the CPU to the GPU is fundamental in many PyTorch applications. It’s crucial for users to understand the most effective tools and options available for moving data between devices. This tutorial examines two key methods for device-to-device data transfer in PyTorch: [`pin_memory()`](https://docs.pytorch.org/docs/stable/generated/torch.Tensor.pin_memory.html#torch.Tensor.pin_memory "(in PyTorch v2.14)") and [`to()`](https://docs.pytorch.org/docs/stable/generated/torch.Tensor.to.html#torch.Tensor.to "(in PyTorch v2.14)") with the `non_blocking=True` option.
...
<a id="memory-management-basics"></a>
...
<a id="cuda-and-non-pageable-memory"></a>
...
<a id="asynchronous-vs-synchronous-operations-with-non-blocking-true-cuda-cudamemcpyasync"></a>
...
In practice, when calling [`to()`](https://docs.pytorch.org/docs/stable/generated/torch.Tensor.to.html#torch.Tensor.to "(in PyTorch v2.14)"), PyTorch always makes a call to [cudaMemcpyAsync](https://docs.nvidia.com/cuda/cuda-runtime-api/group__CUDART__MEMORY.html#group__CUDART__MEMORY_1g85073372f776b4c4d5f89f7124b7bf79). If `non_blocking=False` (default), a `cudaStreamSynchronize` will be called after each and every `cudaMemcpyAsync`, making the call to [`to()`](https://docs.pytorch.org/docs/stable/generated/torch.Tensor.to.html#torch.Tensor.to "(in PyTorch v2.14)") blocking in the main thread. If `non_blocking=True`, no synchronization is triggered, and the main thread on the host is not blocked. Therefore, from the host perspective, multiple tensors can be sent to the device simultaneously, as the thread does not need to wait for one transfer to be completed to initiate the other.
...
1.  The device must have at least one free DMA (Direct Memory Access) engine. Modern GPU architectures such as Volterra, Tesla, or H100 devices have more than one DMA engine.

2.  The transfer must be done on a separate, non-default cuda stream. In PyTorch, cuda streams can be handled using [`Stream`](https://docs.pytorch.org/docs/stable/generated/torch.cuda.Stream_class.html#torch.cuda.Stream "(in PyTorch v2.14)").
...
PyTorch offers the possibility to create and send tensors to page-locked memory through the [`pin_memory()`](https://docs.pytorch.org/docs/stable/generated/torch.Tensor.pin_memory.html#torch.Tensor.pin_memory "(in PyTorch v2.14)") method and constructor arguments. CPU tensors on a machine where CUDA is initialized can be cast to pinned memory through the [`pin_memory()`](https://docs.pytorch.org/docs/stable/generated/torch.Tensor.pin_memory.html#torch.Tensor.pin_memory "(in PyTorch v2.14)") method. Importantly, `pin_memory` is blocking on the main thread of the host: it will wait for the tensor to be copied to page-locked memory before executing the next operation. New tensors can be directly created in pinned memory with functions like [`zeros()`](https://docs.pytorch.org/docs/stable/generated/torch.zeros.html#torch.zeros "(in PyTorch v2.14)"), [`ones()`](https://docs.pytorch.org/docs/stable/generated/torch.ones.html#torch.ones "(in PyTorch v2.14)") and other constructors.
...
![Device casting runtime (pin-memory)](../_images/sphx_glr_pinmem_nonblock_001.png)

    pageable_tensor.to('cuda:0'):  0.3705 ms
    pinned_tensor.to('cuda:0'):  0.3164 ms
    pageable_tensor.pin_memory():  0.1143 ms
    pageable_tensor.pin_memory().to('cuda:0'):  0.4364 ms
...
However, contrary to a somewhat common belief, calling [`pin_memory()`](https://docs.pytorch.org/docs/stable/generated/torch.Tensor.pin_memory.html#torch.Tensor.pin_memory "(in PyTorch v2.14)") on a pageable tensor before casting it to GPU should not bring any significant speed-up, on the contrary this call is usually slower than just executing the transfer. This makes sense, since we’re actually asking Python to execute an operation that CUDA will perform anyway before copying the data from host to device.

Note

The PyTorch implementation of [pin_memory](https://github.com/pytorch/pytorch/blob/5298acb5c76855bc5a99ae10016efc86b27949bd/aten/src/ATen/native/Memory.cpp#L58) which relies on creating a brand new storage in pinned memory through [cudaHostAlloc](https://docs.nvidia.com/cuda/cuda-runtime-api/group__CUDART__MEMORY.html#group__CUDART__MEMORY_1gb65da58f444e7230d3322b6126bb4902) could be, in rare cases, faster than transitioning data in chunks as `cudaMemcpy` does. Here too, the observation may vary depending on the available hardware, the size of the tensors being sent or the amount of available RAM.

<a id="non-blocking-true"></a>
...
![Device casting runtime (non-blocking)](../_images/sphx_glr_pinmem_nonblock_002.png)

    copy_to_device(*tensors):  19.5636 ms
    copy_to_device_nonblocking(*tensors):  14.7996 ms
...
Let’s see the call stack with a regular `to(device)` first:
    ...
    -------------------------  ------------  ------------  ------------  ------------  ------------  ------------
                     aten::to         4.15%     977.290us       100.00%      23.549ms      23.549us          1000
               aten::_to_copy        10.59%       2.494ms        95.85%      22.572ms      22.572us          1000
          aten::empty_strided        20.20%       4.757ms        20.20%       4.757ms       4.757us          1000
                  aten::copy_        26.18%       6.165ms        65.06%      15.320ms      15.320us          1000
        cudaStreamIsCapturing         1.86%     437.398us         1.86%     437.398us       0.437us          1000
              cudaMemcpyAsync        16.16%       3.805ms        16.16%       3.805ms       3.805us          1000
        cudaStreamSynchronize        20.86%       4.912ms        20.86%       4.912ms       4.912us          1000
    -------------------------  ------------  ------------  ------------  ------------  ------------  ------------
...
and now the `non_blocking` version:
    ...
    -------------------------  ------------  ------------  ------------  ------------  ------------  ------------
                         Name    Self CPU %      Self CPU   CPU total %     CPU total  CPU time avg    # of Calls
    -------------------------  ------------  ------------  ------------  ------------  ------------  ------------
                     aten::to         5.00%     971.069us        99.87%      19.396ms      19.396us          1000
               aten::_to_copy        13.54%       2.630ms        94.87%      18.425ms      18.425us          1000
          aten::empty_strided        24.51%       4.760ms        24.51%       4.760ms       4.760us          1000
                  aten::copy_        33.84%       6.573ms        56.82%      11.036ms      11.036us          1000
        cudaStreamIsCapturing         2.21%     428.922us         2.21%     428.922us       0.429us          1000
              cudaMemcpyAsync        20.77%       4.034ms        20.77%       4.034ms       4.034us          1000
        cudaDeviceSynchronize         0.13%      25.151us         0.13%      25.151us      25.151us             1
    -------------------------  ------------  ------------  ------------  ------------  ------------  ------------
    Self CPU time total: 19.422ms
...
The benefits of using [`pin_memory()`](https://docs.pytorch.org/docs/stable/generated/torch.Tensor.pin_memory.html#torch.Tensor.pin_memory "(in PyTorch v2.14)") are more pronounced for somewhat large batches of large tensors:
...
![Runtime (pin-mem and non-blocking)](../_images/sphx_glr_pinmem_nonblock_003.png)

    copy_to_device(*tensors):  388.7401 ms
    copy_to_device_nonblocking(*tensors):  309.0382 ms
    copy_to_device(*tensors_pinned):  316.9789 ms
    copy_to_device_nonblocking(*tensors_pinned):  299.8560 ms
    pin_copy_to_device(*tensors):  561.3477 ms
    pin_copy_to_device_nonblocking(*tensors):  327.2634 ms

<a id="other-copy-directions-gpu-cpu-cpu-mps"></a>

### Other copy directions (GPU -\> CPU, CPU -\> MPS)
> 

Until now, we have operated under the assumption that asynchronous copies from the CPU to the GPU are safe. This is generally true because CUDA automatically handles synchronization to ensure that the data being accessed is valid at read time \_\_whenever the tensor is in pageable [memory\_\_](#id2).
...
<a id="practical-recommendations"></a>
...
One might now legitimately ask what use there is for the [`pin_memory()`](https://docs.pytorch.org/docs/stable/generated/torch.Tensor.pin_memory.html#torch.Tensor.pin_memory "(in PyTorch v2.14)") method. In the following section, we will explore further how this can be used to accelerate the data transfer even more.
...
PyTorch notoriously provides a [`DataLoader`](https://docs.pytorch.org/docs/stable/data.html#torch.utils.data.DataLoader "(in PyTorch v2.14)") class whose constructor accepts a `pin_memory` argument. Considering our previous discussion on `pin_memory`, you might wonder how the `DataLoader` manages to accelerate data transfers if memory pinning is inherently blocking.
...
To illustrate this, we will use the TensorDict primitive from the homonymous library. When invoking [`to()`](https://docs.pytorch.org/tensordict/stable/reference/generated/tensordict.TensorDict.html#tensordict.TensorDict.to "(in tensordict v0.14)"), the default behavior is to send tensors to the device asynchronously, followed by a single call to `torch.device.synchronize()` afterwards.
...
![Device casting runtime](../_images/sphx_glr_pinmem_nonblock_004.png)

    td.to('cuda:0', non_blocking=False):  392.5374 ms
    td.to('cuda:0'):  311.3985 ms
    td.to('cuda:0', non_blocking_pin=True, num_threads=0):  316.1530 ms
    td.to('cuda:0', non_blocking_pin=True, num_threads=4):  301.3059 ms
...
Throughout this tutorial, we have explored several critical factors that influence transfer speeds and memory management when sending tensors from the host to the device. We’ve learned that using `non_blocking=True` generally accelerates data transfers, and that [`pin_memory()`](https://docs.pytorch.org/docs/stable/generated/torch.Tensor.pin_memory.html#torch.Tensor.pin_memory "(in PyTorch v2.14)") can also enhance performance if implemented correctly. However, these techniques require careful design and calibration to be effective.
...
If you are dealing with issues with memory copies when using CUDA devices or want to learn more about what was discussed in this tutorial, check the following references:
...
**Total running time of the script:** (1 minutes 3.909 seconds)
...
[](tensorboard_tutorial.html "previous page")

previous

Visualizing Models, Data, and Training with TensorBoard

[](intermediate_data_loading_tutorial.html "next page")
...
- [Background](#background)
  ...
  - [Asynchronous vs. Synchronous Operations with `non_blocking=True` (CUDA `cudaMemcpyAsync`)](#asynchronous-vs-synchronous-operations-with-non-blocking-true-cuda-cudamemcpyasync)
- [A PyTorch perspective](#a-pytorch-perspective)
  - [`pin_memory()`](#pin-memory)
  - [`non_blocking=True`](#non-blocking-true)
  - [Synergies](#synergies)
  - [Other copy directions (GPU -\> CPU, CPU -\> MPS)](#other-copy-directions-gpu-cpu-cpu-mps)
