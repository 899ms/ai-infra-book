...
async def main(a):
 ...
 try:
  with (a.out/'requests.jsonl').open('w',buffering=1) as log:
   for i,name in enumerate(['base','base','a','a','b','b','a','base']):
    rid=f'{i}-{name}';events=[];start=time.monotonic();final=None
    async for result in engine.generate({'prompt_token_ids':ids},SamplingParams(temperature=0,max_tokens=64,ignore_eos=True),rid,lora_request=adapters.get(name)):
     final=result;events.append([time.monotonic(),len(result.outputs[0].token_ids)])
    seq=final.outputs[0];assert final.prompt_token_ids==ids and len(seq.token_ids)==64
  ...
  (a.out/'completion.json').write_text(json.dumps(dict(done=True,requests=8))+'\n')
