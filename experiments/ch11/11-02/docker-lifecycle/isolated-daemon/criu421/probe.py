...
commands=[]
def cmd(args):
 if args[0]=="docker":args=args[:1]+["-H","unix://"+"/run/book1102-c421.sock"]+args[1:]
...
for mode in ['default','unconfined']:
 ...
 try:
  ...
  def watch():
   roots=[Path('/run/book1102-c421-ctd/io.containerd.runtime.v2.task/book1102')/cid,Path('/run/book1102-c421/runtime-runc/book1102')/cid,Path('/run/book1102-c421/containerd/daemon/io.containerd.runtime.v2.task/book1102')/cid,P/'data/containers'/cid]
  ...
  if restore and 'already exists' in restore['stderr']:
   ...
   ctr=['ctr','--address','/run/book1102-c421-ctd.sock','--namespace','book1102','content']
   listing=cmd(ctr+['ls'])
   assert digest in listing['stdout']
   # Only the digest from this own-container restore failure, in the private store.
   assert cmd(ctr+['rm',digest])['rc']==0
