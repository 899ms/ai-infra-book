...
def draw(out):
    ...
    text(a, .02, .95, "horizontal wire: hundreds of μm, capacitance accumulates along length", 13)
    box(a, .02, .60, .16, .16, "driver gate", 'green', 11); box(a, .82, .60, .16, .16, "receiver gate", 'green', 11)
    ...
    text(a, .50, .36, "each 0→1 charge draws C·V² energy from supply", 12, ha='center')
    text(a, .02, .19, "after folding: vertical link between top and bottom layers is only a few μm", 13)
    box(a, .30, .00, .18, .11, "top layer", 'blue', 11); box(a, .52, .00, .18, .11, "bottom layer", 'blue', 11)
    arrow(a, (.48, .055), (.52, .055))
    b = f.add_axes([.14, .12, .82, .32])
    labels = ["baseline", "after voltage drop\ndynamic power", "after folding\npower", "after folding\nprojected area", "after folding\npower density"]
    ...
    b.set(xticks=range(5), xticklabels=labels, ylim=(0, 1.55), ylabel="relative to baseline")
    ...
    rows = [("KB-scale local SRAM", epb['shared_l1']['pj_per_byte']), ("NVLink-C2C link", epb['nvlink']['pj_per_byte']),
            ("MB-scale on-chip SRAM", epb['l2']['pj_per_byte']), ('HBM2', epb['hbm']['pj_per_byte']),
    ...
    a.set_xscale('log'); a.set(xlim=(1, 400), yticks=list(ys), yticklabels=[r[0] for r in rows], xlabel="energy per byte (pJ, log scale)")
    a.invert_yaxis(); a.tick_params(labelsize=11); a.spines[['top', 'right']].set_visible(False)
    b = f.add_axes([.30, .10, .66, .36])
    names = ["actual accounting (HBM)", "if all from KB-scale SRAM", "if all from NVLink-C2C", "if all from MB-scale SRAM", "if all from HBM2"]
    w, k, c = split['weight_joules'], split['kv_joules'], split['compute_joules']
    b.barh(0, w, color=COL['blue'], edgecolor=COL['line'], height=.62, label="weight read")
    b.barh(0, k, left=w, color=COL['green'], edgecolor=COL['line'], height=.62, label="KV read")
    b.barh(0, c, left=w+k, color=COL['orange'], edgecolor=COL['line'], height=.62, label="matrix computation")
    ...
    b.set(xlim=(0, .72), yticks=range(5), yticklabels=names, xlabel="energy per decode step (J, single request, 8K context)")
    ...
    f, a = canvas(4.6)
    text(a, .5, .95, "each die area cannot exceed reticle exposure area (H100: 814 mm²)", 12, ha='center')
    box(a, .05, .13, .90, .74, '', 'gray')
    text(a, .07, .17, "interposer", 11)
    box(a, .32, .30, .17, .46, 'die 0', 'orange', 12); box(a, .51, .30, .17, .46, 'die 1', 'orange', 12)
    a.plot([.49, .51], [.53, .53], color=COL['line'], lw=2.2)
    text(a, .50, .81, "die-to-die link", 11, ha='center')
    for i in range(4):
        y = .24 + i*.14
    ...
    text(a, .5, .05, "per-stack bandwidth = 1024 pins × pin rate; per-stack capacity = layers × per-layer capacity; stacks arranged along die edge", 11, ha='center')
    ...
    a.plot(x, 1/x, color='#527fa0', lw=2, label="Voltage constant: P ∝ f, sustained frequency ratio = b/e")
    a.plot(x, x**(-1/3), color='#a56c28', lw=2, label="Voltage scales with frequency: P ∝ f³, sustained frequency ratio = (b/e)^(1/3)")
    ...
    a.text(1.9, .955, "e ≤ b: peak frequency sustainable", fontsize=11)
    a.set(xlim=(1.0, 3.0), ylim=(.3, 1.05), xlabel="actual energy per FLOP e ÷ budget b", ylabel="sustained frequency ÷ peak frequency")
