...
f,a=canvas(8)
label(a,.025,.96,'多卡协作：一个实例共同处理一批请求')
label(a,.53,.96,'完整副本：分别接收独立请求')
box(a,.025,.59,.44,.27,'统一调度与完整模型','八张卡协作执行',col='pale')
for i in range(8):
 x=.045+(i%4)*.10;y=.61+(i//4)*.06
 a.add_patch(Rectangle((x,y),.075,.035,fc=C['blue']))
for x in [.54,.78]:
 box(a,x,.62,.19,.19,'完整副本','权重、KV、调度',col='green',fs=12)
 a.text(x+.095,.88,'请求',ha='center',fontsize=11);arrow(a,(x+.095,.85),(x+.095,.815))
...
a.text(.76,.37,'激活往返',ha='center',fontsize=11)
a.text(.035,.06,'不同阶段可分别排队、组成批次',fontsize=11)
a.text(.54,.06,'同一请求每层都经过两侧计算',fontsize=11)
...
f,a=canvas(6)
stages=['P','D','工具等待','下一轮 P'];edges=[.19,.37,.56,.78,.96]
...
rows=[('权重',.72,.19,.96,'blue'),('输入 KV',.54,.23,.96,'orange'),('生成 KV',.36,.40,.96,'orange'),('EC（可选）',.18,.19,.96,'teal')]
...
for messages,col in [(1,'blue'),(72,'orange')]:ax.plot(alpha,base+messages*alpha/1000,color=C[col],lw=2.3,label=f'{messages} 次消息')
ax.set(xlabel='每次启动与同步 / μs',ylabel='串行交接时间 / ms',xlim=(0,20),ylim=(48.2,50),xticks=[0,5,10,15,20])
...
a.text(.50,.965,'读入 64 页',ha='center',fontsize=15,weight='bold')
a.text(.10,.11,'前 63 页：1008 位置可复用',color=C['teal'],fontsize=13)
a.text(.62,.11,'末页：16 位置仍需处理',color=C['orange'],fontsize=13)
...
(HERE/'manifest.json').write_text(json.dumps({'chapter':9,'generator':'manuscripts/ch09/build.py','figures':17,'font_family':family,'outputs':[{'path':str(p.relative_to(ROOT)),'sha256':hashlib.sha256(p.read_bytes()).hexdigest()} for p in artifacts]},ensure_ascii=False,indent=2)+'\n')
