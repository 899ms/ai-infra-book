...
The fault writer blocks immediately after committing checkpoint metadata.
...
def child(root,fault):
    ...
    class Writer(dcp.FileSystemWriter):
        ...
        def finish(self,metadata,results):
            ...
            if fault and self.label=='checkpoint-2':
                (root/'fault-gate-ready').write_text('metadata committed; parent may terminate\n')
    ...
    (root/'environment.json').write_text(json.dumps(dict(torch=torch.__version__,device='cpu',threads=1,
        ...
        failure_gate='after metadata commit after data write' if fault else None),indent=2)+'\n')
    events.close()
...
def run(args):
    ...
    latest=empty();dcp.load(latest,checkpoint_id=fault/'checkpoint-2',no_dist=True)
    latest_hashes={k:hashlib.sha256(v.numpy().tobytes()).hexdigest() for k,v in latest.items()}
    assert latest_hashes==json.loads((fault/'expected.json').read_text())['checkpoint-2']
    assert (fault/'checkpoint-2/.metadata').exists() and int(latest['cursor'])==23
    outcomes['fault'].update(recovered_cursor=int(latest['cursor']),recovered_hashes=latest_hashes,
                            last_completed_training_cursor=43,redo_steps_to43=20,
                            previous_cursor=int(target['cursor']),committed_load_succeeded=True)
    ...
    print(json.dumps(dict(normal_completed=True,fault_exit=outcomes['fault']['returncode'],
                          recovered_cursor=int(latest['cursor']),committed_load_succeeded=True)))
