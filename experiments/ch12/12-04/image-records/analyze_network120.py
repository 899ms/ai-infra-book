...
D=R/'network120-results'/args.profile
for phase in ['before','after']:
 shaping=json.loads((R/'network120-results'/(args.profile+'-'+phase+'.json')).read_text())
...
assert env['body_request_deadline_s']==120
...
assert hashlib.sha256((R/'run_network120.py').read_bytes()).hexdigest()==env['source_sha256']
