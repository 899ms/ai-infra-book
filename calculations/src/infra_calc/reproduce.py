...
from .topics import v41_flash, kv_comparison, v41_forward
...
def input_hashes() -> list[dict]:
    ...
    return [{"file": str(path.relative_to(PROJECT)), "sha256": hashlib.sha256(path.read_bytes()).hexdigest()}
            for path in paths if is_source_input(path)]
...
def verify_results(include_figures=True) -> dict:
    ...
    if include_figures:
        ...
        from .real_scaling_plot import verify as verify_real_scaling
        ...
        figures["verified_figures"] += verify_shapes()["verified_figures"]
        from .image_request_plot import verify as verify_image_request
        figures["verified_figures"] += verify_image_request()["verified_figures"]
        from .supernode_cost_plot import verify as verify_supernode
        figures["verified_figures"] += verify_supernode()["verified_figures"]
...
def run() -> dict:
    ...
    decoder_rows, variant_rows = [], []
    for row in scenarios.get("flux_vae_decode", []):
        result = flux_vae_decode.calculate(**row["inputs"])
        save(row["id"], result)
        decoder_rows.append((row["id"], result))
    for row in scenarios.get("architecture_variants", []):
        result = architecture_variants.calculate(**{k:v for k,v in row.items() if k != "id"})
        save(row["id"], result)
        variant_rows.append((row["id"], result))
    prefix_rows_v4 = []
    for row in scenarios.get("v4_prefix_continuation", []):
        result = v4_prefix_continuation.calculate(**{k:v for k,v in row.items() if k != "id"})
        save(row["id"], result)
        prefix_rows_v4.append((row["id"], result))
    placement235_rows = []
    for row in scenarios.get("qwen235_placement", []):
        result = qwen235_placement.calculate(**{k:v for k,v in row.items() if k != "id"})
        save(row["id"], result)
        placement235_rows.append((row["id"], result))
    dense_lowbit_rows = []
    for row in scenarios.get("dense_quantized_placement", []):
        result = dense_quantized_placement.calculate(**{k:v for k,v in row.items() if k != "id"})
        save(row["id"], result)
        dense_lowbit_rows.append((row["id"], result))
    request_comparison_rows = []
    for row in scenarios.get("request_model_comparison", []):
        result = request_model_comparison.calculate(**{k:v for k,v in row.items() if k != "id"})
        save(row["id"], result)
        request_comparison_rows.append((row["id"], result))
    fish_export_rows = []
    for row in scenarios.get("fish_wave_export", []):
        result = fish_wave_export.calculate(**{k:v for k,v in row.items() if k != "id"})
        save(row["id"], result)
        fish_export_rows.append((row["id"], result))
    strategy_cost_rows = []
    for row in scenarios.get("strategy_record_cost", []):
        result = strategy_record_cost.calculate()
        save(row["id"], result)
        strategy_cost_rows.append((row["id"], result))
    training_nonmatrix_rows = []
    for row in scenarios.get("training_nonmatrix", []):
        result = training_nonmatrix.calculate(**{k:v for k,v in row.items() if k != "id"})
        save(row["id"], result)
        training_nonmatrix_rows.append((row["id"], result))
    real_scaling_rows = []
    for row in scenarios.get("real_scaling_fit", []):
        result = real_scaling_fit.calculate()
        save(row["id"], result)
        real_scaling_rows.append((row["id"], result))
    v4_training_rows = []
    for topic, module in (("v4-training-primitives", v4_training_primitives), ("v4-hc-training", v4_hc_training), ("v4-attention-training", v4_attention_training), ("v4-attention-projections", v4_attention_projections), ("v4-compressor-training", v4_compressor_training), ("v4-compressor-overlap", v4_compressor_overlap)):
        for row in scenarios.get(topic, []):
            result = module.calculate(**{k: v for k, v in row.items() if k != "id"})
            save(row["id"], result)
            v4_training_rows.append((row["id"], result))
    stage_resource_bounds_rows = []
    for row in scenarios.get("stage_resource_bounds", []):
        result = stage_resource_bounds.calculate(**{k: v for k, v in row.items() if k != "id"})
        save(row["id"], result)
        stage_resource_bounds_rows.append((row["id"], result))
    v4_compressor_online_rows = []
    for row in scenarios.get("v4_compressor_online", []):
        result = v4_compressor_online.calculate(**{k: v for k, v in row.items() if k != "id"})
        save(row["id"], result)
        v4_compressor_online_rows.append((row["id"], result))
    training_input_supply_rows = []
    for row in scenarios.get("training_input_supply", []):
        result = training_input_supply.calculate(**{k: v for k, v in row.items() if k != "id"})
        save(row["id"], result)
        training_input_supply_rows.append((row["id"], result))
    qwen235_execution_rows = []
    for row in scenarios.get("qwen235_execution", []):
        result = qwen235_execution.calculate(**{k: v for k, v in row.items() if k != "id"})
        save(row["id"], result)
        qwen235_execution_rows.append((row["id"], result))
    qwen235_expert_granularity_rows = []
    for row in scenarios.get("qwen235_expert_granularity", []):
        result = qwen235_expert_granularity.calculate(**{k: v for k, v in row.items() if k != "id"})
        save(row["id"], result)
        qwen235_expert_granularity_rows.append((row["id"], result))
    architecture_tile_work_rows = []
    for row in scenarios.get("architecture_tile_work", []):
        result = architecture_tile_work.calculate(**{k: v for k, v in row.items() if k != "id"})
        save(row["id"], result)
        architecture_tile_work_rows.append((row["id"], result))
    trace_resource_bridge_rows = []
    for row in scenarios.get("trace_resource_bridge", []):
        result = trace_resource_bridge.calculate(**{k: v for k, v in row.items() if k != "id"})
        save(row["id"], result)
        trace_resource_bridge_rows.append((row["id"], result))
    paired_projection_rows = []
    for row in scenarios.get("paired_projection_cost", []):
        result = paired_projection_cost.calculate(**{k:v for k,v in row.items() if k != "id"})
        save(row["id"], result)
        paired_projection_rows.append((row["id"], result))
    matrix_vector_rows = []
    for row in scenarios.get("matrix_vector_handoff", []):
        result = matrix_vector_handoff.calculate(**{k:v for k,v in row.items() if k != "id"})
        save(row["id"], result)
        matrix_vector_rows.append((row["id"], result))
    v4_copy_rows = []
    for row in scenarios.get("v4_copy_coordinates", []):
        result = v4_copy_coordinates.calculate(**{k:v for k,v in row.items() if k != "id"})
        save(row["id"], result)
        v4_copy_rows.append((row["id"], result))
    attention_input_rows = []
    for row in scenarios.get("attention_input_pipeline", []):
        result = attention_input_pipeline.calculate(**{k:v for k,v in row.items() if k != "id"})
        save(row["id"], result)
        attention_input_rows.append((row["id"], result))
    fa4_resource_rows = []
    for row in scenarios.get("fa4_resource_balance", []):
        result = fa4_resource_balance.calculate()
        save(row["id"], result)
        fa4_resource_rows.append((row["id"], result))
    storage_generation_rows = []
    for row in scenarios.get("storage_generation_comparison", []):
    ...
    for row in scenarios.get("granularity_selection", []):
        ...
        save(row["id"], result)
        granularity_selection_rows.append((row["id"], result))
    trace_cache_lifecycle_rows = []
    for row in scenarios.get("trace_cache_lifecycle", []):
        result = trace_cache_lifecycle.calculate(**{k: v for k, v in row.items() if k != "id"})
        save(row["id"], result)
        trace_cache_lifecycle_rows.append((row["id"], result))
    v4_mtp_forward_rows = []
    for row in scenarios.get("v4_mtp_forward", []):
        result = v4_mtp_forward.calculate(**{k: v for k, v in row.items() if k != "id"})
        save(row["id"], result)
        v4_mtp_forward_rows.append((row["id"], result))
    request_hardware_bridge_rows = []
    for row in scenarios.get("request_hardware_bridge", []):
        result = request_hardware_bridge.calculate(**{k: v for k, v in row.items() if k != "id"})
        save(row["id"], result)
        request_hardware_bridge_rows.append((row["id"], result))
    omni_audio_preprocess_rows = []
    for row in scenarios.get("omni_audio_preprocess", []):
        result = omni_audio_preprocess.calculate(**{k: v for k, v in row.items() if k != "id"})
        save(row["id"], result)
        omni_audio_preprocess_rows.append((row["id"], result))
    v4_optimizer_rows = []
    for row in scenarios.get("v4_optimizer", []):
        result = v4_optimizer.calculate(**row["scenario"])
        save(row["id"], result)
        v4_optimizer_rows.append((row["id"], result))
    v4_moe_training_rows = []
    for row in scenarios.get("v4_moe_training", []):
        result = v4_moe_training.calculate(**{k: v for k, v in row.items() if k != "id"})
        save(row["id"], result)
        v4_moe_training_rows.append((row["id"], result))
    vision_preprocess_rows = []
    for row in scenarios.get("vision_preprocess", []):
        result = vision_preprocess.calculate(**{k: v for k, v in row.items() if k != "id"})
        save(row["id"], result)
        vision_preprocess_rows.append((row["id"], result))
    training_gemm_rows = []
    for row in scenarios.get("training_pipeline_gemm_state", []):
        result = training_pipeline_gemm_state.calculate(**{k: v for k, v in row.items() if k != "id"})
        save(row["id"], result)
        training_gemm_rows.append((row["id"], result))
    training_pipeline_rows = []
    for row in scenarios.get("training_pipeline_schedule", []):
        result = training_pipeline_schedule.calculate(**{k: v for k, v in row.items() if k != "id"})
        save(row["id"], result)
        training_pipeline_rows.append((row["id"], result))
    real_lifecycle_rows = []
    for row in scenarios.get("real_scaling_lifecycle", []):
        result = real_scaling_lifecycle.calculate(**{k:v for k,v in row.items() if k != "id"})
        save(row["id"], result)
        real_lifecycle_rows.append((row["id"], result))
    profile_rows = []
    for row in scenarios.get("workload_profiles", []):
    ...
    for group, module in (("protocol_handshake", protocol_handshake), ("protocol_early_stream", protocol_early_stream), ("protocol_retry", protocol_retry), ("shared_media_transport", shared_media_transport), ("transport_closed_loop", transport_closed_loop)):
        for row in scenarios.get(group, []):
            ...
            save(row["id"], result)
            protocol_rows.append((row["id"], result))
    connection_sequence_rows = []
    for row in scenarios.get("connection_sequence", []):
        result = connection_sequence.calculate(**row["inputs"])
        save(row["id"], result)
        connection_sequence_rows.append((row["id"], result))
    connection_window_rows = []
    for row in scenarios.get("connection_window", []):
        result = connection_window.calculate(**row["inputs"])
        save(row["id"], result)
        connection_window_rows.append((row["id"], result))
    image_stream_rows = []
    for row in scenarios.get("image_request_streaming", []):
        result = image_request_streaming.calculate(**row["inputs"])
        save(row["id"], result)
        image_stream_rows.append((row["id"], result))
    image_request_rows = []
    for row in scenarios.get("image_request_budget", []):
        result = image_request_budget.calculate(**row["inputs"])
        save(row["id"], result)
        image_request_rows.append((row["id"], result))
    gradient_rows = []
    for row in scenarios.get("hierarchical_gradient", []):
    ...
    for row in scenarios.get("supernode_cohort_cost", []):
        ...
        save(row["id"], result)
        supernode_rows.append((row["id"], result))
    growing_rows = []
    for row in scenarios.get("growing_remote_kv", []):
        result = growing_remote_kv.calculate(**row["inputs"])
        save(row["id"], result)
        growing_rows.append((row["id"], result))
    pool_rows = []
    for row in scenarios.get("memory_pool_access", []):
        result = memory_pool_access.calculate(copies=row["copies"])
        save(row["id"], result)
        pool_rows.append((row["id"], result))
        if row["copies"] == 1:
    ...
    qwen36_rows = []
    for group, module in (("v41_forward", v41_forward), ("v41_flash", v41_flash), ("kv_comparison", kv_comparison), ("qwen36_forward", qwen36_forward), ("qwen36_capacity", qwen36_capacity)):
        for row in scenarios.get(group, []):
            result = module.calculate(**row["inputs"])
            save(row["id"], result)
            qwen36_rows.append((row["id"], result))
    qwen35_rows = []
    ...
    fabric_rows = []
    for row in scenarios.get('ub_fabric', []):
        result = ub_fabric.calculate(**{key: value for key, value in row.items() if key != 'id'})
        save(row['id'], result)
        fabric_rows.append((row['id'], result))
    ub_rows = []
    ...
    decision_rows = []
    for row in scenarios.get('decision_request', []):
        result = decision_request.calculate(**{key:value for key,value in row.items() if key != 'id'})
        save(row['id'],result)
        decision_rows.append((row['id'],result))
    restore_rows = []
    ...
    lines.extend(["", "决策请求（共享一次前向、不生成 token）与 LLM 路径的 GPU 秒、牌价与升级份额：", "",
                  "| 结果 | 输入 token | 决策路径 GPU 秒 | 决策路径 $/M 输入 | V4.1 Flash CED $/M 输入 | Jev 定价/稠密下界 | LLM 路径 GPU 秒 | LLM 路径延迟 s |", "| --- | ---: | ---: | ---: | ---: | ---: | --- | --- |"])
    for name,result in decision_rows:
        s_=result["summary"]
        lines.append(f"| [{name}]({name}.md) | {s_['input_tokens']} | {s_['decision_gpu_seconds']} | {s_['decision_usd_per_million_input_tokens']} | {s_['v41_ced_usd_per_million_input_tokens']} | {s_['jev_price_over_dense_floor']} | {s_['llm_gpu_seconds']} | {s_['llm_latency_seconds']} |")
    ...
    lines.extend(['', 'UB 互联从第一性原理推导：状态按 N+M 增长、上下文缓存溢出点、一次 64 B 读取的阶段和、连接建立与硅面积代价。', '',
                  '| 场景 | 1024×1024 状态比 | RoCE 溢出端点数 | UB 溢出端点数 | LD/ST 往返 ns | RoCE 往返 ns | 缓存内主机数 RoCE/UB |', '| --- | ---: | ---: | ---: | ---: | ---: | ---: |'])
    for name, result in fabric_rows:
        s_ = result['summary']
        lines.append(f"| [{name}]({name}.md) | {s_['state_ratio_max']} | {s_['roce_spill_endpoints']} | {s_['ub_spill_endpoints']} | {s_['ub_loadstore_round_trip_ns']:.0f} | {s_['roce_dma_round_trip_ns']:.0f} | {s_['roce_max_hosts_in_cache']}/{s_['ub_max_hosts_in_cache']} |")
    ...
    lines.extend(["", "Qwen3.6-35B-A3B真实混合MoE：逐算子与必要容量，非实际运行时间。", ""])
    lines.extend(['', 'TLS/QUIC消息依赖与长早期上传：', ''])
    for name, result in protocol_rows:
        lines.append(f'- [{name}]({name}.md)')
    lines.extend(['', '连续请求与连接窗口：', ''])
    for name, result in connection_sequence_rows:
        lines.append(f'- [{name}]({name}.md)')
    lines.extend(['', '有限ACK窗口图片请求：', ''])
    for name, result in connection_window_rows:
        lines.append(f'- [{name}]({name}.md)')
    lines.extend(['', '图片分块与预览：', ''])
    for name, result in image_stream_rows:
        lines.append(f'- [{name}]({name}.md)')
    ...
    for name, result in gradient_rows:
        lines.append(f'- [{name}]({name}.md)')
    lines.extend(['', '超节点规模同cohort比较：[图6-9](../figures/supernode-cost/figure.svg)，全部时长/费率为声明条件。', ''])
    for name, result in supernode_rows:
        lines.append(f"- [{name}]({name}.md)：符合容量/SLO的最低费用候选{result['selection']['winners']}，空集合不选。")
    for name, result in growing_rows:
        lines.append(f"- [{name}]({name}.md)：增长KV旧历史/追加/副本提交，通信骨架不是实测decode时间。")
    for name, result in pool_rows:
        lines.append(f"- [{name}]({name}.md)：内存池容量/声明周期读取/副本依赖，见[图6-8](memory-pool-layout.svg)。")
    for name, result in qwen36_rows:
        lines.append(f"- [{name}]({name}.md)：{result['calculation']}，固定官方输入和明确作用域。")
    ...
    lines.extend(["", "封存请求画像与离线p95：", ""])
    for name, result in profile_rows:
        lines.append(f"- [{name}]({name}.md)：{result['summary']['requests']}条封存请求，零新增请求。")
    for title, rows in (("FLUX VAE解码器（已有阶段的细化，不重复累加）", decoder_rows),
                        ("官方基线与未训练架构变体", variant_rows)):
        lines.extend(["", title + "：", ""])
        for name, result in rows:
            lines.append(f"- [{name}]({name}.md)")
    lines.extend(["", "V4完整前缀状态之后的顺序已知token续算：", ""])
    for name, result in prefix_rows_v4:
        lines.append(f"- [{name}]({name}.md)：{result['summary']['forward_calls']}次完整基础前向；不是并行chunk prefill。")
    lines.extend(["", "Qwen235八rank逐张量与条件容量：", ""])
    for name, result in placement235_rows:
        lines.append(f"- [{name}]({name}.md)")
    lines.extend(["", "Dense三模型分片后量化与条件容量：", ""])
    for name, result in dense_lowbit_rows:
        lines.append(f"- [{name}]({name}.md)")
    lines.extend(["", "统一S/P/G/B的四模型完整请求逻辑预算：", ""])
    for name, result in request_comparison_rows:
        lines.append(f"- [{name}]({name}.md)")
    lines.extend(["", "Fish CLI代码块到波形导出：", ""])
    for name, result in fish_export_rows:
        lines.append(f"- [{name}]({name}.md)")
    lines.extend(["", "实验3-3严格协议与全部候选消耗：", ""])
    for name, result in strategy_cost_rows:
        lines.append(f"- [{name}]({name}.md)")
    lines.extend(["", "Qwen8训练非矩阵、AdamW与保存对象：", ""])
    for name, result in training_nonmatrix_rows:
        lines.append(f"- [{name}]({name}.md)")
    lines.extend(["", "真实C4训练点与预定留出拟合：", ""])
    for name, result in real_scaling_rows:
        lines.append(f"- [{name}]({name}.md)")
    lines.extend(["", "真实C4拟合的条件生命周期：", ""])
    for name, result in real_lifecycle_rows:
        lines.append(f"- [{name}]({name}.md)")
    lines.extend(["", "V4 router与mHC可微训练子图：", ""])
    for name, result in v4_training_rows:
        lines.append(f"- [{name}]({name}.md)")
    lines.extend(["", "Qwen8 GPipe/1F1B训练流水与激活寿命：", ""])
    for name, result in training_pipeline_rows:
        lines.append(f"- [{name}]({name}.md)")
    lines.extend(["", "RGB预处理至视觉编码器入口：", ""])
    for name, result in vision_preprocess_rows:
        lines.append(f"- [{name}]({name}.md)")
    lines.extend(["", "stage-resource-bounds：", ""])
    for name, result in stage_resource_bounds_rows:
        lines.append(f"- [{name}]({name}.md)")
    lines.extend(["", "v4-compressor-online：", ""])
    for name, result in v4_compressor_online_rows:
        lines.append(f"- [{name}]({name}.md)")
    lines.extend(["", "训练流水GEMM保存身份与重算：", ""])
    for name, result in training_gemm_rows:
        lines.append(f"- [{name}]({name}.md)")
    lines.extend(["", "V4固定选择MoE单层训练：", ""])
    for name, result in v4_moe_training_rows:
        lines.append(f"- [{name}]({name}.md)")
    lines.extend(["", "V4 Muon/AdamW优化器分组与逐矩阵计算：", ""])
    lines.extend(["", "Omni PCM到mel前处理：", ""])
    lines.extend(["", "training-input-supply：", ""])
    for name, result in training_input_supply_rows:
        lines.append(f"- [{name}]({name}.md)")
    lines.extend(["", "qwen235-execution：", ""])
    for name, result in qwen235_execution_rows:
        lines.append(f"- [{name}]({name}.md)")
    lines.extend(["", "Qwen235专家颗粒度：", ""])
    for name, result in qwen235_expert_granularity_rows:
        lines.append(f"- [{name}]({name}.md)")
    lines.extend(["", "架构矩阵与tile补齐：", ""])
    for name, result in architecture_tile_work_rows:
        lines.append(f"- [{name}]({name}.md)")
    lines.extend(["", "封存Chat轨迹到资源账：", ""])
    for name, result in trace_resource_bridge_rows:
        lines.append(f"- [{name}]({name}.md)")
    lines.extend(["", "四模型请求与精度匹配硬件供给：", ""])
    lines.extend(["", "权重/KV容量与带宽代际对照：", ""])
    lines.extend(["", "FA4单SM资源配比：", ""])
    lines.extend(["", "注意力输入槽与异步流水：", ""])
    lines.extend(["", "V4共享专家FP8复制坐标：", ""])
    lines.extend(["", "矩阵—向量交接与完整行依赖：", ""])
    lines.extend(["", "配对投影费用与功率条件：", ""])
    for name, result in paired_projection_rows:
        lines.append(f"- [{name}]({name}.md)")
    for name, result in matrix_vector_rows:
        lines.append(f"- [{name}]({name}.md)")
    for name, result in v4_copy_rows:
        lines.append(f"- [{name}]({name}.md)")
    for name, result in attention_input_rows:
        lines.append(f"- [{name}]({name}.md)")
    for name, result in fa4_resource_rows:
        lines.append(f"- [{name}]({name}.md)")
    for name, result in storage_generation_rows:
        lines.append(f"- [{name}]({name}.md)")
    for name, result in granularity_selection_rows:
        lines.append(f"- [{name}]({name}.md)")
    for name, result in trace_cache_lifecycle_rows:
        lines.append(f"- [{name}]({name}.md)")
    for name, result in v4_mtp_forward_rows:
        lines.append(f"- [{name}]({name}.md)")
    for name, result in request_hardware_bridge_rows:
        lines.append(f"- [{name}]({name}.md)")
    for name, result in omni_audio_preprocess_rows:
        lines.append(f"- [{name}]({name}.md)")
    for name, result in v4_optimizer_rows:
        lines.append(f"- [{name}]({name}.md)")
