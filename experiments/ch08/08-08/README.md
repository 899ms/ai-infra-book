...
> 当前大纲对应实验 8-6；本目录与原始记录沿用归档编号 8-8。见[编号映射](../../../research/ch08-reorganization-2026-09-10/README.md)。

固定Qwen3-8B的BF16权重与Triton attention，比较BF16 KV和FP8 E4M3 KV；分别保留自然结束的精确检索质量，以及固定输出长度的计时。模型原生状态结构没有改变。本节原始批次使用 BF16 权重；新增[量化权重变体](quantized-weights/README.md)固定在线 FP8 权重比较两种 KV，均为 28/32 但错误任务不同。两包 KV 预算不同，不能直接归因权重量化的性能差异。
...
[量化权重下的并发扫描](concurrency/README.md)完成538次调用；8GiB KV预算下，在已测档位BF16的7请求、FP8的15请求没有抢占，下一档8/16开始抢占。自然答案98/132与90/132，容量收益不代表质量保持；部分重复输出不同。保存逐步调度与完整任务记录，尚不构成通用SLO容量结论。

## 量化权重下的 Q 精度控制

[新增控制](quantized-q-control/README.md)复用两组原始参考，只新增97次调用；固定相同FP8权重与KV scales、保留BF16 Q后自然答案32/32，默认路径28/32。实际36层Q类型与参数SHA验证通过。仅八个独立任务，不推广为通用质量保持。

## decode attention实际访存

[NCU硬件计数](memory-counters/README.md)采固定量化权重下首层decode attention主体：BF16/FP8 KV的DRAM读取29.67/14.85MB，L2请求30.47/15.62MB。只采单个kernel、每格式一次，未含写缓存/Q量化/归约/其他层，不外推全模型流量或稳定加速。

## 实际重试成本

[新文档与回归题](retry-cost/README.md)分开实跑：新16题两轮32/32首答正确；已知4题两轮6/8首答正确，两次BF16 Q重试后8/8，完整任务约10.01秒，含错误首答和两次重试的92个额外token。已知标准答案校验器，不当作开放业务的免费判断。
