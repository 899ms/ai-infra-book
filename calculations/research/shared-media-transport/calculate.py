...
def scenarios():
    ...
    for scheduler in ("fifo", "priority"):
        ...
        p["businesses"] = [
            {"id": "image", "kind": "image", "required": [f"B{i}" for i in range(4)]},
            {"id": "audio", "kind": "asr", "required": ["A0"]},
        ]
        rows["schedule-" + scheduler] = p
    for streams in (1, 3):
        p = example()
        ...
        p["packets"] = [
            {
                "id": f"p{i}",
                "stream": f"s{i%streams}",
                "direction": "c2s",
                "offset": i if streams == 1 else 0,
                "payload_bytes": 1,
            }
            for i in range(3)
        ]
        p["businesses"] = []
        rows[f"credit-{streams}-streams"] = p
    for playback in ("reliable", "slots"):
        p = example()
        p["packets"] = [
            {
                "id": f"a{i}",
        ...
        p["businesses"] = [
            {
                ...
                "playback": playback,
                "blocks": [
                    {"packets": [f"a{i}"], "duration": 2, "slot_start": 3 + 2 * i}
                    for i in range(3)
                ],
            }
        ]
        rows["playback-" + playback] = p
    p = example()
    p["packets"] = [
        {
            "id": "up",
    ...
    p["tasks"] = [
        {"id": "infer", "dependencies": ["up"], "duration": 3, "cancel_tag": "screen"}
    ]
    p["businesses"] = [
        {
            "id": "screen",
    ...
    rows["cancel-running-work"] = p
    for scheduler in ("fifo", "priority"):
        p = mixed_example()
        p["scheduler"] = scheduler
        rows["mixed-" + scheduler] = p
    p = mixed_example()
    p["packets"] = [
        x for x in p["packets"] if x["id"].startswith(("image-up-", "image-final-"))
    ]
    p["tasks"] = [x for x in p["tasks"] if x["id"] == "image-model"]
    p["businesses"] = [x for x in p["businesses"] if x["kind"] == "image"]
    rows["image-30mb-5mb"] = p
    for stale in (False, True):
        p = mixed_example()
        p["packets"] = [
            x for x in p["packets"] if x["id"] in ("screenshot-v1", "screen-action")
        ]
        p["tasks"] = [x for x in p["tasks"] if x["id"] == "screen-infer"]
        p["businesses"] = [x for x in p["businesses"] if x["kind"] == "screenshot"]
        if not stale:
            p["businesses"][0]["version_changes"] = []
        rows["screenshot-stale" if stale else "screenshot-complete"] = p
    p = mixed_example()
    p["scheduler"] = "priority"
    upload = [x["id"] for x in p["packets"] if x["id"].startswith("image-up-")]
    p["tasks"].insert(
        0, {"id": "preview-model", "dependencies": upload, "duration": "0.05"}
    )
    preview = []
    for offset in range(0, 500000, 25000):
        name = f"preview-{offset//25000}"
        preview.append(name)
        p["packets"].append(
            {
                "id": name,
    ...
    for b in p["businesses"]:
        if b["kind"] == "image":
            b.update(
                ...
                preview_quality="declared 500kB preview; no equivalence to final image quality",
            )
    rows["mixed-preview-priority"] = p
    p = example()
    p["packets"] = [
        {
            "id": "lost-unit",
    ...
    p["shared_credit_bytes"] = 1
    p["businesses"] = []
    rows["unreliable-loss-credit"] = p
    return rows
...
def mixed_example():
    ...
    def blocks(prefix, stream, direction, total, chunk, **kwargs):
        names = []
        for offset in range(0, total, chunk):
            name = f"{prefix}-{offset//chunk}"
            names.append(name)
            p["packets"].append(
                dict(
                    id=name,
                    ...
                    direction=direction,
                    offset=offset,
                    payload_bytes=min(chunk, total - offset),
                    **kwargs,
                )
            )
        return names

    upload = blocks("image-up", "image-upload", "c2s", 30000000, 25000)
    p["tasks"].append({"id": "image-model", "dependencies": upload, "duration": "0.3"})
    final = blocks(
        "image-final",
        "image-final",
        "s2c",
        5000000,
        25000,
        dependencies=["image-model"],
    )
    p["businesses"].append({"id": "image", "kind": "image", "required": final})
    asr = []
    for i in range(8):
        name = f"asr-{i}"
        asr.append(name)
    ...
    p["tasks"].append(
        {"id": "asr-model", "dependencies": asr, "duration": "0.05", "priority": 100}
    )
    p["packets"].append(
        {
            "id": "asr-text",
            ...
            "offset": 0,
            "payload_bytes": 64,
            "dependencies": ["asr-model"],
            "priority": 100,
        }
    )
    p["businesses"].append({"id": "asr", "kind": "asr", "required": ["asr-text"]})
    tts = []
    for i in range(8):
        task = f"tts-produce-{i}"
        name = f"tts-{i}"
        p["tasks"].append(
            {
                "id": task,
                "dependencies": ["asr-model" if i == 0 else f"tts-produce-{i-1}"],
                "duration": "0.012",
                "priority": 100,
            }
        )
        ...
        tts.append(
            {
                "packets": [name],
                "duration": "0.02",
                "slot_start": str(F(2, 5) + F(i, 50)),
            }
        )
    p["businesses"].append(
        {"id": "tts", "kind": "tts", "playback": "slots", "blocks": tts}
    )
    p["packets"].append(
        {
            "id": "screenshot-v1",
    ...
    p["businesses"].append(
        {
            ...
            "version": "v1",
            "version_changes": [{"at": "0.08", "version": "v2"}],
        }
    )
    return p
