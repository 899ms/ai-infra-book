...
def main():
    ...
    save(O/'environment.json',dict(runid=runid,base_image_id=base,
        ...
        source_hashes={n:hashlib.sha256((R/n).read_bytes()).hexdigest() for n in ['run_same_mutated.py','worker_unix.py','PROTOCOL.md']}))
    ...
    try:
        for trial,path in order:
            ...
            if path=='pause':
                ...
                mutation_stream,_=connect(cid)
                row['same_before']=call(mutation_stream,'add',value=7)
                mutation_stream.close()
                time.sleep(.2)
                row['same_capture']=docker('checkpoint','create',cid,'same',check=False)
                row['same_restore']=docker('start','--checkpoint','same',cid,check=False)
                if row['same_restore']['exit_code']!=0:
                    row['fallback_cold_start']=docker('start',cid,check=False)
        ...
        save(O/'completion.json',dict(status='same_container_diagnostic_recorded',count=len(rows),runid=runid))
    finally:
