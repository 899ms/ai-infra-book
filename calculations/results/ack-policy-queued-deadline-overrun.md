...
截至 horizon 实际串行线上字节：**2640 B**；已开始包的完整声明字节可能包含截止后尚未发完部分。接收唯一字节：**2336 B**。
...
| up | 7 | ack | `{"ranges": [[0, 1]], "ack_delay": "1", "app_limited": true, "flow_limited": false}` |
...
{
  "wire_bytes": 2640,
  "serialized_wire_bytes_by_horizon": "2640",
...
声明策略：`{"mode": "count_or_timer", "every": 1, "max_delay": "1/2"}`。
