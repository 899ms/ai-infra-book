...
| down | 368 | 1 | 2 | 128 | 56 | 184 |

截至 horizon 实际串行线上字节：**3868 B**；已开始包的完整声明字节可能包含截止后尚未发完部分。接收唯一字节：**3504 B**。
...
| down/0 | ack | 控制 | 2 | 4 | 5 | 5 | 92 | False | None | —/— |
| up/2 | data | business:[2336,3504) | 2 | 3 | 4 | 4 | 1228 | False | None | —/— |
| down/1 | ack | 控制 | 4 | 6 | 7 | 7 | 92 | False | None | —/— |
...
| 4 | application_prefix | up | `{"prefix": 3504}` |

## 发送方实际反馈与等待

| 方向 | 时刻 s | 事件 | 已知反馈 |
|---|---|---|---|
| up | 5 | ack | `{"ranges": [[0, 0]], "ack_delay": "0", "app_limited": true, "flow_limited": false}` |
| up | 7 | ack | `{"ranges": [[0, 2]], "ack_delay": "0", "app_limited": true, "flow_limited": false}` |
...
声明策略：`{"mode": "count_or_timer", "every": 1, "max_delay": 10}`。
...
|---|---|---|---|---|---:|---|
| down/0 | 2 | [[0, 0]] | 0 | 0 | 5 | False |
| down/1 | 4 | [[0, 2]] | 0 | 0 | 5 | False |
