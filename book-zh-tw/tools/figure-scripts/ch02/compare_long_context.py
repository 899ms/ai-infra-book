...
table=["| 模型 | 8K 上下文互動（GFLOPs） | 1M 上下文互動（GFLOPs） | 8K 狀態（GiB） | 1M 狀態（GiB） |",'| --- | ---: | ---: | ---: | ---: |']
...
request=["| 模型 | 完整請求矩陣運算量（TFLOPs） | 本次計算採用的實作方式 |",'| --- | ---: | --- |']
paths=["CED＋末尾視窗重放，候選集內索引","有效因果注意力","矩形全注意力與塊式線性分支","有效主注意力與參考索引","緊湊 MLA 與塊式 KDA；decode 採用遞推 KDA"]
