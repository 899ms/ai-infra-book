...
async def main(a):
 ...
 cfg=dict(model=a.model,dtype='bfloat16',kv_cache_dtype='auto',max_model_len=8192,max_num_seqs=4,max_num_batched_tokens=2048,enable_chunked_prefill=True,enable_prefix_caching=a.apc,enforce_eager=True,async_scheduling=False,gpu_memory_utilization=.30,kv_cache_memory_bytes=2*1024**3,seed=302,attention_backend='TRITON_ATTN')
 ...
 state={'active':[]};stats=(a.out/'scheduler.jsonl').open('w',buffering=1)
 class Logger(StatLoggerBase):
  ...
  def record(self,scheduler_stats,iteration_stats,mm_cache_stats=None,engine_idx=0):
   stats.write(json.dumps(dict(time=time.monotonic(),active=list(state['active']),running=scheduler_stats.num_running_reqs,waiting=scheduler_stats.num_waiting_reqs,kv_usage=scheduler_stats.kv_cache_usage,preemptions=getattr(iteration_stats,'num_preempted_reqs',0) if iteration_stats else 0))+'\n')
 engine=None
 try:
  ...
  with (a.out/'requests.jsonl').open('w',buffering=1) as log:
   async def one(task,turn):
     ...
     rid=f'c{client}-t{turn}';state['active'].append(rid);start=time.monotonic();events=[];final=None
     ...
     log.write(json.dumps(row)+'\n');history.append(dict(role='assistant',content=seq.text));state['active'].remove(rid);print(rid,row['cached_tokens'],len(seq.token_ids),flush=True)
   for turn in range(3):
    await asyncio.gather(*(one(task,turn) for task in tasks))
