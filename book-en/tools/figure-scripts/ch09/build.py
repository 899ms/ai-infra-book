...
f,a=canvas(8)
label(a,.025,.96,"multi-card collaboration: one instance jointly handles a batch of requests")
label(a,.53,.96,"full replica: each receives independent requests")
box(a,.025,.59,.44,.27,"unified scheduling with full model","eight cards collaborate on execution",col='pale')
for i in range(8):
 x=.045+(i%4)*.10;y=.61+(i//4)*.06
 a.add_patch(Rectangle((x,y),.075,.035,fc=C['blue']))
for x in [.54,.78]:
 box(a,x,.62,.19,.19,"full replica","weights, KV, scheduling",col='green',fs=12)
 a.text(x+.095,.88,"request",ha='center',fontsize=11);arrow(a,(x+.095,.85),(x+.095,.815))
label(a,.025,.49,"PD: division by generation stage")
box(a,.025,.16,.18,.18,"P service pool","process input",col='pale')
box(a,.29,.16,.18,.18,"D service pool","stepwise generation",col='green')
arrow(a,(.21,.25),(.285,.25));a.text(.245,.36,"context KV",ha='center',fontsize=11)
label(a,.53,.49,"AF: division by intra-layer operator")
box(a,.54,.16,.19,.18,'Attention',"attention computation",col='pale')
box(a,.79,.16,.19,.18,"FFN/expert","feedforward computation",col='green')
arrow(a,(.735,.29),(.785,.29));arrow(a,(.785,.20),(.735,.20))
a.text(.76,.37,"activation round trip",ha='center',fontsize=11)
a.text(.035,.06,"different stages can queue and batch separately",fontsize=11)
a.text(.54,.06,"same request passes through both sides each layer",fontsize=11)
...
f,a=canvas(6)
stages=['P','D',"tool waiting","next round P"];edges=[.19,.37,.56,.78,.96]
...
rows=[("weights",.72,.19,.96,'blue'),("input KV",.54,.23,.96,'orange'),("generated KV",.36,.40,.96,'orange'),("EC (optional)",.18,.19,.96,'teal')]
...
a.text(.67,.045,"computation can pause, state stays resident",ha='center',fontsize=12)
...
ax.set(xticks=range(5),yticks=range(5),xlabel="A100 count for P",ylabel="H20 count for P")
cax=f.add_axes([.84,.15,.025,.73]);f.colorbar(im,cax=cax).set_label("request rate bound / req·s⁻¹")
save(f,'figure-9-3-pd')
# Local CPU/GPU execution.
f,a=canvas(8)
label(a,.025,.96,"weight handoff: GPU executes expert")
box(a,.03,.70,.25,.14,'CPU DRAM',"expert weights",col='sand');box(a,.39,.70,.24,.14,"GPU weight buffer","wait for payload ready");box(a,.75,.70,.22,.14,"GPU expert computation","output activation",col='green');arrow(a,(.285,.77),(.38,.77),'orange');arrow(a,(.635,.77),(.74,.77));a.text(.335,.86,'W',ha='center',color=C['orange'])
label(a,.025,.56,"activation handoff: CPU computes expert in place")
box(a,.03,.30,.25,.15,'GPU attention',"input activation X");box(a,.39,.30,.24,.15,"CPU expert computation","local weight read",col='sand');box(a,.75,.30,.22,.15,"synchronous merge","continue to next layer",col='green');arrow(a,(.285,.375),(.38,.375));arrow(a,(.635,.375),(.74,.375));a.text(.335,.465,'X',ha='center',color=C['teal']);a.text(.69,.465,'Y',ha='center',color=C['teal'])
box(a,.39,.04,.24,.14,"GPU-resident expert","independent branch executable");arrow(a,(.17,.295),(.385,.11),rad=.15);arrow(a,(.635,.11),(.86,.295),rad=.15)
...
for messages,col in [(1,'blue'),(72,'orange')]:ax.plot(alpha,base+messages*alpha/1000,color=C[col],lw=2.3,label=f'{messages} messages')
ax.set(xlabel="per launch and sync / μs",ylabel="serial handoff time / ms",xlim=(0,20),ylim=(48.2,50),xticks=[0,5,10,15,20])
ax.annotate("difference ~0.36 ms",xy=(5,base+.360),xytext=(7,49.0),arrowprops={'arrowstyle':'->','color':C['muted']},fontsize=12)
...
ax.plot(m,cpu,color=C['teal'],lw=2.3,label="CPU in-place compute, AVX-512");ax.plot(m,amx,color=C['blue'],lw=2.3,label="CPU in-place compute, AMX");ax.plot(m,gpu,color=C['orange'],lw=2.3,label="move weights once + GPU compute")
ax.set_xscale('log',base=2);ax.set(xlabel="tokens per hot expert",ylabel="sequential execution time of eight experts / ms",xlim=(1,1024),ylim=(0,30),xticks=[1,4,16,64,256,1024],xticklabels=['1','4','16','64','256','1024']);ax.minorticks_off();ax.legend(frameon=False);ax.grid(alpha=.15)
...
net={}
for key,col,title,dy in [('nvlink','teal',"within same HGX via NVLink",4),('cx7','orange',"cross-server via ConnectX-7",-4)]:
 setup=num(zr[key]['serialized_copy_setup_ns_exact'])/1e6;k=zr[key]['algebraic_strict_payback_batches'];assert k==int(setup//delta)+1
 net[key]=(n*delta-setup).tolist();ax.plot(n,n*delta-setup,color=C[col],lw=2.3,label=f'{title}: preparing {setup:.2f} ms')
 ax.scatter([k],[k*delta-setup],color=C[col],zorder=3);ax.annotate(f'Step {k} batches to break even',(k,k*delta-setup),xytext=(k+4,k*delta-setup+dy),fontsize=12,arrowprops={'arrowstyle':'->'})
ax.axhline(0,color=C['line']);ax.set(xlabel="batches hotspot persists",ylabel="cumulative net savings / ms",xlim=(0,64),ylim=(-7,17),yticks=[-5,0,5,10,15],xticks=[0,16,32,48,64]);ax.legend(frameon=False,loc='upper left');ax.grid(alpha=.15)
...
a.text(.50,.965,"read 64 pages",ha='center',fontsize=15,weight='bold')
a.text(.10,.11,"first 63 pages: 1008 positions reusable",color=C['teal'],fontsize=13)
a.text(.62,.11,"last page: 16 positions still needed",color=C['orange'],fontsize=13)
...
f,a=canvas(7)
box(a,.025,.71,.21,.16,'P',"generate context KV");box(a,.755,.71,.21,.16,'D',"read context KV",col='green');arrow(a,(.245,.79),(.745,.79),'orange');a.text(.50,.86,'1.125 GiB',ha='center',color=C['orange'],fontsize=12)
a.text(.025,.96,"direct handoff",fontsize=14,weight='bold')
box(a,.025,.36,.21,.16,'P',"generate context KV");box(a,.39,.36,.21,.16,"shared pool","save then publish",col='sand');box(a,.755,.36,.21,.16,'D',"read context KV",col='green')
for x in [.245,.61]:arrow(a,(x,.44),(x+.135,.44),'orange');a.text(x+.065,.55,'1.125 GiB',ha='center',color=C['orange'],fontsize=11)
a.text(.025,.62,"relay via pool",fontsize=14,weight='bold')
box(a,.755,.055,.21,.14,"subsequent instance","retrieve instead of recompute",col='green');arrow(a,(.50,.35),(.75,.125),'teal',rad=.12);a.text(.43,.15,"reuse same object again",fontsize=12,color=C['teal'])
...
for mu,col,label in [(mu_pd,'teal',"direct PD"),(mu_ch,'blue',"ideal chunked co-location"),(mu_co,'orange',"unchunked co-location")]:
 q=np.where(tt<=10,lam*tt,np.maximum(0,lam*10-(mu-lam)*(tt-10)));series[f'{mu:.2f}']=q.tolist();ax.plot(tt,q,label=f'{label}：{mu:.2f} req/s',color=C[col],lw=2.3)
 if mu>lam:drain.append(10+lam*10/(mu-lam))
ax.axvline(10,color=C['line'],ls='--');ax.axvline(60,color=C['red'],ls=':');ax.text(11,50,"ready: 35 requests backlog",fontsize=12);ax.text(61,50,"deadline 60 s",fontsize=12,color=C['red'])
ax.set(xlabel="time since startup / s",ylabel="backlog request count",ylim=(-2,70),xlim=(0,80),xticks=[0,10,20,30,40,50,60,70,80]);ax.legend(frameon=False,fontsize=11);ax.grid(alpha=.15)
...
page="<!doctype html><html lang=\"en\"><head><meta charset=\"utf-8\"><meta name=\"viewport\" content=\"width=device-width,initial-scale=1\"><title>Chapter 9 Distributed Inference</title><style>"+css+math_css+"</style></head><body><main><nav aria-label=\"Chapter contents\">"+nav+'</nav>'+body+'</main></body></html>'
import sys
sys.path.insert(0,str(HERE.parent))
