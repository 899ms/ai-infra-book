...
class H3(QuicConnectionProtocol):
 ...
 async def reply(self,stream,record):
  ...
  self.http.send_headers(stream,[(b':status',b'200'),(b'content-length',str(len(RESPONSE)).encode()),(b'content-type',b'text/plain; charset=utf-8')])
 ...
 async def h1_server(reader,writer):
  ...
  try:
   while True:
    ...
    record['service_start']=NOW();await asyncio.sleep(SERVICE_S);record['service_end']=NOW()
    writer.write(conn.send(h11.Response(status_code=200,headers=[(b'content-length',str(len(RESPONSE)).encode()),(b'content-type',b'text/plain; charset=utf-8')])))
