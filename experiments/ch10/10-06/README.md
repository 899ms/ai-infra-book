...
[预取与packing恢复](input-state-recovery/README.md)已补真实文本管线、非空残留、终止后重读及两个漏状态负对照。[改变worker数量](worker-count-recovery/README.md)进一步从2个worker恢复为1或4个：12次恢复、420更新和最终完整状态均与原基线精确一致，派发游标随预取数量改变而消费游标不变。确定性顺序读取成立，不代表随机增强或大模型I/O争用已验证。

## 输入与写入共存补测

[真实读取和检查点写入](input-io-contention/README.md)完成无保存/同步/异步三路径三轮，576更新和18检查点全状态相同；异步记录到读取与写入重叠，但首次worker就绪占据大部分输入获取时间，小负载未建立稳定存储争用结论。窗口中位709/731/704ms，不据此外推大模型带宽。
