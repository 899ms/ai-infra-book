...
meta=json.loads((r/'reference-provenance.json').read_text());assert hashlib.sha256((r/'reference-expert.pt').read_bytes()).hexdigest()==meta['artifact_sha256']
p=torch.load(r/'reference-expert.pt',map_location='cpu',weights_only=True)
...
results=[]
for m in [1,8,64,512]:
 f=torch.load(r/f'matrix-results/fixture-m{m}.pt',map_location='cpu',weights_only=True)
 assert torch.equal(f['w'],reference) and torch.equal(f['qw'],p['int8_weight']) and torch.equal(f['sw'],p['int8_scale'])
 x=f['x'];assert torch.equal(x,torch.load(r/'activation-pool.pt',map_location='cpu',weights_only=True)['input_bf16'][:m].float());ref64=x.double()@reference.double()
 assert torch.allclose(f['reference'].double(),ref64,atol=2e-5,rtol=2e-5)
 checks={}
 for mode in ['int8_direct','int8_dequant_bf16']:
  y=torch.load(r/f'matrix-results/output-m{m}-{mode}.pt',map_location='cpu',weights_only=True)
...
(r/'matrix-audit.json').write_text(json.dumps(dict(blocks_checked=192,source_tensor_hashes=True,all_shapes_same_weights=True,cpu_reference='FP64 matmul of actual routed input and decoded checkpoint matrix',rows=results),indent=2)+'\n');print(json.dumps(results))
