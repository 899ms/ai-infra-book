...
降低 KV 位宽也能释放容量。第二章的 DeepSeek V4.1-Flash 同时使用全局历史与滑动窗口注意力（SWA，保留最近一段上下文）。一条主 KV 记录的 512 个值采用 FP4，连同分组缩放因子共占 288 bytes；局部窗口的一条记录采用 FP8，连同缩放因子共占 528 bytes。主 KV 在参与注意力乘法前反量化，即利用缩放因子恢复为计算所用的数值格式。存储格式压缩了常驻状态，执行时再恢复操作数。[^v41-case]
...
NVLink、昇腾 Unified Bus 等互联提供设备间的传输路径，完成通知让接收方在数据到达后开始计算。[^package]

> **思考：合并消息如何改变完成时间与首条响应？** 四条 8 KiB 消息依次在 0、1、2、3 μs 就绪。仍取 100 GB/s、每次启动 2 μs，在同一链路上比较就绪后分别发送与等齐后合并发送：所有消息何时送达？第一条消息又何时可供接收方使用？
...
沿用第 4.2.4 节每 32 个值共享一字节 scale 的分块格式；MX 表示这种微缩放表示。MXFP8 需要 $16+0.5=16.5$ MiB，MXFP4 需要 $8+0.5=8.5$ MiB。RTX 4090 的 1,008 GB/s 带宽读取这份 BF16 权重需 33.29 μs；RTX 5090 的 1,792 GB/s 读取同一份 BF16 权重需 18.72 μs，读取 MXFP4 权重则需 4.97 μs。前一步来自带宽提高，后一步来自位数与分组表示改变。
...
用同一个注意力块计算接口工作量。分数与概率各按 FP32 交接，每个张量占 64 KiB。一次交接经过外层交换接口写出、读入，共 128 KiB；QK→Softmax 和 Softmax→PV 两次合计 256 KiB。若两次交接都由同一外层接口承担，要在 1.024 μs 内完成，该接口需要约 256 GB/s。把早期论文中每核分配的 94 GB/s 末级缓存（LLC）带宽代入，这笔工作需要约 2.79 μs，已经超过 1 GHz 下 Cube 的 1.024 μs 矩阵服务时间。矩阵计算即使继续加快，外层接口仍需搬完这些字节。

950 在 Cube 的一级缓冲 L1 与 Vector 的统一缓冲 UB 之间增加 CV（Cube–Vector）直接通路。两次各 64 KiB 的中间张量通过直接连接交接，共传递 128 KiB，原来外层接口的 256 KiB 写读由此移出。若要求这条直接通路在 1.024 μs 内完成两次交接，需要约 128 GB/s；若交接格式改为 FP16／BF16，需求再减半至约 64 GB/s。
...
950 同时将单个 Vector Core 的 FP16／FP32 吞吐提高一倍，在 UB 与向量算术逻辑单元（ALU）之间增加寄存器文件，并优化 Softmax、GELU 的执行。对相同普通向量工作 $F_v$，$F_v/P_v$ 降为 $F_v/(2P_v)$；如果原来向量阶段是矩阵阶段的两倍，这项改动能把二者重新配平。前面的注意力块进一步把指数吞吐需求确定为每周期 16 个结果。新增 NDDMA 承担多维地址与布局准备，BufferID 用缓冲区标识组织同步交接。[^ascend]
...
从 5 到 36 再到 144，增加的是可写状态，模型权重并未增加。片上保存权重减少了一类读取，但持续服务还需要为增长的状态提供容量。增加加速器后，分布式 KV 又带来跨加速器数据汇集；容量扩展与通信需求由同一工作负载同时产生。
...
作者的 OpenTallas 项目研究将不可变权重放入掩模 ROM 的架构；这种只读存储器在制造时固定存储内容。下面以项目中的 Qwen3-8B 为例，通过公式估算说明“从独立只读存储器（ROM）读取权重、从 HBM 读写 KV”的设计怎样改变各接口的传输量。[^opentallas] 每批只读取一次本步计算所需的权重，约 $W=15.1$ GB；每条请求完整读取 8,192 个 token 的 KV，约 $K=1.21$ GB。完整驻留权重约 16.4 GB，每步读取其中参与计算的部分。
...
同一个 100 μs 时限也可以用来求计算单元数量。沿用项目历史执行产物中的一组张量工作，共 15,134,641,792 次操作。设每条运算通道（lane）在 1 GHz 下每周期完成一次乘加（FMA），100 μs 内所需的运算通道数至少为
...
多卡运行的张量切分与专家分派在第六章展开。下面用运行记录把单设备的资源预算换算为任务时间。
...
加速器分析从模型工作量出发：用状态大小筛选容量，用流量与带宽计算读取时间，用矩阵形状、精度与指令速率计算运算时间，再沿依赖安排执行。三家架构的演进展示了模型需求怎样改变资源配比。访问计数、内核记录与计时将这些预算落实为运行速度和任务成本。第五章继续研究布局、融合与调度，说明软件怎样用好这些硬件资源。
