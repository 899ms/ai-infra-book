<!-- 从 vllm-attention-doc.html 迁移的资料快照；原始 HTML SHA-256: 0403b2c95ea8695ed77d6408a1b73269d913c92e47f6e1eaf7d98d06a19faf5f。 -->
...
Attention Backend Feature Support [ Attention Backend Feature Support ](./)

Table of contents

- [ Setting the Attention Backend ](#setting-the-attention-backend)
  - [ Command Line ](#command-line)
  - [ Python API ](#python-api)
- [ Backend Selection Behavior ](#backend-selection-behavior)
  - [ Manual Selection ](#manual-selection)
  - [ Automatic Selection ](#automatic-selection)
- [ Backend Priority (CUDA) ](#backend-priority-cuda)
  - [ Standard Attention (MHA, MQA, GQA) ](#standard-attention-mha-mqa-gqa)
...
- [ Standard Attention (MHA, MQA, GQA) Backends ](#standard-attention-mha-mqa-gqa-backends)
  - [ b12x ](#b12x)
- [ MiniMax M3 Sparse Attention Backends ](#minimax-m3-sparse-attention-backends)
- [ MLA (Multi-head Latent Attention) Backends ](#mla-multi-head-latent-attention-backends)
  ...
  - [ DeepSeek V4 Decode Backends ](#deepseek-v4-decode-backends)
...
[![](data:image/svg+xml;base64,)](https://github.com/vllm-project/vllm/edit/main/docs/design/attention_backends.md "Edit this page")

# Attention Backend Feature Support[¶](#attention-backend-feature-support "Permanent link")

The priority and feature tables on this page are auto-generated from the attention backend registry by `docs/mkdocs/gen_files/generate_attention_backends.py`, based on the checks in `AttentionBackend.validate_configuration()`.

## Setting the Attention Backend[¶](#setting-the-attention-backend "Permanent link")
...
Use [`AttentionConfig`](../../api/vllm/config/attention/#vllm.config.attention.AttentionConfig "            AttentionConfig") with the [`LLM`](../../api/vllm/entrypoints/llm/#vllm.entrypoints.llm.LLM "            LLM") class:
...
## Backend Selection Behavior[¶](#backend-selection-behavior "Permanent link")

### Manual Selection[¶](#manual-selection "Permanent link")

When you explicitly set a backend via `--attention-backend` or [`AttentionConfig`](../../api/vllm/config/attention/#vllm.config.attention.AttentionConfig "            AttentionConfig"):
...
### Automatic Selection[¶](#automatic-selection "Permanent link")
...
## Backend Priority (CUDA)[¶](#backend-priority-cuda "Permanent link")
...
### Standard Attention (MHA, MQA, GQA)[¶](#standard-attention-mha-mqa-gqa "Permanent link")
...
### MLA Attention (DeepSeek-style)[¶](#mla-attention-deepseek-style "Permanent link")
...
## Standard Attention (MHA, MQA, GQA) Backends[¶](#standard-attention-mha-mqa-gqa-backends "Permanent link")
...
>
> On Blackwell, when the FlashAttention backend is selected, `head_size=256` is served by a dedicated FA4 kernel that requires a KV cache block size of 128 (advertised automatically, so it is picked as long as `--block-size` is not pinned) and does not support logit soft capping, attention sinks, mm_prefix/R-SWA masking, DCP, or windowed encoder attention. Those configurations transparently fall back to FA2. A pinned `--block-size` that is not a multiple of 128 instead makes FlashAttention ineligible for such models, which is an error if the backend was requested explicitly.

### b12x[¶](#b12x "Permanent link")
...
## MiniMax M3 Sparse Attention Backends[¶](#minimax-m3-sparse-attention-backends "Permanent link")
...
## MLA (Multi-head Latent Attention) Backends[¶](#mla-multi-head-latent-attention-backends "Permanent link")

MLA uses separate backends for prefill and decode phases.

### Prefill Backends[¶](#prefill-backends "Permanent link")

To explicitly select a prefill backend, use `-ac.mla_prefill_backend=<BACKEND>` (e.g., `FLASH_ATTN`, `FLASHINFER`). Otherwise, the prefill backend is selected automatically at runtime based on hardware and configuration.
...
### Decode Backends[¶](#decode-backends "Permanent link")

MLA decode backends are selected using the standard `-ac.backend=<BACKEND>` argument (e.g., `FLASHMLA`, `TRITON_MLA`).
...
| `FLASHMLA_SPARSE` | bf16 | `auto`, `bfloat16`, `fp8_ds_mla`, `nvfp4_ds_mla` | 64 | 576 | ❌ | ❌ | ✅ | ❌ | ✅ | Decoder | 9.x-10.x |
...
### DeepSeek V4 Decode Backends[¶](#deepseek-v4-decode-backends "Permanent link")

DeepSeek V4 sparse MLA uses its own decode backends, selected via `--attention-backend=<BACKEND>` (e.g., `FLASHMLA_SPARSE_DSV4`, `FLASHINFER_MLA_SPARSE_DSV4`). They share the V4 sparse-index pipeline (compressor + SWA + indexer, 256-token blocks, head 512); default on NVIDIA is `FLASHINFER_MLA_SPARSE_DSV4` on SM12x and `FLASHMLA_SPARSE_DSV4` on other supported CUDA architectures.
