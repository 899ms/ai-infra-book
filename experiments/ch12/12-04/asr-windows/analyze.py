...
R=Path(__file__).resolve().parent;O=R/'results';order=json.loads((O/'order.json').read_text());offsets=json.loads((R/'tcp-offsets.json').read_text());F=json.loads((R.parent/'speech-records/asr-fixture.json').read_text())
...
for c in order:
 ...
 ci=json.loads((O/f'{name}-container.json').read_text())[0];hc=ci['HostConfig'];assert ci['Image']==image['Id'] and hc['NetworkMode']=='none' and hc['NanoCpus']==2000000000 and hc['Memory']==2147483648 and 'NET_ADMIN' in [cap.removeprefix('CAP_') for cap in hc['CapAdd']];assert (O/f'{name}-removed.txt').read_text().strip()=='book-asrwindow1204'
