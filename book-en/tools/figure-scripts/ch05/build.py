...
f,a=canvas(8.5)
a.text(.04,.965,"save full matrix first",fontsize=17,weight='bold')
a.text(.54,.965,"compute and activate block by block",fontsize=17,weight='bold')
box(a,.035,.56,.405,.34,'',color='pale')
a.text(.06,.865,'for i, j:\n    acc = 0\n    for k:\n        acc += A[i,k] × W[k,j]\n    C[i,j] = BF16(acc)',va='top',fontsize=13,linespacing=1.6)
box(a,.035,.36,.405,.12,"full C: 24 MiB","write once, read once",'sand',14)
arrow(a,(.24,.55),(.24,.49))
...
a.text(.55,.865,"for io, jo: 16 × 192 output blocks",fontsize=13)
...
a.text(.595,.63,"for ko: 128 reduction blocks",fontsize=13)
a.text(.595,.55,"A block 64 × 32: 4 KiB\nW block 32 × 64: 4 KiB",fontsize=12,va='top',linespacing=1.7)
a.text(.595,.395,"acc += A block × W block",fontsize=13)
a.add_patch(Rectangle((.555,.155),.38,.12,facecolor=C['light'],edgecolor=C['teal']))
a.text(.575,.235,"c = BF16(acc)\nwrite SiLU(c)",fontsize=13,va='top',linespacing=1.6)
arrow(a,(.745,.35),(.745,.28))
a.text(.76,.315,"reduction done",fontsize=10,color=C['teal'])
a.text(.04,.025,"C write and read: 48 MiB",fontsize=13,color=C['orange'])
a.text(.54,.025,"input slot updates with ko; acc persists across ko",fontsize=12,color=C['blue'])
...
p=np.linspace(0,1,301);a.plot(p*100,np.full_like(p,10),lw=2.4,color=C['blue'],label="original implementation: 10 μs")
a.plot(p*100,20-15*p,lw=2.4,color=C['teal'],label="new implementation: 20 − 15p μs")
a.axvline(200/3,color=C['orange'],ls='--');a.text(200/3+2,19,"A share 2/3",color=C['orange'],fontsize=11)
for x,y,label,offset in [(50,12.5,"each occurs once: new implementation 25% slower",(-160,65)),(10000/101,520/101,"A ×100, B ×1: new implementation ~1.9x faster",(-260,-30))]:
...
a.set_xticks(range(0,101,20));a.set(xlim=(0,103),ylim=(0,22),xlabel="Call share of shape A / %",ylabel="Average time per call / μs");a.legend(frameon=False,loc='lower left')
...
f,a=plt.subplots(figsize=(12,5.5));f.subplots_adjust(left=.10,bottom=.17,right=.95,top=.93);rs=np.arange(1,151);labels={'generic':"generic",'bucket':"bucketed",'specialized':"per-shape specialization"}
...
a.set_xticks(range(0,151,25));a.set(xlabel="repeat count for 10-call group",ylabel="Preparation + cumulative execution / ms",xlim=(0,150),ylim=(0,2850));a.legend(frameon=False);a.text(14,2600,"1–64: generic",fontsize=11);a.text(66,2600,"65–89: bucketed",fontsize=11);a.text(102,2600,"90+: specialized",fontsize=11)
...
for a,z,label in zip(axs,ranges,["normal submission","fusion","graph replay","fusion + graph replay"]):
 ...
 a.set_yticks([.28,.78],["device kernel","host launch"]);a.set_ylim(0,1.05);a.set_xlim(0,900);a.set_xticks(range(0,901,150));a.set_xlabel("Relative to own capture start / μs",fontsize=10);a.set_title(f"{label}: host kernel launch {z['launch_api_count']-z['graph_launch_count']}，graph launch {z['graph_launch_count']}; device kernel {z['kernel_count']}",loc='left',fontsize=12)
...
f,a=canvas(6.3)
for y,hot,total,critical,label in [(.58,60,80,'A',"original accelerator"),(.10,15,60,'B',"after A 4x speedup")]:
 a.text(.035,y+.29,label+'：'+str(total)+' μs',fontsize=15,weight='bold')
 box(a,.035,y+.07,.145,.105,"prep 10 μs",color='pale',size=12)
 ...
 box(a,.735,y+.07,.22,.105,"wrap-up 10 μs",color='pale',size=12)
 ...
 a.text(.615,y+.26,"wait for both branches",fontsize=11,color=C['muted'])
a.text(.035,.02,"orange marks path determining completion time",fontsize=11,color=C['orange'])
...
a.axhline(np.median(v),ls='--',color=C['orange'],label=f'median paired time diff: {np.median(v):.1f} ms')
a.set_yticks(range(-3,6));a.set_xticks(x);a.set(xlim=(.5,11.5),ylim=(-3.2,5.3),xlabel="paired round",ylabel="Request time before − after replacement / ms");a.legend(frameon=False,loc='upper left',fontsize=11)
a.text(7.3,4.6,"9 pairs faster, 2 pairs slower\ntotal request time ≈817 ms",ha='right',va='top',fontsize=11,linespacing=1.7)
...
for y,kernel,label in [(1,20,"original kernel"),(0,5,"kernel 4x faster")]:
 start=0
 for duration,name,col in [(3,"submission",'orange'),(8,'H2D','teal'),(kernel,'kernel','blue'),(4,'D2H','muted')]:
 ...
 a.text(start+.5,y,f'{start} μs\nresult available',va='center',fontsize=11)
a.set(xlim=(0,40),ylim=(-.65,1.65),xlabel="Time from CPU submission start / μs");a.set_yticks([0,1],["kernel 4x faster","original kernel"]);a.set_xticks([0,3,11,20,31,35]);a.grid(axis='x',alpha=.15)
...
for y,fused in [(.57,False),(.12,True)]:
 a.text(.025,y+.31,"requantize in matmul" if fused else "save one FP8 quantized result",fontsize=16,weight='bold')
 box(a,.03,y+.05,.18,.18,"FP16 input",'32 MiB',size=13)
 box(a,.34,y+.05,.24,.18,"compute row scale" if fused else "compute row scale, quantize and save","don't save FP8 matrix" if fused else "write FP8: 16 MiB",color='sand' if fused else 'light',size=12)
 arrow(a,(.22,y+.14),(.33,y+.14))
 box(a,.74,y+.05,.23,.18,"12 output column blocks","each reads FP16 and quantizes" if fused else "each reads same FP8",size=12)
 if fused:
  arrow(a,(.59,y+.18),(.73,y+.18),col='line')
  a.text(.66,y+.235,"row scale",ha='center',fontsize=10,color=C['muted'])
  arrow(a,(.21,y+.075),(.74,y+.075),col='orange',rad=.16)
  a.text(.45,y+.005,"original input: 12 × 32 MiB",ha='center',fontsize=11,color=C['orange'])
 else:arrow(a,(.59,y+.14),(.73,y+.14),col='teal')
 if not fused:a.text(.66,y+.235,'12 × 16 MiB',ha='center',fontsize=11,color=C['teal'])
 a.text(.34,y-.040,"read/write: 32 + 12 × 32 = 416 MiB" if fused else "read/write: 32 + 16 + 12 × 16 = 240 MiB",fontsize=12)
a.text(.025,.015,"doubled width of repeated reads outweighs the one write saved",fontsize=12,color=C['blue'])
...
a.set_yticks([2,1,0],["normal execution","graph replay: 2 MiB input","graph replay: 16 MiB input"]);a.set(xlim=(0,52),xticks=range(0,51,10),ylim=(-.65,2.6),xlabel="Completion time / μs");a.axvline(40,color=C['muted'],ls='--',lw=1)
a.legend(handles=[Rectangle((0,0),1,1,color=C[c],label=t) for c,t in [('orange',"Preparation / submission wait"),('teal',"copy input"),('blue',"device compute")]],loc='lower left',bbox_to_anchor=(-.2,1),frameon=False,ncol=3,fontsize=11)
...
for a,fine in zip(axs,[False,True]):
 ...
 end=max(b[3] for b in bars);a.axvline(bars[0][2],ls='--',color=C['orange']);a.set_xticks(range(0,131,20));a.set(xlim=(0,135),ylim=(0,1.03),xlabel="time / μs");a.set_yticks([.27,.78],["activation","projection"]);a.set_title(("start activation block by block" if fine else "start activation after all projections complete")+f': approx. {end:.0f} μs',loc='left',fontsize=14)
...
figure_refs=re.findall("^!\\[Figure (5-\\d+) ([^\\]]+)\\]\\(ch05/([^)]+)\\)",raw,re.M)
...
def wrap_figure(match):
 ...
 content=('<div class="diagram-scroll">'+img+"</div><p class=\"diagram-hint\">Swipe left/right to view full figure</p>") if revised else img
...
page="<!doctype html><html lang=\"zh-CN\"><meta charset=\"utf-8\"><meta name=\"viewport\" content=\"width=device-width,initial-scale=1\"><title>Chapter 5 Operators and Runtime</title><style>"+css+math_css+'</style><main><nav>'+nav+'</nav>'+body+'</main></html>'
import sys
sys.path.insert(0,str(HERE.parent))
from teaching_reading import readable_diagrams
from preview_output import preview_path
page=readable_diagrams(page)
html_path=preview_path(HERE.parent/"05-operators-and-runtime.html");html_path.write_text(page)
...
excerpt=preview_path(HERE.parent/"05-operators-and-runtime-5.2-5.3.html")
