...
Figure 10-36 plots the two runs from their logs. Pro restarted 14 times. Its interruption intervals totaled about 30.1 h (23.6% of total runtime), corresponding to approximately \$619,000. Before Pro's fifth restart, 2.76 h had elapsed since the previous step completed, costing about \$56,700; the sixth and eighth intervals lasted 2.63 h and 2.95 h, corresponding to about \$54,100 and \$60,700. The restarts had three main types of cause: hardware faults such as GPU-memory double-bit errors; service failures such as container crashes and loss of network access to the grader; and memory exhaustion, including exhausted KV caches during inference, GPU out-of-memory errors caused by expert-load imbalance during training, and host-memory exhaustion during sequence packing. In Flash, an infrastructure error went undetected until after step 17 had completed, forcing training to restart from step 15. The roughly four hours already spent on steps 16 and 17 were consequently wasted.[^rl-interrupt]

![Figure 10-36: Each timeline starts at its run's launch. Blue intervals end in a completed step; red intervals extend from the preceding event to a restart, marked by a triangle. Hatching marks the two Flash steps invalidated by rollback. Estimated wasted time is the sum of red and hatched intervals; percentages in parentheses are shares of total runtime. The lower panel converts those intervals to costs at the published rates. The savings scenario assumes unchanged training work and a runtime reduction equal to the interruption time avoided.](images/figure-10-mimo-interruptions.pdf)
...
![Figure 10-38: Loading generation weights and allocating the KV pool first, then releasing training state, gives a peak of about 83.3 GiB, exceeding the H100 SXM's 74.5 GiB. The horizontal axis is ordered by operation sequence.](images/figure-10-15-rl.pdf)

![Figure 10-39: Loading generation weights first, then releasing training state, then allocating the KV pool. The peak drops to about 59.3 GiB; both figures share the 74.5 GiB capacity line and the same vertical axis.](images/figure-10-rl-staged.pdf)
...
![Figure 10-40: Weight lifecycles for fixed-version serving and policy training. The ROM above repeatedly serves the same version; the training below produces new versions and publishes them to the generation side. KV writes and weight updates use different data paths.](images/figure-10-weight-update.pdf)
...
![Figure 10-42: The synchronous loop generates the current batch, learns the current batch, and synchronizes weights in sequence, taking 40, 16, and 4 s respectively, totaling 60 s.](images/figure-10-16-async-cycle.pdf)

![Figure 10-43: In steady state, generating the next batch overlaps with learning the previous batch on independent resources; once both finish, weights are synchronized, giving a 44 s cycle. The orange synchronization stage blocks both sides.](images/figure-10-async-overlap.pdf)
...
![Figure 10-44: Discrete expert IDs connect the generation side and the training side, while the current weights continue to participate in the numerical computation. The illustrated record is the top-2 selection for sample A, token 17, layer 3; the dashed line represents ID replay, and the solid line represents the current computation data flow.](images/figure-10-17-replay.pdf)
...
[^precision]: [The mixed-precision training paper](https://github.com/bojieli/ai-infra-book/blob/main/references/text/mixed-precision.txt) (loss-scaling mechanism, scaling factors from 8 to 32K, FP16 representable range), [the FP8 formats paper](https://github.com/bojieli/ai-infra-book/blob/main/references/text/fp8-formats.txt) (E4M3/E5M2 encoding and maximum normal values), [the DeepSeek-V3 technical report](https://github.com/bojieli/ai-infra-book/blob/main/references/files/papers/deepseek-v3.pdf) §3.3 (1×128 activation and 128×128 weight block scaling, promotion to FP32 accumulation every 128 elements, relative loss error below 0.25%). The exponent and mantissa comparison between FP16 and BF16 is given in [Section 4.6.1](https://github.com/bojieli/ai-infra-book/blob/main/manuscripts/04-%E5%8A%A0%E9%80%9F%E5%99%A8%E6%9E%B6%E6%9E%84.md).
...
[^rl-interrupt]: Total costs and failure causes are reported in the [MiMo-V2.6 Technical Report](https://github.com/bojieli/ai-infra-book/blob/main/references/files/papers/mimo-v26-tech-report.pdf), Sections 4.1 and 5.5. Per-event timestamps, rates, and rollback notices are in the [archived logs](https://github.com/bojieli/ai-infra-book/blob/main/references/files/documents/mimo-v26-rl-run-log/README.md); see the [case-study memo](https://github.com/bojieli/ai-infra-book/blob/main/case-studies/mimo-v26-rl-interruptions.md) and [recalculation results](https://github.com/bojieli/ai-infra-book/blob/main/references/files/documents/mimo-v26-rl-run-log/recalculated.json) for the calculations. Costs are estimates obtained by multiplying event intervals by the dashboard's fixed rates, not a separate failure-cost bill published in the report. Rework, detection, and waiting are not separately timed within those intervals, and potentially reusable rollout results are not deducted.
...
[^opd]: [DeepSeek V4 technical report](https://github.com/bojieli/ai-infra-book/blob/main/references/files/papers/deepseek-v4.pdf), §5.1 on domain experts and multi-teacher OPD; stage work division follows [Chapter 3](https://github.com/bojieli/ai-infra-book/blob/main/manuscripts/03-%E6%8E%A8%E7%90%86%E4%B8%8E%E8%AE%AD%E7%BB%83%E8%B4%9F%E8%BD%BD.md).
