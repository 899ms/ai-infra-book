...
import base64, hashlib, html, json, re, argparse
import matplotlib
matplotlib.use('Agg')
...
def canvas(title,subtitle,height=8):
 ...
 ax.text(.045,.958,title,fontsize=23,weight='bold',va='top');ax.text(.045,.898,subtitle,fontsize=11,color=C['muted'],va='top')
 return fig,ax
...
box(a,.055,.75,.18,.075,'输入 X','[m,4096]')
box(a,.30,.75,.18,.075,'RMSNorm',size=12);arrow(a,(.24,.785),(.29,.785))
box(a,.555,.72,.37,.13,'Q、K、V 投影','Q [m,4096]；K/V 各 [m,1024]',color='light');arrow(a,(.485,.785),(.545,.785))
box(a,.555,.535,.37,.12,'QK Norm → RoPE → Attention','QK^T → Mask / Softmax → AV',color='light');arrow(a,(.74,.71),(.74,.665))
box(a,.30,.545,.18,.10,'输出投影 Wo','[4096,4096]');arrow(a,(.55,.59),(.49,.59))
box(a,.055,.545,.18,.10,'残差相加','返回 [m,4096]');arrow(a,(.295,.59),(.24,.59));arrow(a,(.145,.745),(.145,.652),'orange')
box(a,.055,.365,.18,.095,'RMSNorm');arrow(a,(.145,.535),(.145,.468))
box(a,.30,.365,.27,.095,'gate / up 两路升维','各 [m,4096] × [4096,12288]',color='sand');arrow(a,(.24,.415),(.29,.415))
box(a,.65,.365,.275,.095,'SiLU(gate) ⊙ up','[m,12288]',color='sand');arrow(a,(.58,.415),(.64,.415))
box(a,.65,.19,.275,.095,'down 降维','[m,12288] × [12288,4096]',color='sand');arrow(a,(.787,.355),(.787,.292))
box(a,.30,.19,.27,.095,'残差相加 → 层输出','[m,4096]');arrow(a,(.64,.24),(.58,.24));arrow(a,(.055,.59),(.025,.59),'orange');arrow(a,(.025,.59),(.025,.24),'orange');arrow(a,(.025,.24),(.29,.24),'orange')
...
save(f,'figure-2-3-cache');data['figure_2_3']={'history':8192,'qwen_variants_mib':[4608,1152,144],'k3_mla_paths_mib':[216,11520]}
...
a.text(.06,.035,'索引仍扫描压缩索引历史；未完成块与更新峰值单列。图中载荷不是 HBM 实测。',fontsize=10,color=C['muted'])
save(f,'figure-2-4-sparse');data['figure_2_4']=v['summary']|{'components':com}
...
f.text(.055,.18,'A：K3 含卷积槽；V4 含压缩缓冲。B：Qwen 为旧 KV 读取，V4 为主注意力＋索引载荷，',fontsize=11)
...
save(f,'figure-2-5-state-growth');data['figure_2_5']={'lengths':lengths.tolist(),'resident_bytes':dict(zip(['qwen','v4','k3'],[qr.tolist(),vr.tolist(),kr.tolist()])),'accounted_access_bytes':dict(zip(['qwen','v4','k3'],[qaccess.tolist(),vaccess.tolist(),kaccess.tolist()])),'prefill_matrix_tflops':pre,'scope':'Structural length extrapolation; partial state access; pinned compact K3 prefill, no measured runtime or quality.'}
...
for y,title,body in [(.66,'GQA 注意力','QKV → 历史访问 → Wo'),(.45,'残差相加／归一化','[m,4096]'),(.24,'稠密 SwiGLU','所有 token 使用同组 FFN 权重')]:box(a,.10,y,.28,.12,title,body,color='light')
arrow(a,(.24,.65),(.24,.58));arrow(a,(.24,.44),(.24,.37));arrow(a,(.24,.23),(.24,.15));a.text(.24,.115,'残差相加 → 下一层',ha='center',fontsize=12)
for y,title,body in [(.66,'mHC 汇合 → 注意力','窗口／压缩／索引；输入 [m,4096]'),(.45,'mHC 残差混合','保留四路状态；子层维度不乘 4'),(.24,'mHC 汇合 → MoE','256 选 6＋1 共享专家；每专家 t_e 行')]:box(a,.60,y,.31,.12,title,body,color='sand')
...
save(f,'figure-2-6-residual');data['figure_2_6']={'qwen_hidden':4096,'v4_sublayer_hidden':4096,'v4_residual_streams':4,'v4_experts':256,'v4_top_k':6,'v4_shared':1}
...
for i,z in enumerate(entries):a.plot([z['capacity_bytes']/1e9]*2,[i-.32,i+.32],color=C['ink'],lw=2);a.text(2,i+.37,f"容量 {z['capacity_bytes']/1e9:.0f} GB：算术上限 {z['maximum_requests']} 条",fontsize=10)
...
a=axs[1];a.axis('off');a.set(xlim=(0,1),ylim=(0,1));a.set_title('B  专家颗粒度（教学对照）',loc='left',fontsize=14)
for y,title,body,width in [(.66,'真实基线：128 专家，选 8','每专家宽 1536',.35),(.29,'教学变体：64 专家，选 4','每专家宽 3072',.70)]:
 a.text(.03,y+.17,title,fontsize=12,weight='bold');a.text(.03,y+.10,body,fontsize=11)
 for j in range(3):a.add_patch(plt.Rectangle((.03,y-j*.035),width,.024,color=C['teal'] if width<.5 else C['orange']))
a.text(.03,.05,'同专家参数预算、同激活矩阵工作；\nrouter 改变，完整参数与质量不等价。',fontsize=11,linespacing=1.8)
f.text(.05,.095,'低位方案保留部分高精度权重并计 scale／尾部；70B 4-bit 在 32K 历史下的算术上限降为 3 条。',fontsize=11)
f.text(.05,.045,'容量与专家宽度使用不同坐标；未训练变体不证明同质量，容量通过不证明真实可服务并发。',fontsize=11,color=C['muted'])
save(f,'figure-2-7-capacity');data['figure_2_7']={'capacity_rows':entries,'expert_shapes':[[128,1536,8],[64,3072,4]]}
...
for i,(title,body) in enumerate([('Prefill','输入 128 → 输出 y₁\n保留 128 位置'),('Decode 1','输入 y₁ → 输出 y₂\n保留 129 位置'),('Decode 2','输入 y₂ → 输出 y₃\n保留 130 位置'),('Decode 3','输入 y₃ → 输出 y₄\n保留 131 位置')]):
 x=.055+i*.235;box(a,x,.66,.195,.15,title,body,color='light' if i==0 else 'pale',size=13)
 if i<3:arrow(a,(x+.203,.735),(x+.225,.735))
a.text(.055,.60,'y₄ 已返回但尚未再次进入模型；最终状态不是 132 个位置。',fontsize=12,color=C['orange'])
...
box(a,.65,.235,.29,.27,'另一个证据：同消息检索','Qwen 与 V4 Flash 各 8/8\n输入 token 数、部署与精度不同\nPro／K3 未测',color='sand',size=13)
a.text(.055,.115,'左：计算结果按各自声明路径累计；右：八题实测只支持这组窄任务，不是左侧资源请求的性能测量。',fontsize=10.5)
a.text(.055,.055,'已有记录未闭合完整 HBM、运行峰值或时延；同 token 几何不保证同文本与同质量。',fontsize=11,color=C['muted'])
save(f,'figure-2-8-request');data['figure_2_8']={'contract':r['contract'],'matrix_tflops':vals,'model_order':[z['model'] for z in r['comparisons']]}
(HERE/'figure-data.json').write_text(json.dumps(data,ensure_ascii=False,indent=2)+'\n')
md=HERE.parent/'02-模型架构.md';raw=md.read_text();body=markdown.markdown(raw,extensions=['tables','footnotes','fenced_code','toc'],output_format='html')
for p in out:
...
artifacts=out+[HERE/'figure-data.json',html_path,md]
manifest={'chapter':2,'generator':'manuscripts/ch02/build.py','font_family':family,'figures':8,'outputs':[{'path':str(p.relative_to(ROOT)),'sha256':hashlib.sha256(p.read_bytes()).hexdigest()} for p in artifacts]}
