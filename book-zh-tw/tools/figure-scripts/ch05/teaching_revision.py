...
def draw(here, data):
    ...
    with plt.rc_context(style):
        ...
        for row,j in enumerate([0,1]):
            y=.60-row*.45
            title(a,"先計算一個輸出" if j==0 else "再計算相鄰輸出",y+.31)
        ...
        f,a=canvas(3.9)
        title(a,"一對輸入塊，更新整個輸出塊")
        box(a,.04,.54,.23,.20,"A 塊\nm × k",'blue')
        box(a,.385,.54,.23,.20,"W 塊\nk × n",'orange')
        ...
        box(a,.09,.15,.56,.13,"沿 K 換入下一對 A、W 塊",'gray')
        arrow(a,(.65,.215),(.74,.475))
        text(a,.5,.055,"輸入塊更換；輸出累加器一直保留",12,ha='center')
        ...
        a.text(28,.43,"各讀寫一次：128 MiB",fontsize=11)
        a.set(xlim=(0,98),ylim=(0,7),xticks=[8,24,48,80],xlabel="區域性儲存需求（KiB）",ylabel="介面讀寫量（GiB）")
        ...
        f,a=canvas(3.0);title(a,"一個 SM 的 100 KB 共享記憶體能放幾份？")
        for y,size,c in [(.54,24,'blue'),(.19,80,'orange')]:
            ...
            if used<budget:
                ...
                text(a,.04+.92*(used+budget)/2/budget,y+.08,"空餘",11,ha='center')
            text(a,.04,y-.075,"四份 64 × 64 輸出塊的工作集" if size==24 else "一份 128 × 128 輸出塊的工作集",12)
        text(a,.96,.075,"灰色：每塊 1 KiB 預留",11,ha='right')
        ...
        for y,stride in [(.58,32),(.09,33)]:
            title(a,f'行跨度 {stride} 個字',y+.34)
            text(a,.98,y+.34,"數字為字地址",11,ha='right')
            for i in range(4):
                x=.05+i*.235
        ...
        f,a=canvas(4.6)
        title(a,"一組處理一行")
        box(a,.04,.72,.25,.13,"保留整行輸入",'blue');box(a,.38,.72,.25,.13,"求縮放因子",'green');box(a,.73,.72,.23,.13,"歸一化",'green')
        ...
        text(a,.50,.555,"每段 512 項 → 一個區域性平方和",12,ha='center')
        box(a,.33,.21,.34,.11,"合併，求縮放因子",'green')
        box(a,.04,.035,.30,.10,"重讀原輸入",'orange')
        box(a,.65,.035,.31,.10,"歸一化並寫出",'green')
        arrow(a,(.67,.265),(.805,.145));arrow(a,(.35,.085),(.64,.085))
        save(f,'figure-5-4-reduction')

        f,a=canvas(4.2)
        title(a,"分開執行：中間結果經過下一級儲存")
        ...
        text(a,.24,.59,"寫出",11);text(a,.71,.59,"讀回",11)
        title(a,"融合執行：逐片傳給下一步",.34)
        box(a,.035,.055,.93,.20,'','gray')
        box(a,.08,.10,.24,.10,'SiLU(G)','green');box(a,.40,.10,.20,.10,"區域性 t",'orange');box(a,.69,.10,.23,.10,'t × U','green')
        ...
        a.set(yticks=range(3),yticklabels=["全部分開","融合前兩步","三步融合"],xlim=(0,177),xticks=[0,60,120,160],xlabel="讀寫量（MiB）");a.invert_yaxis();a.spines['left'].set_visible(False);a.tick_params(axis='y',length=0)
        f.legend(handles=[Patch(fc=COL[c],ec=COL['line'],label=l) for c,l in [('blue',"必要讀寫"),('orange',"中間 T"),('purple',"中間 Z")]],loc='upper center',ncol=3,frameon=False,handlelength=1,columnspacing=1)
        ...
        for a,mode,end in zip(axs,['serial','double_buffer'],d['completion_us']):
            ...
            a.set(xlim=(0,6.6),ylim=(0,1),xticks=[0,1,2,3,4,5,6],yticks=[.27,.79],yticklabels=["計算","搬移"])
            a.set_title(title_,loc='left',pad=9);a.axvline(end,ls='--',lw=.8,color=COL['line'])
        axs[1].set_xlabel("時間（μs）")
        f.legend(handles=[Patch(fc=COL[c],ec=COL['line'],label=l) for c,l in [('blue',"槽 A：塊 0、2"),('green',"槽 B：塊 1、3")]],loc='upper center',ncol=2,frameon=False)
        save(f,'figure-5-6-fusion-buffer')

        f,a=canvas(4.2)
        title(a,"儲存完整中間矩陣")
        box(a,.04,.63,.24,.19,"Q、K\n計算分數",'blue');box(a,.38,.63,.24,.19,"S → P\n整行 Softmax",'orange');box(a,.72,.63,.24,.19,"P、V\n計算輸出",'green')
        arrow(a,(.285,.72),(.37,.72));arrow(a,(.625,.72),(.71,.72))
        text(a,.5,.545,"S、P 各 256 MiB，分別寫出再讀回",12,ha='center')
        title(a,"處理當前塊，留下統計量",.42)
        box(a,.04,.15,.25,.18,"當前分數塊",'orange');box(a,.39,.15,.25,.18,"更新\nm、ℓ、u",'green');box(a,.75,.15,.21,.18,"下一塊",'orange')
        arrow(a,(.30,.24),(.38,.24));arrow(a,(.65,.24),(.74,.24))
        text(a,.5,.055,"傳遞統計量；分數緩衝留給下一塊",12,ha='center')
        ...
        f,a=canvas(4.5)
        for y,label,body_,c in [(.72,"① 處理第一塊",'s = 0，V = 1\nm = 0，ℓ = 1，u = 1','blue'),
                                  (.43,"② 讀入第二塊","s = ln 2，V = 3\nm′ = ln 2，r = 1/2\n舊 ℓ、u 同乘 1/2",'orange'),
                                  (.14,"③ 加入第二塊",'ℓ′ = 0.5 + 1 = 1.5\nu′ = 0.5 + 3 = 3.5','green')]:
            text(a,.025,y+.09,label,12);box(a,.39,y,.56,.19,body_,c,12)
        arrow(a,(.67,.715),(.67,.635));arrow(a,(.67,.425),(.67,.345))
        text(a,.50,.06,"全部塊處理完：u′ / ℓ′ = 7/3",12,ha='center')
        ...
        axs[0].text(4,len(offsets)-.62,"精確值",fontsize=11,color=COL['ink'],va='center')
        ...
        axs[0].set_title("段數如何改變平方和",loc='left',fontsize=13)
        ...
        axs[1].set_title(f"同樣分成 {order['merge_order']['splits']} 段：{order['merge_order']['orders']:,} 種合併次序，只有 {order['merge_order']['distinct_results']} 個結果",loc='left',fontsize=13)
        for a_ in axs:
            a_.set_xlabel("平方和與精確值的距離（ULP）",fontsize=11)
        ...
        a.set_yscale('log');a.set(xlim=(230,665),ylim=(3500,1e6),xticks=[300,400,500,600],xlabel="介面讀寫量（MiB）",ylabel="塊對更新次數（對數刻度）")
        a.set_yticks([1e4,1e5,1e6],["1 萬","10 萬","100 萬"]);a.grid(alpha=.15)
