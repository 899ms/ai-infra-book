...
- [ ] F01 原文清单：正文小节、练习、扩写资料与所有案例逐条纳入，审查计算范围及旧脚本迁移。清单本身已捕获 95 份原件、1,268 个文本块；本轮补上一直缺的那一半——从文本块回到工作包的映射，见[覆盖清单](inventory/f01-coverage.json)与 `python3 calculations/calc.py coverage --format md`。映射按各条自带的小节引用机械生成，直接命名与仅命名其子节两种关系分开记，并逐块记录量化信号与已引用的生成结果。当前：988 块有量化信号，823 块有直接候选工作包，27 块仅通过子节相关，350 块有量化信号却无候选；129 块引用了生成结果且全部解析成功（0 处失效链接）。**由此发现并已修复的计划欠账**：第 3 章改写后小节全部移位，C15–C20 仍写着旧编号，C16/C17/C18 声称的视觉、语音与训练小节实际已成为无人认领的空白；C56 的 `10.4.1、3` 简写此前也解析不出 10.4.3。已按标题逐条对应更新 C15–C20 与 C56/C57 的小节引用并注明原编号，剩余唯一有小节的空白是第 1 章 1.1 全景（3 块），是否需要计算由作者判断。另有 333 个未映射块来自案例笔记——没有任何工作包按文件名认领案例，这是工作包写法的问题而非案例未计算。**仍未完成**：1,268 块的 review_status 全部仍为 pending，本轮不设置任何一块；机械映射不是审查，有候选工作包不等于该块要求已实现。逐块审查、1.1 与案例笔记的认领决定、旧脚本迁移仍待。22 项独立tests通过。
...
- [ ] C57 故障率、checkpoint 周期 c/τ+λτ/2+λr、最优间隔和相关故障敏感性（10.4.2、10.4.4；原仅 10.4.2，故障规模与保存周期现位于 10.4.4）。[周期与重试](src/infra_calc/topics/checkpoint_interval.py)已实现官方载荷的阻塞c、一阶分项／连续最优、独立设备与作业共同冲击、另列Poisson重试期望／数值最优，五场景含高率／零率／恢复敏感性；实际故障分布、一般相关故障、恢复再失败和异步策略仍待。
