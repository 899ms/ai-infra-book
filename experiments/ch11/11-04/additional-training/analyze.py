...
result=dict(status='new_batch_training_verified',logical_samples=12,completed_training_tokens=sum(x['completed_training_tokens'] for x in ledger),distinct_content_streams=len(unique),distinct_content_output_tokens=sum(unique.values()),preparation_s=raw['preparation_s'],process_wall_s=raw['end_s']-raw['start_s'],ledger=ledger,scope='Independent supervised adapter checkpoints for newly received samples. No live generation-to-training clock or RL improvement.')
