...
三条路线都可以用本节的工作量变化理解：Tensor Core 缩短规则矩阵计算，img2col 省去卷积展开的中间写读，CV 通路减少注意力的外层交换，统一内存扩展模型与 KV 的驻留空间。哪项变化更有价值，取决于目标模型的矩阵形状、状态大小和数据路径。图 4-3 的跨代协同由这些量连接起来：负载改变了资源需求，架构改变了执行成本，模型的精度、批次和并行方式再据此调整。第 4.8 节将这些代际参数用于同一模型的设备比较；下面先考察进一步固定计算与数据条件的专用设计。
...
矩阵列统一采用 BF16 输入、FP32 累加、稠密执行的速率；横线对应硬件表中尚未给出这一完整口径的速率。Apple 的 Neural Engine TOPS、昇腾的不同精度或累加口径，需要先匹配模型实际执行路径，再进入这一列。两列分别计算不同阶段的主要资源工作，后续按第 4.8.3 节的实测方法加入其余步骤。
...
**大内存的 Apple 是否比 NVIDIA 更高效？** 表中的单请求稠密模型，RTX 4090、5090 的读取下界都低于 M3 Ultra。换成第 4.6.3 节已经算过容量与活跃专家读取量的 Qwen3-235B-A22B，三张消费级卡都无法单卡驻留，M3 Ultra 256 GB 可以。模型改变后，大容量进入了选型的关键条件。

将该 MoE 同时放入候选比较，H200 的 141 GB 能容纳其模型与工作区预算，每步 13.697 GB 的读取约需 2.85 ms，为 M3 Ultra 读取时间的约 1/5.86。Apple 提供大容量本地运行，H200 以更高带宽缩短服务时间；价格、整机功率和请求利用率再按第 4.8.4 节换算为每任务成本与能耗。用多张容量较小的 GPU 则要把专家或张量切分，并把跨卡通信加入时间，不能只把显存数字相加后沿用单卡速度。
...
容量筛选、阶段时间和执行路径共同确定候选设备。接下来用实际计时与硬件访问记录校准这些预算，再把完成时间换算为成本与能耗。

### 4.8.3 预测与实测为什么有差距

第 4.8.2 节的设备比较给出了容量与主要资源预算。运行库为数学算子选择具体内核，缓存改变片外流量，提交和同步也占用时间。测量的用途是识别这些执行步骤，补全第 4.8.1 节的时间模型。

先给每一阶段写出执行路径，再把峰值换成同一负载下的有效速率。令 $\eta_P$、$\eta_R$ 分别表示有效矩阵速率和有效带宽相对峰值的比例，则该阶段的服务预算变为 $F/(\eta_P P)$ 和 $V/(\eta_R R)$。随后沿依赖加入向量、转换、启动与同步，能重叠的工作按流水安排，串行步骤依次相加。$\eta$ 从匹配形状、精度与访问方式的测量中取得，缓存流量则按实际命中层次重新计算。

以上一节表中 RTX 4090 的单请求读取为例，若有效带宽达到峰值的 80%，16.22 ms 变为约 20.27 ms；若只有 50%，则为约 32.43 ms。它们对应约 49.3 和 30.8 token/s 的读取能力，其他串行工作还要计入完成时间。这里给出的两个利用率是供读者代入的条件；下面的 Q 投影测量将展示如何用计时、DRAM／L2 流量和内核记录求出差距的来源。把一次投影的有效速率直接套给所有层，也会漏掉注意力、激活和小专家矩阵的不同形状。
