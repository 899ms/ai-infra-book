...
![Figure 10-2: The five categories of training state together amount to about eight times the capacity of the BF16 weights. The model is Qwen3-8B, using the data formats in the table; the horizontal axis shows the cumulative capacity occupied by each category of state.](images/figure-10-1-state.pdf)
...
![Figure 10-3: Card count required at 30% matrix compute efficiency. The task is Qwen3-8B processing 100B tokens with sequence length 8192, with all 30 days spent on execution; blue bars are the compute requirement, orange diamonds are the training-state capacity lower bound. A100 is 80 GB SXM, H100 is SXM.](images/figure-10-2-budget.pdf)

![Figure 10-4: As matrix compute efficiency rises to 40%, the card count required for the same task falls; orange diamonds still denote the capacity lower bound. Each lower bound is measured in card count; the capacity term is the minimum card count required by the memory constraint.](images/figure-10-budget-1.pdf)

![Figure 10-5: Card count required at 50% matrix compute efficiency. The three figures use the same model ordering, vertical-axis range, task, and 30-day execution deadline. Each lower bound is measured in card count; the capacity term is the minimum card count required by the memory constraint.](images/figure-10-budget-2.pdf)
...
![Figure 10-6: Plain DP: each card keeps the full weights, gradients, master weights, and two moment states.](images/figure-10-zero-0.pdf)

![Figure 10-7: ZeRO-1: master weights and the two moment states are sharded by parameter, while weights and gradients are still fully replicated.](images/figure-10-zero-1.pdf)

![Figure 10-8: ZeRO-2: gradients are further partitioned, with each card retaining only the gradient shard it owns.](images/figure-10-zero-2.pdf)
...
![Figure 10-9: ZeRO-3: weights are also sharded. When executing a module, each card obtains the full weights it needs through communication.](images/figure-10-zero-3.pdf)
...
![Figure 10-10: The weights of the same module are normally distributed across four cards; during execution, each card gathers the full weights. The figure expands the gathering process on GPU 0, with the four colors representing the four parameter shards. The full-weight buffer can be released once used; the original shards each card keeps long-term remain in place.](images/figure-10-3-sharding.pdf)
...
![Figure 10-11: Matrix backward pass produces two gradient branches: dX is passed to the previous layer to continue backward propagation, dW is passed to this layer's reduction and parameter update. Both branches read the upstream gradient dY.](images/figure-10-gradient-branches.pdf)
...
![Figure 10-12: Retaining the product h: the blue bar represents the always-retained a and u; the orange bar represents the product h that persists from the forward pass through the down-projection's backward pass. For a micro-batch of 128 tokens using FP32, h has shape [128,12288], occupying 6 MiB.](images/figure-10-4-recompute.pdf)

![Figure 10-13: Retaining a, u, and re-multiplying to obtain h just before backward use. The orange 6 MiB buffer exists only briefly around the point of use; the horizontal axis represents operation order.](images/figure-10-recompute-rebuild.pdf)
...
![Figure 10-14: First transfer the 96 MiB BF16 gradient to the CPU, then convert it to 192 MiB FP32 on the CPU. Arrows represent data flow; both the input and output of the conversion reside on the CPU side.](images/figure-10-5-casting.pdf)

![Figure 10-15: First convert the 96 MiB BF16 gradient to 192 MiB FP32 on the GPU, then transfer it to the CPU. During the conversion, input and output coexist, giving a GPU peak of 288 MiB.](images/figure-10-casting-gpu.pdf)
...
![Figure 10-17: Fill-drain first completes the forward pass of eight micro-batches, then executes the backward pass, and finally updates parameters. Blue is forward, orange is backward, green is the parameter update. All four stages share the same timescale, and under the computation and transfer conditions in the text, the total is 337 ms.](images/figure-10-7-pipeline.pdf)

![Figure 10-18: 1F1B interleaves forward and backward passes after warmup, completing in 347 ms in this example. Blue is forward, orange is backward, green is the parameter update; the same timescale as the previous figure is used.](images/figure-10-pipeline-1f1b.pdf)

![Figure 10-19: Zoomed-in view of stage 3 in 1F1B: the second micro-batch finishes backward computation at 93 ms, the next forward input arrives at 95 ms, forming a 2 ms wait.](images/figure-10-pipeline-gap.pdf)

![Figure 10-20: Peak activation and send/receive buffer usage per stage across the two schedules. Earlier backward computation releases activations sooner, dropping the maximum from about 2.57 GB to 0.97 GB.](images/figure-10-pipeline-memory.pdf)
...
![Figure 10-21: Per-stage timeline of interleaved 1F1B ($v=2$), completing in 298 ms. Blue is forward, orange is backward, green is the parameter update, and hatched blocks represent each card's second layer block. Each block is about half the length of those in Figure 10-18; the gaps during warmup and drain are filled by computation from another layer block, and stage 3's first forward pass moves up from 33 ms to about 20 ms; each micro-batch now crosses four more boundaries.](images/figure-10-pipeline-interleaved.pdf)

![Figure 10-22: Per-stage timeline of zero-bubble (ZB-H1) scheduling, completing in 283 ms. Blue is forward, orange is $dX$, purple is $dW$, green is the parameter update. In steady state, each stage rotates through forward, $dX$, $dW$; the dependency gaps between backward passes and the trailing drain gap in Figure 10-18 are both filled by the deferred $dW$, and the four stages finish almost simultaneously; stage 3's $dW$ lags its $dX$ by three micro-batches, so activations there are retained the longest.](images/figure-10-pipeline-zero-bubble.pdf)

![Figure 10-23: Per-stage timeline of DualPipe, completing in 306 ms. Colors are the same as Figure 10-22, and hatched blocks represent computation for the other half of the micro-batches (reverse direction) entering from stage 3. Each stage simultaneously carries both directions; one direction's warmup and drain gaps are filled by the other direction's blocks; the cost is that each card stores two copies of the parameters.](images/figure-10-pipeline-dualpipe.pdf)
...
![Figure 10-25: When the link is idle, a 3 ms reduction (orange) can complete before a 5 ms independent computation (blue) finishes. The dashed line marks the end of computation.](images/figure-10-8-overlap.pdf)

![Figure 10-26: The link is occupied by other communication for 4 ms first (gray), delaying the reduction to 4–7 ms. The extra 2 ms after the dashed line extends the training step.](images/figure-10-overlap-busy.pdf)
...
![Figure 10-27: Under Qwen3-235B-A22B's routing shape (E=128, k=8, 8,192 tokens, half the experts 1.5x hot), as the capacity factor increases, the fraction of dispatches dropped decreases while the fraction of zero-padded rows increases.](images/figure-10-moe-capacity.pdf)
...
![Figure 10-28: Causal attention pairs for two sequences of 4096 tokens each. Each query token reads positions in its own sequence no later than itself, forming two triangles; the attention computation for the two sequences is independent, and the total pair count is 16781312.](images/figure-10-9-attention-area.pdf)

![Figure 10-29: Splitting the same 8192 tokens into 7168 and 1024 increases the total causal pair count to 26218496. The extra area of the triangle for the longer sequence exceeds the area lost from the triangle for the shorter sequence.](images/figure-10-attention-unequal.pdf)
...
![Figure 10-32: Capture consistent training state, copy it to an independent buffer to let training continue; only after the background process finishes writing the data and commits the complete snapshot does the recovery program use this checkpoint. Arrows indicate ordering dependencies.](images/figure-10-checkpoint-commit.pdf)
...
![Figure 10-33: Two 112 GB snapshots are written at 7 GB/s, each first taking 0.5 s to copy to the buffer (orange), then uploading (blue). At the 50 s failure, the first snapshot has been committed and the second has not; the diagonal line shows the remaining upload absent a failure, and the hollow points mark the originally scheduled commit times.](images/figure-10-12-recovery.pdf)

![Figure 10-34: The same snapshots written at 20 GB/s, committing at 26.1 and 46.1 s. At the 50 s failure, recovery can reach the training state at 40 s, redoing only 10 s.](images/figure-10-recovery-fast.pdf)
...
![Figure 10-36: The three stages can process at most 12, 6, and 8 equal-length trajectories/s respectively. Verification retains 75%, so only 4.5/s enter learning. The return arrow indicates that new weights produced by learning affect subsequent generation; the synchronization time is expanded on in the later stage-switching and asynchronous examples.](images/figure-10-14-rl-flow.pdf)
...
![Figure 10-37: Loading generation weights and allocating the KV pool first, then releasing training state, gives a peak of about 83.3 GiB, exceeding the H100 SXM's 74.5 GiB. The horizontal axis is ordered by operation sequence.](images/figure-10-15-rl.pdf)

![Figure 10-38: Loading generation weights first, then releasing training state, then allocating the KV pool. The peak drops to about 59.3 GiB; both figures share the 74.5 GiB capacity line and the same vertical axis.](images/figure-10-rl-staged.pdf)
...
![Figure 10-39: Weight lifecycles for fixed-version serving and policy training. The ROM above repeatedly serves the same version; the training below produces new versions and publishes them to the generation side. KV writes and weight updates use different data paths.](images/figure-10-weight-update.pdf)
...
![Figure 10-40: μ produces training samples, πold marks the starting point of this round's optimization, πθ changes as this round's update proceeds. The dashed line indicates the order of version evolution. All three probabilities must be computed for the same prefix and the same token.](images/figure-10-policy-versions.pdf)
...
![Figure 10-41: The synchronous loop generates the current batch, learns the current batch, and synchronizes weights in sequence, taking 40, 16, and 4 s respectively, totaling 60 s.](images/figure-10-16-async-cycle.pdf)

![Figure 10-42: In steady state, generating the next batch overlaps with learning the previous batch on independent resources; once both finish, weights are synchronized, giving a 44 s cycle. The orange synchronization stage blocks both sides.](images/figure-10-async-overlap.pdf)
...
![Figure 10-44: Each bar accumulates, in order, the base training time, the additional time for saving and failure recovery, and the reserved 5-day planned stall. The base training time already includes communication and input waiting; the small orange segment is the extra time obtained from the checkpoint model. Both schemes use the same task and global batch size; the dashed line marks the 30-day deadline.](images/figure-10-18-deadline.pdf)
...
![Figure 10-45: The benefit of a change in a resource's capability is determined by its original proportion of waiting time. The curve is computed as $T'/T=1-f+f/r$, holding per-card computation and dependencies fixed, with communication time inversely proportional to effective capability.](images/figure-10-19-hardware.pdf)
...
[^critical-batch]: [Critical batch size recomputation](https://github.com/bojieli/ai-infra-book/blob/main/calculations/results/critical-batch-book.md) and [comparison with a noise scale of twenty million tokens](https://github.com/bojieli/ai-infra-book/blob/main/calculations/results/critical-batch-noise-20m.md). The definition of gradient noise scale is given in Equation (2.8) of [the empirical model for large-batch training](https://github.com/bojieli/ai-infra-book/blob/main/references/text/large-batch-empirical.txt); the step-count-vs.-samples relation, Equation (5.1), the definition of critical batch size, Equation (5.2), and the one-to-two-million-token range at convergence are given in [the Scaling Laws paper](https://github.com/bojieli/ai-infra-book/blob/main/references/text/scaling-laws.txt). $B_{\mathrm{noise}}$ is the value used in this example and has not been measured on this chapter's model; neither weak scaling nor strong scaling includes changes in parallel efficiency.

[^precision]: [The mixed-precision training paper](https://github.com/bojieli/ai-infra-book/blob/main/references/text/mixed-precision.txt) (loss-scaling mechanism, scaling factors from 8 to 32K, FP16 representable range), [the FP8 formats paper](https://github.com/bojieli/ai-infra-book/blob/main/references/text/fp8-formats.txt) (E4M3/E5M2 encoding and maximum normal values), [the DeepSeek-V3 technical report](https://github.com/bojieli/ai-infra-book/blob/main/references/files/papers/deepseek-v3.pdf) §3.3 (1×128 activation and 128×128 weight block scaling, promotion to FP32 accumulation every 128 elements, relative loss error below 0.25%). The exponent and mantissa comparison between FP16 and BF16 is given in [Section 4.6.1](https://github.com/bojieli/ai-infra-book/blob/main/manuscripts/04-加速器架构.md).

[^schedules]: [Interleaved event model](https://github.com/bojieli/ai-infra-book/blob/main/calculations/results/training-pipeline-interleaved-m8.md), [zero-bubble event model](https://github.com/bojieli/ai-infra-book/blob/main/calculations/results/training-pipeline-zero-bubble-m8.md), [DualPipe event model](https://github.com/bojieli/ai-infra-book/blob/main/calculations/results/training-pipeline-dualpipe-m8.md), and the sixteen-micro-batch variants ([interleaved](https://github.com/bojieli/ai-infra-book/blob/main/calculations/results/training-pipeline-interleaved-m16.md), [zero-bubble](https://github.com/bojieli/ai-infra-book/blob/main/calculations/results/training-pipeline-zero-bubble-m16.md), [DualPipe](https://github.com/bojieli/ai-infra-book/blob/main/calculations/results/training-pipeline-dualpipe-m16.md)). Bubble formulas: 1F1B and interleaved are given in [the Megatron-LM paper](https://github.com/bojieli/ai-infra-book/blob/main/references/text/megatron-scale.txt) ($(p-1)/m$ and $(1/v)(p-1)/m$); ZB-H1/H2 are given in Table 2 of [the zero-bubble paper](https://github.com/bojieli/ai-infra-book/blob/main/references/text/zero-bubble.txt); the parameter count and activation split for DualPipe are given in Table 2 of [the DeepSeek-V3 technical report](https://github.com/bojieli/ai-infra-book/blob/main/references/files/papers/deepseek-v3.pdf); the steady-state definition for 1F1B is given in [PipeDream](https://github.com/bojieli/ai-infra-book/blob/main/references/text/pipedream.txt). The zero-bubble slot ordering follows the upper half of Figure 3 in the paper for ZB-H1; for the interleaved and DualPipe schemes, the archival text gives only the ideas of chunking and bidirectional feeding, and the slot ordering follows the declared_schedule_rule recorded in each result file; the split ratio of $T_B=T_W=10$ ms is the value used in this example; the completion times are derived from the event model according to the dependencies, not measured.

[^capacity]: [Capacity factor and drop recomputation](https://github.com/bojieli/ai-infra-book/blob/main/calculations/results/moe-capacity-book.md). The capacity rule is given in Equation (3) of [Switch Transformer](https://github.com/bojieli/ai-infra-book/blob/main/references/text/switch-transformer.txt) (per-expert capacity = tokens per batch / number of experts × capacity factor) and in [GShard](https://github.com/bojieli/ai-infra-book/blob/main/references/text/gshard.txt) (capacity taken as O(N/E), with overflowed tokens' representation passed to the next layer via the residual connection); bias-based balancing without an auxiliary loss is given in [the DeepSeek-V3 technical report](https://github.com/bojieli/ai-infra-book/blob/main/references/files/papers/deepseek-v3.pdf). The distribution in which half the experts are 1.5x hot is the value used in this example, giving the same 3:1 ratio as the 96/32 example in the main text.

[^straggler]: [Straggler maximum model (σ as 2% of the mean)](https://github.com/bojieli/ai-infra-book/blob/main/calculations/results/straggler-max-sigma-2pct.md) and [(σ as 5% of the mean)](https://github.com/bojieli/ai-infra-book/blob/main/calculations/results/straggler-max-sigma-5pct.md). The observation that about 0.5% of machines are noticeably slower is given in [MegaScale](https://github.com/bojieli/ai-infra-book/blob/main/references/text/megascale.txt); the mean time between failures of 7.9 hours for a 1024-card job and 47.7 days for an 8-card job are given in [the Meta cluster reliability paper](https://github.com/bojieli/ai-infra-book/blob/main/references/text/meta-cluster-reliability.txt), and the per-card failure rate in the save-period section is converted directly from the 1024-card 7.9-hour figure. The standard deviation of per-card computation time and the loss-spike rollback interval are the values used in this example; the expected maximum is obtained by numerical integration based on order statistics, without modeling correlation, periodic jitter, or persistent slow cards.

[^state]: [Official parameters and Adam/ZeRO state recomputation](https://github.com/bojieli/ai-infra-book/blob/main/calculations/results/training-state-book.md); [the FP32 gradient variant](https://github.com/bojieli/ai-infra-book/blob/main/calculations/results/training-state-fp32-gradient.md). This chapter uses the specified five-item state representation; other optimizers and low-precision states must be computed separately.

[^deadline]: [Training deadline and device lower bound](https://github.com/bojieli/ai-infra-book/blob/main/calculations/results/training-deadline-book.md) and [the calendar-time variant](https://github.com/bojieli/ai-infra-book/blob/main/calculations/results/training-deadline-calendar.md). Precision, sparsity, and hardware sources are recorded item by item in the JSON. The 38%–43% BF16 MFU figures are given in §3.3.2 and Table 4 of [the Llama 3 report](https://github.com/bojieli/ai-infra-book/blob/main/references/files/papers/llama3.pdf); the PCIe 4.0 interface and absence of NVLink on the RTX 4090 are given on [the RTX 4090 specification page](https://github.com/bojieli/ai-infra-book/blob/main/references/files/specs/nvidia-rtx4090.md).

[^fsdp]: [CPU FSDP2 four-configuration record](https://github.com/bojieli/ai-infra-book/blob/main/experiments/ch10/10-01/fsdp-cpu/README.md). The four configurations on the small CPU model produce parameters consistent with the unsharded reference result; the peak memory allocated during training is 39.16/30.14 MiB.

[^pipeline]: [Fill-drain event model](https://github.com/bojieli/ai-infra-book/blob/main/calculations/results/training-pipeline-gpipe-m8.md), [1F1B event model](https://github.com/bojieli/ai-infra-book/blob/main/calculations/results/training-pipeline-1f1b-m8.md), [retention relationships for GEMM input tensors](https://github.com/bojieli/ai-infra-book/blob/main/calculations/results/pipeline-gemm-save-1f1b.md), [selective recomputation](https://github.com/bojieli/ai-infra-book/blob/main/calculations/results/pipeline-gemm-recompute-products.md). The GEMM inputs use an FP32 reference representation; the reconstructible products for nine layers total 90 MiB, with a maximum per-layer reconstruction working set of 6 MiB. The pipeline diagram uses the intermediate results saved by the save_nonlinear strategy and their send/receive buffers.

[^offload]: [Gradient conversion recomputation](https://github.com/bojieli/ai-infra-book/blob/main/calculations/results/gradient-cast-book.md), [fast-link variant](https://github.com/bojieli/ai-infra-book/blob/main/calculations/results/gradient-cast-fast-link.md), [SuperOffload reading and conversion case](https://github.com/bojieli/ai-infra-book/blob/main/case-studies/training-offload-and-casting.md), [CPUAdam offload record](https://github.com/bojieli/ai-infra-book/blob/main/experiments/ch10/10-01/deepspeed-offload/README.md). The conversion path starts from GPU gradient readiness and ends with CPU-readable FP32 gradients; both paths use page-locked host memory as a buffer. GPU conversion throughput uses the RTX 4090's 1008 GB/s memory bandwidth (calculations/configs/hardware.json); CPU conversion throughput uses the eight-channel DDR5-4800 from the [Xeon Platinum 8480+ specification](https://github.com/bojieli/ai-infra-book/blob/main/references/files/specs/intel-xeon-8480plus-ark.md), $8\times4800\ \mathrm{MT/s}\times8$ B $=307.2$ GB/s; both are upper bounds computed from memory bandwidth. NVLink-C2C at 450 GB/s per direction is documented in the [GH200 architecture description](https://github.com/bojieli/ai-infra-book/blob/main/references/files/documents/nvidia-grace-hopper-blog.md).

[^megascale]: The paper reports per-step latency of 23.66/6.34 s, with scaling efficiency of about 93.3%; this corresponds to the full system scaling experiment with a fixed global batch size. [The official MegaScale paper](https://github.com/bojieli/ai-infra-book/blob/main/references/proceedings/NSDI/2024/selected/nsdi24-jiang-ziheng.pdf) and [fixed task, historical configuration, and measurement notes](https://github.com/bojieli/ai-infra-book/blob/main/case-studies/collective-paths-and-diagnosis.md).

[^moe]: [DeepSeek-V3 technical report](https://github.com/bojieli/ai-infra-book/blob/main/references/files/papers/deepseek-v3.pdf), [expert exchange and backward bucketing case](https://github.com/bojieli/ai-infra-book/blob/main/case-studies/expert-dispatch-and-resizing.md). The 72 MiB split into three buckets, four-member ring AllReduce, 0.02 ms startup, and three 2 ms gaps are the given input; the configuration of one 200 Gbit/s NIC per card is given in the [4090 cluster configuration analysis](https://github.com/bojieli/ai-infra-book/blob/main/references/files/documents/h100-vs-4090.md), and 400 Gbit/s is given in the [ConnectX-7 datasheet](https://github.com/bojieli/ai-infra-book/blob/main/references/files/specs/nvidia-connectx7-datasheet.pdf). Expert load balancing and routing mechanisms follow DeepSeek-V3; the chunked scheduling follows the linked case.

[^length]: [Training workload and length distribution estimation](https://github.com/bojieli/ai-infra-book/blob/main/case-studies/training-compute.md).

[^supply]: [Training data input and shared storage model](https://github.com/bojieli/ai-infra-book/blob/main/calculations/results/training-supply-shared-starvation.md), [input and save coexistence experiment](https://github.com/bojieli/ai-infra-book/blob/main/experiments/ch10/10-06/input-io-contention/README.md). The event model uses small-scale parameters to demonstrate how the prefetch queue gets drained when checkpointing and data input share storage; the design case in this chapter uses four data preparation processes, each preparing two sequences/s.

[^checkpoint]: [Checkpoint layout, ByteCheckpoint, and background save reading](https://github.com/bojieli/ai-infra-book/blob/main/case-studies/checkpoint-layout-and-loading.md), [logical resharding computation](https://github.com/bojieli/ai-infra-book/blob/main/calculations/results/checkpoint-reshard-book.md), [112 GB timeline](https://github.com/bojieli/ai-infra-book/blob/main/calculations/results/checkpoint-async-rounded.md).

[^resume]: [DCP recovery record](https://github.com/bojieli/ai-infra-book/blob/main/experiments/ch10/10-06/README.md), [real text pipeline recovery](https://github.com/bojieli/ai-infra-book/blob/main/experiments/ch10/10-06/input-state-recovery/README.md). The 2,000+6,192 figure is a teaching example illustrating leftover tokens during packing. The DCP small model restores two row-sharded copies into three column-sharded copies plus full state; the parameters, Adam state, random state, and next parameter-update result match uninterrupted training.

[^fault]: [Asynchronous save and pre-commit failure experiment](https://github.com/bojieli/ai-infra-book/blob/main/experiments/ch10/10-07/README.md) and [no-save/synchronous/asynchronous comparison](https://github.com/bojieli/ai-infra-book/blob/main/experiments/ch10/10-07/save-baseline/README.md). The original normal-path API takes about 7.25 ms, with metadata commit at about 48.30 ms; the failure experiment sets a barrier before commit and terminates the process.

[^interval]: [Save period and Poisson retry model](https://github.com/bojieli/ai-infra-book/blob/main/calculations/results/checkpoint-interval-book.md). The precise data volume is 114,670,295,040 bytes; at a write speed of 7 GB/s, the save cost is about 16.381 s; the first-order optimum is about 965.29 s, and the Poisson retry model including in-save failures gives about 954.40 s. The 7 GB/s figure is the aggregate write throughput of a single SU in the Good tier of Table 6 in the [DGX SuperPOD reference architecture](https://github.com/bojieli/ai-infra-book/blob/main/references/files/documents/dgx-superpod-h100-ra.pdf); a 1024-card job interrupts once every 7.9 hours on average, and mean time between failures is inversely proportional to card count, per the [Meta cluster reliability paper](https://github.com/bojieli/ai-infra-book/blob/main/references/text/meta-cluster-reliability.txt). The choice in this chapter only needs to distinguish five, fifteen, and thirty minutes.

[^verl]: [Fixed verl small-model real closed loop](https://github.com/bojieli/ai-infra-book/blob/main/experiments/ch10/10-08/README.md), [source-code verification of loss and update](https://github.com/bojieli/ai-infra-book/blob/main/research/2026-infra-survey/qa/verl-recipe-loss-closure.md). The fixed Qwen2.5-0.5B-Instruct training configuration uses single-card NO_SHARD with no_sync=False; the source code and version lock are recorded in the linked material. 0.32/0.30/0.04 are derived using the real length structure with a teaching scalar loss.

[^handoff]: [Qwen3-8B shared-device peak](https://github.com/bojieli/ai-infra-book/blob/main/calculations/results/weight-handoff-qwen8.md), [Qwen3-235B-A22B sharded weight synchronization](https://github.com/bojieli/ai-infra-book/blob/main/calculations/results/weight-handoff-book.md). 40 GiB is the given input for GPU memory footprint after sharding/offloading; the two switchover peaks are precisely about 83.256/59.256 GiB, compared against the H100 SXM nominal 80 GB (74.506 GiB).

[^opd]: [DeepSeek V4 technical report](https://github.com/bojieli/ai-infra-book/blob/main/references/files/papers/deepseek-v4.pdf), §5.1 on domain experts and multi-teacher OPD; stage work division follows [Chapter 3](https://github.com/bojieli/ai-infra-book/blob/main/manuscripts/03-推理与训练负载.md).

[^identity]: [RL state, probability ratio, and version](https://github.com/bojieli/ai-infra-book/blob/main/case-studies/rl-state-and-reproducibility.md), [DeepSeek V4 technical report](https://github.com/bojieli/ai-infra-book/blob/main/references/files/papers/deepseek-v4.pdf).

[^replay]: [Routing metadata budget](https://github.com/bojieli/ai-infra-book/blob/main/calculations/results/routing-metadata-book.md), [logs and analysis published by the R3 authors](https://github.com/bojieli/ai-infra-book/blob/main/experiments/ch10/10-09/source-readiness/README.md). The four paired settings share seed42; reward is a training-batch metric, while logprob is a log-level statistic from the R3 authors.

[^scale]: [Dense scale and deadline](https://github.com/bojieli/ai-infra-book/blob/main/calculations/results/dense-training-scale-book.md), [variant with data volume scaling proportionally with parameters](https://github.com/bojieli/ai-infra-book/blob/main/calculations/results/dense-training-scale-proportional.md). At 16,384 H100s, DP=128, and an 8K sequence length, 41% BF16 MFU is reported in Table 4 of the [Llama 3 report](https://github.com/bojieli/ai-infra-book/blob/main/references/files/papers/llama3.pdf).

[^public]: [SmolLM3 original training record verification](https://github.com/bojieli/ai-infra-book/blob/main/experiments/ch10/10-10/public-training/README.md). The SmolLM3 record includes rank stages at 384, 288, and 192, with the final resume actually running for 14,000 steps; the cumulative token count at the endpoint includes prior work.

[^design]: [Inputs, derivation, and recomputation for this chapter's design case](https://github.com/bojieli/ai-infra-book/blob/main/manuscripts/ch10/design-case.md). The capacity reservation, input waiting, and planned pauses for 32/48 cards are the given input; per-card compute efficiency uses Llama 3's MFU; the model's matrix work comes from the existing training-deadline computation, and the ZeRO-3 communication volume is derived from the same parameter table. The RTX 4090's PCIe 4.0 with no NVLink is given in the [RTX 4090 specification page](https://github.com/bojieli/ai-infra-book/blob/main/references/files/specs/nvidia-rtx4090.md); no P2P support between cards and eight 200 Gbit/s NICs per eight-card host are given in the [4090 cluster configuration analysis](https://github.com/bojieli/ai-infra-book/blob/main/references/files/documents/h100-vs-4090.md); within a single step, only the first AllGather and the last ReduceScatter cannot be hidden, per [MegaScale](https://github.com/bojieli/ai-infra-book/blob/main/references/text/megascale.txt) §3.2. Itemized values and independent recomputation are stored in [design-case.json](https://github.com/bojieli/ai-infra-book/blob/main/manuscripts/ch10/design-case.json).

[^v41-case]: [DeepSeek V4.1 official technical report](https://github.com/bojieli/ai-infra-book/blob/main/calculations/sources/deepseek-v4.1-flash/DeepSeek_V41_Tech_Report.pdf), Sections 1, 2, 3, and 6; [fixed conditions and recomputation across chapter sessions](https://github.com/bojieli/ai-infra-book/blob/main/calculations/results/v41-throughline.json).
