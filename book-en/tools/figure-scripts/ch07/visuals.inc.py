...
ax.set(xlim=(0,11),ylim=(3.7,-.7),xticks=range(12),yticks=range(4),yticklabels=[f'Stage {i+1}' for i in range(4)],xlabel="Time / ms")
ax.set_title("same micro-batch passes through four stages sequentially; different micro-batches compute concurrently",pad=18,fontsize=14)
ax.text(1,3,"idle at start",ha='center',va='center',fontsize=10,color=C['muted']);ax.text(9.5,0,"idle at end",ha='center',va='center',fontsize=10,color=C['muted'])
...
for i,title in enumerate(["direct remote read each time","move back once, reuse locally"]):
 y=.59-i*.48;a.text(.01,y+.28,title,fontsize=12,weight='bold');box(a,.02,y,.21,.20,"remote snapshot",size=11);box(a,.51,y,.20,.20,"local copy" if i else "local computation",size=11)
 arrow(a,(.24,y+.10),(.50,y+.10),'orange');a.text(.37,y+.21,"cross network once" if i else "cross network every time",ha='center',fontsize=10,color=C['orange'])
 box(a,.79,y,.19,.20,"repeated read" if i else "request again next time",size=10);arrow(a,(.72,y+.10),(.78,y+.10))
r=teaching['snapshot'];record={}
for idx,(fraction,limit,title) in enumerate([(1,4,"read full snapshot each time"),(.1,22,"read only 10% each time")]):
 ...
 ax.plot(xs,remote,c=C['orange'],label="remote read each time",lw=2);ax.plot(xs,stage,c=C['teal'],label="move back first",lw=2)
 cross=r['setup_s']/(fraction*(r['remote_per_read_s']-r['local_per_read_s']));cy=cross*r['remote_per_read_s']*fraction*1000
 ax.scatter([cross],[cy],c=C['ink'],s=24,zorder=4);ax.annotate(f'~ {cross:.1f} times',xy=(cross,cy),xytext=(cross*.48,cy+2.2),arrowprops={'arrowstyle':'->','color':C['muted']},fontsize=11)
 ax.set(xlim=(0,limit),ylim=(0,16 if idx==0 else 10),xlabel="read count",ylabel="cumulative time / ms",title=title);ax.legend(frameon=False,loc='upper left')
...
ax.set(xlim=(0,4),ylim=(-.6,1.6),yticks=[1,0],yticklabels=["128 slots","391 slots"],xlabel="time / μs",xticks=[0,.82,2,2.82,4]);ax.set_title("link has no data to send while slot not yet released",pad=16,fontsize=13)
ax.text(1.42,1,"waiting for slot",ha='center',va='center',fontsize=10);ax.text(3.4,1,"waiting for slot",ha='center',va='center',fontsize=10)
...
ax.plot(ns,bound,c=C['blue'],lw=2,label="consider only in-flight count and link limit");ax.plot(ns,np.minimum(bound,cap),c=C['orange'],lw=2,label="plus 18.6 ns startup interval limit");ax.axhline(50,c=C['muted'],ls=':',lw=1)
for n,val in [(128,16.384),(391,50)]:ax.scatter([n],[val],c=C['blue'],s=22);ax.text(n+7 if n<300 else n-7,val-3,f'{n} items: approx {val:.1f} GB/s',fontsize=10,ha='left' if n<300 else 'right')
ax.set(xlim=(0,400),ylim=(0,48),xlabel="concurrent request slot count",ylabel="throughput bound / GB/s");ax.legend(frameon=False,loc='upper left',fontsize=10)
...
for x,y,t in [(2.5,1.95,"sending"),(9.5,1.95,"source buffer reusable"),(4,.95,"destination buffer usage"),(10,.95,"receiver use")]:ax.text(x,y,t,ha='center',va='center',fontsize=11)
ax.plot([6,6],[.2,1.35],color=C['red'],lw=1.5);ax.scatter([6],[.95],marker='x',s=75,c=C['red']);ax.text(6,.04,"6 μs overwrite: new data replaces old data",ha='center',fontsize=10,color=C['red'])
ax.set(xlim=(0,14),ylim=(-.2,2.6),yticks=[1.95,.95],yticklabels=["source buffer","destination buffer"],xticks=[0,5,8,12,14],xlabel="time / μs",title="after send completes, receiver may not yet use data")
...
ax.scatter([3],[2.2],c=C['teal'],s=45);ax.text(3.12,2.2,"ready flag visible",va='center',fontsize=11)
ax.plot([1,5],[1.3,1.3],c=C['red'],lw=2);ax.scatter([1],[1.3],c=C['red'],s=50,zorder=3);ax.scatter([5],[1.3],facecolors='white',edgecolors=C['red'],s=65,zorder=3);ax.text(1,1.58,"read 0",ha='center',fontsize=10);ax.text(5,1.58,"still delivers 0",ha='center',fontsize=10)
ax.scatter([4],[.45],c=C['ink'],s=35);ax.plot([4,6],[.45,.45],c=C['teal'],lw=2);ax.scatter([6],[.45],facecolors='white',edgecolors=C['teal'],s=65,zorder=3);ax.text(4,.13,"recheck then reread",ha='center',fontsize=10);ax.text(6,.13,"get 1",ha='center',fontsize=10)
ax.set(xlim=(0,6.7),ylim=(-.1,3.6),xticks=range(7),yticks=[3,2.2,1.3,.45],yticklabels=["remote data","publish notification","reorder delivery only","check and reread"],xlabel="time / μs",title="late delivery doesn't turn an already-read old value into new")
...
for ax,name,title in zip(axs,['completion-reclaim-book','completion-reclaim-more-slots'],["8 slots: later submission waits for reclaim","16 slots: transfer finishes early, still waits for reclaim"]):
 ...
 ax.set(ylim=(15.8,-.8),yticks=[0,3,7,11,15],yticklabels=[1,4,8,12,16],ylabel="request ID",title=title,xlim=(0,82),xticks=[0,20,40,48,60,80])
axs[0].legend(handles=[Patch(color=C['blue'],label="in transit"),Patch(color='#eac799',label="sent, awaiting completion notice")],ncol=2,frameon=False,loc='upper center',bbox_to_anchor=(.5,1.30),fontsize=11)
axs[1].set_xlabel("time / μs");save(f,'figure-7-14-reclaim');data['7-14']={'kind':'saved events','operations':rec}
...
box(a,.01,.30,.21,.38,"sender",'100 → 40 GB/s',size=12);box(a,.39,.30,.25,.38,"512 KiB buffer","initial 256 KiB",col='sand',size=12);box(a,.80,.30,.19,.38,"egress",'50 GB/s',size=12);arrow(a,(.23,.49),(.38,.49));arrow(a,(.65,.49),(.79,.49));a.annotate('',xy=(.12,.22),xytext=(.51,.22),arrowprops={'arrowstyle':'->','ls':'--','color':C['red']});a.text(.32,.01,"feedback takes effect at 20 μs",ha='center',fontsize=10,color=C['red'])
...
ax.plot(tt,qq,c=C['orange'],lw=2.5);ax.fill_between(tt,qq,color=C['sand']);ax.axvspan(256*1024/50e9*1e6,20,color=C['red'],alpha=.14);ax.axvline(20,c=C['red'],ls='--',lw=1);ax.text(22,535,"reduced to 40 GB/s",fontsize=10,color=C['red']);ax.text(14.4,580,"buffer full, excess data dropped",ha='center',fontsize=10,color=C['red']);ax.annotate("~72 μs to drain",xy=(72.4288,0),xytext=(49,130),arrowprops={'arrowstyle':'->','color':C['muted']},fontsize=11)
ax.set(xlim=(0,82),ylim=(0,640),xticks=[0,5.2,20,40,60,72.4,80],yticks=[0,256,512],xlabel="time / μs",ylabel="queue occupancy / KiB")
...
for ax,name,title in zip(axs,['packet-reorder-balanced','packet-reorder-skewed'],["both paths 1 μs: ~1.33 μs full delivery","propagation delay 1/9 μs: ~9.33 μs full delivery"]):
 ...
 ax.set(ylim=(7.8,-.8),yticks=range(8),ylabel="packet sequence number",title=title,xlim=(0,10),xticks=[0,1,2,4,6,8,10]);pack[name]={'transmissions':z['transmissions'],'delivery':z['delivery']}
axs[0].text(5.8,2,"Dot: arrival\nLine: waiting for gap ahead\nVertical line: in-order delivery",fontsize=11,linespacing=1.6)
axs[1].set_xlabel("time / μs");save(f,'figure-7-17-packets');data['7-17']={'kind':'saved packet events','cases':pack}
...
cases=[("contiguous ring, serial",20,p['flat_s']*1000),("hierarchical reduction, serial",20,p['hier_s']*1000),("hierarchical, only 128 in flight",20,st['hier_window_limited_comm_s']*1000),("contiguous ring, ready at 17 ms",17,p['flat_s']*1000),("hierarchical, ready at 17 ms",17,p['hier_s']*1000),("hierarchical, only one NIC left",17,st['hier_one_nic_comm_s']*1000)];steps=[]
...
ax.axvline(20,c=C['muted'],ls=':',lw=1);ax.set(xlim=(0,39),ylim=(.1,11),yticks=[],xticks=[0,5,10,15,20,25,30,35],xlabel="Time / ms");ax.legend(handles=[Patch(color='#dbe2e6',label="compute"),Patch(color=C['blue'],label="communication"),Patch(color=C['teal'],label="update")],ncol=3,frameon=False,loc='upper center',bbox_to_anchor=(.5,1.09))
...
ax.loglog(size,startup,c=C['orange'],ls=':',lw=1.7,label="startup: 14 × 5 μs");ax.loglog(size,payload,c=C['muted'],ls='--',lw=1.7,label="payload only: 50 GB/s")
ax.loglog(size,startup+payload,c=C['blue'],lw=2.4,label="total time: 50 GB/s, 5 μs");ax.loglog(size,startup+payload/3,c=C['teal'],lw=2,label="bandwidth increased to 150 GB/s");ax.loglog(size,28+payload,c=C['red'],lw=2,label="per-round startup down to 2 μs")
ax.axvline(2e6,c=C['line'],lw=1);ax.text(2.36e6,1.8,"two terms equal: 2 MB",fontsize=10);ax.text(2500,190,"small messages: startup dominates",fontsize=11);ax.text(2.2e6,1800,"large messages: transfer dominates",fontsize=11)
ax.set(xlim=(1024,16*2**20),ylim=(.05,4000),xlabel="input size per participant",ylabel="one ring all-reduce time / μs");ax.set_xticks([1024,8192,65536,2**20,8*2**20],['1 KiB','8 KiB','64 KiB','1 MiB','8 MiB']);ax.legend(frameon=False,loc='lower right',fontsize=10)
