"""Ten official scenarios, twenty real unified CLI calls, streamed byte comparison."""
...
def readiness():
 cases=[r for r in json.loads((PROJECT/'scenarios/book.json').read_text())['transport_closed_loop'] if r['id'].startswith('ack-policy-')]
 assert len(cases)==10 and len({r['id'] for r in cases})==10
...
def run():
 ...
 for case in cases:
  for fmt in ('json','md'):
   ...
   with tempfile.TemporaryDirectory(prefix='ack-policy-cli-') as td:
 ...
 return dict(status='passed',scenario_count=len(cases),cli_calls=len(checks),checks=checks,source_hashes_before=code_before,source_hashes_after=code_after,source_fingerprint=code_fingerprint,results_manifest_sha256=manifest_before,elapsed_seconds=round(time.monotonic()-started,3),scope='Twenty actual CLI calls and full streamed byte equality against official manifested artifacts; not a reproduction run')
