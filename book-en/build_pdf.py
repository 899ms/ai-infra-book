#!/usr/bin/env python3
"""Build the English edition PDF (Pandoc + XeLaTeX), mirroring book/build_pdf.py.

The chapters here are already assembled (see assemble.py): figures point at
images/ and repository links are absolute GitHub URLs. This script adds the
cover, the preface as front matter, page numbering and the same build report
that the Chinese build writes, so CI can validate both editions alike.
"""
...
def run(command, *, cwd=HERE, log=None):
    env = dict(os.environ)
    # The current directory must come first so cover.tex resolves to the
    # English cover, not the Chinese one that TEXINPUTS also exposes.
    env['TEXINPUTS'] = f'.:{BOOK}:{BOOK / "template"}:'
    # Large book: extend XeTeX memory at runtime rather than rebuilding the format.
    env.setdefault('extra_mem_top', '8000000')
    env.setdefault('extra_mem_bot', '8000000')
    # Without Menlo, kpathsea would otherwise spawn METAFONT for a Menlo.tfm.
    ...
    def image(match):
        ...
        if selected.read_bytes().startswith(LFS_POINTER):
            raise ValueError('Figure is an LFS pointer; run '
                             f'git lfs pull --include="book-en/images/**" --exclude="": {selected}')
        assets.append(selected)
        return f'![{match[1]}]({selected.as_posix()})'
    text = re.sub(r'!\[([^\n]*)\]\(([^)]+)\)', image, text)
    if source_ref:
        # Pin repository links in a released PDF to the commit it was built from.
...
def main():
    ...
    parser.add_argument('--output-dir', type=Path, default=HERE,
                        help='Output directory relative to book-en/ (default: book-en/)')
    ...
    command = ['pandoc', *map(str, inputs), '--file-scope', '--standalone',
               ...
               '--lua-filter=' + str(BOOK / 'layout.lua'),
               '-V', 'documentclass=elegantbook', '-V', 'classoption=lang=en',
               ...
               '--metadata', 'author-meta=Bojie Li (English edition)',
    ...
    # XeLaTeX runs from book-en/ so cover.tex, images/ and ../book/ resolve.
