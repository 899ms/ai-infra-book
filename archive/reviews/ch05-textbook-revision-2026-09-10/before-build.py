...
a.set(xlim=(0,100),ylim=(0,6.7),xlabel='片上活动空间 / KiB',ylabel='工作缓冲与下一层之间的访问 / GiB')
...
save(f,'figure-5-2-fusion-buffer');data['5-2']={'kind':'teaching_timeline','relationship':'same four tiles, different overlap','copy_us':2,'compute_us':3,'serial':serial,'double_buffer':pipelined,'completion_us':[20,14]}
...
save(f,'figure-5-3-boundaries');data['5-3']={'kind':'fixed_scale_teaching','relationship':'materialized boundaries versus interface bytes','required_MiB':base.tolist(),'T_write_read_MiB':t.tolist(),'Z_write_read_MiB':z.tolist()}

f,a=canvas(9)
steps=[('原始循环','1024 × 12288 个输出\n完整 BF16 中间矩阵：24 MiB'),('split / tile','输出块 64 × 64\n16 × 192 = 3072 个输出块'),('reorder','io → jo → ko → 块内工作\n每个输入元素供多个输出复用'),('局部缓存与计算位置','当前 A/W：各 4 KiB\nFP32 累加器：16 KiB'),('完成归约后做 SiLU','完成 128 个 k 块后再激活\n省去完整 C 的 48 MiB 读写')]
for i,(t,b) in enumerate(steps):
 y=.80-i*.175;box(a,.06,y,.57,.145,t,b,'light' if i==4 else 'pale',13)
 if i<4:arrow(a,(.34,y-.006),(.34,y-.026))
a.text(.71,.83,'遍历顺序改变',fontsize=13,color=C['muted'])
a.text(.71,.62,'矩阵乘工作量不变\n约 103 GFLOPs',fontsize=13,linespacing=1.8,color=C['blue'])
a.text(.71,.39,'当前工作集\n8 + 16 = 24 KiB',fontsize=13,linespacing=1.8,color=C['blue'])
a.text(.71,.17,'先完成归约\n再消费结果',fontsize=13,linespacing=1.8,color=C['teal'])
save(f,'figure-5-4-polyhedral');data['5-4']={'kind':'loop_transformation','relationship':'move production and consumption into a tile','steps':[x[0] for x in steps],'tile':[64,64,32],'output_tiles':3072,'k_tiles':128,'input_KiB':8,'accumulator_KiB':16,'eliminated_read_write_MiB':48}

f,a=plt.subplots(figsize=(10.5,5.6));f.subplots_adjust(left=.11,right=.97,top=.92,bottom=.16)
p=np.linspace(0,1,301);a.plot(p*100,np.full_like(p,10),lw=2.4,color=C['blue'],label='原始：10 μs')
a.plot(p*100,20-15*p,lw=2.4,color=C['teal'],label='候选：20 − 15p μs')
a.axvline(200/3,color=C['orange'],ls='--');a.text(200/3+2,19,'A 占比 2/3',color=C['orange'],fontsize=11)
for x,y,label,offset in [(50,12.5,'各出现一次：候选慢 25%',(-160,65)),(10000/101,520/101,'A ×100、B ×1：候选约快 1.9 倍',(-260,-30))]:
...
save(f,'figure-5-6-specialization');data['5-6']={'kind':'teaching_from_calculations','source':'calculations/results/specialization-medium.json','policies':policies}
...
save(f,'figure-5-7-runtime');data['5-7']={'kind':'measured_profile','source':'experiments/ch05/05-08/results/trace-analysis.json','ranges':ranges}
...
a.set_yticks(range(-3,6));a.set_xticks(x);a.set(xlim=(.5,11.5),ylim=(-3.2,5.3),xlabel='配对轮次',ylabel='原生 − 候选的完整请求时间 / ms');a.legend(frameon=False,loc='upper left',fontsize=11)
a.text(7.3,4.6,'9 对更快，2 对更慢\n请求总时间约 817 ms',ha='right',va='top',fontsize=11,linespacing=1.7)
save(f,'figure-5-8-request');data['5-8']={'kind':'measured_paired_requests','relationship':'paired full-request time savings','source':'experiments/ch05/05-09/results/summary.json','pairs':pairs,'median_saving_ms':float(np.median(v)),'baseline_median_ms':summary['rows'][0]['median_latency_ms']}
...
(HERE/'manifest.json').write_text(json.dumps({'chapter':5,'generator':'manuscripts/ch05/build.py','figures':8,'font_family':family,'outputs':[{'path':str(p.relative_to(ROOT)),'sha256':hashlib.sha256(p.read_bytes()).hexdigest()} for p in artifacts]},ensure_ascii=False,indent=2)+'\n')
