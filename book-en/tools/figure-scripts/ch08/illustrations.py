...
def draw(save, canvas, box, arrow, C, data):
    ...
    a.text((first+last)/2,.49,"255 output intervals × 26.5 ms = 6.76 s",ha='center',va='center',color='white',fontsize=13)
    a.annotate("queueing 0.1 s",xy=(.05,.63),xytext=(.05,1.00),arrowprops={'arrowstyle':'-','color':C['ink']},fontsize=11)
    ...
    a.text(first,.22,"first token\n0.196 s",ha='center',va='top',fontsize=11)
    a.text(last-.25,.22,"last token\n6.95 s",ha='center',va='top',fontsize=11)
    a.axvline(7,color=C['red'],ls='--',lw=1.4)
    a.text(7.05,.90,"7 s deadline",color=C['red'],fontsize=11)
    a.set(xlim=(0,7.8),ylim=(-.18,1.15),yticks=[],xticks=[0,1,2,3,4,5,6,7],xlabel="since request arrival / s")
    ...
    for a,h in zip(axs,[0,8]):
        ...
        a.set(xlim=(-.05,12.05),ylim=(4.1,-.1),aspect='equal',yticks=[.5,1.5,2.5,3.5],yticklabels=["new 1","new 2","new 3","new 4"],xticks=[])
        a.tick_params(length=0);a.spines[['left','bottom']].set_visible(False)
        a.set_title(f'history {h} positions:'+("10 within-block pairs" if h==0 else "32 history pairs + 10 within-block pairs = 42"),loc='left',fontsize=12,pad=12)
        a.text(h+2,4.65,"4 key positions in this block",ha='center',fontsize=11)
        if h:a.text(h/2,4.65,"8 old history key positions",ha='center',fontsize=11)
        pairs.append({'history':h,'new':4,'history_pairs':4*h,'within_pairs':10,'total_pairs':4*h+10})
    f.text(.5,.035,"green: old history access    blue: within-block causal access    gray: future position, not accessed",ha='center',fontsize=11)
    data['attention_geometry']=pairs;save(f,'figure-8-4-attention')

    f,a=canvas(4.5)
    a.text(.04,.93,"same 864 MiB, how to use it?",fontsize=15,weight='bold')
    box(a,.06,.58,.63,.19,'A：864 MiB',col='blue')
    va=round(worked['cache']['A_gen5'][2]*1e3,1);vb=round(worked['cache']['B_gen5'][2]*1e3,1)
    a.text(.76,.675,f'total saved {va} ms',va='center',fontsize=14,color=C['blue'])
    for i in range(3):box(a,.06+i*.21,.20,.203,.23,f'B{i+1}：288 MiB',f'saved {vb} ms',size=12)
    a.text(.76,.315,f'total saved {3*vb:.1f} ms',va='center',fontsize=14,color=C['teal'])
    ...
    for y,name,payload,metadata in [(2,'BF16',64,0),(1,'q8_0',32,2),(0,'q4_0',16,2)]:
        ...
        if metadata:
            ...
            a.annotate("scale factor 2 bytes",xy=(payload+1,y+.25),xytext=(payload+7,y+.39),fontsize=10,arrowprops={'arrowstyle':'-','color':C['orange']})
        a.text(payload+metadata+1,y,f'total {payload+metadata}',va='center',fontsize=11)
    a.set(xlim=(0,73),ylim=(-.65,2.6),yticks=[0,1,2],yticklabels=['q4_0','q8_0','BF16'],xticks=[0,16,32,48,64],xlabel="bytes used by same group of 32 values")
    ...
    f,a=canvas(5.8)
    xs=[.23,.40,.57,.74]
    for x,label in zip(xs,["position 1","position 2","position 3","position 4"]):a.text(x+.055,.94,label,ha='center',fontsize=11)
    for y,label in [(.70,"draft"),(.41,"target verification"),(.10,"final output")]:a.text(.035,y+.075,label,va='center',fontsize=13)
    for x,token in zip(xs,['a','b','c','d']):box(a,x,.70,.11,.15,token,size=16)
    for x,token,col in zip(xs,['a','b','x',"discard"],['teal','teal','orange','gray']):
    ...
    a.text(.625,.635,"third mismatch",ha='center',fontsize=11,color=C['orange'],bbox={'fc':'white','ec':'none','pad':1})
    a.text(.83,.31,"downstream dependency on c\nno longer used",ha='center',fontsize=11,color=C['ink'])
    a.text(.78,.06,"keep 2 + correct 1 = 3 outputs",ha='center',fontsize=11)
    ...
    a.text(12.3,1.2,"12 GiB limit",color=C['orange'],fontsize=11)
    a.text(.4,7.25,"7 s deadline",color=C['red'],fontsize=11)
    labels={'A':"insufficient memory",'B':"shared, timeout",'C':"timeout",'D':"shared + speculative"}
    ...
    a.text(1,2.0,"sufficient memory, on-time completion",fontsize=13,color=C['teal'])
    a.set(xlim=(0,21),ylim=(0,16.5),xlabel="KV and auxiliary buffer / GiB",ylabel="total batch time / s",xticks=[0,6,12,18],yticks=[0,3,6,7,9,12,15]);a.grid(alpha=.12)
