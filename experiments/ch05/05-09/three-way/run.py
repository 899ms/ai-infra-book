...
async def main(a):
    ...
    result=dict(config=config,protocol=protocol,source_hashes={f:sha(ROOT/f) for f in ['run.py','replacement_worker.py','candidate.py','agent_candidate.py','input.json','engine-config.json','selection-provenance.json']},requests=[],audits=[],switches=[],status='running')
    ...
    try:
        ...
        for mode in ['native','schedule','agent']:
        ...
        with (a.output/'requests.jsonl').open('w',buffering=1) as log:
            for trial in range(protocol['trials']):
                order=['native','schedule','agent'];rng.shuffle(order)
                for mode in order:
                    result['switches'].append(await engine.collective_rpc('set_replacement',args=(mode,False)))
