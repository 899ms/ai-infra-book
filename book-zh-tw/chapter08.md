...
同一個模型執行在同一臺加速器上，單個使用者每秒可能只收到幾十個 token，所有使用者合計每秒卻能收到上千個 token。decode 每讀一遍權重只做很少的計算，速度受限於視訊記憶體頻寬而不是峰值算力；把多個請求合併計算、共用一次讀取的權重，總吞吐就能成倍提高。與此同時，每條請求的上下文長度、排隊時間和執行順序又決定了使用者何時能收到答案。本章討論如何組織這些請求，在給定時間內回傳更多正確答案。

推理實例可以使用一張卡，也可以由多張卡、多臺伺服器協同執行。第 6、7 章已經說明模型計算如何切分，以及加速器如何通訊。本章以給定加速器組合為基礎，研究批處理、請求排程、記憶體管理、壓縮、卸載和推測解碼，比較不同執行設定的執行效率。第 9 章再討論計算和狀態應分佈在哪裡，如何組織完整副本、階段服務池和運算子分工，並據此確定服務規模。

本章從具體的設計問題出發。範例中的 Qwen3-8B 實例執行在一張 RTX PRO 6000 Blackwell 工作站版顯示卡上：視訊記憶體 96 GB，頻寬 1792 GB/s，BF16 輸入、FP32 累加的稠密峰值為 503.8 TFLOP/s，功耗上限 600 W。[^device]除了在 Apple M2 Max 上執行的實驗 8-7，本章實驗都在這張卡上完成。模型權重採用每元素 2 位元組的 BF16 浮點格式，約佔 15.3 GiB。KV 快取儲存上下文 token 的鍵（K）和值（V），供後續注意力計算讀取。與實驗 8-9 的設定相同，本實例分到 32 GiB 視訊記憶體，為 KV 快取及輔助緩衝區預留 12 GiB，其餘約 4.7 GiB 用於活化值、計算圖重放（第 5.5 節）的固定緩衝，以及其他執行工作區。

實例處理兩類請求：短對話輸入 2048 個 token，長上下文請求輸入 8192 個 token，二者都輸出 256 個 token。長請求的前 6144 個 token 是相同的系統提示和工具定義。要求每條請求從到達到回傳完整的正確答案，耗時不超過 7 秒。一條短請求在這張卡上單獨執行約需 6.95 秒（第 8.1.1 節），該時限只留出約 0.05 秒的餘量。後文將反覆使用這組條件。
...
首先計算實例能夠同時儲存多少條請求的 KV 快取，再計算增大 batch 能為每個輸出 token 減少多少權重讀取。隨後引入分頁、字首共享、壓縮、卸載和推測解碼，分別分析各自節省了什麼、增加了什麼。章末將這些分析用於同一個問題：16 條長請求同時到達時，應選擇哪種設定？
...
![請求身份與每輪 batch](images/figure-8-request-iterations.pdf)
...
隨後進入 decode 階段。模型將剛生成的 token 作為下一次前向計算的輸入，讀取上下文 KV，再生成下一個 token。同一條請求必須依次完成這些步驟，不同請求的當前步驟則可以一起計算。因此，即使單條請求只能逐個生成 token，實例仍能透過合批提高總吞吐。

在這張卡上，實驗 8-1 單獨執行一條 2K 輸入的短請求：prefill 並回傳首個 token 用時約 0.096 秒，此後平均每隔 26.5 ms 回傳一個 token。假設這條請求先排隊 0.1 秒，使用者便在到達後的第 0.196 秒收到第一個 token。總共輸出 256 個 token，需要經歷 255 個輸出間隔，因此請求總耗時為
...
圖 8-2 把這 6.95 秒畫成一條時間線。首個 token 的回傳時刻將時間線分成兩段：此前的排隊和輸入處理決定使用者何時看到回應，此後的 255 個間隔決定使用者何時收到完整答案。後續所有排程最佳化，都可以理解為改變這條時間線上某一段的長度。

![請求從到達到最後一個輸出的時間線](images/figure-8-1-lifecycle.pdf)
...
要讓多條請求沿各自的時間線向前執行，實例必須同時儲存權重、請求狀態和計算中間結果。本節的算例只用一張卡，因此只列出這張卡上的記憶體佔用。多卡實例則要按各卡儲存的權重分片、KV 和緩衝區分別列式：某張卡上的空閒記憶體，只有改變資料放置，才能用於另一張卡的計算。

用 $M_w$ 表示常駐權重的容量，$M_{\mathrm{KV}}$ 表示去除重複引用後的物理 KV 塊總容量（KV 按固定大小的塊分配，多條請求共用的塊只計一次），$M_a$ 表示活化值與普通工作區的容量，$M_u$ 表示計算圖重放等其他緩衝區的容量。這些記憶體佔用之和必須小於或等於加速器可用容量 $C$：

$$
M_w+M_{\mathrm{KV}}+M_a+M_u\le C.
$$

在這幾項記憶體佔用中，權重在請求到達前就已載入到記憶體，KV 則隨著請求到達和生成不斷增長。要算出實例能同時處理多少請求，先求每個 token 要儲存多少 KV。Qwen3-8B 有 36 層，每層有 8 個 KV 頭，每個頭的維度為 128。每個 token 在每層都需要儲存 K 和 V，BF16 每個元素佔 2 bytes，因此每個 token 所需的 KV 容量為
...
把第 8.1.1 節時間線的兩段寫成可以設定時限的指標。設請求到達時刻為 $t_a$，共輸出 $G$ 個 token，第一個與最後一個 token 回傳給使用者的時刻分別為 $t_1$、$t_G$。於是
...
第 8.1.1 節的例子中，TTFT 為 0.196 秒，後續 255 個輸出間隔合計 6.76 秒。較短的 TTFT 讓使用者儘早看到回應，均勻的輸出間隔便於連續閱讀，而總耗時決定完整答案何時可用。SLO 將這些要求量化為時限。章首算例要求總耗時不超過 7 秒，排程算例還會比較最長輸出間隔。

這些指標針對單條請求；實例整體的產出，還取決於一次同時處理多少請求，即 batch size（下文簡稱 batch）。吞吐表示單位時間內完成的工作量。實驗 8-1 同時處理 16 條 2K 請求時，引擎每輪 decode 約 27.44 ms，每輪共輸出 16 個 token，生成吞吐約為 $16/0.02744\approx583$ token/s；單條請求每 26.26 ms 輸出一個 token 時，該值只有約 38。增加每輪輸出數或縮短每輪時間，都能提高吞吐；使用者實際等待多久，還取決於請求開始執行前的排隊時間。

衡量服務效果還要檢查答案是否正確。固定輸出長度，便於在相同工作量下比較執行時間；讓模型自然停止，則可以觀察答案正確率、回答長度和重試次數。第 8.6 節將計算單位時間內正確且按期完成的請求數，同時評價速度與品質。
...
![batch 增大時每個輸出 token 分攤的權重與上下文讀取量](images/figure-8-2-batch.pdf)
...
落實到請求時間上，還要看更大的 batch 是否延遲了首個 token，以及相鄰輸出之間是否等得更久。batch 掃描實驗同時記錄了這兩項變化。Qwen3-8B 輸入 2K、輸出 256 個 token 時，batch 從 1 增至 64，總吞吐從約 37 增至 1165 token/s；但首個 token 的回傳時間從約 96 ms 延後到 3.29 秒，平均輸出間隔從約 27 增至 40 ms。每輪輸出雖然更多，但處理更多輸入、執行更大的 batch 也花了更多時間。該實驗中每批請求同時提交；線上服務若額外等待 batch 集滿，這段時間還要計入首 token 延遲。[^batch-measure]
...
如果權重改由足夠快的獨立儲存提供，讀取耗時減少，批處理的收益就需要重新計算。沿用第 4 章的 8K 範例，設 $W$ 為整批讀取的權重位元組數，$B$ 為批內請求數，$K$ 為單請求本步讀取的 KV 位元組數。傳統方案中，每個輸出 token 分攤的 HBM 讀取量為 $W/B+K$；權重由足夠快的獨立 ROM 提供時，HBM 每輸出一個 token 仍需讀取 $K$。[^core-calculation] 圖 8-4 中兩條曲線的距離隨 batch 增大而縮小，說明原先依靠 batch 攤薄權重讀取的收益正在消失。

不過，batch 仍會影響矩陣計算對計算單元的利用率，以及 batch 內的 KV 讀取量。將新的讀取量代入上述時間模型，就能求出計算與 KV 讀取的交點，再按首 token 延遲和輸出間隔選擇 batch。

![每輸出 token 分攤的 HBM 讀取量](images/figure-8-batch-counterfactual.pdf)
...
![decode 優先的分塊執行](images/figure-8-scheduling-chunked.pdf)
...
$ch$ 來自新 token 對舊上下文的存取，三角形項來自塊內的因果注意力。取 $c=512$，首塊的 131,328 個配對全部來自塊內；8K 輸入的末塊已有 7680 個上下文 token，配對數增至 4,063,488，約為首塊的 31 倍。

圖 8-8 將配對數畫成面積：每個新 token 都要存取全部舊上下文，形成左側矩形；塊內只能關注當前 token 位置及其之前的位置，形成右側三角形。塊長固定時，右側三角形不變，左側矩形隨上下文增長而變寬。這就是末塊需要更多注意力計算的原因。

![同一塊新 token 存取舊上下文和塊內位置的範圍](images/figure-8-4-attention.pdf)

*圖 8-8：首次處理 4 個 token 時，沒有舊上下文，只有新 token 之間的因果注意力配對，形成 1 + 2 + 3 + 4 = 10 個綠色格。白格表示未來 token，不參與當前查詢。*

![同一塊存取更長上下文](images/figure-8-attention-history.pdf)
...
第 8.1 節按最大輸出長度為每條請求預留空間，保證它能持續生成。如果請求提前結束，預留空間的尾部就一直沒有使用。另一種辦法是隨生成過程逐步擴充記憶體，但若必須保持空間連續，鄰接區域被其他請求佔用時，就需要搬移資料或等待。
...
![從邏輯塊查到物理塊](images/figure-8-page-map.pdf)
...
最多浪費 $p-1$ 個 token 的 KV 空間。塊越小，這一上限越低，塊表項和分配次數也越多。2023 年，伯克利等機構的研究者在 vLLM 中提出 PagedAttention，將作業系統分頁管理的思路用於 KV 快取，減少預留空間與記憶體碎片對服務併發的限制。PagedAttention 將這種對映帶入注意力執行；FlashAttention 則在運算子內部組織分塊計算與中間結果，兩者分別作用於狀態分配和計算訪存。
...
![整段預留、分頁與字首共享的 KV 分配容量](images/figure-8-5-pages.pdf)

*圖 8-11：四條請求分別含 9、13、5、15 個 token，每條預留可儲存 16 個 token 的 KV 空間，共分配可儲存 64 個 token 的 KV 空間。藍色已用，灰色預留未用。*

![改用每塊 4 個 token 的分頁](images/figure-8-pages-paged.pdf)
...
![兩個請求引用同一字首](images/figure-8-pages-shared.pdf)

*圖 8-13：A、B 的兩個共同字首塊只儲存一次，各自塊表都指向它們。A、B 保留各自的私有尾塊；四條請求實際分配的 KV 容量進一步降至 44 個 token。*

### 8.3.2 分支共享、寫時複製與記憶體釋放

字首共享圖中，B 與 A 引用相同的兩個物理塊。如果 A 先結束，這兩個塊仍要留給 B；如果 B 要修改其中的內容，又不能影響 A。要實作共享，就需要確定何時釋放共享塊，以及如何避免不同請求互相覆蓋資料。每張塊表中指向某個物理塊的記錄稱為一個**引用**，共享塊由多個塊表引用；**引用計數**記錄還有多少使用者需要它。每當一條請求獲得引用，引用數加一；請求結束時釋放引用。最後一個引用釋放後，該塊才能回到空閒池；此時還要確認加速器操作不再存取它。這樣，同一份共同上下文既能服務多個分支，也能在其中一條分支結束後繼續服務其餘分支。

![根據引用計數釋放共享塊](images/figure-8-reference-release.pdf)
...
![分支寫入前複製共享尾塊](images/figure-8-copy-on-write.pdf)
...
塊用完之後，還要正確釋放才能交給後續請求。釋放記憶體需要同時滿足兩個條件：沒有使用者繼續持有引用，已提交的加速器操作也不再存取該塊。使用者取消請求後，排程器停止安排新計算，等待已提交的操作完成，隨後釋放私有塊。一次觀察中，取消呼叫約 1.6 ms 就回傳了，而對應的 104 個塊直到約 31 ms 才釋放。後續請求在釋放之後使用這些塊，便不會覆蓋舊請求仍在讀取的資料。[^cancel]

![取消後先停止排程，再等待釋放](images/figure-8-cancel-lifetime.pdf)

*圖 8-16：取消介面回傳表示已接收取消請求。已提交的加速器操作完成後，再釋放私有塊和相關引用；觀測中的 1.6 ms 與 31 ms 對應不同事件。*
...
![多輪輸入的壓縮字首樹與共同字首長度](images/figure-8-6-prefix.pdf)

*圖 8-17：前四輪程式碼 Agent 輸入的壓縮字首樹。根部已有 206 個共同 token，邊上是新增數量，葉子是輸入輪次。分叉表示後續內容不同。*

![十二輪輸入中的共同字首](images/figure-8-prefix-lengths.pdf)

*圖 8-18：藍色為與上一輪逐 token 相同的字首，橙色為其餘輸入。此圖描述輸入內容的可複用程度，實際快取命中還取決於狀態是否保留。*

每輪在末尾追加內容，可以保留已有的公共字首，但上下文越長，KV 佔用和後續讀取量也越大。

**擴充：混合注意力模型如何確定可恢復的字首位置。** 全注意力保留逐 token KV，遞推模型通常只保留當前狀態。設文字匹配到第 10,752 個 token，遞推快照分別儲存在第 4096 和第 8192 個 token；系統從 8192 恢復，再計算到 10,752，重算 2560 個 token。增加快照可以減少重算的 token 數，但要儲存更多狀態。以第 2 章的 KDA 為例，一種實作將層內張量分到八張卡（TP=8），此時每卡每份快照約 53.6 MiB，儲存 2 份約 107 MiB，儲存 32 份約 1714 MiB。快照儲存得越密，恢復時需要重算的 token 就越少，保留的狀態也越多。[^hybrid]

![從最近遞推快照恢復](images/figure-8-prefix-restore.pdf)
...
![同一歷史的三種更新方式](images/figure-8-context-edits.pdf)

*圖 8-20：同一歷史的三種更新方式。藍色表示可複用字首，橙色表示需要重新處理的輸入；總結方案先生成摘要，再重建快取。長度以 K token 示意。*

模型側的字首複用還涉及更細的狀態區分。DeepSeek V4.1 在複用字首時，對兩類區域性狀態分別處理。第 2 章介紹的 CED 結構包含因果編碼器和解碼器，兩者各有 SWA 的區域性狀態：**編碼器 SWA**可以在主機 DRAM 池中短期保留，供下一輪沿已有字首繼續處理新輸入；**解碼器 SWA**只為本輪後續生成使用，論文中的部署不把它作為字首快取儲存。全域 KV 則進入可較長期複用的快取。因此，生成期間駐留的 40 層 SWA 與跨輪次保留的字首快取，需要分別計算容量。[^v41-case]

這兩類狀態是否命中，決定了下一輪續算時編碼器要恢復多少字首。設工具回傳後，恢復所需的 token 或輸入表示仍可取得，模型版本、token 位置索引與輸入保持一致。全域 KV 和編碼器 SWA 都命中時，編碼器直接處理追加輸入；僅全域 KV 命中時，編碼器重放已快取字首的最近最多 128 個 token，並與未快取字尾一起執行。重放段重建編碼器 SWA，繼續使用既有全域 KV；新增字尾產生新的全域 KV 和 SWA。全域 KV 也未命中時，則重算缺失字首。
...
短視窗重放得到的是近似恢復的狀態，誤差來自更早的區域性依賴被截斷。每層雖然只存取最近 128 個 token，多層疊加後，區域性狀態仍可透過前一層間接依賴更早的輸入。從視窗邊界重新執行，會改變這部分歷史資訊。技術報告第 3.2.2 節因此將編碼器重建的狀態定義為近似狀態；這一狀態又參與後續計算，使新增字尾的全域 KV 和 SWA 隨恢復起點變化。

兩類狀態的區分最終體現在快取容量上。在 DeepSeek V4 技術報告採用的工作負載與快取策略下，SWA 約佔長期快取的一半。V4.1 將這部分移出持久化快取，再將全域 KV 壓到約四分之一，長期儲存量因此約為原來的 $1/2\times1/4=1/8$。短期保留的編碼器 SWA 放在 DRAM 中，解碼器 SWA 則在本輪生成時駐留加速器。快取按使用時段分層儲存，進一步降低了長期容量需求。

![編碼器的三條字首恢復路徑與共同的解碼器重放](images/figure-8-v41-recovery.pdf)
...
以章首長請求的 6144 個 token 公共字首為例，它的 KV 佔 864 MiB（0.906 GB）。在 RTX PRO 6000 上重算這段字首要做 96.5 TFLOP 的矩陣運算，按實驗 8-1 中 batch 1 prefill 達到的 BF16 峰值的 62%（第 8.6.3 節）計，約需 307.9 ms。把這段字首換出到主機記憶體後，經這張卡的 PCIe Gen5 x16 鏈路（每方向 64 GB/s）取回約需 14.2 ms；[^pcie]維護開銷取換出時寫回主機的一次傳輸，也是 14.2 ms。命中機率為 50% 時平均節省 132.7 ms。要讓保留字首節省時間，需滿足

$$
p_h>\frac{T_m}{T_r-T_f}=\frac{14.2}{293.7}=4.8\%.
$$

若主機鏈路換成 PCIe Gen4 x16（每方向 32 GB/s），取回和換出各增至 28.3 ms，每次命中只省 279.6 ms，所需的最低命中機率升至 10.1%。換出釋放了加速器空間，但只有字首足夠常用，才能抵消取回和維護所需的時間。
...
![相同快取空間保留一個大字首或三個小字首](images/figure-8-7-cache-choice.pdf)
...
**Q2_K** 是張量計算庫 GGML 系列實作中的一種分組量化格式，主編碼使用 2 bit，並為組內的 scale 儲存額外資訊。以該格式為例，每組有 256 個值，2-bit 編碼佔 $256\times2/8=64$ bytes，scale 和最小值等後設資料另佔 20 bytes。因此，一組共需 84 bytes，平均每值為 $84\times8/256=2.625$ bit。位寬降低後，後設資料在總容量中的比重相應提高，在該格式中約佔 24%。[^gguf]

**擴充算例：量化權重載入到記憶體後，還能容納多長的上下文？** Qwen3-235B-A22B 的一個 Q2_K 檔案變體採用以 2 位元為主的分組量化，並混合使用多個位寬，量化編碼與未量化的浮點資料約 64.43 GiB、量化後設資料約 15.37 GiB，加上檔案頭與填充，共約 79.81 GiB。另一個 `UD-Q2_K_XL` 變體約 81.97 GiB。以實驗 8-7 所用的 Apple M2 Max 為例，它的統一記憶體標稱 96 GB（十進位制），給系統與工作區預留 8 GiB，剩餘約 81.41 GiB。[^files]
...
在統一記憶體系統中，CPU 和 GPU 共用實體記憶體。將權重檔案對映到地址空間後，存取相應資料時，檔案頁面才需要駐留在實體記憶體中。如果執行時要存取的資料總量超過記憶體容量，就會不斷換入頁面。因此，要解決容量不足的問題，可以減少資料本身的位元組數，也可以只保留當前需要的資料，其餘資料在使用前搬入。下面先計算 KV 壓縮的收益，再分析權重搬入所需的時間。
...
![同一組 32 個值在三種 KV 格式中的位元組佈局](images/figure-8-8-kv-format.pdf)
...
用壓縮 KV 做注意力計算時，讀取量減少了，但格式轉換需要額外時間。設一次計算少讀 $\Delta D$ bytes，有效頻寬為 $\beta$，轉換增加的非重疊執行時間為 $T_c$，則淨節省時間為
...
本節開頭提到，上下文表示決定每個 token 要儲存多少個值；混合注意力模型的區域性與全域兩類狀態，也可以按這種方式比較容量。在 8K 上下文基線下，把全域歷史與有效 SWA 的容量相加。DeepSeek V4.1 的兩項分別為 6.953 MiB 與 2.578 MiB，共 9.531 MiB；V4 的對應兩項合計為 30.521 MiB，約為 V4.1 的 3.20 倍。區域性視窗占固定空間，因此短上下文下的合計容量差距小於全域歷史的 3.95 倍。

### 8.4.3 權重卸載、預取與每步搬移

壓縮減少了資料本身的大小。另一種擴充容量的方法是讓資料輪流駐留在 GPU 上：只在計算需要時搬入，用完後複用這塊空間。權重卸載將部分原本常駐 GPU 的權重移到主記憶體，並在計算相應層之前搬回 GPU。節省的加速器記憶體可以用於 KV，但每次前向計算都需要重新搬入這些權重。請求逐步生成輸出時，搬移也隨每輪 decode 重複。

Qwen3-8B 一層 FFN 的三個 BF16 矩陣共佔 288 MiB。36 層中每四層卸載一層，共卸載九份 FFN 權重，釋放 2592 MiB。GPU 上還要為預取（在計算某層之前，提前把它的權重複制到 GPU）預留緩衝區：一組緩衝佔 288 MiB，淨省 2304 MiB；兩組佔 576 MiB，淨省 2016 MiB。以一條 8K 輸入的 1152 MiB KV 為單位，前者能多容納兩條，後者只能多容納一條。若按生成完畢時所需的 1188 MiB 預留空間，兩種方案均只能增加一條完整請求。[^offload]

![預取緩衝如何改變新增 KV 容量](images/figure-8-9-offload.pdf)

*圖 8-24：卸載九份 FFN 共騰出 2592 MiB。橙色為留在加速器上的預取緩衝，綠色為可重新分配的淨空間；一組緩衝淨省 2304 MiB，兩組淨省 2016 MiB。*
...
先計算搬移的鏈路成本。這張卡經 PCIe Gen5 x16 連線主機，每方向標稱 64 GB/s。每輪要搬入 2592 MiB，即 2.72 GB，需要

$$
T_{\mathrm{copy}}\ge\frac{2.72\ \mathrm{GB}}{64\ \mathrm{GB/s}}\approx42.5\ \mathrm{ms}.
$$

這已經超過 batch 1 一輪 decode 的 26.26 ms。即使其餘計算都能與複製同時完成，每輪仍至少需要約 42.5 ms。若每輪為四條請求各生成一個 token，總輸出吞吐最多約為 $4/0.0425\approx94$ token/s，每條請求的輸出間隔也至少約為 42.5 ms。生成 256 個輸出 token 需要 255 次後續前向計算，僅權重搬移就要約 10.8 秒。因此，經 PCIe 卸載這部分權重，無法滿足 7 秒的完成時限。

CPU 與 GPU 之間也有快得多的鏈路。GH200 用 NVLink-C2C 連線 Grace CPU 與 Hopper GPU，每方向 450 GB/s。同樣的 2.72 GB 每輪至少約需 6.0 ms，255 輪合計約 1.54 秒。每輪複製短於一輪 decode 的計算，預取就能用計算掩蓋複製。

![相同權重在兩種頻寬下的複製時間](images/figure-8-offload-copy.pdf)
...
若先壓縮權重再搬移，還要加上解壓時間。設原始資料為 $S$ 位元組，壓縮後為 $rS$ 位元組，鏈路頻寬為 $B_{\mathrm{link}}$，解壓吞吐為 $R_{\mathrm{dec}}$（按輸出的原始位元組計）。先傳完再解壓需要 $rS/B_{\mathrm{link}}+S/R_{\mathrm{dec}}$，直接傳輸需要 $S/B_{\mathrm{link}}$，因此只有當

$$
R_{\mathrm{dec}}>\frac{B_{\mathrm{link}}}{1-r}
$$

時，壓縮後搬移才更快。把 96 MiB 權重壓縮到四分之一，經 PCIe Gen5 x16 直接傳輸約需 1.57 ms，傳輸壓縮資料只需 0.39 ms；要保留這 1.18 ms 的節省，解壓器每秒必須輸出超過 85.3 GB 的原始資料。鏈路越快，對解壓吞吐的要求就越高。[^codec]

這一節的卸載中，模型計算始終由 GPU 執行，主記憶體負責儲存暫不使用的權重。第 9.3 節將進一步改變計算位置：專家權重已在主記憶體時，可由 CPU 直接計算，只向 GPU 傳回較小的活化值，屆時需要比較每個專家的輸入行數、CPU 計算時間與權重傳輸時間。

> **練習 8-5 · 計算：權重卸載節省的容量與增加的搬移。** 設卸載九份 FFN 權重，每份佔 288 MiB。設每組預取緩衝可以容納一份 FFN 權重，分別計算使用一組和兩組緩衝時淨節省的加速器記憶體，並求這些空間能多容納多少份 1152 MiB 的輸入狀態，或多少份 1188 MiB 的完整請求狀態。再分別取 PCIe Gen5 x16 的每方向 64 GB/s 與 GH200 NVLink-C2C 的每方向 450 GB/s，求每步複製以及累計 255 步複製的時間下界。若留給所有複製的總時間為 1 秒，求所需的最低單向頻寬。

### 8.4.4 品質約束下的容量與速度選擇

第 8.4.2 節算出了壓縮節省的空間和增加的轉換時間，但尚未回答改變數值格式後答案是否相同。這裡的執行後端指實際完成注意力計算的 kernel 實作。壓縮 KV 改變儲存的數值，更換執行後端還可能同時改變 Q 的計算格式、轉換過程和歸約運算的數值精度。逐項比較張量的儲存格式與計算精度，能夠找出輸出差異的來源。

下面用八道固定任務做這種逐項比較，三幅圖按相同順序排列這些任務。八道任務來自兩種長度的文件：每份文件含 128 或 512 條六位數字記錄，兩種長度各有四份獨立文件，實驗記錄裡分別標為 n128-r0 至 n128-r3 和 n512-r0 至 n512-r3，圖中依次記為任務 1 至 8。每道任務在兩種併發數下各執行兩次，用相同任務逐一比較不同數值格式的影響。

一組 Qwen3-8B 實驗保持 BF16 權重不變，比較了三種注意力計算方式。32 次自然生成中，使用 BF16 KV 時答對 28 次；改用原 FP8 實作後答對 26 次；保留 FP8 KV、將 Q 恢復為 BF16 後，又答對 28 次。最後一種設定只改變 Q 的精度，結果就發生了變化，說明需要分別考察 KV 的儲存格式和 Q 的計算精度。[^quality]

![同一模型中 Q 精度與正確答案數](images/figure-8-10-kv-quality.pdf)

*圖 8-26：BF16 KV 基線。每行是一道固定任務，四列為併發 1、4 下各兩次自然生成。綠色圓圈正確，橙色叉號錯誤，共 28/32 正確。*

![原 FP8 執行實作](images/figure-8-kv-quality-1.pdf)

*圖 8-27：同一任務與執行順序，原 FP8 實作共 26/32 正確。這一比較包含實作選擇對 Q 精度的影響。圓圈表示回答正確，叉號表示回答錯誤；每行對應同一道題，列表示併發數與重複執行序號。*

圖 8-28 進一步只恢復查詢 Q 的 BF16 精度，KV 仍保持 FP8。按同一列比較三幅圖，就能看出改變的是哪些題目的結果，而不只是總正確數。

![FP8 KV 與 BF16 Q](images/figure-8-kv-quality-2.pdf)

*圖 8-28：將查詢 Q 保持為 BF16，KV 仍用 FP8，共 28/32 正確；答錯的題目與 BF16 基線不同。三幅圖使用相同任務和列順序，模型權重均為 BF16。圓圈表示回答正確，叉號表示回答錯誤；每行對應同一道題，列表示併發數與重複執行序號。*

圖中任務 5（實驗記錄裡的 n512-r0，即含 512 條記錄的第一份長文件）在 BF16 KV 下四次均錯，在「FP8 KV＋BF16 Q」下四次均對；任務 6（n512-r1）則恰好相反。兩種設定都答對 28 次，但答錯的任務不同。將同一任務的結果排在一起，就能看出差異發生在哪裡，以及重複執行時是否一直如此。
...
> **練習 8-6 · 資料分析：KV 壓縮如何影響容量、答案正確性與重試成本。** 設每組包含 32 個值，另用 2 位元組儲存 scale，復算 8K 輸入和完整長請求在 q8_0、q4_0 格式下的 KV 容量。再讀取配套三種 KV／Q 精度設定的逐題結果，列出 BF16 KV 與「FP8 KV＋BF16 Q」中正確性改變的題目。最後讀取重試記錄，對每個成功任務合計首答與重試時間，並與只計入最後一次成功生成時間的結果比較。
...
![草稿第三個 token 不匹配時的保留、修正和丟棄](images/figure-8-11-verification.pdf)
...
![草稿恆為 A 的接受與修正](images/figure-8-sample-A.pdf)

*圖 8-30：接受 A 的機率為 1/4，拒絕後輸出 B 的機率為 3/4。兩條路徑合起來給出目標分佈。*

![草稿恆為 B 的接受與修正](images/figure-8-sample-B.pdf)
...
![草稿查詢時間與平均每個輸出 token 的耗時](images/figure-8-12-speculation.pdf)
...
### 8.5.3 草稿來源與生成方式：逐一或並行

四個草稿 token 能節省多少時間，既取決於其中有多少通過了驗證，也取決於生成這段草稿需要多長時間。實際系統可以從上下文中查詢草稿，也可以呼叫額外的網路生成草稿；兩種選擇會帶來不同的準備時間與記憶體佔用。
...
將每輪分為生成草稿、準備驗證、目標驗證和確定輸出四個階段，就能逐項比較這些方法。以實驗 8-5 的 DFlash 草稿網路為例，其權重約 1.95 GiB（2.10 GB）。按 batch 1 的有效頻寬計（26.26 ms 讀取 15.44 GB，約 588 GB/s），該網路前向一次約需 3.57 ms。若用它逐個生成四個草稿 token，要前向四次，共 14.28 ms；像 DFlash 那樣並行生成整段草稿，只需前向一次，約 3.57 ms。目標驗證都需 26.26 ms，若最終每輪都平均輸出 3 個 token，逐一生成時，每個 token 約需 $(14.28+26.26)/3\approx13.5$ ms，並行方式約需 $(3.57+26.26)/3\approx9.9$ ms。此時，加速來自草稿生成時間的縮短。

再設並行生成的草稿接受率較低，平均輸出數降為 2 個，平均每個輸出 token 的耗時就升至 $(3.57+26.26)/2\approx14.9$ ms，反而慢於逐一生成方式。更快的草稿生成節省了 10.7 ms，但每輪輸出減少，每個輸出 token 分攤的時間隨之增加，整個請求因而變慢。

### 8.5.4 草稿長度的動態選擇與併發競爭
...
預設執行形狀也會影響新增時間 $\Delta T$。如果本次驗證四個或五個 token，都把各 token 的特徵向量補齊為八行矩陣，使用預先準備的八行計算圖，增加一個 token 所需的額外計算較少；若待驗證的 token 從八個增至九個，需要補齊為十六行矩陣並改用相應計算圖，計算量便會突然增加。併發請求數也影響推測解碼的收益：低負載時，多行驗證可以利用閒置算力；高負載時，普通批處理已經充分共享權重，草稿生成和驗證還要與其他請求爭用加速器和記憶體。

一組實測比較了不同草稿長度。K7 和 K15 分別表示每輪生成 7 個和 15 個草稿 token 的設定。Qwen3-8B／DFlash 的短提取任務在併發 1 時，普通 decode、K7、K15 的完整耗時中位數約 90.0、30.6、29.4 ms。普通執行改為 K7 節省約 59 ms，從 K7 改為 K15 只再節省約 1.2 ms。逐輪記錄中還出現兩種塊長都只接受一個草稿 token 的輪次：K7 丟棄六個草稿 token，K15 丟棄十四個，最終輸出長度相同。更大的驗證塊增加了計算量，卻沒有在這一輪帶來更多輸出。[^spec-measure]
...
第 8.2 至 8.5 節討論了如何改變實例的記憶體佔用和執行方式。線上服務還要處理請求不斷到達的情況：一條請求尚未完成，下一條就已經進入佇列；一條請求等待工具回傳時，模型計算暫停，KV 卻仍需保留。排程器因此既要安排當前計算，也要為正在生成或等待繼續執行的請求保留狀態。
...
這些請求有的在排隊，有的正在計算，還有的僅保留狀態。增大 batch 可以提高輸出吞吐，但 KV 讀取、計算和其他開銷會逐漸限制這種增長。當處理速度無法繼續隨到達率提高時，新請求就會在佇列中累積，等待時間也隨之延長。

**准入控制**決定是否開始處理新請求。接收請求之前，先檢查可用的 KV 空間和距離截止時間還剩多久。對第 8.1 節的短請求，0.196 秒首輸出加 6.76 秒生成已用掉 6.95 秒；再多排隊 0.1 秒就會超時。提前識別這種請求，可以拒絕、轉交其他實例，或使用更快的執行設定。取消則在請求失去使用價值時停止安排後續迭代，並在加速器操作完成後釋放空間。

這些最佳化會相互影響。KV 壓縮讓實例能同時接收更多請求，較大的 batch 又讓普通 decode 更有效地共享權重，推測解碼的相對收益也就隨之改變。下面將這些變化應用於同一組請求，計算組合後的執行時間和記憶體佔用。

### 8.6.2 品質與 SLO 約束下的有效吞吐

只看佇列長度和完成速度，還不能判斷使用者是否得到了可用的答案。第 8.4.4 節的品質實驗出現過錯誤答案，排隊則會造成正確但遲到的回答。把這兩種損失一起計入，才能評價服務效果。設觀察視窗長為 $T_{\mathrm{obs}}$，第 $i$ 條請求正確時 $Q_i=1$，按期完成時 $D_i=1$。按完成的請求數計量，有效吞吐為

$$
R_{\mathrm{good}}=\frac{\sum_i Q_iD_i}{T_{\mathrm{obs}}}.
$$

分子只計正確且按期完成的請求。若改用全部完成的請求數作為分子，就得到完成吞吐。兩者的差距說明，有多少處理能力花在了錯誤或遲到的答案上。

一組實測顯示了這兩項損失的大小。Qwen3-8B 檢索實驗使用相同的八道題，要求請求在到達後 3 秒內正確完成，比較逐個接收請求、連續批處理和一次提交全部請求三種方式。每次實驗提交 16 條請求，全部實驗共生成 336 次回答，其中 294 次正確，只有 155 次同時滿足時限。其餘 139 次正確回答因為太晚回傳，仍未滿足服務要求。[^service]

![不同到達條件下的完成吞吐](images/figure-8-13-service.pdf)

*圖 8-33：每種接納與到達條件使用三個 16 請求視窗。柱為全部完成請求的吞吐中位數，圓點為各視窗結果。時間從計劃到達起計算。*

![同一組視窗的有效吞吐](images/figure-8-service-goodput.pdf)

*圖 8-34：分子僅計正確且在 3 秒內完成的請求。兩圖採用相同橫軸順序和縱軸範圍，差額來自錯誤或遲到的回答。*

到達率為 1/s 時，兩種線上策略的有效吞吐均約 0.90 req/s。每批工作較少，逐個接納也能及時處理大部分正確請求。到達率升至 4/s，逐個接納的完成吞吐約 1.22 req/s，有效吞吐卻降至 0.31 req/s；佇列變長，許多正確輸出越過了期限。連續批處理把完成吞吐提高到約 2.33 req/s，有效吞吐達到約 1.02 req/s，更多請求在期限內完成。

再把連續模式的到達率推至 16/s，完成吞吐只增至約 2.38 req/s，有效吞吐反而降到約 0.60 req/s，減少約 41%。完成吞吐已基本不再增長，更多請求到達主要增加了排隊時間。對這組視窗，4/s 比 16/s 按時回傳了更多正確答案。選擇服務設定時，要先找出有效吞吐開始下降的到達率，再限制新接收的請求數，避免排隊時間過長。

逐個接收請求時，其他請求雖然沒有進入引擎，卻仍在應用佇列中等待。因此，應從請求原定的到達時刻開始計時，把引擎外的等待也計入。結合本節的幾組測量，加速器每秒輸出多少 token、單條請求等待多久，以及每秒有多少請求正確且按期完成，需要分別計算。

### 8.6.3 設定、加速器與完整任務成本

比較設定之前，先把實測換算成處理速度。確定批處理、精度和快取設定後，用完成的工作量除以佔用加速器的時間，就得到各階段的實際處理速度：prefill 速度 $r_P$ 按每秒處理的新輸入 token 計，decode 速度 $r_D$ 按每秒完成的 decode 呼叫計。批內多條請求共享一次執行時，每條請求都算完成了一次呼叫，佔用時間只計一次。一條請求有 $L$ 個新輸入 token、輸出 $G$ 個 token，它佔用加速器的時間（以 GPU 秒計）為
...
**綜合算例：在記憶體與時限約束下選擇字首共享、壓縮和推測設定。** 有效吞吐給出了服務的評價標準，容量和時間分析則能在執行之前排除不合適的設定。採用章首給定的推理實例：RTX PRO 6000 分給本實例 32 GiB，扣除權重和基本工作區後，留給 KV 及輔助緩衝區的空間為 12 GiB。16 條長請求同時到達，每條輸入 8192 個 token、輸出 256 個 token，前 6144 個輸入 token 相同，完成時限為 7 秒。下面比較四種方案，並假設四種方案都能給出正確答案。共享方案在請求到達前已經快取了公共字首。推測方案使用實驗 8-5 的 DFlash 草稿模型。啟用後行程視訊記憶體峰值增加約 3.1 GiB（從 17,706 MiB 增至 20,894 MiB），其中草稿模型權重約 1.95 GiB。草稿模型還要處理輸入，實驗 8-5 中輸入約 2300 個 token 時，首 token 時間從 129.4 ms 增至 141.6 ms，多出 9.4%。設這組請求平均每輪輸出 3 個 token。
...
| 方案 | 狀態組織 | 首 token 回傳前（整批 prefill） | 首 token 回傳後的執行 |
|---|---|---:|---|
...
記憶體不足的 A 和超時的 B、C 都已排除，只有 D 能按時回傳 16 個正確答案。這一比較依次使用了字首共享後的容量、壓縮格式的大小、每輪耗時和每輪輸出數量。在這張卡上，一輪 decode 的時間幾乎不隨 batch 和上下文變化，只有減少輪數，才能明顯縮短生成階段。

圖 8-35 將這兩項檢查畫在同一張平面圖上。橫向越過虛線，表示 KV 和輔助緩衝區超過 12 GiB；縱向越過虛線，表示完成時間超過 7 秒。只有 D 同時落在兩個限制以內。

![四種設定相對於記憶體上限和完成時限的位置](images/figure-8-14-design.pdf)

*圖 8-35：16 條長請求在 RTX PRO 6000 上的設定比較。橫軸為 KV 和輔助緩衝區佔用，已扣除共同的權重與基本工作區；縱軸為整批請求總耗時，由實驗 8-1、8-5 的實測推出。A 記憶體不足，不能在本實例上執行，圖中是它所需的時間。陰影區域同時滿足 12 GiB 和 7 秒限制。A 為 BF16 獨立上下文，B 為 BF16 共享字首，C 為 q8_0 獨立上下文，D 為 BF16 共享字首加推測解碼。*
...
以上分析的都是實例內的執行時間。計算完整 Agent 任務的耗時，還要加上工具執行和外部等待。假設一次任務耗時 100 秒，其中 decode 佔 50 秒；將 decode 加速四倍後，總耗時變為 $50+50/4=62.5$ 秒，整體加速 1.6 倍。如果 decode 原本只佔 20 秒，總耗時則變為 $80+20/4=85$ 秒，整體只加速約 1.18 倍。一般地，設 decode 佔原總時間的比例為 $f$，該階段加速 $s$ 倍，其他階段耗時不變，則由 Amdahl 定律得到

$$
S_{\mathrm{task}}=\frac{1}{(1-f)+f/s}.
$$

![區域性 decode 加速對完整任務的影響](images/figure-8-15-task.pdf)

*圖 8-36：其他階段時間固定、沒有新增準備工作的 Amdahl 加速比曲線。三條曲線分別取 decode 佔基線時間 20%、50%、80%。該階段原有的耗時佔比越高，區域性加速帶來的收益就越大。*

圖 8-36 中，decode 佔比越低，曲線越早變平：即使生成階段繼續加速，工具呼叫和其他等待仍佔據原來的時間。比較加速器佔用和能耗時，也要覆蓋這些階段，並按最終得到的合格請求數計算。設觀察期間佔用加速器的總時間為 $T_{\mathrm{GPU}}$，總能耗為 $E_{\mathrm{obs}}$，合格結果數為 $N_g$，則每個合格結果平均佔用 $T_{\mathrm{GPU}}/N_g$ 的 GPU 時間，消耗 $E_{\mathrm{obs}}/N_g$ 的能量。實驗 8-9 在同一張卡上讀取了 GPU 與 CPU package（整個處理器封裝）的能量計數：連續批處理在到達率 1、4、16 req/s 下，每個合格結果分別消耗 615.0、652.0、1083.7 J；逐個接納在 4 req/s 下為 1982.3 J。16/s 時連續批處理的完成吞吐與 4/s 相近，遲到的回答卻更多，每個合格結果的能耗高出約 66%。即使答案錯誤或回傳太晚，已經消耗的能量也必須計入總成本。[^service]
...
混合注意力與遞推模型把快取索引擴充為狀態恢復問題，Marconi 等工作討論恢復機會和快取價值。權重卸載、張量編碼以及統一記憶體執行，則將加速器容量與鏈路成本聯絡起來；配套 M2 Max 記錄展示了小模型的真實 KV 分配與輸入處理。[^hybrid][^codec][^mac]
...
[^batch]: 固定模型在 RTX PRO 6000 上的[批處理計算](https://github.com/bojieli/ai-infra-book/blob/main/calculations/results/batch-reuse-pro6000-2k.md)及[8K 對照](https://github.com/bojieli/ai-infra-book/blob/main/calculations/results/batch-reuse-pro6000-8k.md)。完整 BF16 權重為 16,381,470,720 bytes，一步理想共享矩陣權重讀取為 15,136,811,008 bytes；embedding 按請求查行，記憶體中仍需儲存完整的 embedding 矩陣。讀取表先用精確位元組計算，再舍入各項。

[^device]: [NVIDIA RTX Blackwell PRO 規格](https://github.com/bojieli/ai-infra-book/blob/main/references/files/specs/nvidia-rtx-blackwell-pro.pdf)附錄 A 表 4，整理在[硬體參數表](https://github.com/bojieli/ai-infra-book/blob/main/calculations/results/hardware.md)的 rtx-pro6000-blackwell-ws 一行。

[^batch-measure]: [batch 掃描原始記錄](https://github.com/bojieli/ai-infra-book/blob/main/experiments/ch08/08-01/README.md)。RTX PRO 6000、vLLM 0.23.0、BF16，三輪；正文報告中位數，吞吐覆蓋剩餘 prefill 和生成，排除載入與預熱；8K 條件先建立共享字首。

[^schedule-survey]: [Orca 原文](https://github.com/bojieli/ai-infra-book/blob/main/references/files/papers/orca.pdf)與[分塊計算和版本說明](https://github.com/bojieli/ai-infra-book/blob/main/case-studies/chunking-and-state-transfer.md)。

[^schedule]: [固定 batch](https://github.com/bojieli/ai-infra-book/blob/main/calculations/results/iteration-batching-pro6000-fixed.md)、[連續批處理](https://github.com/bojieli/ai-infra-book/blob/main/calculations/results/iteration-batching-pro6000-continuous.md)、[分塊策略](https://github.com/bojieli/ai-infra-book/blob/main/calculations/results/iteration-batching-pro6000-chunked.md)。每輪耗時參數由[實驗 8-1 的逐輪記錄](https://github.com/bojieli/ai-infra-book/blob/main/experiments/ch08/08-01/efficiency.json)擬合：batch 1 的 2K decode 一輪 26.26 ms、prefill 94.8 ms；最後一個輸出 token 在請求結束時尚未寫入 KV。

[^chunk]: [逐塊計算和封存測量](https://github.com/bojieli/ai-infra-book/blob/main/calculations/results/chunk-history-book.md)。11 次請求、每次 16 塊，首末塊時間中位數 25.30、35.07 ms；CUDA event 區間包括完整模型路徑和可能的主機間隙，不含外部 logits／取樣。各塊按原始順序執行，輸入 token 與內容逐塊變化。

[^nanoflow]: [OSDI 2025 調研](https://github.com/bojieli/ai-infra-book/blob/main/research/2026-infra-survey/reading-osdi-2025.md)、[NanoFlow 正式稿](https://github.com/bojieli/ai-infra-book/blob/main/references/proceedings/OSDI/2025/selected/osdi25-zhu-kan.pdf)與[資源共享算例](https://github.com/bojieli/ai-infra-book/blob/main/case-studies/resource-sharing-and-placement.md)。

[^replay]: [六請求實測](https://github.com/bojieli/ai-infra-book/blob/main/experiments/ch08/08-02/README.md)及[圖執行取捨](https://github.com/bojieli/ai-infra-book/blob/main/case-studies/graph-execution-tradeoffs.md)。實驗在共享 GPU 上按固定順序執行六條合成文字請求。

[^pages]: [分頁、取消與搶佔](https://github.com/bojieli/ai-infra-book/blob/main/experiments/ch08/08-03/README.md)、[四分支共享](https://github.com/bojieli/ai-infra-book/blob/main/experiments/ch08/08-03/branch-sharing/README.md)、[冷分支對照](https://github.com/bojieli/ai-infra-book/blob/main/experiments/ch08/08-03/cold-branches/README.md)。共享分支的輸出相同；冷、熱分支具有相同的塊峰值，但 prefill 量不同。

[^lora]: [MLSys 2024 調研](https://github.com/bojieli/ai-infra-book/blob/main/research/2026-infra-survey/reading-mlsys-2024.md)與[多 LoRA 計算](https://github.com/bojieli/ai-infra-book/blob/main/case-studies/multi-lora-serving.md)。

[^context]: [上下文工程案例](https://github.com/bojieli/ai-infra-book/blob/main/case-studies/author-context-and-design.md)。

[^hybrid]: [混合狀態恢復與 checkpoint 計算](https://github.com/bojieli/ai-infra-book/blob/main/case-studies/hybrid-prefix-state.md)、[MLSys 2025 調研](https://github.com/bojieli/ai-infra-book/blob/main/research/2026-infra-survey/reading-mlsys-2025.md)。Kimi K3 的容量按指定儲存格式和並行設定計算。

[^prefix]: [12 輪實際輸入的快取回放](https://github.com/bojieli/ai-infra-book/blob/main/experiments/ch08/08-04/README.md)。回放採用原任務的輸入序列，依序執行請求，每輪生成一個 token。

[^files]: [固定模型檔案與業務預算](https://github.com/bojieli/ai-infra-book/blob/main/case-studies/inference-training-scenarios.md)及[235B 分片後設資料](https://github.com/bojieli/ai-infra-book/blob/main/references/outline-checks/2026-09-07/systems-cases/qwen235-gguf-metadata.json)。精確位元組、GB 與 GiB 分別報告。

[^kv-format]: [KV 格式和執行成本](https://github.com/bojieli/ai-infra-book/blob/main/case-studies/kv-quantization-and-execution.md)。資料包含 GGML 分組格式、FP8 scale 粒度及 H100 上的擬合結果。

[^offload]: [權重卸載與預取計算](https://github.com/bojieli/ai-infra-book/blob/main/case-studies/weight-offload-execution.md)；[PCIe Gen5](https://github.com/bojieli/ai-infra-book/blob/main/calculations/results/weight-offload-pcie5.md)與[NVLink-C2C](https://github.com/bojieli/ai-infra-book/blob/main/calculations/results/weight-offload-gh200-c2c.md)兩檔鏈路的複製與預取排程，每層計算時間取 batch 1 一輪 decode 的 26.26 ms 除以 36 層；[GH200 架構說明](https://github.com/bojieli/ai-infra-book/blob/main/references/files/documents/nvidia-grace-hopper-blog.md)：NVLink-C2C 合計 900 GB/s，每方向 450 GB/s。

[^codec]: [張量壓縮與傳輸](https://github.com/bojieli/ai-infra-book/blob/main/case-studies/tensor-codec-and-transfer.md)。資料包含 LLM.265 的影片引擎實作、專用硬體設計與效能估計。

[^mac]: [M2 Max 小模型容量記錄](https://github.com/bojieli/ai-infra-book/blob/main/experiments/ch08/08-07/README.md)及[32K 補測](https://github.com/bojieli/ai-infra-book/blob/main/experiments/ch08/08-07/context32k/README.md)。各槽依次執行 prefill。

[^quality]: [KV 與 Q 精度對照](https://github.com/bojieli/ai-infra-book/blob/main/experiments/ch08/08-08/README.md)、[固定 FP8 權重的 Q 控制](https://github.com/bojieli/ai-infra-book/blob/main/experiments/ch08/08-08/quantized-q-control/README.md)、[併發容量記錄](https://github.com/bojieli/ai-infra-book/blob/main/experiments/ch08/08-08/concurrency/README.md)。正文采用 BF16 權重組。另一組固定 FP8 權重時，BF16 Q 與預設路徑分別為 32/32、28/32。

[^retry]: [自然輸出與重試成本](https://github.com/bojieli/ai-infra-book/blob/main/experiments/ch08/08-08/retry-cost/README.md)。實驗使用已知答案校驗，重試成本包含錯誤首答的生成成本。

[^spec]: [推測取樣原文](https://github.com/bojieli/ai-infra-book/blob/main/references/files/papers/speculative-sampling.pdf)、[推測解碼原文](https://github.com/bojieli/ai-infra-book/blob/main/references/files/papers/speculative-decoding.pdf)、[框架計數與預算筆記](https://github.com/bojieli/ai-infra-book/blob/main/case-studies/speculative-execution.md)、[調研中的框架覆蓋](https://github.com/bojieli/ai-infra-book/blob/main/research/2026-infra-survey/framework-coverage.md)。基礎機率演算法、草稿路線和具體實作分別引用。

[^history]: [上下文草稿的分佈與成本計算](https://github.com/bojieli/ai-infra-book/blob/main/case-studies/history-drafts-and-rollout.md)及[有理數窮舉結果](https://github.com/bojieli/ai-infra-book/blob/main/research/2026-infra-survey/history-speculation-arithmetic.json)。算例採用二符號目標分佈；RTX PRO 6000 上的每輪時間見 [AAAA](https://github.com/bojieli/ai-infra-book/blob/main/calculations/results/speculative-round-pro6000-aaaa.md)、[BBBB](https://github.com/bojieli/ai-infra-book/blob/main/calculations/results/speculative-round-pro6000-bbbb.md) 與[含 2 秒索引的 256 token 請求](https://github.com/bojieli/ai-infra-book/blob/main/calculations/results/speculative-budget-pro6000-index.md)；普通 decode 一輪 26.26 ms 取自[實驗 8-1 的逐輪記錄](https://github.com/bojieli/ai-infra-book/blob/main/experiments/ch08/08-01/efficiency.json)，DFlash 的每輪耗時取自[實驗 8-5](https://github.com/bojieli/ai-infra-book/blob/main/experiments/ch08/08-05/README.md)的逐步記錄。

[^spec-measure]: [DFlash 同執行器對照](https://github.com/bojieli/ai-infra-book/blob/main/experiments/ch08/08-05/README.md)及[逐請求階段觀測](https://github.com/bojieli/ai-infra-book/blob/main/experiments/ch08/08-05/kernel-trace/README.md)。實驗在低併發下，按固定順序執行給定短題集。併發 1 通過短題的普通、K7、K15 時間中位數分別為 90.02、30.58、29.43 ms；配對輸出共 32 對。

[^service]: [服務與能量原始記錄](https://github.com/bojieli/ai-infra-book/blob/main/experiments/ch08/08-09/README.md)。同一裝置、八道題、七條件各三個 16 請求視窗；同題跨條件輸出一致，42 個錯誤來自同一道題。吞吐表為各視窗指標中位數。每視窗包含 16 條請求，按排序後向上取整的位置計算 p95，得到的就是最大值。NVML 與 RAPL 元件計數視窗略寬於請求視窗，其他服務仍在執行。

[^service-plan]: [實驗 8-1 的效率表](https://github.com/bojieli/ai-infra-book/blob/main/experiments/ch08/08-01/efficiency.json)由[計算指令碼](https://github.com/bojieli/ai-infra-book/blob/main/experiments/ch08/08-01/efficiency.py)從[batch 掃描的逐輪記錄](https://github.com/bojieli/ai-infra-book/blob/main/experiments/ch08/08-01/README.md)匯出，峰值取硬體表中 RTX PRO 6000 的 1792 GB/s 與 503.8 TFLOP/s；DFlash 的視訊記憶體峰值、首 token 時間與每輪耗時取自[實驗 8-5](https://github.com/bojieli/ai-infra-book/blob/main/experiments/ch08/08-05/README.md)。各方案的記憶體與時間由[算例檢查](https://github.com/bojieli/ai-infra-book/blob/main/manuscripts/ch08/teaching-check.py)復算。

[^task]: [生成實驗與 Agent 執行記錄](https://github.com/bojieli/ai-infra-book/blob/main/experiments/ch08/08-06/README.md)。Agent 記錄採用非流式呼叫；階段時間圖採用題設參數。

[^gguf]: [Q2_K 的逐張量佈局](https://github.com/bojieli/ai-infra-book/blob/main/calculations/results/gguf-layout-q2k.md)、[8K 檔案容量](https://github.com/bojieli/ai-infra-book/blob/main/calculations/results/gguf-inventory-8k.md)與[32K 對照](https://github.com/bojieli/ai-infra-book/blob/main/calculations/results/gguf-inventory-32k.md)。檔案總量 85,691,002,112 bytes，碼值／浮點載荷 69,178,275,840 bytes，量化後設資料 16,506,720,256 bytes，檔案頭與填充 6,006,016 bytes。上述檔案大小按已歸檔檔案頭與釋出後設資料計算。

[^pcie]: [RTX PRO 6000 Blackwell 工作站版規格](https://github.com/bojieli/ai-infra-book/blob/main/references/files/specs/nvidia-rtx-pro6000-spec.pdf)：系統介面 PCIe 5.0 x16，頻寬為 PCIe Gen4 的兩倍；[A100 80GB 規格](https://github.com/bojieli/ai-infra-book/blob/main/references/files/specs/nvidia-a100-80-spec.pdf)：PCIe 4.0 為 64 GB/s（收發合計），即 x16 每方向 32 GB/s。由此 PCIe Gen5 x16 每方向 64 GB/s。正文按標稱值計算，得到的是傳輸時間的下界。字首的重算時間按[實驗 8-1 的效率表](https://github.com/bojieli/ai-infra-book/blob/main/experiments/ch08/08-01/efficiency.json)計算。

[^cancel]: 取消觀察來自[狀態實驗](https://github.com/bojieli/ai-infra-book/blob/main/experiments/ch08/08-03/README.md)。記錄值約 1.55 ms 與 31.41 ms，計時包含觀察器開銷。
...
KV 的管理方式決定了實例能同時處理多少請求。分頁按需分配空間，共享只儲存一份公共字首，字首快取還省去了重複計算。上下文的追加、改寫和總結會改變快取複用以及後續狀態儲存與讀取的開銷，比較這些上下文組織方式與其他服務最佳化方案時，應採用相同的任務品質要求。壓縮進一步減少儲存量，卸載將部分權重移出加速器，代價是反覆搬移。推測解碼增加了草稿生成和驗證的開銷，卻能讓一次目標模型計算確定多個輸出。這些變化都可以透過記憶體佔用、每輪耗時和每輪輸出數來比較。
...
完成本章的設定比較後，應記錄三類結果：在給定加速器、精度和請求長度下能同時儲存多少條請求的狀態，prefill 與 decode 分別以多快的速度處理工作，以及請求延遲如何隨 batch 和到達率變化。這些結果已經包含排程、快取和 kernel 效率的影響。

下一章以這些結果為基礎，決定各類加速器承擔什麼工作、需要多少執行單元，以及 KV 如何傳輸和共享。多個加速器既可以共同執行一個實例，也可以組成多個完整副本或分別排程的階段服務池。批處理決定已有資源如何處理請求，部署方式決定這些資源如何分工，兩者共同影響完整服務的效能。
