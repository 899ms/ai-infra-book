...
def save(f,name):
 ...
 for t in f.findobj(matplotlib.text.Text):
  ...
  if re.search("圖\\s*\\d+\\s*[-－–]\\s*\\d+",t.get_text()):raise ValueError('Figure number inside artwork')
...
for label,y,c in [("原方案",240/bw+.8,'blue'),("處理十倍加速",240/bw+.53,'teal'),("輸入減半，新增編解碼",120/bw+.95,'red')]:a.plot(bw,y,color=C[c],lw=2,label=label)
a.set(xscale='log',xlabel="上行速率 / Mbit/s",ylabel="完整成片時間 / s",ylim=(0,51));a.set_xticks([5,10,20,50,100,200],['5','10','20','50','100','200']);a.set_yticks([0,10,20,30,40,50]);a.grid(alpha=.18);a.legend(frameon=False)
...
for i,z in enumerate(chunks[:5]):
 ...
 a.broken_barh([(z['playback_start_ns']/1e6,20)],(y-.13,.26),facecolors=C['teal'],label="實際播放" if i==0 else None)
 a.scatter(z['arrival_ns']/1e6,y,color=C['blue'],s=35,zorder=3,label="到達" if i==0 else None)
a.annotate("晚到 5 ms",(123,2),(140,2.6),arrowprops={'arrowstyle':'->','color':C['muted']},fontsize=11)
a.set(yticks=range(5),yticklabels=["塊 5","塊 4","塊 3","塊 2","塊 1"],xlabel="從開始採集起的時間 / ms",xlim=(25,185),ylim=(-.6,4.7));a.set_xticks([40,60,80,100,120,140,160,180]);a.grid(axis='x',alpha=.18);a.legend(frameon=False,ncol=3,loc='lower left',bbox_to_anchor=(0,1.01))
...
a.set(yticks=[1,0],yticklabels=["壓縮圖片","完整視覺特徵"],xlabel="理想傳送時間 / s",xlim=(0,12),ylim=(-.65,1.65));a.grid(axis='x',alpha=.15)
...
comp={}
for files,title,labels in [(['shared-media-schedule-fifo','shared-media-schedule-priority'],"只改傳送順序",['FIFO',"音訊優先"]),(['shared-media-hol-connection','shared-media-hol-per_stream'],"固定傳送，只改交付順序",["整體有序","逐流交付"])]:
 ys=[]
 for name in files:
...
f,a=plot_canvas();x=np.arange(2);v=np.array(comp["固定傳送，只改交付順序"]['image_audio_seconds'])
for j,(lab,c) in enumerate([("圖片",'blue'),("音訊",'orange')]):
...
a.set(yticks=x,yticklabels=["整體有序","逐流交付"],xlim=(0,8.5),xlabel="應用交付完成時間 / s");a.invert_yaxis();a.legend(frameon=False,ncol=2,loc='lower left',bbox_to_anchor=(0,1.01));a.grid(axis='x',alpha=.15)
...
for j,(lab,c) in enumerate([("接入等待",'muted'),("資料 PPDU",'blue'),('SIFS','orange'),('MAC ACK','teal')]):
...
a.set(yticks=[1,0],yticklabels=["承載資料","承載端到端 ACK"],xlim=(0,350),ylim=(-.6,1.6),xlabel="一次成功交換的空口時間 / μs");a.legend(frameon=False,ncol=4,fontsize=10,loc='lower left',bbox_to_anchor=(-.08,1.01))
...
for j,(label,c) in enumerate([("直接路徑",'blue'),('Queqiao','teal')]):
...
a.set(xticks=x,xticklabels=["新建連線","保持連線並調優"],ylabel="請求完成時間中位數 / s",ylim=(0,1.4));a.legend(frameon=False);a.grid(axis='y',alpha=.15)
...
a.plot(b,cloud,color=C['blue'],lw=2,label="雲地域");a.axhline(near_s,color=C['teal'],lw=2,label="附近工作站");a.axhline(deadline,color=C['red'],ls='--',label="任務期限");a.scatter([feasible],[deadline],s=40,color=C['orange'],zorder=3)
a.annotate(f'滿足期限：約 {feasible:.1f} Mbit/s',(feasible,deadline),(6,58),fontsize=11,arrowprops={'arrowstyle':'->','color':C['muted']})
a.set(xlabel="雲路徑上行速率 / Mbit/s",ylabel="剩餘 20 輪完成時間 / s",xlim=(2,20),ylim=(0,80));a.legend(frameon=False,loc='upper right');a.grid(alpha=.16)
...
relationships=["上行速率與完整成片時間","到達延遲與按序播放","資料表示與傳送時間","交付依賴與完成時間","固定開銷與空口交換時間","連線基線與請求時間差","上行速率與部署時間邊界"]
...
body=re.sub("<p>(<img [^>]+>)</p>\\s*<p><em>(圖 12-[\\s\\S]*?)</em></p>",r'<figure>\1<figcaption>\2</figcaption></figure>',body)
for p in outputs:
...
page="<!doctype html><html lang=\"zh-CN\"><head><meta charset=\"utf-8\"><meta name=\"viewport\" content=\"width=device-width,initial-scale=1\"><title>第 12 章 端邊雲協同</title><style>"+css+math_css+"</style></head><body><main><nav aria-label=\"本章目录\">"+nav+'</nav>'+body+'</main>'+viewer+'</body></html>'
import sys
sys.path.insert(0,str(HERE.parent))
