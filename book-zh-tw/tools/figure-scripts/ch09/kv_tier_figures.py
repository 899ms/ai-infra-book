...
def draw(here):
    ...
    with plt.rc_context(STYLE):
        ...
        f, a = canvas(4.6)
        text(a, .02, .95, "一張 A100 可用的 KV 儲存層次", 13)
        rows = [('HBM', f"≤ {num(tiers['HBM']['capacity_bytes'])/1e9:.1f} GB", 'green', .24, "已在 GPU 上", "取回 0 ms"),
                ("主機記憶體", f"{num(tiers['host DRAM']['capacity_bytes'])/1024**3:.0f} GiB", 'blue', .34,
                ...
                ("本地 SSD", f"{num(tiers['local NVMe']['capacity_bytes'])/1e12:.2f} TB", 'orange', .44,
                 "SSD 順序讀 7.1 GB/s", f"取回 {num(tiers['local NVMe']['read_s_exact'])*1e3:.0f} ms"),
                ("遠端儲存池", "隨節點數增加", 'purple', .54,
                 "網路卡 25 GB/s，再經 PCIe", f"取回 {num(tiers['remote pool']['read_s_exact'])*1e3:.1f} ms")]
        for i, (name, cap, c, w, path, t) in enumerate(rows):
            y = .70 - i * .19
            box(a, .31 - w / 2, y, w, .15, f'{name}\n{cap}', c, 12)
            text(a, .63, y + .105, path, 11)
            text(a, .63, y + .045, t, 11)
        text(a, .02, .05, f"對照：在 A100 上重算這份 8K 字首約 {num(s['prefill_prefix_s_exact'])*1e3:.0f} ms", 11)
        ...
        for name, cap, c in [('HBM', num(tiers['HBM']['capacity_bytes']) / 1e9, 'green'),
                             ("主機記憶體", num(tiers['host DRAM']['capacity_bytes']) / 1e9, 'blue'),
        ...
        a.text(10 * 1.12, 1100, "API：80% 的複用\n在 10 s 內", fontsize=11)
        a.text(600 / 1.12, 1.3, "對話：80% 的複用\n在 10 min 內", fontsize=11, ha='right')
        a.set(xlim=(1, 3600), ylim=(1, 20000), xlabel="同一字首兩次使用的間隔（s，對數刻度）", ylabel="所需容量（GB，對數刻度）",
        ...
        a.text(.075, 53, "容量不限時也只有 51%", fontsize=11, color=LINE['orange'])
        ...
        for x, y, row in zip(xs, ys, m):
            ...
            a.annotate(f"{row['blocks']:,} 塊", (x, y), xytext=(x * fx, y + dy), fontsize=11, ha=ha)
        a.set_xscale('log')
        a.set(xlim=(.05, 12), ylim=(20, 60), xlabel="快取容量（換算為 Qwen3-8B 的 KV，TB）", ylabel="LRU 命中率（%）",
        ...
        for mode, name, total in [('serial', 'serial', num(t['serial_s_exact'])),
            ...
            if mode == 'preload':
                a.text(-k * ld / 2, 1.62, f'提前讀入 {k} 層', fontsize=11, ha='center')
            a.set(xlim=(-22, 90), ylim=(-.6, 1.9), yticks=[0, .95], yticklabels=["GPU 計算", "PCIe 讀取"],
                  xlabel="從這個請求開始執行起計時（ms）")
        ...
        f, a = canvas(4.0)
        a.set_ylim(0, .88)
        text(a, .02, .66, "等待佇列", 12)
        for i, j in enumerate(['J1', 'J2', 'J3', 'J4', 'J5', 'J6']):
            box(a, .20 + i * .13, .60, .11, .12, j + ("\n執行中" if i == 0 else ''), 'green' if i == 0 else 'gray', 11)
        a.plot([.33, .33, .59, .59], [.745, .77, .77, .745], color=COL['line'], lw=.9)
        text(a, .46, .82, "接下來的請求：J2—J4", 11, ha='center')
        text(a, .02, .40, "主機記憶體", 12)
        ...
        text(a, .575, .25, "讀入 J3", 11)
        arrow(a, (.715, .34), (.715, .16))
        text(a, .725, .25, "換出 J6", 11)
