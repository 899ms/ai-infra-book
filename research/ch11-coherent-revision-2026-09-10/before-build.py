...
f,a=canvas(5.3)
box(a,.04,.58,.23,.23,'Agent 控制器','任务、历史与验收')
box(a,.60,.58,.29,.23,'模型服务','返回工具参数')
arrow(a,(.27,.73),(.60,.73));arrow(a,(.60,.64),(.27,.64));a.text(.435,.79,'模型请求',ha='center',fontsize=12)
box(a,.04,.13,.23,.23,'环境平台','准入与节点分配');arrow(a,(.155,.58),(.155,.36));a.text(.20,.46,'明确工具调用',fontsize=12)
...
a.axvline(4,color=C['red'],ls='--',lw=1.4);a.text(4,3.55,'明确调用',ha='center',fontsize=12,color=C['red'])
...
a.plot(h*100,cb,color=C['blue'],label='B：命中率变化',lw=2.3);a.axhline(ca,color=C['orange'],label='A：前缀全命中',lw=2)
...
a.annotate('B 达到按时质量要求：约 91.8%',(target*100,.007),(12,.006),arrowprops={'arrowstyle':'->','color':C['teal']},fontsize=12)
a.set(xlabel='B 请求命中率 / %',ylabel='费用单位 / 质量成功任务',xlim=(0,100),ylim=(0,.046));a.legend(frameon=False,fontsize=11,loc='upper right');a.grid(alpha=.15)
...
xx=np.arange(3);vals=[initial,qc,dc];a.bar(xx,vals,color=[C['blue'],C['teal'],C['orange']],width=.55)
for i,v in enumerate(vals):a.text(i,v+.0003,f'{v:.4f}',ha='center',fontsize=13)
a.set(xticks=xx,xticklabels=['仅首次尝试\n成功率 80%','有限恢复\n质量成功约 99.7%','有限恢复\n按时质量成功约 95.0%'],ylim=(0,.018),ylabel='全部支出 / 相应完成任务数');a.grid(axis='y',alpha=.15)
