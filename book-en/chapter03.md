...
>
> Continuing the decision-request example at the end of Section 3.2.4: the decision model costs 0.000081 dollars per request, and handing a request to the LLM costs 0.013880 dollars. Let the success rates before and after hand-off be $p_1,p_2$. Compute the average cost per request for hand-off fractions of 1% and 5%; how large must $p_2/p_1$ be at least for hand-off to lower the cost per successful task?
...
**Example: why Jev answers in the blink of an eye.** In September 2026, TypeSafe AI, founded by former OpenAI researcher Diogo Almeida, released the Jev model. According to the founder's launch post, at OpenAI he helped build the methods that taught language models to follow instructions and hold conversations, work that became the research behind ChatGPT.[^jev] Jev's official demo is striking: for the same customer-service request, GPT-5.6 Terra took 8.566 seconds while Jev took 0.114 seconds, at 1/171 of the cost. Many people's first reaction is: is this still a Transformer? Did they build some remarkable infra? Anyone who has read this book will see there is no real secret here, and readers can easily build it themselves.

Jev does not generate text. The caller submits a piece of state (a support ticket, say) and several questions, each with its options fixed in advance, and the model returns a probability for each option. Back in the call chain of Section 3.1.1, such a request has a prefill and no decode. The state and all the questions are concatenated into one forward pass, and the output distribution at the last position of each question is that question's answer, much like the branches in Figure 3-13 that fork from one prefix, except that each stops after a single step. The questions do not depend on one another, and no KV needs to be kept once the request finishes. Whether these probabilities can be used directly as confidence depends on training: TypeSafe calls its method RLCD, which follows the same idea as RLCR from the research literature, adding a Brier score to the reward so that the probabilities the model reports match how often it is actually right.[^rlcr]

The book's own inputs are enough to check this. Jev charges only for input, 0.042 dollars per million tokens; the demo request cost 0.000081 dollars, which works out to about 1929 input tokens. On Qwen3-8B this request is one forward pass of about 28 TFLOPs, which takes about 70 ms on an H100 SXM at 40% compute efficiency, in line with the demo's 0.114 seconds. On DeepSeek V4.1 Flash along its CED path, the work grows by only about a quarter. An ordinary LLM, after the prefill, would still have to generate tokens one at a time, each step limited by memory bandwidth and taking about 9 ms. Generating 64 tokens pushes latency to 0.65 seconds, 9 times that of the decision request; because decode can be batched, GPU time grows by only 28%. Figure 3-15 puts the two executions on the same timeline: the instant answer comes from skipping serial generation.[^decision-calc]

![Figure 3-15 Two ways to execute the same 1929-token request on an H100 SXM. Top: the decision request reads out the answers to 8 questions at once after one forward pass. Bottom: an ordinary LLM generates 64 tokens one by one after the prefill. The model is Qwen3-8B, with 40% compute efficiency and 50% bandwidth efficiency.](images/figure-3-decision-request.pdf)

Pricing is no mystery either. Taking the DeepSeek inference-system report's figure of 2 dollars per GPU-hour, an 8B dense model or V4.1 Flash spends about 0.02 to 0.025 dollars of compute per million input tokens, and Jev's price is 1.7 to 2 times that. In other words, a model of this class reaches this price with nothing more than an ordinary prefill; conversely, if the price is to cover cost, Jev's work per token is unlikely to exceed about twice that of an 8B dense model. Of the 171-fold cost gap in the demo, only a small part comes from skipping generation: on the same model, dropping 64 to 512 output tokens cuts GPU time only to between 1/1.3 and 1/3.4 of the original. The rest comes from the price of the frontier model itself and the many tokens it generates while reasoning.

In practice, requests the decision model is unsure about still have to be handed to an LLM. Let the hand-off fraction be $e$; the average cost per request is then $c_{\mathrm{dec}}+e\,c_{\mathrm{LLM}}$. With the two costs from the demo, once $e$ exceeds about 0.58%, the hand-off cost is larger than the cost of the decision model itself. The $c$ in the cost per successful task of Section 3.2.2 must therefore include hand-off costs. Whether a decision model saves money depends on how many requests must be handed off and how expensive the receiving LLM is.
...
![Figure 3-16 An image goes from a pixel grid to visual tokens. A 640×640 square image is cut into 40×40 blocks; adjacent 2×2 blocks are merged into one token, forming a 20×20 grid of 400 visual tokens in total.](images/figure-3-vision-shapes.pdf)

![Figure 3-17 The number of visual tokens and the per-token feature width are measured separately. Four sets of 2560-dimensional BF16 encoded features occupy 7.8125 MiB; once these visual tokens enter the language model, they additionally produce per-layer KV state.](images/figure-3-vision-state.pdf)
...
![Figure 3-18 Composable multimodal stages. Encoding forms the model input, the language model generates the reply, an acoustic module converts the reply into audio, and the receiver's buffer and playback device determine when sound is actually produced.](images/figure-3-4-realtime.pdf)

![Figure 3-19 Teaching timeline of playback for eight audio chunks. Each chunk lasts 20 ms; dots mark arrival, short vertical lines mark the originally planned playback moment, and colored bars mark actual playback; the third chunk arrives 5 ms late, delaying subsequent playback accordingly.](images/figure-3-audio-timing.pdf)

Beyond playback continuity, interruption response must be checked separately. Figure 3-20 times from when the user issues an interruption, tracking when local playback actually stops; whether remote generation stops must be judged along a separate control path.

![Figure 3-20 Local interruption in the same teaching scenario. The action is issued at 123 ms, and the device mutes at 130 ms; whether remote computation stops belongs to a separate control path.](images/figure-3-audio-interrupt.pdf)
...
![Figure 3-24 Full-parameter training of Qwen3-8B with an 8192-token input. All input tokens execute the vocabulary head, with no recomputation; accumulated matrix by matrix, forward plus backward comes to 431.368 TFLOPs.](images/figure-3-training-flops.pdf)
...
![Figure 3-25 Roles and data flow in reinforcement learning. The generation side produces responses, the feedback stage evaluates the results, and after filtering they are sent to the learner for the update; the new weights are then used for the next round of generation.](images/figure-3-6-rl.pdf)

![Figure 3-26 Keeping the target of 16 effective samples, the generation count rises from 32 to 64. The input processing, subsequent decoding, and reference-model scoring during generation all grow with the total number of responses, while the number of samples processed by the policy update stays the same.](images/figure-3-rl-stage-work.pdf)
...
Solution: datablations is a public collection of experimental records studying how model and data scale vary; C4 is one of the text corpora used in these records. Taking eight models as an example, we fit a curve using six smaller models and hold out the two models with $N\ge2\times10^9$ for testing. The actual losses of the two large models are approximately 2.574 and 2.337, respectively; the fitted curve predicts 2.583 and 2.363. The predictions are slightly higher than the actual values, with a root-mean-square error of about 0.0193 nats/token. A nat is the unit of information when using the natural logarithm; nats/token denotes the average loss per token. Figure 3-27 distinguishes fitting points from test points using different markers.[^fit]

![Figure 3-27 Predicted versus actual loss for eight public C4 observations. 6 points are used for fitting, 2 points are held out in advance for testing; the diagonal represents predicted equal to observed, and deviation from the line reflects error.](images/figure-3-7-scaling.pdf)

![Figure 3-28 A zoomed-in view of the prediction errors for the same set. F1–F6 are fitting points, H1–H2 are held-out points; the vertical axis is predicted minus observed, retaining sign.](images/figure-3-scaling-residual.pdf)
...
![Figure 3-30 Models with similar parameter scale invest different amounts of training tokens. Bar values are reported training data volume divided by parameter count; Qwen uses the data budget disclosed at the model-family level.](images/figure-3-8-history.pdf)
...
The usage figures above come from three different accelerators, so GPU-hours cannot be directly compared in scale of investment across devices. Converting by BF16 dense peak compute: A100 80GB is 312 TFLOP/s, and H100 and H800 are both 989.4 TFLOP/s (the H800 only has lower interconnect bandwidth); one H100 or H800 GPU-hour is thus equivalent to about $989.4/312\approx3.17$ A100 GPU-hours. Figure 3-31 converts three sets of public usage figures uniformly into A100 80GB equivalent hours; this conversion assumes similar actual utilization across devices, and is intended for comparing scale of investment, not for indicating differences in efficiency or cost.

![Figure 3-31 Public training usage of Llama and DeepSeek-V3 converted uniformly to A100 80GB equivalent GPU-hours. Llama 1/Llama 2 are measured A100 hours; H100 and H800 hours are converted by the BF16 dense peak ratio $989.4/312\approx3.17$, and DeepSeek-V3 counts only the pretraining stage. The horizontal axis is a logarithmic scale; the conversion assumes similar actual utilization across devices and does not indicate differences in efficiency or cost.](images/figure-3-9-gpu-hours.pdf)
...
[^model]: [Chapter 2, Model Architecture](https://github.com/bojieli/ai-infra-book/blob/main/manuscripts/02-模型架构.md), request invocation conventions in §2.2 and Qwen3-8B KV counting; [request calculations for four models](https://github.com/bojieli/ai-infra-book/blob/main/calculations/results/request-four-models-book.md).
...
[^jev]: [TypeSafe AI homepage](https://github.com/bojieli/ai-infra-book/blob/main/references/text/typesafe-home.txt), pricing and the side-by-side demo; the [launch post](https://github.com/bojieli/ai-infra-book/blob/main/references/text/typesafe-jev-blog.txt) includes the founder's own account and describes parallel sampling, RLCD and the evaluation method; the [API documentation](https://github.com/bojieli/ai-infra-book/blob/main/references/text/typesafe-docs-intro.txt) states that questions are evaluated in parallel against the same state.

[^rlcr]: [Beyond Binary Rewards: Training LMs to Reason About Their Uncertainty](https://github.com/bojieli/ai-infra-book/blob/main/references/files/papers/rlcr.pdf), the RLCR reward in §3.

[^decision-calc]: [Execution paths and pricing comparison for the decision request](https://github.com/bojieli/ai-infra-book/blob/main/calculations/results/decision-request-book.md), including the V4.1 Flash full-layer path, the LLM path with 512 output tokens, and the DeepSeek price table; reproduce with `python3 calculations/calc.py decision-request --format md`; the 2 dollars per GPU-hour figure from the [DeepSeek inference system overview](https://github.com/bojieli/ai-infra-book/blob/main/references/text/deepseek-serving-report.txt); [DeepSeek models and pricing](https://github.com/bojieli/ai-infra-book/blob/main/references/text/deepseek-pricing-zh.txt).
