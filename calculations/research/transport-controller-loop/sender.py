...
def create_sender(inputs):
    ...
    if controller_config and controller_config["name"] != "newreno":
        ...
        filename, classname = {
            "cubic_hystart": ("cubic_adapter.py", "CubicAdapter"),
            "bbr": ("bbr_adapter.py", "BbrAdapter"),
        }[name]
        module_path = Path(__file__).resolve().parent / filename
        spec = importlib.util.spec_from_file_location("loop_" + name, module_path)
        module = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(module)
        cfg = dict(controller_config)
        cfg["bbr_rtt_seed_known"] = bool(p.get("rtt_seed"))
        cfg["external_pacer"] = True
        adapter = getattr(module, classname)(mds, cwnd, smoothed, cfg)
    pacer_hook = None
    ...
    def check_persistent():
        ...
        if (
            ...
            return
        path = Path(__file__).resolve().parent / "persistent_congestion.py"
        spec = importlib.util.spec_from_file_location("loop_persistent", path)
        module = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(module)
        records = [
            dict(
