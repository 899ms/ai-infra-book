...
async def main(args):
    ...
    base=root/'reference'/args.format
    ...
    result=dict(config=config,input_sha256=hashlib.sha256((base/'inputs.json').read_bytes()).hexdigest(),
        sources={name:hashlib.sha256((root/name).read_bytes()).hexdigest() for name in ['run.py','trace_probe.py','probe.py']})
    ...
    try:
        ...
        result['weights']=await engine.collective_rpc('weight_snapshot')
