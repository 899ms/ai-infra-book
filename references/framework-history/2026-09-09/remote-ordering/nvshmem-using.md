<!-- 从 nvshmem-using.html 迁移的资料快照；原始 HTML SHA-256: 63d6cf91c14b73ba5e718e1e4ce4a4c90c418be0c18b4d4a2abae7a3bea34870。 -->
...
[![NVSHMEM Memory Model](_images/mem_model.png)](_images/mem_model.png)

Shared and private memory regions at each PE. The aggregation of the shared memory segments across all PEs is referred to as a Partitioned Global Address Space (PGAS).[\#](#fig-using-mem-model "Link to this image")
...
<a id="example-nvshmem-program"></a>
...
[![Example communication ring using NVSHMEM](_images/ring.png)](_images/ring.png)

Illustration of the communication performed by the simple_shift kernel.[\#](#fig-ring "Link to this image")
...
<a id="using-gpudirect-async-kernel-initiated-communication"></a>
...
The GPUNetIO transport optionally depends on the [DOCA SDK](https://developer.nvidia.com/networking/doca) to enable advanced NIC features such as Data Direct Placement (DDP). The DOCA SDK path can be specified using the DOCA_SDK_LIB_PATH environment variable. The minimum required version of the DOCA SDK is 3.4. For details, refer to the [GPUNetIO documentation](https://github.com/NVIDIA-DOCA/gpunetio).

Further information on IBGDA and GPUNetIO with environment variables affecting that transport can be found in the [Environment Variables](gen/env.html#environment-variables) section of these documents.

<a id="selecting-and-enabling-a-gdaki-transport"></a>
...
<a id="using-nvshmem-with-mpi-or-openshmem"></a>
...
There are some references to more complex build environments in [Troubleshooting and FAQs](faq.html#troubleshooting-and-faqs).
...
`NVCC_GENCODE` is an nvcc option to specify which GPU archtecture to build the code for. more information on that option can be found at [https://docs.nvidia.com/cuda/cuda-compiler-driver-nvcc/index.html](https://docs.nvidia.com/cuda/cuda-compiler-driver-nvcc/index.html).
...
<a id="running-nvshmem-programs"></a>

## Running NVSHMEM Programs
By default, the NVSHMEM library compiles with support for the PMI and PMI-2 interfaces. However, it can also be compiled with support for PMIx. Refer to the [installation guide](https://docs.nvidia.com/hpc-sdk/nvshmem/release-notes-install-guide/install-guide/nvshmem-install-proc.html#building-and-installing-nvshmem) for more information on compiling the NVSHMEM libraries.
...
NVSHMEM follows the OpenSHMEM memory model. However, several important exceptions are made to adapt OpenSHMEM to the weakly consistent memory model provided by the GPU architecture, as noted in [Differences Between NVSHMEM and OpenSHMEM](gen/mem-model.html#differences-between-nvshmem-and-openshmem).
...
<a id="building-nvshmem-applications-libraries"></a>
...
[](cuda-interactions.html "next page")
...
- [Example NVSHMEM Program](#example-nvshmem-program)
- [Using GPUDirect Async Kernel-Initiated Communication](#using-gpudirect-async-kernel-initiated-communication)
  - [Selecting and Enabling a GDAKI Transport](#selecting-and-enabling-a-gdaki-transport)
- [Using NVSHMEM With MPI or OpenSHMEM](#using-nvshmem-with-mpi-or-openshmem)
- [Compiling NVSHMEM Programs](#compiling-nvshmem-programs)
- [Running NVSHMEM Programs](#running-nvshmem-programs)
- [Communication Model](#communication-model)
- [Data Consistency](#data-consistency)
- [Multiprocess GPU Support](#multiprocess-gpu-support)
- [Building NVSHMEM Applications/Libraries](#building-nvshmem-applications-libraries)
