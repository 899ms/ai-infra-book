...
labs=sorted(set(map(int,re.findall(r'\*\*实验 3-(\d+)',s))))
check(labs==list(range(1,11)),'ten exercises missing')
figs=re.findall(r'!\[[^\]]*\]\((ch03/[^)]+\.svg)\)',s)
check(len(figs)==11,'eleven figures missing')
check(len(re.findall(r'^\*图 3-\d+：',s,re.M))==11,'external captions missing')
...
for z in json.loads((HERE/'browser-validation.json').read_text()):
 check(z['width']==z['documentWidth'] and z['imagesLoaded'] and z['images']==11 and z['mathErrors']==0 and not z['unresolvedMath'],'browser check')
