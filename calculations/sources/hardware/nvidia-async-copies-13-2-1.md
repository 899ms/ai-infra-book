<!-- 从 nvidia-async-copies-13-2-1.html 迁移的资料快照；原始 HTML SHA-256: e601b5f21450aba8f96d78906d81621745f58758f1b791cf8280ef4be8d2a0c2。 -->

- [](../contents.html)
- [CUDA Programming Guide](../index.html)
- [4. CUDA Features](../part4.html)
- 4.11. Asynchronous Data Copies

[Is this page helpful?](https://surveys.hotjar.com/4904bf71-6484-47a7-83ff-4715cceabdb5)

<a id="asynchronous-data-copies"></a>
...
Table 18 Asynchronous copies with possible source and destination memory spaces and completion mechanisms using LDGSTS. An empty cell indicates that a source-destination pair is not supported.[\#](#table-async-source-dest-state-spaces-ldgsts "Link to this table") {#table-async-source-dest-state-spaces-ldgsts}
...
<a id="batching-loads-in-conditional-code"></a>
...
<a id="producer-consumer-pattern-through-warp-specialization"></a>
...
<a id="using-the-tensor-memory-accelerator-tma"></a>
...
Table 19 Asynchronous copies with possible source and destination memory spaces and completion mechanisms using TMA. An empty cell indicates that a source-destination pair is not supported.[\#](#table-async-source-dest-state-spaces-tma "Link to this table") {#table-async-source-dest-state-spaces-tma}

<a id="using-tma-to-transfer-one-dimensional-arrays"></a>
...
Table 20 Asynchronous copies with possible source and destination memory spaces and completion mechanisms using bulk-asynchronous TMA. An empty cell indicates that a source-destination pair is not supported.[\#](#table-async-source-dest-state-spaces-tma-bulk-sync "Link to this table") {#table-async-source-dest-state-spaces-tma-bulk-sync}
...
**TMA write and sync**. The write from shared to global memory is again initiated by a single thread. The completion of the write is not tracked by a shared memory barrier. Instead, a thread-local mechanism is used. Multiple writes can be batched into a so-called *bulk async-group*. Afterwards, the thread can wait for all operations in this group to have completed reading from shared memory (as in the code above) or to have completed writing to global memory, making the writes visible to the initiating thread. For more information, refer to the PTX ISA documentation of [cp.async.bulk.wait_group](https://docs.nvidia.com/cuda/parallel-thread-execution/index.html#data-movement-and-conversion-instructions-cp-async-bulk-wait-group). Note that the bulk-asynchronous and non-bulk-asynchronous copy instructions have different async-groups: there exist both `cp.async.wait_group` and `cp.async.bulk.wait_group` instructions.
...
Table 21 Alignment requirements for one-dimensional bulk-asynchronous operations.[\#](#table-alignment-one-dim-tma "Link to this table") {#table-alignment-one-dim-tma}

<a id="async-copies-tma-one-dim-staging"></a>
...
<a id="using-tma-to-transfer-multi-dimensional-arrays"></a>
...
Table 22 Asynchronous copies with possible source and destination memory spaces and completion mechanisms using bulk-tensor asynchronous TMA. An empty cell indicates that a source-destination pair is not supported.[\#](#table-async-source-dest-state-spaces-tma-bulk-tensor-async "Link to this table") {#table-async-source-dest-state-spaces-tma-bulk-tensor-async}
...
Table 23 Alignment requirements for multi-dimensional bulk tensor asynchronous copy operations.[\#](#table-alignment-multi-dim-tma "Link to this table") {#table-alignment-multi-dim-tma}

<a id="encoding-a-tensor-map-on-device"></a>
...
<a id="device-side-encoding-and-modification-of-a-tensor-map"></a>
...
<a id="usage-of-a-modified-tensor-map"></a>
...
<a id="creating-a-template-tensor-map-value-using-the-driver-api"></a>
...
<a id="shared-memory-bank-swizzling"></a>
...
[![The shared memory data layout without swizzle](../_images/swizzle-example1.png)](../_images/swizzle-example1.png)

Figure 48 In the shared memory data layout without swizzle, the shared memory indices are equivalent to the global memory indices. Per load instruction, one row is read and stored in a column of the transpose buffer. Since all matrix elements of the column in the transpose fall in the same bank, the store must be serialized, resulting in eight store transactions, giving an eight-way bank conflict per stored column.[\#](#id2 "Link to this image")

[![The shared memory data layout with CU_TENSOR_MAP_SWIZZLE_128B swizzle.](../_images/swizzle-example2.png)](../_images/swizzle-example2.png)

Figure 49 The shared memory data layout with `CU_TENSOR_MAP_SWIZZLE_128B` swizzle. One row is stored in a column, each matrix element is from a different bank for both the rows and columns, and so without any bank conflicts.[\#](#id3 "Link to this image")
...
Figure 50 An Overview of TMA Swizzle Patterns[\#](#id4 "Link to this image")
...
Table 24 Swizzle Pattern Pointer Offset Formula and Index Relation[\#](#table-swizzle-pattern-offset "Link to this table") {#table-swizzle-pattern-offset}
...
Table 25 Requirements and properties of the different swizzle patterns for Compute Capability 9[\#](#table-swizzle-pattern-properties-and-requirements "Link to this table") {#table-swizzle-pattern-properties-and-requirements}
...
[](pipelines.html "previous page")

previous

4.10. Pipelines

[](cluster-launch-control.html "next page")
...
- [4.11.1. Using LDGSTS](#using-ldgsts)
  - [4.11.1.1. Batching Loads in Conditional Code](#batching-loads-in-conditional-code)
  - [4.11.1.2. Prefetching Data](#prefetching-data)
  - [4.11.1.3. Producer-Consumer Pattern Through Warp Specialization](#producer-consumer-pattern-through-warp-specialization)
- [4.11.2. Using the Tensor Memory Accelerator (TMA)](#using-the-tensor-memory-accelerator-tma)
  - [4.11.2.1. Using TMA to transfer one-dimensional arrays](#using-tma-to-transfer-one-dimensional-arrays)
    - [4.11.2.1.1. Prefetching Data](#async-copies-tma-one-dim-staging)
  - [4.11.2.2. Using TMA to transfer multi-dimensional arrays](#using-tma-to-transfer-multi-dimensional-arrays)
    - [4.11.2.2.1. Encoding a Tensor Map on Device](#encoding-a-tensor-map-on-device)
    - [4.11.2.2.2. Device-side Encoding and Modification of a Tensor Map](#device-side-encoding-and-modification-of-a-tensor-map)
    - [4.11.2.2.3. Usage of a Modified Tensor Map](#usage-of-a-modified-tensor-map)
    - [4.11.2.2.4. Creating a Template Tensor Map Value Using the Driver API](#creating-a-template-tensor-map-value-using-the-driver-api)
    - [4.11.2.2.5. Shared-Memory Bank Swizzling](#shared-memory-bank-swizzling)
- [4.11.3. Using STAS](#using-stas)
