...
def socket_record(sock):
 return dict(rcvbuf=sock.getsockopt(socket.SOL_SOCKET,socket.SO_RCVBUF),sndbuf=sock.getsockopt(socket.SOL_SOCKET,socket.SO_SNDBUF),tcp_info_hex=sock.getsockopt(socket.IPPROTO_TCP,socket.TCP_INFO,256).hex())
...
class H3(QuicConnectionProtocol):
 ...
 async def reply(self,stream,record):
  ...
  self.http.send_headers(stream,[(b':status',b'200'),(b'content-length',str(len(response)).encode()),(b'content-type',b'application/json')]);self.http.send_data(stream,response,end_stream=True);self.transmit()
...
async def main(out,smoke):
 ...
 async def h1_server(reader,writer):
  ...
  try:
   while True:
    ...
    fixture=BYHASH[record['sha256']];record['step']=fixture['step'];response=fixture['response'];record['service_start']=NOW();await asyncio.sleep(fixture['service_s']);record['service_end']=NOW()
    for event in [h11.Response(status_code=200,headers=[(b'content-length',str(len(response)).encode()),(b'content-type',b'application/json')]),h11.Data(data=response),h11.EndOfMessage()]:writer.write(conn.send(event))
 ...
 env=dict(python=platform.python_version(),platform=platform.platform(),machine=platform.machine(),openssl=ssl.OPENSSL_VERSION,packages={p:importlib.metadata.version(p) for p in ['aioquic','h11','cryptography','pylsqpack']},certificate_sha256=fingerprint,source_sha256=hashlib.sha256(Path(__file__).read_bytes()).hexdigest(),mode=MODE,requested_window=WINDOW,listener_settings=listener_settings,quic_max_data=qc.max_data,quic_max_stream_data=qc.max_stream_data,tcp_port=tcp_port,udp_port=udp_port,fixture=FIXTURE,smoke=smoke,server_and_client_same_event_loop=True,body_request_deadline_s=REQUEST_TIMEOUT)
