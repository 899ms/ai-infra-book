...
axes[0].set_ylabel('Complete image validation (seconds)')
fig.suptitle('Image transfer: receive configuration comparison')
fig.text(.5,.015,'Six formal transfers per cell; dots = samples, line = median. Includes 0.484 s recorded CPU service.',ha='center',fontsize=9)
fig.tight_layout(rect=[0,.04,1,.94])
for ext in ['png','svg','pdf']:fig.savefig(R/f'window-results.{ext}',dpi=160)
plt.close(fig)
s='''# Receive configuration and complete image transfer
...
(R/'manifest.json').write_text(json.dumps(dict(scope='Actual image network receive-configuration comparison; parent incomplete',files=files),indent=2)+'\n')
