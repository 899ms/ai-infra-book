...
class KVProbe:
    ...
    def weight_snapshot(self):
        ...
        for name,p in self.model_runner.model.named_parameters():
            raw=p.detach().contiguous().view(torch.uint8).cpu().numpy().tobytes()
            parameters.append(dict(name=name,shape=list(p.shape),dtype=str(p.dtype),bytes=len(raw),sha256=hashlib.sha256(raw).hexdigest()))
        methods=[]
        for name,m in self.model_runner.model.named_modules():
            method=getattr(m,'quant_method',None)
            if method is not None:
                methods.append(dict(name=name,method=type(method).__module__+'.'+type(method).__name__))
        return dict(parameters=parameters,methods=methods)
