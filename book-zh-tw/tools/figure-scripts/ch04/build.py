...
f,a=canvas(7)
box(a,.03,.78,.18,.13,"主機 CPU\n準備輸入、提交任務")
box(a,.32,.78,.24,.13,"片外記憶體\n權重、輸入、最終輸出")
box(a,.71,.78,.23,.13,"裝置控制與任務排程")
...
box(a,.31,.57,.25,.12,"共享快取 / 記憶體介面");arrow(a,(.435,.77),(.435,.70))
a.add_patch(FancyBboxPatch((.04,.05),.90,.43,boxstyle='round,pad=.012',facecolor='none',edgecolor=C['gray'],linestyle='--'))
a.text(.06,.445,"區域性計算單元組（可有多組）",fontsize=11,color=C['gray'])
box(a,.08,.25,.20,.12,"區域性輸入緩衝\n分塊與複用",color='light')
box(a,.38,.25,.20,.12,"矩陣計算單元\n乘加與區域性累積",color='sand')
box(a,.69,.25,.20,.12,"累加儲存\n暫存器 / 專用儲存")
box(a,.69,.08,.20,.10,"向量與通用計算")
arrow(a,(.37,.56),(.18,.38));a.text(.12,.52,"載入 / 搬移",fontsize=11)
arrow(a,(.29,.31),(.37,.31));arrow(a,(.59,.31),(.68,.31));arrow(a,(.79,.24),(.79,.19));arrow(a,(.68,.12),(.48,.12));arrow(a,(.48,.12),(.48,.24));a.text(.35,.075,"中間結果繼續計算",fontsize=10)
arrow(a,(.91,.37),(.91,.62));arrow(a,(.90,.63),(.57,.63));a.text(.70,.65,"按需要寫回",fontsize=11)
...
for i,(label,col) in enumerate([("矩陣",'blue'),("共享記憶體讀取",'teal'),("指數",'orange')]):
...
a.set_yticks(range(4),["原設定","僅矩陣 ×2","矩陣與指數 ×2","三項同時 ×2"]);a.invert_yaxis();a.set_xlabel("單個 SM 處理一塊所需週期");a.set_xlim(0,1150);a.legend(ncol=3,frameon=False,loc='upper left',bbox_to_anchor=(0,1.18))
...
# Precision paths.
f,a=canvas(7)
box(a,.03,.43,.20,.23,"壓縮權重 + scale\n8.5 MiB",'light')
for y in [.78,.22]:arrow(a,(.235,.55),(.32,y))
box(a,.33,.68,.22,.20,"展開 / 反量化\nBF16 副本 32 MiB")
box(a,.63,.68,.22,.20,"矩陣計算\n匹配累加精度",'sand');arrow(a,(.56,.78),(.62,.78))
box(a,.33,.12,.22,.20,"低精度矩陣計算\n填充 / 部分和",'sand')
box(a,.63,.12,.22,.20,"縮放、合併與轉換\n規定輸出格式");arrow(a,(.56,.22),(.62,.22))
...
for threshold,rate in zip(thresholds,[b/1e12 for b in bandwidths]):
 a.plot(threshold,rate,'o',color=C['orange']);a.annotate(f'至少 {threshold} 個',xy=(threshold,rate),xytext=(threshold+350,rate-.23),fontsize=11,arrowprops={'arrowstyle':'-','color':C['gray']})
a.axvline(4096,color=C['gray'],ls=':',lw=1);a.set(xlim=(0,10000),ylim=(0,2.25),xlabel="可同時未完成的請求數 N",ylabel="可達到的頻寬上界 / TB/s");a.legend(frameon=False,loc='upper left');a.grid(axis='y',alpha=.15)
...
for a,slots in zip(axes,[1,2,3]):
 ...
 a.set_yticks(range(4),["塊 0","塊 1","塊 2","塊 3"]);a.invert_yaxis();a.set_xlim(0,1320);a.set_title(f'{slots} 個輸入槽：完成 {r["finish_tick"]} tick',loc='left',fontsize=12);a.set_xticks([0,320,640,960,1280]);a.tick_params(labelbottom=True)
axes[-1].set_xlabel("時間 / tick（B200 SM 時鐘週期）")
f.text(.12,.966,"藍：輸入傳輸　橙線：資料就緒　綠：計算　淺灰：槽位佔用至計算結束",fontsize=11)
...
for a,size,label in zip(axes,payload,['M=1：8 KiB','M=256：2 MiB']):
 ser=size/rates*1e6;total.append((ser+alpha).tolist());a.bar([0,1],[alpha,alpha],color=C['orange'],label="啟動");a.bar([0,1],ser,bottom=alpha,color=C['blue'],label="傳輸");a.set_xticks([0,1],['A100 300 GB/s','H100 450 GB/s']);a.set_title(label,loc='left',fontsize=12);a.set_ylabel("時間 / μs");a.set_ylim(0,max(ser+alpha)*1.2)
...
b=np.arange(1,33);W=15136811008;K=1207959552;a.plot(b,np.full(len(b),W/1e9),color=C['blue'],lw=2,label="本步所用權重：每批讀一次");a.plot(b,b*K/1e9,color=C['teal'],lw=2,label="KV：每請求讀取一次");a.axvline(13,color=C['orange'],ls=':');a.annotate("從 batch 13 起，KV 超過權重",xy=(13,13*K/1e9),xytext=(16,9),arrowprops={'arrowstyle':'->','color':C['orange']},fontsize=11);a.set_xlabel('batch B');a.set_ylabel("每步讀取 / GB");a.set_xlim(1,32);a.set_ylim(0,42);a.legend(frameon=False,fontsize=11,loc='upper left')
...
labels=[f'M={n}\n'+("複用" if mode=='reused' else "輪換") for n,mode in conditions];colors=[C['blue'],C['teal']]*2
...
for a,values,ylabel,limit in [(axes[0],wall,"每次呼叫總耗時 / μs",62),(axes[1],dram,"單獨測得的 DRAM 讀取 / MiB",40)]:
...
page="<!doctype html><html lang=\"zh-CN\"><meta charset=\"utf-8\"><meta name=\"viewport\" content=\"width=device-width,initial-scale=1\"><title>第 4 章 加速器架構</title><style>"+css+'</style><main><nav>'+nav+'</nav>'+body+'</main></html>'
import sys
sys.path.insert(0,str(HERE.parent))
from teaching_reading import readable_diagrams
from preview_output import preview_path
page=readable_diagrams(page)
html_path=preview_path(HERE.parent/"04-加速器架構.html");html_path.write_text(page)
