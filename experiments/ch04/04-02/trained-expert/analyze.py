...
def main(out):
 ...
 (out/'summary.json').write_text(json.dumps(dict(smoke=e['smoke'],rows=result,scope='Checkpoint expert matrix, synthetic activations, GPU-resident timed operator paths; no full model quality or cold-memory claim'),indent=2)+'\n');print('Verified',len(rows),'shapes')
