...
def calculate(jobs=None,capacity_bytes_per_second=50*10**9,window_ns=None,
    ...
    return dict(schema_version=1,calculation='periodic-fluid-queue',
                ...
                assumptions=[
                    '；'.join(f"作业{i}：周期{j['period_ns']/1e6:g}ms、持续{j['on_ns']/1e6:g}ms、偏移{j['offset_ns']/1e6:g}ms、{j['rate_bytes_per_second']/1e9:g}GB/s" for i,j in enumerate(jobs))+f"；共用{capacity_bytes_per_second/1e9:g}GB/s出口。到达速率是给定输入，不是CASSINI实测。观察窗口默认为周期最小公倍数。",
