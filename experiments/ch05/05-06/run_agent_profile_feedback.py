...
async def main(a):
    ...
    environment=dict(config=config,thinking=False,temperature=0,max_tokens=1400,
        ...
        protocol_sha256=sha(ROOT/'protocol.json'),source_hashes={f:sha(ROOT/f) for f in ['run_agent_profile_feedback.py','evaluate_agent_profile.py','agent_guard.py','schedule.py']})
...
This run uses an exclusive GPU. The starting kernel launches BLOCK=256 and num_warps=4; submitting those unchanged is a duplicate. Make a concrete change, such as a different block size or shape specialization. Every passing candidate receives actual CUDA profiler kernel names, durations and launch dimensions in tool feedback. Use those observations for your next hypothesis. Profiler durations are instrumented and are not the timing score.
'''
    system+='Allowed attribute names: '+', '.join(sorted(ATTRS))
    baseline=json.loads((ROOT/'results/exclusive-profile-001/schedule/3-schedule.json').read_text())
    assert baseline['status']=='passed'
    profiler_baseline=[dict(t=r['t'],profiler_feedback=r['profiler_feedback'],median_graph_us=statistics.median(r['samples_us'])) for r in baseline['rows']]
    system+='\nActual exclusive-device profiler observations for the starting BLOCK256/4warps kernel: '+json.dumps(profiler_baseline)
    environment['baseline_profile_sha256']=sha(ROOT/'results/exclusive-profile-001/schedule/3-schedule.json')
    dump(a.output/'environment.json',environment)
        ...
        for turn in range(protocol['maximum_candidates_per_strategy']):
            ...
            try:
                ...
                with (a.output/f'evaluation-{turn}.log').open('w') as log:
                    result=subprocess.run([sys.executable,str(ROOT/'evaluate_agent_profile.py'),'--kind','agent','--candidate',str(candidate.resolve()),'--output',str(result_path.resolve())],stdout=log,stderr=subprocess.STDOUT,timeout=protocol['candidate_process_timeout_seconds'])
            ...
            if reply.get('status')=='failed':feedback['unchanged_starting_kernel_profile']=profiler_baseline
