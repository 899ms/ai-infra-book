...
f,a=canvas(5)
for row,(groups,label) in enumerate([(8,"eight single-card instances"),(2,"two four-card instances"),(1,"one eight-card instance")]):
 ...
 for g in range(groups):
  ...
  if groups<=2:a.text(x+w/2,y+.013,"one independent inference instance",ha='center',fontsize=9,color=C['teal'])
a.set_ylim(.32,1)
...
f,a=canvas(8)
box(a,.015,.42,.17,.21,"complete input X",'m × h',size=14)
for y,r,col in [(.69,0,'light'),(.30,1,'pale')]:
 box(a,.255,y,.265,.22,f'card {r}: up-projection and activation',f'Wg,{r} / Wu,{r}：h × (f/2)\nZ{r}：m × (f/2)',col=col,size=12)
 box(a,.60,y,.16,.22,f'down-projection Wd,{r}','(f/2) × h',col=col,size=12)
 box(a,.835,y,.14,.22,f'partial sum Y{r}','m × h',col='sand',size=13)
 arrow(a,(.19,.53),(.25,y+.11));arrow(a,(.525,y+.11),(.595,y+.11));arrow(a,(.765,y+.11),(.83,y+.11))
box(a,.60,.04,.375,.13,"complete output Y = Y0 + Y1","AllReduce: each card gets m × h after summation",col='light',size=13)
arrow(a,(.98,.80),(.98,.105),'orange',rad=-.07)
arrow(a,(.905,.295),(.905,.175),'orange')
a.text(.03,.965,"up-projection column-sharded → activation stays local → down-projection row-sharded → output summed",fontsize=13,va='top')
...
ax.set(xlim=(0,7.1),ylim=(-.6,3.6),yticks=[3,2,1,0],yticklabels=["stage 0","stage 1","stage 2","stage 3"],xticks=range(8),xlabel="Time / ms")
ax.axvline(4,color=C['muted'],ls=':',lw=1);ax.text(4.05,3.4,"first result",fontsize=11)
ax.spines['left'].set_visible(False);ax.tick_params(axis='y',length=0)
save(f,'figure-6-3-pipeline')
# Independent token dispatch/combine ownership.
f,a=canvas(6.7)
box(a,.02,.29,.26,.29,"card 0: token A","router selects experts 1, 6",col='light',size=14)
box(a,.40,.64,.25,.21,"local expert 1","compute y1",size=14)
box(a,.40,.12,.25,.21,"card 3: expert 6","compute y6",size=14)
box(a,.75,.29,.23,.29,"card 0: weighted merge",'a1 y1 + a6 y6',col='sand',size=14)
arrow(a,(.285,.53),(.395,.74));arrow(a,(.285,.36),(.395,.225))
arrow(a,(.655,.74),(.745,.53));arrow(a,(.655,.225),(.745,.36),'orange')
a.text(.305,.20,'dispatch',fontsize=11,ha='right');a.text(.70,.18,"return y6",fontsize=11)
a.text(.05,.84,"device storing input",fontsize=12);a.text(.76,.84,"device merging results",fontsize=12)
save(f,'figure-6-4-dispatch')
# Ring: follow one block, then show completed block ownership.
f,a=canvas(8)
panel(a,.025,.965,"block 0: add contribution as it passes each card")
for r,val in enumerate([1,11,111,1111]):
 x=.025+r*.248
 box(a,x,.70,.21,.16,f'card {r}',f'block 0:{val}',col='light' if r==3 else 'pale',size=13)
 if r<3:
  arrow(a,(x+.215,.78),(x+.24,.78))
  a.text(x+.23,.64,f'Step {r+1} rounds',ha='center',fontsize=10)
panel(a,.025,.54,"ReduceScatter complete: each card holds one fully reduced block")
for r,block in enumerate([1,2,3,0]):
 box(a,.025+r*.248,.34,.21,.13,f'card {r}: block {block}',size=13)
 arrow(a,(.13+r*.248,.33),(.13+r*.248,.23))
 box(a,.025+r*.248,.045,.21,.17,f'card {r}',"blocks 0, 1, 2, 3",col='light',size=13)
a.text(.5,.265,"three more AllGather rounds exchange reduced blocks",ha='center',fontsize=12)
save(f,'figure-6-6-ring-rounds')
# 3: Equal assignments, different expert reuse.
m=np.arange(1,65);expected=128*(1-(1-8/128)**m)
f,a=canvas(6)
for y,title,experts,rows,col in [(.61,"uniform coverage",128,4,'blue'),(.15,"concentrated on same eight experts",8,64,'orange')]:
 a.text(.02,y+.23,title,fontsize=15,weight='bold')
 box(a,.025,y,.23,.17,f'{experts} experts × {rows} rows',f'total {experts*rows} dispatches',size=13)
...
a.text(.37,.96,"weight read volume",fontsize=15,weight='bold',va='top')
...
ax.set(xlabel="expert index",ylabel="layer index",xticks=[0,64,128,192,255],yticks=[0,10,20,30,42]);f.colorbar(im,ax=ax,label="Selection count / token")
...
ax.loglog(M,ring*1e6,color=C['blue'],lw=2.2,label='Ring');ax.loglog(M,tree*1e6,color=C['orange'],lw=2.2,label="unsegmented binomial tree");ax.set(xlabel="Full input per card / bytes",ylabel="Model time / μs",xlim=(100,1e8),ylim=(3,5000));ax.legend(frameon=False,loc='upper left');ax.grid(which='major',alpha=.2)
...
ax.annotate("~680 KiB: sort flip",(cross,14*A*1e6+1.75*cross/BW*1e6),xytext=(3e6,40),fontsize=12,arrowprops={'arrowstyle':'->','color':C['muted']})
...
ax.set(yticks=[1,0],yticklabels=['4 MiB','64 MiB'],xlabel="Time / ms",xlim=(0,50),ylim=(-.65,1.65));ax.set_title("communication runs alone",fontsize=14,loc='left')
ax=axs[1]
for base,comm,compute,col in [(1.1,6.141,11.523,'blue'),(0,45.209,12.885,'orange')]:
 ...
 ax.text(.015,base+.15,"communication",va='center',fontsize=10)
 ax.text(.015,base-.15,"compute",va='center',fontsize=10,color='white')
...
ax.set(yticks=[1.1,0],yticklabels=['4 MiB','64 MiB'],xlabel="Time from simultaneous start / ms",xlim=(0,56),ylim=(-.65,1.75));ax.set_title("run concurrently",fontsize=14,loc='left')
...
for idx,z in enumerate(paths):
 ...
 ax.set_title(("recursion" if idx==0 else 'Swing')+f': round 3, 0 → {route["receiver"]}',fontsize=13)
ax=f.add_axes([.66,.21,.30,.61]);xx=np.arange(3)
for i,z in enumerate(paths):
 vals=[v['peak_link_bytes']/2**20 for v in z['rounds']]
 ax.bar(xx+(i-.5)*.34,vals,.32,color=C[['blue','orange'][i]],label=["recursion",'Swing'][i])
 for x,v in zip(xx+(i-.5)*.34,vals):ax.text(x,v+.12,f'{v:g}',ha='center',fontsize=11)
ax.set(xticks=xx,xticklabels=['1','2','3'],xlabel="round",ylabel="Max unidirectional link transfer / MiB",ylim=(0,5.6));ax.legend(ncol=2,frameon=False,fontsize=11)
...
for y,title,left,right,middle,detail in [
 (.72,"NVIDIA: rack-scale NVLink","GPU / CPU tray","GPU / CPU tray","NVLink switching",'GB200 NVL72：72 GPU、36 CPU'),
 (.40,"TPU: electrical interconnect unit","64-chip electrical interconnect unit","64-chip electrical interconnect unit","OCS optical switching","TPU v4: 4×4×4 chips per unit"),
 (.08,"UB: cross-host resource interconnect","NPU / CPU resources","NPU / CPU resources","UB switching",'CloudMatrix384：384 NPU、192 CPU')]:
...
ax.axhline(50,color=C['red'],ls='--',label="Path: 50 GB/s")
ax.axhline(win,color=C['orange'],ls=':',label="128 in-flight transactions: ~4.36 GB/s")
for i,v in enumerate(demand_bw):
 ax.scatter(freq[i],v,color=C['blue'],s=45,zorder=3)
 label=["Once per minute\n~0.33 GB/s","Once per second\n20 GB/s","20 times per second\n400 GB/s"][i]
 offset=[(12,-5),(12,-40),(-125,12)][i]
 ax.annotate(label,(freq[i],v),xytext=offset,textcoords='offset points',fontsize=11)
ax.set(xlabel="Frequency of reading 20 GB fully / times per second",ylabel="Required average bandwidth / GB/s",xlim=(.01,40),ylim=(.1,1500))
...
f=plt.figure(figsize=(12,5.8));colors=['muted','blue','teal','orange'];labels=["eight single-card instances","four two-card instances","two four-card instances","one eight-card instance"]
curve_data={}
for pos,(phase,title) in enumerate([('healthy',"no-failure condition"),('fault',"20 ms failure, redo starts at 60 ms")]):
 ...
 ax.axvline(130,color=C['line'],ls='--');ax.set(xlim=(20,240),ylim=(0,1.9),xlabel="Eight-step continuation deadline / ms",ylabel="Per on-time session / GPU·s")
...
page="<!doctype html><html lang=\"zh-CN\"><head><meta charset=\"utf-8\"><meta name=\"viewport\" content=\"width=device-width,initial-scale=1\"><title>Chapter 6 Supernode</title><style>"+css+math_css+"</style></head><body><main><nav aria-label=\"Chapter contents\">"+nav+'</nav>'+body+'</main></body></html>'
import sys
sys.path.insert(0,str(HERE.parent))
...
outputs += draw_ub_ep(6, HERE)
from parallel_figures import draw as draw_parallel
outputs += draw_parallel(HERE)
from book_assets import sync_figure_index
active_assets=sync_figure_index(HERE)
artifacts=outputs+[HERE.parent/'ub_ep_figures.py',HERE.parent/'parallel_figures.py',HERE/'parallel-layout-validation.json',HERE/'ub-ep-layout-validation.json',ROOT/'calculations/results/ep-skew-book.json',HERE/'teaching_revision.py',HERE/'figure-index.json',HERE/'teaching-layout-validation.json',HERE/'visual_examples.py',HERE/'figure-catalog.json',HERE/'figure-data.json',HERE/'continuity-model.json',HERE/'continuity_model.py',hp,md]+active_assets
(HERE/'manifest.json').write_text(json.dumps({'chapter':6,'generator':'manuscripts/ch06/build.py','figures':len(figure_catalog['figures']),'supplementary_figures':1,'font_family':family,'outputs':[{'path':str(p.relative_to(ROOT)),'sha256':hashlib.sha256(p.read_bytes()).hexdigest()} for p in artifacts if not (p.parent==ROOT/'manuscripts' and re.match(r'^[01][0-9]-',p.name))]},ensure_ascii=False,indent=2)+'\n')
