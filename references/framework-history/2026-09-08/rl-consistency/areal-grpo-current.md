<!-- 从 areal-grpo-current.html 迁移的资料快照；原始 HTML SHA-256: 0b772a961e02c1fdc1f8d6ee54fc76e4a8df50f752b42a6de2d09d277e98dc29。 -->

- [ Repository](https://github.com/areal-project/AReaL "Source repository")
- [ Open issue](https://github.com/areal-project/AReaL/issues/new?title=Issue%20on%20page%20%2Ftutorial/gsm8k_grpo.html&body=Your%20issue%20content%20here. "Open an issue")

- [ .md](../_sources/tutorial/gsm8k_grpo.md "Download source file")
-  .pdf
...
- [Overview: How AReaL Works](#overview-how-areal-works)
  - [Single-Controller Architecture](#single-controller-architecture)
  - [Data Flow with RTensor](#data-flow-with-rtensor)
- [Launching the Experiment](#launching-the-experiment)
  - [How Single-Controller Mode Works](#how-single-controller-mode-works)
  - [Configuration Files](#configuration-files)
- [The Training Script: Entry Point](#the-training-script-entry-point)
- [The PPOTrainer: Controller-Based Training](#the-ppotrainer-controller-based-training)
  - [Controller Architecture](#controller-architecture)
- [Rollout: Generating Training Data](#rollout-generating-training-data)
  - [Workflow Specification](#workflow-specification)
  - [RLVRWorkflow: Single-Turn Reward Learning](#rlvrworkflow-single-turn-reward-learning)
  - [Asynchronous Rollout Collection](#asynchronous-rollout-collection)
    - [Three-Process Architecture](#three-process-architecture)
    - [Three Levels of Concurrency](#three-levels-of-concurrency)
    - [Request Flow](#request-flow)
- [Training: Controller-Worker Pattern](#training-controller-worker-pattern)
  - [TrainController: Dispatch Mechanism](#traincontroller-dispatch-mechanism)
  - [Training Workers: Algorithm Implementation](#training-workers-algorithm-implementation)
  - [The Training Loop](#the-training-loop)
- [Weight Synchronization](#weight-synchronization)
  - [Transfer Methods](#transfer-methods)
  - [Weight Update Process](#weight-update-process)
...
<a id="running-grpo-on-gsm8k-dataset"></a>

# Running GRPO on GSM8K Dataset
This guide walks you through how AReaL runs the GRPO algorithm on the GSM8K dataset. We’ll use the example training script [`examples/math/gsm8k_rl.py`](https://github.com/areal-project/AReaL/blob/main/examples/math/gsm8k_rl.py) and configuration file [`examples/math/gsm8k_grpo.yaml`](https://github.com/areal-project/AReaL/blob/main/examples/math/gsm8k_grpo.yaml) to explain the key concepts step by step.

<a id="overview-how-areal-works"></a>

## Overview: How AReaL Works
<a id="single-controller-architecture"></a>
...
<a id="launching-the-experiment"></a>

## Launching the Experiment
AReaL supports launching experiments with different scheduler backends for different environments. As shown in the [quickstart guide](quickstart.html), you can launch experiments with:
...
<a id="how-single-controller-mode-works"></a>
...
- Per-engine `backend` fields (e.g., `rollout.backend`, `actor.backend`): Determine GPU allocation and parallel strategies for each engine
...
### Configuration Files
Configuration files are YAML files that specify options from [`areal/api/cli_args.py`](https://github.com/areal-project/AReaL/blob/main/areal/api/cli_args.py). You can override settings via CLI:
...
See [CLI Reference](../cli_reference.html) for all available options.

<a id="the-training-script-entry-point"></a>

## The Training Script: Entry Point
The training script ([`examples/math/gsm8k_rl.py`](https://github.com/areal-project/AReaL/blob/main/examples/math/gsm8k_rl.py)) follows this pattern:
...
See [CLI Reference](../cli_reference.html) for configuration options, and [Customization: Dataset](../customization/dataset.html) for custom datasets.

<a id="the-ppotrainer-controller-based-training"></a>

## The PPOTrainer: Controller-Based Training
The [`PPOTrainer`](https://github.com/areal-project/AReaL/blob/main/areal/trainer/rl_trainer.py) orchestrates distributed training by initializing the scheduler and creating controllers for actors (policy/critic) and rollout workers.

<a id="controller-architecture"></a>
...
<a id="rollout-generating-training-data"></a>

## Rollout: Generating Training Data
<a id="workflow-specification"></a>
...
<a id="rlvrworkflow-single-turn-reward-learning"></a>

### RLVRWorkflow: Single-Turn Reward Learning
The [`RLVRWorkflow`](https://github.com/areal-project/AReaL/blob/main/areal/workflow/rlvr.py) defines how prompts become training samples. Each trajectory goes through these steps:
...
**GSM8K Reward**: Binary reward (1.0 for correct answer, 0.0 otherwise). See [`gsm8k_reward_fn`](https://github.com/areal-project/AReaL/blob/main/areal/reward/gsm8k.py).

**NOTE:** This workflow adopts the low-level API of inference engines — the `agenerate` API. It is preferable if you want more fine-grained control over token IDs. `agenerate` inputs token IDs to the inference server and produces output token IDs for user’s processing. We also provide high-level API for convenient agentic workflow orchestration. We refer to the [agentic RL guide](agentic_rl.html).

<a id="asynchronous-rollout-collection"></a>
...
<a id="three-process-architecture"></a>
...
<a id="three-levels-of-concurrency"></a>

#### Three Levels of Concurrency
**Level 1 - Controller Thread**: [`BatchTaskDispatcher`](https://github.com/areal-project/AReaL/blob/main/areal/core/workflow_executor.py) runs in a background thread, continuously submitting rollout requests to workers via HTTP:
...
As such, **rollout and training happen simultaneously** in AReaL, even though the code looks like a synchronous orchestration.

**Level 2 - Worker RPC Server**: Each rollout worker runs a Flask HTTP server ([`rpc_server.py`](https://github.com/areal-project/AReaL/blob/main/areal/infra/rpc/rpc_server.py)) on **CPU**:
...
**Staleness Control**: [`StalenessManager`](https://github.com/areal-project/AReaL/blob/main/areal/infra/staleness_manager.py) limits concurrent inflight requests:
...
<a id="training-controller-worker-pattern"></a>
...
<a id="traincontroller-dispatch-mechanism"></a>

### TrainController: Dispatch Mechanism
[`TrainController`](https://github.com/areal-project/AReaL/blob/main/areal/infra/controller/train_controller.py) provides the core RPC dispatch:
...
<a id="training-workers-algorithm-implementation"></a>

### Training Workers: Algorithm Implementation
On each training worker, [`FSDPPPOActor`](https://github.com/areal-project/AReaL/blob/main/areal/trainer/ppo/actor.py) implements the GRPO/PPO algorithm:
...
<a id="the-training-loop"></a>

### The Training Loop
The `trainer.train()` method orchestrates the complete loop. See [`PPOTrainer.train()`](https://github.com/areal-project/AReaL/blob/main/areal/trainer/rl_trainer.py) for the full implementation:
...
<a id="weight-update-process"></a>

### Weight Update Process
The weight sync process in `PPOTrainer.train()` follows this pattern:

1.  Pause rollout servers to interrupt all inflight generations back to the rollout client (e.g., `RemoteSGLangEngine`)

2.  Transfer weights via configured method (NCCL or disk)

3.  Update version tracking for staleness management

4.  Resume rollout with updated weights with re-computed KV cache

See [`PPOTrainer.train()`](https://github.com/areal-project/AReaL/blob/main/areal/trainer/rl_trainer.py) lines 861-874 for the full implementation.

<a id="monitoring-and-utilities"></a>
...
**Saver** creates HuggingFace-compatible checkpoints that can be loaded with `transformers` or published to HuggingFace Hub. Each save creates a new directory.

**RecoverHandler** saves complete training state (model, optimizer, dataloader, RNG) for fault tolerance. Checkpoints are backend-specific and require the same parallelism configuration to load. Each save overwrites the previous checkpoint.

Both are called automatically during `trainer.train()`. For details, see the [Checkpointing Reference](../reference/checkpointing.html).

<a id="evaluation"></a>

### Evaluation
The [`Evaluator`](https://github.com/areal-project/AReaL/blob/main/areal/utils/evaluator.py) runs periodic evaluations on validation sets. Configure via `config.evaluation`. Called automatically in `trainer.train()`.
...
**`stats_tracker`** ([source](https://github.com/areal-project/AReaL/blob/main/areal/utils/stats_tracker.py)): Collects statistics with two paradigms optimized for different use cases:

- **Streaming metrics** for rollout workers: Each workflow logs scalars individually (e.g., `reward`), which are aggregated across workers by the controller

- **Batch metrics** for training: Tensor statistics with boolean masks are logged per batch, then all-reduced across data-parallel ranks
...
**`StatsLogger`** ([source](https://github.com/areal-project/AReaL/blob/main/areal/utils/stats_logger.py)): Sends aggregated metrics to logging backends (Weights & Biases, SwanLab, TensorBoard) from rank 0. At each training step, `PPOTrainer` collects metrics from all components and commits them:
...
For the complete API reference, see the [Metrics Tracking Reference](../reference/metrics_tracking.html).
...
- [Evaluation](eval.html) - Evaluate your trained model

- [Training Large MoE Models](megatron.html) - Scale to massive models with Megatron integration

- [Agentic RL](agentic_rl.html) - Build agents that use tools and any agentic frameworks

**Customization Guides**:

- [Custom Datasets](../customization/dataset.html) - Use your own data sources

- [Custom Workflows](../customization/agent.html) - Build agentic/RLVR workflows with custom reward functions

[](../cli_reference.html "previous page")

previous

Configurations

[](../best_practices/algo_perf.html "next page")
