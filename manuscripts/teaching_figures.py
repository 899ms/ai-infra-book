...
def draw(ch,save,root):
 ...
 if ch==1:
  ...
  a.set(xscale='log',xlabel=r'batch $B$',ylabel='每批时间（ms）',ylim=(0,80));a.set_title('A  读取与计算的转折',loc='left',fontsize=15);a.axvline(cross,color=TEAL,ls=':');a.text(cross*1.07,59,r'$B_*\approx148$',fontsize=13,color=TEAL);a.legend(fontsize=12,loc='upper left')
  a=axs[1];a.plot(batches,batches/lower,color=TEAL,lw=2.5);a.axvline(cross,color=TEAL,ls=':');a.set(xscale='log',xlabel=r'batch $B$',ylabel='理想吞吐（token/s）',ylim=(0,8000));a.set_title('B  同一模型预测的吞吐',loc='left',fontsize=15)
  a.text(3,6800,r'计算上限 $\Pi/(2N)$',fontsize=13);a.text(2,3700,'读取主导：增加复用\n计算主导：吞吐趋于平台',fontsize=12,linespacing=1.8)
 ...
 if ch==3:
  ...
  variants=[('A  工具串行，总时间 21 秒',[(0,2,3,BLUE),(2,6,2,TEAL),(8,10,1,ORANGE),(18,3,3,BLUE)],16),('B  工具并行，总时间 15 秒',[(0,2,3,BLUE),(2,6,2,TEAL),(2,10,1,ORANGE),(12,3,3,BLUE)],10)]
  for a,(title,blocks,wait) in zip(axs,variants):
   ...
   a.set(xlim=(0,22),ylim=(-.7,3.8),yticks=[0,1,2,3],yticklabels=['保留状态','工具二','工具一','模型'],xlabel='任务开始后的时间（s）');a.set_title(title,loc='left',fontsize=14);a.grid(axis='x',alpha=.15);a.set_xticks([0,2,8,12,18,21])
