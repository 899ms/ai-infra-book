...
config={'experimental':True,'debug':True,'data-root':str(P/'data'),'exec-root':'/run/book1102-c421','pidfile':str(P/'daemon.pid'),'hosts':['unix://'+'/run/book1102-c421.sock'],'bridge':'none','iptables':False,'ip6tables':False,'ip-forward':False,'ip-masq':False,'containerd-namespace':'book1102','containerd-plugins-namespace':'plugins.book1102'}
config['containerd']='/run/book1102-c421-ctd.sock'
ctlog=(P/'containerd.log').open('w')
ct=subprocess.Popen(['containerd','--root',str(P/'containerd-root'),'--state','/run/book1102-c421-ctd','--address',config['containerd']],stdout=ctlog,stderr=subprocess.STDOUT)
for _ in range(100):
...
host='unix://'+'/run/book1102-c421.sock'
try:
 for _ in range(100):
  if proc.poll() is not None:raise RuntimeError('daemon exited')
  if Path('/run/book1102-c421.sock').exists() and cmd(['docker','-H',host,'info','--format','{{.DockerRootDir}}'])['rc']==0:break
  time.sleep(.2)
