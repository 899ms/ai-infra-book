...
[四客户三轮实际回放](multiturn-cache/README.md)保留实际模型回答作为后续历史。APC关闭／轮转开启／客户内连续开启三组各12/12正确，全部输入输出相同；命中输入token比例0%／0.40%／66.32%。顺序改变可复用量，不据此换算速度；明确为合成控制负载，不冒充ServeGen匿名生产会话。

## 多轮并发补测

[同负载四客户并发](multiturn-concurrency/README.md)新增24请求全部正确、与串行输入输出一致；KV池采样峰由51.27%升至100%，等待峰3，APC关/开观测抢占3/0。应用4条同时在途但引擎运行峰2；无单次性能因果排名。
