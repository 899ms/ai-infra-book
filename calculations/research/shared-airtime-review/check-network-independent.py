...
# PHY truncation: access idle ends 34us, preamble spans [34,50]us.
# At 44us no DATA PSDU has reached the AP and therefore no WAN bytes exist.
p=copy.deepcopy(inputs['same-PN-MAC-ACK-lost'])
p['network']['wireless_access']['failures']=[]
p['network']['wireless_access']['profile']=json.loads((HERE.parent/'shared-airtime-inputs/profiles.json').read_text())['source_backed_reference_selection']
p['network']['until']='0.000044'
r=m.calculate(p);generic('PHY-preamble-horizon44us',r)
invariant('PHY-preamble.no fabricated uniform IP bytes',r['summary']['serialized_wire_bytes_by_horizon'] is None)
invariant('PHY-preamble.no WAN bytes before AP reception',F(r['summary']['wan_serialized_ip_bytes_by_horizon'])==0)
invariant('PHY-preamble.no application delivery',not r['delivered'])
invariant('PHY-preamble.one started unreached DATA',len(attempts(r))==1 and not attempts(r)[0]['received'])
invariant('PHY-preamble.actual TXSTART34us',F(attempts(r)[0]['data_start'])==F(34,1000000))
invariant('PHY-preamble.actual reserved elapsed44us',F(r['wireless_summary']['observed_reserved_seconds'])==F(44,1000000))
results['PHY-preamble-horizon44us']=r

# Downstream is still serializing on WAN at 0.5s; scheduled AP arrival 1s
# cannot be published as client arrival, and no air reservation exists yet.
p=copy.deepcopy(inputs['busy-client-ACK-refresh'])
p['network']['until']='1/2'
r=m.calculate(p);generic('down-WAN-midflight',r)
invariant('down-WAN.no radio attempt yet',not attempts(r))
invariant('down-WAN.client arrival unknown',all(t['arrival'] is None and t['received_at'] is None for t in tx(r)))
invariant('down-WAN.AP scheduled arrival separately labeled',tx(r)[0]['wan_arrival']=='1')
invariant('down-WAN.partial WAN614IPbytes',F(r['summary']['wan_serialized_ip_bytes_by_horizon'])==614)
invariant('down-WAN.no application delivery',not r['delivered'])
results['down-WAN-midflight']=r
