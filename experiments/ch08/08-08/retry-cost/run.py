...
async def main(a):
 ...
 tasks=[fixture(tok,512,8808000+i,f'heldout-{i:02d}') for i in range(16)]
 ...
 env=dict(config=cfg,torch=torch.__version__,vllm=vllm.__version__,hashes={n:hashlib.sha256((root/n).read_bytes()).hexdigest() for n in ['run.py','fixture.py','probe.py','switch_probe.py']},input_sha256=hashlib.sha256((a.output/'inputs.json').read_bytes()).hexdigest(),policy='Known-answer synthetic exact checker; one FP8-Q attempt then at most one BF16-Q retry on incorrect answer; no retries after second failure.')
