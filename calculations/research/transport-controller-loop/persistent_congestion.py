...
RFC = Path(__file__).resolve().parents[1] / "transport-feedback/sources/rfc9002.txt"
RFC_SHA = "3a8a54eea1ad5d1c134a548bf15edfa0e21bfb4106dbd7db3c09cace842099af"
...
def evaluate(packets, *, smoothed_rtt, rttvar, max_ack_delay, granularity,
             max_datagram_size=1200, trigger="ack"):
    ...
    """
    if hashlib.sha256(RFC.read_bytes()).hexdigest() != RFC_SHA:
        raise ValueError("RFC9002 source mismatch")
    smoothed, variance, delay, tick = map(rational, (smoothed_rtt, rttvar, max_ack_delay, granularity))
    ...
    for packet in packets:
        if set(packet) != {"pn", "sent_at", "status", "ack_eliciting", "rtt_known_at_send"}:
        ...
        if at < 0 or packet["status"] not in ("sent", "lost", "acked", "lost_then_acked"):
            raise ValueError("Invalid packet time/status")
        if any(type(packet[k]) is not bool for k in ("ack_eliciting", "rtt_known_at_send")):
    ...
    for _, _, packet in records:
        if prior_seen and not packet["rtt_known_at_send"]:
            raise ValueError("Prior RTT knowledge cannot disappear on the confirmed path")
    ...
    def finish_span():
        if first is not None and last is not None and last[0] - first[0] > duration:
            evidence.append(dict(first_pn=first[1], last_pn=last[1],
                                 start=str(first[0]), end=str(last[0]),
    ...
    for at, pn, packet in records:
        if packet["status"] in ("acked", "lost_then_acked"):
            finish_span()
            first = last = None
        elif packet["status"] == "lost" and packet["ack_eliciting"] and packet["rtt_known_at_send"]:
            first = first or (at, pn)
            last = (at, pn)
    finish_span()
    return dict(established=bool(evidence) and trigger == "ack", trigger=trigger,
                threshold_duration=str(duration), qualifying_spans=evidence,
                required_cwnd_bytes=2 * max_datagram_size if evidence and trigger == "ack" else None,
