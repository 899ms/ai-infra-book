...
ax.set(yticks=[1,0],yticklabels=['单卡部署','八卡 TP 中的每卡'],xlim=(0,560),xticks=[0,80,200,300,400,500],ylim=(-.65,1.7),xlabel='每卡内存占用 / GB');ax.legend(loc='lower left',frameon=False,ncol=3,fontsize=10)
...
for key,label,color in [('local_s','本地内存访问','blue'),('communication_s','128 次归约','orange')]:
...
ax.set(yticks=ys,yticklabels=['TP 1','TP 2','TP 4','TP 8'],xlim=(0,34),xticks=[0,5,10,15,20,25,30],xlabel='一次 decode 的执行时间 / ms');ax.legend(loc='upper center',bbox_to_anchor=(.5,1.18),ncol=3,frameon=False)
save(f,'tp-time')
# Ports are discrete resources; the same 32 slots must be split between directions.
f,a=canvas(5.8)
for y,down,up in [(.62,32,32),(.18,48,16)]:
 box(a,.015,y,.21,.23,f'{down} 个 GPU 端口',f'总输入 {down*25} GB/s',col='light')
 box(a,.77,y,.215,.23,f'{up} 个上联端口',f'总带宽 {up*25} GB/s',col='sand')
 arrow(a,(.23,y+.12),(.30,y+.12));arrow(a,(.70,y+.12),(.76,y+.12),'orange')
 a.text(.5,y+.27,f'64 个端口：{down} 下联 / {up} 上联',ha='center',fontsize=13)
 for i in range(64):
  row=i//32;col=i%32
...
for ax,title in zip(axs,['借用前：任务 0 还缺 20 GB','借用后：20 GB 保存在节点 1']):
 ax.bar(range(4),[80]*4,color='white',edgecolor=C['line'],width=.65)
 ax.set(xticks=range(4),xticklabels=['节点 0','节点 1','节点 2','节点 3'],ylim=(0,111),yticks=[0,20,40,60,80,100]);ax.set_title(title,fontsize=13)
...
axs[1].annotate('任务 0 的远端数据',xy=(1,80),xytext=(1.6,99),arrowprops={'arrowstyle':'->','color':C['blue']},fontsize=11,color=C['blue'])
axs[0].set_ylabel('内存需求与占用 / GB')
...
for ax,c in zip(axs.flat,continuous['candidates'][1:]):
 ...
 ax.set(yticks=range(shown),yticklabels=[f'实例 {i}' for i in range(shown-1,-1,-1)],xlim=(0,180),ylim=(-.6,max(shown-.4,1)),xticks=[0,30,60,90,120,150,180])
