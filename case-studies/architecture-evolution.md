...
本笔记支撑第 4 章各节中的代际比较。每次只选影响当前模型的一两项变化：先给旧负载与资源限制，再换负载，计算新增机制能省去什么以及代价在哪里。完整型号规格留在资料库；正文采用局部数据路径、时间线和可复算的例子。
...
厂商或论文明确说明的目标，注明出处与发布时间；由工作负载和资源约束推导的合理性，明确写成本书分析。每次设计比较都保留反问：原任务不变时是否有收益，换成长上下文、小专家矩阵或低 batch 后瓶颈怎样移动，省下的时间是否抵得过面积、功耗、质量与开发成本。第 4 章第 4.6 节按 NVIDIA、昇腾、Apple 的顺序集中比较代际演进，前面的机制小节提供分析基础。各代变化按模型工作、执行压力与硬件机制组织，结合数据通路图和对照表。

## 2026-09-10：量化演进与选型

第四章的集中比较已改为逐项计算：V100 普通 FMA／Tensor Core；3090／4090／5090、工作站与 A100／H100／H200；910A 的 img2col、向量吞吐需求与 CV 接口字节；Apple 容量／带宽与 MoE 路由。原始工作量和参数读取 calculations，计算程序与独立检查见 [architecture-evolution-quantitative](../calculations/research/architecture-evolution-quantitative/README.md)。正文不用显存容量或峰值单项决定优劣；分别给出驻留、读取、矩阵与实际路径校准。
