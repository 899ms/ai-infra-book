...
def draw(here,data):
    ...
    with plt.rc_context(STYLE):
        ...
        a.scatter([life['queue_s']+life['prefill_s'],life['completion_s']],[0,0],color='#252525',zorder=5);a.axvline(life['deadline_s'],ls='--',color='#a56c28');a.set(yticks=[],xlim=(0,7.4),xlabel="從到達起計時（s）");f.subplots_adjust(top=.78);a.legend(ncol=3,frameon=False,loc='upper center',bbox_to_anchor=(.5,1.28),columnspacing=1.5,handlelength=1.4);save(f,'1-lifecycle')
        f,a=plot(3.7);b=np.array(data['batch']['batch']);w=data['batch']['shared_weight_bytes']/2**30/b
        for L,c in [(2048,'#267398'),(8192,'#388768')]:a.plot(b,w+L*144/2**20,color=c,label=f'上下文 {L} token：總讀取量');a.axhline(L*144/2**20,color=c,ls=':')
        a.plot(b,w,ls='--',color='#777777',label="每個輸出 token 分攤的權重讀取量");a.set(xscale='log',yscale='log',xlabel="批內請求數",ylabel="每生成一個 token 的讀取量（GiB）");a.legend(frameon=False,fontsize=11);save(f,'2-batch')
        for i,key in enumerate(['fixed','continuous','chunked']):
            ...
            a.scatter([0,0,20,30],range(4),marker='^',color='#252525',s=18,zorder=5);a.set(yticks=range(4),yticklabels=[f'請求 r{j}' for j in range(4)],xlim=(0,900),xlabel="時間（ms）");a.invert_yaxis();save(f,'3-scheduling' if i==0 else f'scheduling-{key}')
        for i,d in enumerate(data['attention_geometry']):
            ...
            text(a,.04,.91,f'舊上下文 {h} token + 新輸入 4 token',14);text(a,.5,.12,f'舊上下文配對 {4*h} + 塊內配對 10 = {4*h+10}',12,ha='center');save(f,'4-attention' if i==0 else 'attention-history')
        f,a=canvas(4.5);text(a,.04,.94,"邏輯塊按序排列，物理塊分散存放",14)
        for i,target in enumerate([2,0,3]):
            x=.06+i*.31;box(a,x,.68,.24,.13,f'邏輯塊 {i}','blue',11);text(a,x+.12,.54,f'塊表：{i} → {target}',11,ha='center');arrow(a,(x+.12,.46),(.14+target*.235,.31))
        for i in range(4):box(a,.04+i*.235,.12,.20,.18,f'物理塊 {i}','green' if i in [0,2,3] else 'gray',11)
        save(f,'page-map')
        ...
        f,a=canvas(4.4);box(a,.04,.67,.27,.15,"A 的塊表",'blue');box(a,.69,.67,.27,.15,"B 的塊表",'green');box(a,.31,.34,.38,.16,"兩個共同字首塊\n各有兩個引用",'purple',11)
        arrow(a,(.175,.67),(.40,.50));arrow(a,(.825,.67),(.60,.50));box(a,.03,.07,.28,.13,"A 私有字尾",'blue',11);box(a,.69,.07,.28,.13,"B 私有字尾",'green',11);arrow(a,(.175,.67),(.175,.20));arrow(a,(.825,.67),(.825,.20));save(f,'pages-shared')
        f,a=canvas(4.3);box(a,.25,.70,.50,.16,"共享尾塊：[a, b, c, 空]",'purple',12)
        for x,label,c in [(.03,"分支 A：[a, b, c, x]",'blue'),(.54,"分支 B：[a, b, c, y]",'green')]:box(a,x,.20,.43,.19,label,c,11);arrow(a,(.5,.70),(x+.215,.39))
        text(a,.5,.52,"寫入分歧前，取得各自的尾塊副本",12,ha='center');save(f,'copy-on-write')
        f,a=canvas(4.3)
        for row,(label,count,col) in enumerate([("A、B 使用共同塊",2,'purple'),("A 結束，B 繼續使用",1,'green'),("B 結束，加速器已不再存取",0,'gray')]):
        ...
        for i,node in enumerate(d['tree']):
            ...
            if len(node['ids'])==1:text(a,x,y+.09,f'輪 {node["ids"][0]+1}',11,ha='center')
        text(a,.04,.95,"前四輪：從共同 206 個 token 開始",14);text(a,.5,.03,"邊上數字：新增 token 數",11,ha='center');save(f,'6-prefix')
        f,a=plot(3.8);v=np.array(d['adjacent_lcp']);a.bar(range(1,13),v,color=COL['blue'],label="與上一輪共同字首");a.bar(range(1,13),np.array(d['input_lengths'])-v,bottom=v,color=COL['orange'],label="其餘輸入");a.set(xlabel="輸入輪次",ylabel="token 數",xticks=[1,3,6,9,12]);a.legend(frameon=False);save(f,'prefix-lengths')
        f,a=canvas(3.8);text(a,.04,.94,"文字匹配到 10752，狀態從 8192 恢復",13);a.plot([.05,.95],[.48,.48],color='#777777')
        for x,label,c in [(.10,"4096\n快照",'blue'),(.48,"8192\n最近快照",'green'),(.85,"10752\n匹配末端",'orange')]:box(a,x-.07,.39,.17,.22,label,c,11)
        arrow(a,(.57,.49),(.77,.49));text(a,.68,.22,"重算 2560 個 token",12,ha='center');save(f,'prefix-restore')
        f,a=canvas(4.1)
        for row in range(2):
            y=.63-row*.40
            cc=data['cache_choice'];va=round(cc['A']['net_ms'],1);vb=round(cc['B']['each_net_ms'],1)
            if row==0:box(a,.04,y,.92,.19,f'A：864 MiB，淨節省 {va} ms','blue')
            else:
                for i in range(3):box(a,.04+i*.31,y,.30,.19,f'B：288 MiB\n{vb} ms','green',11)
            text(a,.5,y-.09,f'合計節省 {va if row==0 else round(3*vb,1)} ms',12,ha='center')
        ...
        for i,m in enumerate(data['kv']['correct_by_task']):
            ...
            a.set(yticks=range(8),yticklabels=[f'任務 {j+1}' for j in range(len(data['kv']['task_ids']))],xticks=range(4),xticklabels=['1 / 1','1 / 2','4 / 1','4 / 2'],xlabel="併發數 / 重複序號");save(f,'10-kv-quality' if i==0 else f'kv-quality-{i}')
        f,a=canvas(4.6)
        ...
        for draft,n in [('A','sample-A'),('B','sample-B')]:
            ...
            for x,label,prob,c in [(.03,"接受 "+draft,p,'green'),(.54,"拒絕，輸出 "+('B' if draft=='A' else 'A'),1-p,'orange')]:box(a,x,.18,.43,.19,label,c,12);arrow(a,(.5,.71),(x+.215,.37));text(a,x+.215,.48,f'機率 {prob:g}',12,ha='center')
            text(a,.5,.06,"最終：P(A) = 1/4，P(B) = 3/4",12,ha='center');save(f,n)
        ...
        a.axhline(sp['ordinary_ms'],color='#777777',ls='--',label="普通 decode");a.set(xlabel="每輪草稿查詢（ms）",ylabel="每輸出 token 耗時（ms）",xlim=(0,60),ylim=(0,70));a.legend(frameon=False);save(f,'12-speculation')
        for field,name in [('completed_per_s','13-service'),('slo_goodput_per_s','service-goodput')]:
            ...
            for i,g in enumerate(groups):
                vals=[x[field] for x in g];a.bar(i,np.median(vals),color=COL['blue' if field=='completed_per_s' else 'green'],edgecolor=COL['line']);a.scatter([i]*len(vals),vals,color='#252525',s=16);labels.append(("逐個" if g[0]['service']=='serial' else "連續" if g[0]['service']=='continuous' else "突發")+('\n'+str(g[0]['rate']) if g[0]['rate'] is not None else "\n一次到達"))
            a.set(xticks=range(len(groups)),xticklabels=labels,ylim=(0,3.1),ylabel="完成吞吐（req/s）" if field=='completed_per_s' else "正確且按期完成（req/s）",xlabel="接納策略；下行為到達率（req/s）");save(f,name)
        ...
        a.axvline(dp['capacity_gib'],ls='--',color='#777777');a.axhline(dp['deadline_s'],ls='--',color='#777777');a.set(xlim=(0,21),ylim=(0,16.5),xlabel="KV 與輔助緩衝（GiB）",ylabel="整組完成時間（s）");save(f,'14-design')
        f,a=plot(3.6);s=np.array(data['task']['local_speedups'])
        for frac,col in zip(data['task']['decode_fractions'],['#267398','#388768','#a56c28']):a.plot(s,1/(1-frac+frac/s),label=f'decode 佔 {frac:.0%}',color=col)
        a.set(xlabel="decode 加速比",ylabel="完整任務加速比",xlim=(1,8));a.legend(frameon=False);save(f,'15-task')
