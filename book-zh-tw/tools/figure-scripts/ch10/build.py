...
for j,(label,col) in enumerate([("BF16 權重",'blue'),("BF16 梯度",'teal'),("FP32 主權重",'orange'),("Adam 一階矩",'red'),("Adam 二階矩",'muted')]):
...
ax.set(yticks=[1,0],yticklabels=["推理權重","訓練狀態"],xlabel="容量 / GB",xlim=(0,153),xticks=[0,20,40,60,80,100,120,140],ylim=(-.65,1.75));ax.legend(ncol=3,fontsize=10,frameon=False,loc='upper left');ax.grid(axis='x',alpha=.15)
...
for j,(ef,label,col) in enumerate([('3/10',"計算：30%",'blue'),('2/5',"計算：40%",'teal'),('1/2',"計算：50%",'orange')]):
...
caps=[next(r['persistent_capacity_count_bound'] for r in rows if r['device']==d) for d in devs];ax.scatter(x,caps,marker='D',s=55,facecolors='white',edgecolors=C['red'],linewidths=1.7,label="訓練狀態容量下限",zorder=5)
ax.set(xticks=x,xticklabels=labels,ylabel="必要裝置數（向上取整）",ylim=(0,49));ax.legend(ncol=2,frameon=False,loc='upper right',fontsize=11);ax.grid(axis='y',alpha=.18)
...
ax.plot(participants,total,color=C['blue'],lw=2.3,label="訓練狀態分片 + 10 GiB 附加量")
ax.plot(participants,persist,color=C['teal'],lw=1.6,ls=':',label="訓練狀態分片")
ax.fill_between(participants,total,22,where=total<=22,interpolate=True,color=C['teal'],alpha=.17,label="剩餘視訊記憶體")
ax.axhline(22,color=C['red'],ls='--',label="RTX 4090 每卡 22 GiB 可用視訊記憶體")
for d in [8,16]:
...
ax.set(xlim=(4,32),ylim=(0,47),xticks=[4,8,12,16,24,32],xlabel="分片參與者數",ylabel="每卡容量 / GiB");ax.legend(frameon=False,fontsize=10,loc='upper right');ax.grid(alpha=.15)
...
f=plt.figure(figsize=(13,11));policies=[('training-pipeline-gpipe-m8',"填滿排空"),('training-pipeline-1f1b-m8','1F1B')];pipedata={}
for idx,(name,label) in enumerate(policies):
 ...
 ax.set(yticks=[0,1,2,3],yticklabels=["階段 3","階段 2","階段 1","階段 0"],xlim=(0,355),xticks=[0,50,100,150,200,250,300,350],ylim=(-.65,3.65),xlabel="時間 / ms");ax.set_title(label+f"：{d['summary']['step_makespan_seconds']*1000:.0f} ms",loc='left',fontsize=13);ax.grid(axis='x',alpha=.15)
 if idx==1:
  ax.plot([93,95],[0,0],color=C['red'],lw=4,zorder=6)
  ax.annotate("輸入未到：2 ms",xy=(94,0),xytext=(145,.43),fontsize=9,color=C['red'],arrowprops={'arrowstyle':'->','color':C['red']},bbox={'facecolor':'white','edgecolor':'none','alpha':.94})
...
ax.set(xticks=x,xticklabels=["階段 0","階段 1","階段 2","階段 3"],ylabel="中間結果與緩衝區峰值 / GB",ylim=(0,3.1));ax.legend(frameon=False,fontsize=10)
f.text(.71,.23,"藍：前向；橙：反向\n綠：參數更新\n塊內數字：micro-batch 編號",fontsize=11,va='top',linespacing=1.65)
...
ax.set(xlim=(18,59),xticks=[20,30,40,50],ylim=(-.6,4.1),yticks=[3,2,1,0],yticklabels=["7 GB/s · 快照 1","7 GB/s · 快照 2","20 GB/s · 快照 1","20 GB/s · 快照 2"],xlabel="時間 / s");ax.grid(axis='x',alpha=.15)
f.text(.83,.73,"恢復到 20 s",fontsize=12,color=C['blue']);f.text(.83,.51,"恢復到 40 s",fontsize=12,color=C['teal']);f.text(.83,.28,"橙：複製到緩衝區\n實心點：已提交\n空心點：尚未提交",fontsize=10,linespacing=1.8)
...
for ys,label,col in [(restore,"先恢復權重和 KV，再釋放訓練狀態",'red'),(staged,"先同步權重，再釋放訓練狀態、分配 KV",'blue')]:
...
ax.set(xticks=x,xticklabels=["訓練迭代結束","載入生成權重","釋放訓練狀態","開始生成"],ylabel="視訊記憶體佔用 / GiB",ylim=(0,103),yticks=[0,20,40,60,80,100],xlim=(-.1,3.15));ax.legend(frameon=False,fontsize=11,loc='upper left');ax.grid(axis='y',alpha=.15)
...
f,a=canvas(6.2)
box(a,.03,.65,.27,.20,"生成時選擇","樣本 A · token 17 · 層 3",col='pale',size=13)
box(a,.38,.65,.24,.20,"儲存專家 ID",'[2, 7]',col='sand',size=13)
box(a,.71,.65,.26,.20,"當前路由計算","當前 token 的隱藏狀態與權重",size=13)
arrow(a,(.305,.75),(.37,.75))
box(a,.37,.19,.27,.22,"按記錄選專家 2、7","離散選擇保持一致",col='sand',size=13)
box(a,.71,.19,.26,.22,"計算輸出和梯度","使用當前分數與專家權重",col='light',size=13)
...
a.text(.04,.33,"按樣本、位置和層查詢記錄\n樣本 A / token 17 / 層 3",fontsize=11,linespacing=1.8)
...
for frac,col in [(.05,'teal'),(.2,'blue'),(.5,'orange')]:
 y=1-frac+frac/rat;curves[str(frac)]=y.tolist();ax.plot(rat,y,color=C[col],lw=2.4,label=f'原方案通訊等待佔比 {frac:.0%}');ax.scatter([.5],[1-frac+frac/.5],color=C[col])
ax.axvline(1,ls='--',color=C['line']);ax.axhline(1,ls='--',color=C['line']);ax.set(xlabel="新方案通訊頻寬 / 原方案頻寬",ylabel="新方案耗時 / 原方案耗時",xlim=(.25,1.5),xticks=[.25,.5,.75,1,1.25,1.5],ylim=(.8,2.65));ax.legend(frameon=False,fontsize=12);ax.grid(alpha=.15)
...
for dev,label,col in [('a100-80gb-sxm','A100 80GB SXM','blue'),('h100-sxm','H100 SXM','teal'),('b200-sxm','B200','orange')]:
 ...
 ax.annotate(f'{yy[-1]/1e4:.0f} 萬卡',(10,yy[-1]/1e4),xytext=(7,0),textcoords='offset points',va='center',fontsize=10,color=C[col])
...
ax.axhline(1.6384,color=C['muted'],ls='--',lw=1);ax.text(6.5,1.76,"16,384 張卡",fontsize=11,color=C['muted'])
ax.text(5.6,112,"各曲線上方滿足對應計算需求",fontsize=11,color=C['blue'])
ax.set(yscale='log',xlabel="Dense 參數量 / T",ylabel="90 天所需裝置 / 萬張（對數刻度）",xticks=[.1,1,5,10],xlim=(.1,10.6),ylim=(.1,150))
...
sched={}
for key,stem in [('1F1B','training-pipeline-1f1b'),("交錯式 v=2",'training-pipeline-interleaved'),("零氣泡",'training-pipeline-zero-bubble'),('DualPipe','training-pipeline-dualpipe')]:
 ...
 if key!='1F1B':
  ...
  data['pipeline_timeline_'+stem.removeprefix('training-pipeline-')]={'label':{"交錯式 v=2":"交錯式 1F1B（v=2）","零氣泡":"零氣泡（ZB-H1）",'DualPipe':'DualPipe'}[key],'events':events,'summary':s8,'scenario':d8['scenario'],'declared_schedule_rule':d8.get('declared_schedule_rule')}
...
page="<!doctype html><html lang=\"zh-CN\"><head><meta charset=\"utf-8\"><meta name=\"viewport\" content=\"width=device-width,initial-scale=1\"><title>第 10 章 訓練系統</title><style>"+css+math_css+"</style></head><body><main><nav aria-label=\"本章目录\">"+nav+'</nav>'+body+'</main></body></html>'
import sys
sys.path.insert(0,str(HERE.parent))
