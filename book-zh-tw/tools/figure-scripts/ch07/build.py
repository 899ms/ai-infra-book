...
names=['gradient-fp32-flat-contiguous-nic8','gradient-fp32-flat-interleaved-nic8','gradient-fp32-hierarchical-nic8'];g=[calc(x) for x in names];labels=["連續環","交錯環","分層歸約"]
...
f,a=canvas(9)
rows=[("主機處理的 RPC",["應用 / CPU","NIC / 網路","對端 CPU"],["請求序列化","載荷傳送","執行並回應"]),("CPU 提交的 GPUDirect RDMA",["CPU 提交","NIC ↔ GPU 記憶體","遠端 NIC ↔ GPU"],["提交 / 讀取完成狀態","資料直達","目標記憶體寫入"]),("GPU 經 NVLink 存取",["發起 GPU","NVLink / 交換","對端 GPU 記憶體"],["裝置指令","存取路徑","等待存取完成"]),("裝置發起的 URMA 非同步讀寫",["發起裝置 / 佇列","UB / 交換","目標裝置記憶體"],["提交 / 處理完成通知","資料路徑","寫入 / 通知"])]
for i,(title,boxes,notes) in enumerate(rows):
 ...
 a.annotate('',xy=(.17,y-.04),xytext=(.83,y-.04),arrowprops={'arrowstyle':'->','ls':'--','color':C['muted'],'lw':1});a.text(.50,y-.075,"回應 / 完成與同步關係",fontsize=9,ha='center',color=C['muted'])
...
f=plt.figure(figsize=(13,8));a=f.add_axes([.04,.58,.92,.37]);a.set(xlim=(0,1),ylim=(0,1));a.axis('off');panel(a,.01,.98,"應用端點與傳輸狀態分離")
for i in range(3):
 y=.63-i*.24;box(a,.015,y,.23,.17,f'應用端點 {i}',"獨立提交 / 完成",size=11);box(a,.36,y,.23,.17,"關係繫結","目標、權限與身份",size=11);arrow(a,(.25,y+.085),(.35,y+.085));arrow(a,(.60,y+.085),(.735,.42))
box(a,.745,.30,.23,.26,"同目標傳輸狀態","可靠交付 / 擁塞",col='light',size=11)
...
for j,(title,col) in enumerate([("端點",'blue'),("關係繫結",'orange'),("傳輸狀態",'teal')]):ax.barh(range(3),vals[:,j],left=left,color=C[col],label=title);left+=vals[:,j]
ax.set(yticks=range(3),yticklabels=["逐關係獨佔","按目標共享","八類隔離"],xlabel="狀態容量 / MiB",xlim=(0,10.4),xticks=[0,2,4,6,8,10]);ax.invert_yaxis();ax.axvline(1,ls='--',color=C['red']);ax.legend(frameon=False,ncol=3,loc='lower right')
...
for x,t,c in [(.015,"寫入 A",'pale'),(.26,"恢復 / 可見",'sand'),(.505,"通知 B",'pale'),(.75,"獨立傳輸 C",'light')]:box(graph,x,.32,.20,.34,t,col=c,size=11)
...
graph.text(.365,.83,"寫入後才能通知",ha='center',fontsize=12,color=C['teal']);graph.text(.82,.12,"額外的等待關係",ha='center',fontsize=11,color=C['red'])
ax=f.add_axes([.18,.11,.77,.50])
colors={'write_data':'blue','recover_and_make_visible':'orange','publish_notification':'red','independent_transfer':'teal'};task_names={'write_data':"寫入",'recover_and_make_visible':"恢復",'publish_notification':"通知",'independent_transfer':"獨立傳輸"}
for i,key in enumerate(['strict','necessary_dependencies']):
 ...
 ax.text(-3,2.75-i*2,["全部依次執行","僅保留必要依賴"][i],ha='right',va='center',fontsize=11)
 ax.text([112,10][i]+1,2.45-i*2,f'{[112,10][i]} μs',va='center',color=C['teal'])
ax.set(xlim=(0,126),ylim=(0,3.6),yticks=[],xlabel="時間 / μs",xticks=[0,20,60,100,112]);ax.legend(handles=[Patch(color=C[v],label=task_names[k]) for k,v in colors.items()],frameon=False,ncol=4,loc='upper center',bbox_to_anchor=(.5,1.12))
...
queue_data={}
for name,label,col,ls in [('periodic-queue-aligned',"重疊 20 ms",'blue','-'),('periodic-queue-drift',"重疊 5 ms",'orange',':'),('periodic-queue-staggered',"不重疊",'teal','--')]:
...
axs[0].set(ylim=(-3,96),ylabel="到達速率 / GB/s");axs[0].legend(frameon=False,ncol=3,loc='upper center',bbox_to_anchor=(.5,1.23))
axs[1].set(xlim=(0,60),ylim=(-20,1120),xlabel="時間 / ms",ylabel="積壓 / MB");axs[1].text(21,1010,'1000 MB',color=C['blue']);axs[1].text(21,260,'250 MB',color=C['orange'])
...
f,a=canvas(6);panel(a,.015,.98,"導致迴圈等待的資源依賴");panel(a,.53,.98,"預留回應資源，規定申請次序")
xy=[(.04,.63),(.30,.63),(.30,.19),(.04,.19)]
for i,(x,y) in enumerate(xy):box(a,x,y,.15,.18,f'資源 {chr(65+i)}',size=12)
arrow(a,(.195,.72),(.29,.72),'red');arrow(a,(.375,.625),(.375,.385),'red');arrow(a,(.295,.28),(.20,.28),'red');arrow(a,(.115,.385),(.115,.625),'red')
a.text(.245,.495,"持有前一項\n等待後一項",ha='center',va='center',fontsize=11,color=C['red'])
for i,(t,b) in enumerate([("請求資源","按次序申請"),("執行資源","處理資料"),("回應資源","預留緩衝 / 優先傳送")]):
 y=.66-i*.25;box(a,.60,y,.31,.17,t,b,col='light',size=12)
 if i<2:arrow(a,(.755,y-.005),(.755,y-.07))
a.text(.755,.03,"回應到達後釋放原請求資源",ha='center',fontsize=10,color=C['muted'])
...
for group,(label,rd,ex) in enumerate([("原始",ready,.4),("交換減半",ready,.2),("消除就緒偏差",[0]*4,.4)]):
 base=10-group*4
...
ax.set(xlim=(0,2.85),xticks=[0,.5,1,1.5,2,2.5],ylim=(-.4,11),yticks=[],xlabel="時間 / ms");ax.legend(handles=[Patch(color='#dbe2e6',label="尚未就緒"),Patch(color='#f6e3c9',label="等待其他參與者"),Patch(color=C['blue'],label="交換")],ncol=3,frameon=False,loc='upper center',bbox_to_anchor=(.5,1.15))
