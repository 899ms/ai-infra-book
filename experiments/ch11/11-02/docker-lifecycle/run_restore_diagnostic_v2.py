...
O = R/'results-restore-diagnostic-v2'
...
def main():
    ...
    save(O/'environment.json',dict(runid=runid,base_image_id=base,
        ...
        source_hashes={n:hashlib.sha256((R/n).read_bytes()).hexdigest() for n in ['run_restore_diagnostic_v2.py','worker_unix.py','PROTOCOL.md']}))
            ...
            if path=='pause':
                ...
                if cp['exit_code']==0:
                    for number in range(2):
                        ...
                        watch=subprocess.Popen(['sudo','-n','python3',str(R/'watch_restore.py'),new,str(folder/f'clone{number}-runtime-logs.json')])
