...
async def main(args):
    ...
    config=dict(model=args.model,dtype='bfloat16',quantization='fp8',kv_cache_dtype=args.kv_dtype,
        ...
        gpu_memory_utilization=.30,kv_cache_memory_bytes=8*1024**3,seed=808,
        attention_backend='TRITON_ATTN',worker_extension_cls='probe_qbf16.KVQBF16Probe')
    env=dict(config=config,torch=torch.__version__,vllm=vllm.__version__,
        hashes={n:hashlib.sha256((Path(__file__).parent/n).read_bytes()).hexdigest() for n in ['run.py','probe.py','probe_qbf16.py']},
    ...
    try:
        ...
        snapshots['q_override_layers']=await engine.collective_rpc('disable_q_quantization')
        snapshots['after_q_override']=await engine.collective_rpc('kv_snapshot')
