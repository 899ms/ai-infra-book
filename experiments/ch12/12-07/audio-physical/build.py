...
for r in json.loads((B/'source-lock.json').read_text())['files']:
 data=(B/'source'/r['path']).read_bytes()
 patch=json.loads((B/'baseline-binding-patch.json').read_text())
 if r['path']==patch['path']:
  assert r['sha256']==patch['before_sha256'] and hashlib.sha256(data).hexdigest()==patch['after_sha256']
 else:assert len(data)==r['bytes'] and hashlib.sha256(data).hexdigest()==r['sha256']
meta={}
