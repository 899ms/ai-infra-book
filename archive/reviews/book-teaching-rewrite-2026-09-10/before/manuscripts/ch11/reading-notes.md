...
CPU 进程记录、E2B 文档契约、抢占恢复、验证反馈／释放、双批次、预热重放、思考预算、Collector 故障、退出语义和扩容决定分别用于其已经测量的边界。失败和反例继续保留：消除 256 个重复 token 没有更早完成；提前更多没有降低工具延迟中位；反馈改善的少量合格轨迹不能证明普遍质量提升；96.13% 模型等待不证明 GPU 饱和。
