...
import subprocess,json,time,uuid,threading,hashlib,os,re
from pathlib import Path
P=Path(__file__).resolve().parent;O=P/'results';O.mkdir(exist_ok=False)
commands=[]
def cmd(args):
 if args[0]=="docker":args=args[:1]+["-H","unix://"+"/run/book1102-c421v2.sock"]+args[1:]
...
for mode in ['default','unconfined']:
 ...
 try:
  ...
  def watch():
   roots=[Path('/run/book1102-c421v2-ctd/io.containerd.runtime.v2.task/book1102')/cid,Path('/run/book1102-c421v2/runtime-runc/book1102')/cid,Path('/run/book1102-c421v2/containerd/daemon/io.containerd.runtime.v2.task/book1102')/cid,P/'data/containers'/cid]
   while not stop.is_set():
    for root in roots:
     for directory,dirs,files in os.walk(root):
      dirs[:]=[d for d in dirs if d not in ['criu-root','rootfs','fs']]
      for name in files:
       if not (name.endswith('.log') or name=='log.json'):continue
  ...
  if restore and 'already exists' in restore['stderr']:
   digest=re.search(r'content (sha256:[0-9a-f]{64}): already exists',restore['stderr']).group(1)
   ctr=['ctr','--address','/run/book1102-c421v2-ctd.sock','--namespace','book1102','content']
   listing=cmd(ctr+['ls'])
   assert listing['rc']==0
   # Only the digest from this own-container restore failure, in the private store.
   if digest in listing['stdout']:assert cmd(ctr+['rm',digest])['rc']==0
   restore=cmd(['docker','start','--checkpoint','probe',cid])
  stop.set();watcher.join()
  state=cmd(['docker','inspect','--format','{{json .State}}',cid])
  after=cmd(['docker','logs',cid])
  if restore and restore['rc']==0:
   assert json.loads(state['stdout'])['Running']
   assert after['stdout']==before['stdout'], 'cold startup printed another nonce'
  row=dict(after=after,initial_restore=initial_restore,mode=mode,cid=cid,before=before,capture=capture,restore=restore,state=state)
