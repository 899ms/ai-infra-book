...
def main(a):
 ...
 os.environ['BOOK_REMOTE_KV_URL']='http://127.0.0.1:18797'
 ...
 try:
  ...
  # Observe acknowledged remote pages, not a fixed sleep before killing async I/O.
  deadline=time.monotonic()+3600;observations=[]
  while True:
   req=urllib.request.Request(os.environ['BOOK_REMOTE_KV_URL']+'/inventory',data=pack({}))
   with urllib.request.urlopen(req,timeout=120) as f:inventory=unpack(f.read())[0]
   observations.append(dict(time_s=time.monotonic(),files=len(inventory['files'])))
   ack_path=Path(os.environ['BOOK_REMOTE_KV_TRACE'])
   acknowledged=[json.loads(line) for line in ack_path.read_text().splitlines()] if ack_path.exists() else []
   if any(x['error'] is not None for x in acknowledged):raise RuntimeError('Recorded remote transport failure; no waiting for a dead backup thread')
   saved=sum(len(x['meta']['entries']) for x in acknowledged if x['op']=='set' and x['error'] is None)
   if len(inventory['files'])>=len(ids)//16 and (a.phase!='producer' or saved>=len(ids)//16):break
   if time.monotonic()>deadline:raise TimeoutError('remote page publication incomplete')
   time.sleep(2)
  result['publication_observations']=observations;result['remote_inventory']=inventory
