...
*Figure 10-38: Loading generation weights and allocating the KV pool first, then releasing training state, gives a peak of about 83.3 GiB, exceeding the H100 SXM's 74.5 GiB. The horizontal axis is ordered by operation sequence.*

![Loading generation weights first, then releasing training state, then allocating the KV pool](ch10/figure-10-rl-staged.svg)
*Figure 10-39: Loading generation weights first, then releasing training state, then allocating the KV pool. The peak drops to about 59.3 GiB; both figures share the 74.5 GiB capacity line and the same vertical axis.*
...
*Figure 10-40: Weight lifecycles for fixed-version serving and policy training. The ROM above repeatedly serves the same version; the training below produces new versions and publishes them to the generation side. KV writes and weight updates use different data paths.*
...
*Figure 10-41: μ produces training samples, πold marks the starting point of this round's optimization, πθ changes as this round's update proceeds. The dashed line indicates the order of version evolution. All three probabilities must be computed for the same prefix and the same token.*
...
*Figure 10-42: The synchronous loop generates the current batch, learns the current batch, and synchronizes weights in sequence, taking 40, 16, and 4 s respectively, totaling 60 s.*
...
*Figure 10-43: In steady state, generating the next batch overlaps with learning the previous batch on independent resources; once both finish, weights are synchronized, giving a 44 s cycle. The orange synchronization stage blocks both sides.*
...
*Figure 10-46: The benefit of a change in a resource's capability is determined by its original proportion of waiting time. The curve is computed as $T'/T=1-f+f/r$, holding per-card computation and dependencies fixed, with communication time inversely proportional to effective capability.*
