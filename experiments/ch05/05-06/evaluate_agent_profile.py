...
from agent_guard import validate_code
...
def native(x):
 out=torch.empty((x.shape[0],x.shape[1]//2),device=x.device,dtype=x.dtype);torch.ops._C.silu_and_mul(out,x);return out
def expression(x):
...
def main(a):
 ...
 if a.kind=='agent':
  validate_code(a.candidate.read_text())
  spec=importlib.util.spec_from_file_location('agent_candidate',a.candidate);module=importlib.util.module_from_spec(spec);spec.loader.exec_module(module);fn=module.run
 elif a.kind=='native':fn=native
 ...
 try:
  for index in [1,0]:
   ...
   assert actual.untyped_storage().data_ptr()!=x.untyped_storage().data_ptr(), "Output aliases input"
   ...
   trace=a.output.with_name(a.output.stem+f'-profile-{t}.json')
   with torch.profiler.profile(activities=[torch.profiler.ProfilerActivity.CPU,torch.profiler.ProfilerActivity.CUDA]) as prof:
    measured=fn(x);torch.cuda.synchronize()
   prof.export_chrome_trace(str(trace))
   torch.testing.assert_close(measured,expected,atol=protocol['atol'],rtol=protocol['rtol'])
   assert torch.equal(x.cpu(),data['input'])
   events=json.loads(trace.read_text())['traceEvents']
   kernels=[e for e in events if e.get('cat')=='kernel']
   row['profiler_feedback']=dict(trace=trace.name,kernels=[dict(name=e['name'],duration_us=e['dur'],grid=e.get('args',{}).get('grid'),block=e.get('args',{}).get('block'),stream=e.get('args',{}).get('stream')) for e in kernels],scope='instrumented CUDA kernel activity; not hardware counters or timing score')
  ...
  result['source_hashes']={f:hashlib.sha256((ROOT/f).read_bytes()).hexdigest() for f in ['evaluate_agent_profile.py','agent_guard.py','protocol.json']}
  result['candidate_sha256']=hashlib.sha256(a.candidate.read_bytes()).hexdigest() if a.candidate else None
  a.output.write_text(json.dumps(result,indent=2)+'\n')
 print(a.kind,a.block,a.warps,result['status'],flush=True)
if __name__=='__main__':
 p=argparse.ArgumentParser();p.add_argument('--kind',choices=['native','compiled','schedule','agent'],required=True);p.add_argument('--candidate',type=Path);p.add_argument('--block',type=int,default=256);p.add_argument('--warps',type=int,default=4);p.add_argument('--output',type=Path,required=True);main(p.parse_args())
