...
async def main(a):
    ...
    cfg=dict(model=a.model,dtype='auto',kv_cache_dtype='auto',attention_backend='TRITON_ATTN',
             ...
             async_scheduling=False,kv_cache_memory_bytes=2*1024**3,gpu_memory_utilization=.30,worker_extension_cls="capture_probe.CaptureProbe",
    ...
    try:
        ...
        capture_info=await engine.collective_rpc('arm_capture',args=(str(out),))
        (out/'capture-info.json').write_text(json.dumps(capture_info,indent=2)+'\n')
        with (out/'requests.jsonl').open('w',buffering=1) as f:
            for c in cases:
                await engine.collective_rpc('set_capture_case',args=(c['id'],))
